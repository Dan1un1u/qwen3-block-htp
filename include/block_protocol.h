...
#ifdef QBH_LLAMA_3B
#define QBH_BLOCK_HIDDEN UINT32_C(3072)
#else
#define QBH_BLOCK_HIDDEN UINT32_C(2048)
#endif
#ifdef QBH_MODEL_LLAMA32
#define QBH_BLOCK_INTERMEDIATE UINT32_C(8192)
#ifdef QBH_LLAMA_3B
#define QBH_BLOCK_HEADS UINT32_C(24)
#define QBH_BLOCK_HEAD_DIM UINT32_C(128)
#define QBH_QWEN3_TRANSFORMER_LAYERS UINT32_C(28)
#else
#define QBH_BLOCK_HEADS UINT32_C(32)
#define QBH_BLOCK_HEAD_DIM UINT32_C(64)
#define QBH_QWEN3_TRANSFORMER_LAYERS UINT32_C(16)
#endif
#define QBH_BLOCK_KV_HEADS UINT32_C(8)
#define QBH_BLOCK_KV_HIDDEN \
    (QBH_BLOCK_KV_HEADS * QBH_BLOCK_HEAD_DIM)
#define QBH_QWEN3_VOCAB_SIZE UINT32_C(128256)
#else
...
#if defined(QBH_LLAMA_LAYER_COUNT)
#define QBH_VERTICAL_SLICE_FIRST_LAYER UINT32_C(0)
#define QBH_VERTICAL_SLICE_LAYER_COUNT QBH_LLAMA_LAYER_COUNT
#elif defined(QBH_EXP0257_LAYER_COUNT)
#define QBH_VERTICAL_SLICE_FIRST_LAYER UINT32_C(0)
#define QBH_VERTICAL_SLICE_LAYER_COUNT QBH_EXP0257_LAYER_COUNT
#elif defined(QBH_EXP0247_DENSE_R3)
#define QBH_VERTICAL_SLICE_FIRST_LAYER UINT32_C(0)
...
enum qbh_block_generation_mode {
    ...
    QBH_BLOCK_GENERATION_GREEDY_F16F16 = 10,
};
...
enum qbh_w4u8_decode_direct_n_mask {
    ...
    /* EXP-0216 independently opts the M64 Q/K/V/O projections into the
     * same direct packed-W4 HMX carrier. */
    QBH_BLOCK_W4U8_DIRECT_N_PREFILL_QKVO = 1U << 5,
    QBH_BLOCK_W4U8_DIRECT_N_ALL = (1U << 6) - 1U,
};
...
/* EXP-0180/0181 keep the V-cache allocation byte-compatible with segmented-v4,
 * but change the mutable 32-row tail contract.  Complete groups of four rows
 ...
 * is published.  V5 publishes from append; V6 reuses the Attention load and
 * publishes from the consumer, except for the final segment-sealing row. */
...
struct qbh_block_projection_desc {
    ...
    uint32_t lpbq_audit_offset; /* Untimed public projection input/output audit. */
};
...
#define QBH_DENSE_R3_CARRIER_BYTES (QBH_BLOCK_M * (QBH_BLOCK_HEADS + QBH_BLOCK_KV_HEADS) * QBH_BLOCK_HEAD_DIM * 2U)
#define QBH_DENSE_R3_BASE_AUDIT_BYTES (2U * QBH_DENSE_R3_CARRIER_BYTES + QBH_BLOCK_M * (QBH_BLOCK_HEADS + QBH_BLOCK_KV_HEADS) * QBH_BLOCK_HEAD_DIM)
...
struct qbh_block_header {
    ...
    uint32_t w4u8_decode_direct_n_gate_up_batch_n_tiles;
    uint32_t w4u8_decode_direct_n_gate_up_continuous;
    ...
    uint32_t w4u8_decode_direct_n_o_single_dma;
    ...
    /* 0 legacy,1 wide SOLE,2 scalar SOLE,3 wide exact,4 wide NR64,5 scalar exact,6 scalar NR64. */
    ...
    uint32_t paper_format_disable; /* bits1/2 modular interfaces; exclusivebit4 independentSP2decode streams; bit8 legacy decode gather */
    ...
    uint32_t evaluation_mode; /* mode3: fixed-token replay, no NLL work */ /* 0 normal; 1 teacher forcing/scoring; 2 free generation */
    ...
    uint64_t dense_r3_total_prepare_ticks, dense_r3_total_matmul_ticks, dense_r3_total_finish_ticks;
    ...
    uint32_t w4u8_o_gate_prefetch_start_count;
    uint32_t w4u8_o_gate_prefetch_consume_count;
    uint64_t w4u8_o_gate_prefetch_wait_ticks;
    uint64_t w4u8_o_gate_prefetch_lifetime_ticks;
    uint32_t w4u8_gate_up_swiglu_publish_count;
    uint32_t w4u8_gate_up_swiglu_consume_count;
    uint32_t w4u8_gate_up_swiglu_overlap_observed;
    ...
    uint32_t u8_cache_v_quartet_attention_publish_count;
