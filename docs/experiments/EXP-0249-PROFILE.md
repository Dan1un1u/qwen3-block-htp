# EXP-0249 complete profiling record — fixed-prefix software U8 cache diagnostics

PC063 scope: independently validated integer attention and conditional software PPL attribution on frozen C64 per-channel W4, with static U8 online activations and real U8 software KV storage. No DSP binary, model package, deployment, device invocation or timed hardware run changed. Repeat1/repeat10, five short/ten formal device rounds: N/A.
...
| New device hashes / mismatches / maximum LSB / physical gates | N/A | N/A | Software diagnostic only |

No missing value is a measured zero. Integer attention agrees exactly with retained EXP0042/248 device boundary captures. Remaining software QDQ, norm, linear and R3 arithmetic plus the offline prefix do not constitute full-device validation. Evaluation elapsed time cannot be interpreted as device throughput. See REPORT.md, summary.json and checks/ for numerical evidence.
