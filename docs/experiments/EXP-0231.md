# EXP-0231: symmetric static group128 software diagnostic

Activated under PC051 after EXP0230 independent acceptance failed; user ordered approval remains in force. Exactly one G128 candidate.
...
## Frozen executable decision rule

Primary512documents, reserve512documents, all4variants F/C8/C64/G128. Use paired stratified document bootstrap5000 seed231. Trigger reserve for all4variants if ANY G128/F interval straddles its applicable gate; otherwise stop at primary. Overall1.05 and all language/domain/cell1.10 limits unchanged. Point failure is fail; all points and upper confidence limits passing is pass; passing points with crossing intervals at maximum budget is inconclusive (insufficient). No further reserve, candidate search or threshold adjustment. C8/C64/F are controls, never alternatives selected from final scores. Development128 reused for descriptive comparison and exact F/C8/C64 regression against EXP230; no algorithm/checkpoint selection. Independent new data must exclude all EXP230 roles. Freeze dataset/inputs/protocol hashes before group scoring.

If G128 passes, stop PC051 escalation and discuss grouped DSP deployment. If fail/inconclusive, register AWQ-style input-channel equalization separately. Software quality only; no hardware execution/profiling, retained full report declares those sections N/A and references EXP230. Other recipes/runtime/packages frozen, no baseline promotion.
