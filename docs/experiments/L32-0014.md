...
Completed all16-layer precision-only software and SDK integer simulation. Hardware runs0; no new DSP R3/R4 implementation, no profiling, no baseline/quality promotion. Report docs/LLAMA32_SMOOTH_R4_ALL_A8_ACCURACY.md.

Fresh transformed float fullvalidation13.64440643; same-W4 DownA16 control15.14164350 vs all112 static A8 inputs15.15255126: only0.072038% extra PPL from DownA8. Matched2048-target bridge: input-only26.74915412; extra Down-outputU8 alone52.07014431; extra residualU8 alone18518.31142225; restore Down/residual highprecision with other BF16-carrier quantizers27.87155044; all BF16 carriers28297.09165154; SDK fullinteger51465.45928709. All-carrier BF16 diagnostics retain float attention and BF16 head; never confuse them with SDK fullinteger contract.

All112 packed W4 matrices/973078528 codes and scales match evaluated control; SDK dot audit113 matrices includinghead. Fixed64-row dense FP32 R3/R4 passed full80 vs actualM64+15 cache equivalence for all16 positions/all128256 vocabulary codes. Failed earlier shape-dependent dense arithmetic evidence retained separately. Transform vector audits max8.6675e-7; custom forward exact against matched HF after fixing RoPE. No affine optimizer updates;8 CE calibration backwards.

Reference source read-only. Temporary independent112 inputscales and dense rotations require later native integration before any hardware claim. Full2048 SDK/device PPL not run; negative native-context simulation already demonstrates this particular static per-tensor U8 residual recipe remains unusable. Do not infer all A8 approaches impossible.
