...
R=Path('/mnt/d/llm_exp/results/llama32-htp/l32-0044')
...
def phase(name,rounds,decode):
 ...
 for a,b in zip(groups['CONTROL'],groups['OPT']):
  for x,y in zip(a,b):
   ...
   assert x['generation_lm_head_command_count']-y['generation_lm_head_command_count']==375
   assert x['hmx_command_count']-y['hmx_command_count']==375
 rng=np.random.default_rng(320044+decode)
 for mode in ['prefill','decode']:
  ...
  result['effects'][mode]=dict(wall_ratio=ratio,ci95=ci,throughput_gain_percent=100*(1/ratio-1),ten_percent_gate=ci[1]<=1.1)
 ...
 return result
def main():
 result=dict(experiment='L32-0044',model='Llama-3.2-3B-Instruct',recipe='W4A8-SP2mode8 FP32 residual, no rotation; AVrows4; candidate head32',quality_claim=False,baseline_promoted=False)
 result['short']=phase('short',5,42);result['formal']=phase('formal',10,42);result['supplement63']=phase('supplement63',5,63)
 result['formal_gate']=all(v['ten_percent_gate'] for v in result['formal']['effects'].values())
 result['external_quant_npu_decode_tps']=28.04;result['external_target_pass']=result['formal']['arms']['OPT']['decode']['tps']>=28.04
 result['control_source']=read(R/'control-l28/runtime.json')['seal']['source_head'];result['measured_source']=read(R/'opt2c-l28/runtime.json')['seal']['source_head']
 save(R/'SUMMARY.json',result)
 lines=['# L32-0044 full-model long decode modules','','Units µs; parentheses complete Host-wall shares. M64+42/cache128, five short and ten formal paired AB/BA cycles,repeat10. Frozen weights/scales,SP2,FP32 residual,no rotations.','','| 模块 | CONTROL Prefill | OPT Prefill | CONTROL Decode | OPT Decode |','|---|---:|---:|---:|---:|']
 ...
 for phase_name in ['formal','supplement63']:
  z=result[phase_name]
  for a in ['CONTROL','OPT']:lines.append(f"| M64+{z['decode_tokens']} | {a} | {z['arms'][a]['prefill']['tps']:.2f} | {z['arms'][a]['decode']['tps']:.2f} |")
 lines+=['','63decode is a five-pair supplement, separate from primary ten-pair formal42decode. Cold loading/session setup and external tokenization excluded. Numerical implementation verification is separate from model quality.']
 (R/'MODULES.md').write_text('\n'.join(lines)+'\n')
 print(json.dumps({n:{a:{m:z[m]['tps'] for m in ['prefill','decode']} for a,z in result[n]['arms'].items()} for n in ['formal','supplement63']},indent=2))
