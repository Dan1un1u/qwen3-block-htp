"""Independent actual-arithmetic oracle for L32-0052 (not a floating/PPL teacher)."""
import os
...
from llama32_3b_reference import layer,norm
from llama_u8_reference import load_qparams_bin,project_w4u8,HmxU8Converter
from long_prefill_3b import R,M,read,put,preflight
def fnv(x):
...
def main(length,decode,tag=None,resume=None):
 ...
 embed=np.memmap(p/'generation_embedding_weight_f16.bin','<f2','r',shape=(128256,3072))
 ...
 for step in range(begin,chunks+decode):
  ...
  for i in range(28):
   x,caches[i],diag=layer(x,p/f'layer{i}',qs[i],cos[pos:pos+rows],sin[pos:pos+rows],caches[i])
  ...
  if head:
   ...
   logits=project_w4u8(act,p,'generation_lm_head',128256,3072,gq['generation_final_norm_output'],gq['generation_lm_head_output'],cv)[0]
