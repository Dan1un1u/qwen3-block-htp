"""L32-0031 fixed full-model factorial, independent frozen references."""
...
R=Path('/mnt/d/llm_exp/results/llama32-htp/l32-0031');OLD=R.parent/'l32-0018';M=Path('/mnt/d/llm_exp/models/llama32-htp/l32-0016')
ARMS={'ALL':(0,0),'SPLIT':(4,0)}
...
def preflight():
 ...
 assert 'ACTIVE_EXPERIMENT=L32-0031' in t

def prepare():
 preflight();prior=R.parent/'l32-0030/EVIDENCE_SHA256.json';assert sha(prior)=='7d6bc80d63ecdfa9d1c7e3040483a30dc68cf1dd0a14264d7aee862f1bee6ed2'
...
def stage(count,trace=False):
 ...
 d=R/f'binaries-l{count}-a{attempt}';d.mkdir();remote=f'/data/local/tmp/llama32-htp/l32-0031-l{count}-a{attempt}'
...
def timing(scope):
 ...
 for phase,cycles in ([('short',5),('formal',10)] if scope==16 else [('formal',10)]):
  ...
  for c in range(cycles):
   j=c%len(keys)
   for a in keys[j:]+keys[:j]:out.append(dict(cycle=c,**execute(key,a,f'l{scope}-{phase}-a01/{c:02d}-{a}',repeat=10)))
