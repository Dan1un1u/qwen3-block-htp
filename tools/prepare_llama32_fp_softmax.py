...
from llama32_fp_softmax_reference import layer,norm
from llama_u8_reference import load_qparams_bin,HmxU8Converter,project_w4u8
from llama_reference import sha256 as sha
M=Path('/mnt/d/llm_exp/models/llama32-htp');O=M/'l32-0033';R=Path('/mnt/d/llm_exp/results/llama32-htp/l32-0033');PR=R.parent/'l32-0032'
...
def replay(key,basekey,layers,origlayer,base):
 ...
 for step in range(2):
  ...
  for i in range(layers):
   root=dst/f'layer{i}';q=load_qparams_bin(root/'qparams_u8.bin');x,past[i],diag=layer(x,root,q,cos,sin,past[i],sp2=True);np.save(dst/f'fp_step{step}_hidden{i}.npy',x)
  ...
  if layers==1:
   for n,v in diag.items():np.save(dst/f'fp_step{step}_{n}.npy',v)
