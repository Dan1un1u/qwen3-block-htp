...
R=Path('/mnt/d/llm_exp/results/llama32-htp/l32-0042')
...
def main():
 ...
 result=dict(experiment='L32-0042',model='Llama-3.2-3B-Instruct',shape='M64+15 cache80,28 layers',short_rounds=5,formal_rounds=10,repeat=10,profiles_per_arm=1600,quality_claim=False,baseline_promoted=False,arms={},effects={},submodules={})
 frozen_counts=['hmx_command_count','hmx_u8s8_tile_pair_count','weight_ddr_read_bytes','scan_cache_ddr_write_bytes','block_invocation_count','generation_lm_head_command_count']
 profiles={}
 for arm in ['CONTROL','OPT2']:
  groups=[]
  ...
  result['arms'][arm]=a
 for cycle in range(10):
  aa=profiles['CONTROL'][cycle];bb=profiles['OPT2'][cycle]
  for a,b in zip(aa,bb):
   for k in frozen_counts:assert a[k]==b[k],(cycle,k,a[k],b[k])
 rng=np.random.default_rng(320042)
 for mode in ['prefill','decode']:
  a=np.array(result['arms']['CONTROL'][mode]['round_host_us']);b=np.array(result['arms']['OPT2'][mode]['round_host_us'])
 ...
 result['matrix_work_and_weight_bytes_identical']=True
 result['vtcm_peak_bytes']=max(v['vtcm_peak_plan_bytes'] for g in profiles['OPT2'] for v in g)
 result['measured_source']=read(R/'opt2-l28/runtime.json')['seal']['source_head']
 result['control_source']=read(R/'control-binaries-3b-l28-a02/runtime.json')['seal']['source_head']
 save(R/'SUMMARY.json',result)
 lines=['# L32-0042: 3B shape-specific pipeline/layout optimization','','M64+15, KV capacity80,28layers;5 short and10 formal paired AB/BA cycles,repeat10. Complete Host wall includes embedding,all blocks,final norm,head,greedy and RPC; excludes cold loading/session setup and external tokenization. Frozen original3B weights/scales/SP2mode8,FP32 residual,no rotations.','',
 '| 模块 | 原实现 Prefill μs（占比） | 优化 Prefill μs（占比） | 原实现 Decode μs/token（占比） | 优化 Decode μs/token（占比） |','|---|---:|---:|---:|---:|']
 cols=[result['arms'][arm][mode]['modules'] for mode,arm in [('prefill','CONTROL'),('prefill','OPT2'),('decode','CONTROL'),('decode','OPT2')]]
 ...
 for arm in ['CONTROL','OPT2']:lines.append(f"| {arm} | {result['arms'][arm]['prefill']['tps']:.2f} | {result['arms'][arm]['decode']['tps']:.2f} |")
 lines+=['','Paired-bootstrap95% CIs (resampling matched cycles,20000 samples,seed320042):']
 ...
 lines+=['','All3200 formal additive profiles reconcile; HMX commands,tile work and weight bytes match by token. VTCM peak'+str(result['vtcm_peak_bytes'])+' bytes; zero intermediate DDR/spill. No quality claim or automatic baseline promotion.','']
