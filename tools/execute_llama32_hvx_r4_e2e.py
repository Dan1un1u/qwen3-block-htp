#!/usr/bin/env python3
"""L32-0070: same binary, fixed trace, full-model ordinary A8 versus online HVX R4."""
...
R=Path('/mnt/d/llm_exp/results/llama32-htp/l32-0070')
...
REMOTE='/data/local/tmp/llama32-htp/l32-0070'
...
def preflight():
 z=subprocess.check_output(['python3',str(ROOT)+'-project-memory/scripts/project_memory.py','preflight','--source-worktree',str(ROOT)],text=True);assert 'ACTIVE_EXPERIMENT=L32-0070' in z;print(z,flush=True)
def fnv(x):
...
def prepare():
 ...
 original=Path('/mnt/d/llm_exp/models/llama3.2-1B-Instruct-origin/model.safetensors');original_sha=sha256(original);assert original_sha==EXPECTED["model.safetensors"];folds=[]
 def had(x):
  x=x.clone()
 ...
 with safe_open(original,framework='pt',device='cpu') as f:
  for i in range(16):
   dst=M/'R4'/f'layer{i}';dst.mkdir(exist_ok=True);w=f.get_tensor(f'model.layers.{i}.mlp.down_proj.weight').double();wr=had(w)
   ...
   pack(code).tofile(dst/'down_weight_w4_hmx.bin');scale.numpy().astype('<f4').tofile(dst/'down_weight_w4_scale_f32.bin');assert np.array_equal(code.numpy(),unpack_w4_codes(dst,'down',2048,8192))
   ...
   q=load_qparams_bin(PARENT/f'layer{i}/qparams_u8.bin');alpha=max(abs(q['middle']['minimum']),abs(q['middle']['maximum']));sc=float(np.float32(alpha/127));q['middle'].update(scale=sc,zero_point=128,minimum=-128*sc,maximum=127*sc);write_qparams(dst/'qparams_u8.bin',q)
 ...
 for arm in MODES:
  p=M/arm;save(p/'manifest.json',dict(experiment='L32-0070',arm=arm,parent_manifest_sha256=sha256(PARENT/'manifest.json'),files={n:dict(bytes=(p/n).stat().st_size,sha256=sha256(p/n)) for n in mf}))
...
def reference():
 preflight();old=R.parent/'l32-0068/oracle-A8/summary.json';d=R/'oracle-A8';d.mkdir(exist_ok=True)
 ...
 mode='R4';p=M/mode;d=R/'oracle-R4';d.mkdir(exist_ok=True);fixture=read(FIXTURE);ids=fixture['prompt_ids'][:64]+fixture['fixed'][:42]
 ...
 for i in range(16):
  ...
  if f.exists():x=np.load(f)
   ...
   x,kv,diag=ref.layer(x,lp,q,co,si,None,sp2=False,r4=True);assert np.isfinite(x).all();np.save(f,x)
   if i==0:
 ...
 for arm in MODES:
  ...
  for start in range(0,len(names),24):
   ...
   for n in batch:
    ...
    if mf[n]['sha256']==parent[n]['sha256']:cmd.append('ln -s /data/local/tmp/llama32-htp/l32-0068/models/A8/'+n+' '+remote+'/'+n)
