#!/usr/bin/env python3
"""L32-0024 fixed ten three-arm exact R4 refinement cost comparison.
One warmup and one measured M64+decode replay per process, ten cycles.
...
R=OLD_R.parent/"l32-0024"
CONTROL=R.parent/"l32-0021"
...
def main():
 ...
 cycles_n=1 if a.short else 10;replays=2
 out=R/a.attempt;out.mkdir(exist_ok=False)
 head=subprocess.check_output(['git','-C',str(ROOT),'rev-parse','HEAD'],text=True).strip()
 cfg={}
 for arm in ['off','hmxboth','exactboth']:
  d=R/('timing-'+arm+'-a01');p=json.loads((d/'protocol.json').read_text());res=json.loads((d/'result.json').read_text());expected_head=head
  assert p['source_head']==head and len(res['steps'])==2 and all(x['pass_ideal_cosine'] and x['finite'] for x in res['steps'])
  audit=R/('validate-'+arm+'-a01');ar=json.loads((audit/'result.json').read_text())
  assert json.loads((audit/'protocol.json').read_text())['source_head']==head
  assert all(x['pass_actual_arithmetic'] and x['pass_ideal_cosine'] for x in ar['steps'])
 ...
 save(out/'protocol.json',dict(experiment='L32-0024',source_head=head,cycles=cycles_n,auxiliary=a.short,arms=cfg,order='cyclic OFF/HMXboth/exactboth; fixed ten cycles',scope='single layer0, M64 prefill + one decode past64; complete Host wall, no audit dumps; loading outside timer; no E2E extrapolation',inner_replays=replays,warmup_replays=1,seed=24024,bootstrap_samples=20000,gate='paired bootstrap95 upper<=1.10; actual-tail/component audits required; ideal-exact CLI failures retained'))
 ...
 for i in range(cycles_n):
  ...
  for arm in arms[i%len(arms):]+arms[:i%len(arms)]:
   ...
   assert z.returncode==(1 if arm=='hmxboth' else 0),(d,z.returncode)
 ...
 rng=np.random.default_rng(24024);idx=rng.integers(0,cycles_n,(20000,cycles_n));speed={};modules={}
 ...
 improvements={}
 for step,mode in enumerate(['prefill','decode']):
  x=np.array([c['hmxboth'][step]['host_wall_ns'] for c in cycles]);y=np.array([c['exactboth'][step]['host_wall_ns'] for c in cycles]);ci=np.quantile(y[idx].mean(1)/x[idx].mean(1),[.025,.975]);improvements[mode]=dict(latency_ratio=float(y.mean()/x.mean()),ci95=ci.tolist())
 save(out/'summary.json',dict(exact_vs_current_hmx=improvements,speed=speed,modules=modules,formal_cycles=0 if a.short else cycles_n,auxiliary=a.short,successful_rpc_boundaries=cycles_n*len(arms)*2*replays,e2e=None));print(json.dumps(speed),flush=True)
