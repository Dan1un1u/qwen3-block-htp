#!/usr/bin/env python3
"""L32-0012 same-build U8/SP2 all-layer M64+15 fixed ten rotated multi-arm cycles."""
...
OUT=Path('/mnt/d/llm_exp/results/llama32-htp/l32-0012/e2e-a01')
MODELS=Path('/mnt/d/llm_exp/models/llama32-htp')
REMOTE='/data/local/tmp/llama32-htp/l32-0012/e2e-a01'
def save(p,v):
...
def deploy():
 ...
 for mode in [5,6,7,8]:cfg['m'+str(mode)]={**cfg['sp2'],'command':cfg['sp2']['command'].replace('QBH_LLAMA_SP2=5 ','QBH_LLAMA_SP2='+str(mode)+' ')}
 del cfg['sp2']
 save(OUT/'protocol.json',dict(experiment='L32-0012',source_head=head,build_seal=seal,arms=cfg,cycles=10,order='rotated u8,m5,m6,m7,m8',prompt_tokens=64,decode_tokens=15,timing_scope='warm full-model Host wall: embedding,16 layers,final norm,LM head,greedy,FastRPC; excludes tokenizer/loading/session preparation',gate='paired bootstrap95 upper latency ratio <=1.10 in both modes',repeat1='auxiliary only'))
...
def formal():
 arms=['u8','m5','m6','m7','m8']
 for arm in arms:assert json.loads((OUT/('gate-'+arm)/'result.json').read_text())['pass_exact']
 cycles=[]
 for i in range(10):
  order=arms[i%len(arms):]+arms[:i%len(arms)];cycle={}
  for arm in order:cycle[arm]=execute(arm,f'formal-{i:02d}-{arm}')
  cycles.append(cycle);print('CYCLE_COMPLETE',i+1,flush=True)
 rng=np.random.default_rng(12012);idx=rng.integers(0,10,size=(20000,10));report={}
 for mode,tokens in [('prefill',64),('decode',15)]:
  timing={arm:np.array([c[arm][mode+'_ns'] for c in cycles],float) for arm in arms}
  report[mode]={'arms':{arm:dict(mean_ns=float(v.mean()),tokens_per_second=tokens*1e9/float(v.mean())) for arm,v in timing.items()},'comparisons':{}}
  for candidate,baseline in [(c,'u8') for c in arms[1:]]+[('m6','m5'),('m7','m6'),('m8','m7'),('m7','m5'),('m8','m5')]:
   a,b=timing[baseline],timing[candidate];ci=np.quantile(b[idx].mean(1)/a[idx].mean(1),[.025,.975]);ratio=float(b.mean()/a.mean())
   report[mode]['comparisons'][candidate+'_vs_'+baseline]=dict(latency_ratio=ratio,latency_increase_pct=100*(ratio-1),throughput_decrease_pct=100*(1-1/ratio),ci95=ci.tolist(),gate_pass=bool(ci[1]<=1.10))
...
if __name__=='__main__':
 ...
 if v.stage=='gate':
  for arm in ['u8','m5','m6','m7','m8']:execute(arm,'gate-'+arm)
