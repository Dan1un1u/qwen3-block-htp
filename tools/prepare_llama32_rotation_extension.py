...
def main():
 ...
 save(NEW/'manifest.json',dict(experiment='L32-0021',original=prov,baseline_manifest_sha256=sha256(BASE/'manifest.json'),calibration_freeze_sha256=sha256(DATA/'freeze.json'),layers=report))
if __name__=='__main__':
 ap=argparse.ArgumentParser();ap.add_argument('--layers',required=True);ap.add_argument('--attempt',required=True);a=ap.parse_args();selected_layers=[int(x) for x in a.layers.split(',')];assert len(set(selected_layers))==len(selected_layers) and all(0<=i<16 for i in selected_layers);NEW=NEW.parent/a.attempt;main()
