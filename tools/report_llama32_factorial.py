...
R=Path('/mnt/d/llm_exp/results/llama32-htp/l32-0029')
...
rng=np.random.default_rng(29029);idx=rng.integers(0,10,(20000,10));out={'experiment':'L32-0029','fullmodel':True,'fp32_residual':1,'rotation':False,'sp2':8,'PPL':None,'arms':{}}
for arm in ARMS:
 ps=[]
 for c in range(10):ps += [x for x in records(R/f'formal/{c:02d}-{arm}/stdout.txt') if x.get('record')=='generation_profile']
 assert len(ps)==1600
 out['arms'][arm]={}
...
lines=['# L32-0029：全模格式 × 流水消融','', '固定 Llama 16 层、M64+15、cache80、无旋转、SP2mode8、FP32 residual mode1。全程同一组权重、量化参数与整数算法。B00 模块接口/常规调度；B10 原生格式/常规调度；B01 模块接口/协同流水；B11 原生格式/协同流水。保留所有组的 GEMM 内部双缓冲。','', '格式范围为每层两处 Norm 输出和 SwiGLU→Down 接口；流水范围为 QKV 后处理、Gate/Up/SwiGLU、O/Down epilogue 与 O→Gate 预取。首尾实现相同且计入端到端时间。该消融不代表移除全部硬件优化。','', '各组通过独立 layers0/7/15、连续3层、全16层末端输出与封存参考比对及完整生成 token/selected-logit 检查。FP32残差固定；模型质量未验收，本次不测PPL。5轮short后10轮formal，四组循环换序；repeat10主结果，repeat1辅助。所有轮次保留。', '']
for mode in ['prefill','decode']:
...
lines += ['', '计时含 warm embedding、16层、finalnorm、head、greedy 与 FastRPC；不含CPU tokenizer、冷加载、ADB。Decode分母为15个连续token的平均Host wall。重叠计数不是可相加的模块时间；交互比小于1表示在墙钟比尺度上的互补，需结合CI解释。没有基线自动晋升。其余独立流水/SP2/softmax等批准阶段尚未完成。']
