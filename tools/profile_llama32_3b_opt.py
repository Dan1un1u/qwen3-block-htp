#!/usr/bin/env python3
"""L32-0042 fixed five short/ten paired formal cycles, frozen 3B trajectory."""
from execute_llama32_3b_opt import execute,read,save,R,BASE,adb,sha
from pathlib import Path
import shlex,sys
CONTROL='control-binaries-3b-l28-a02'
CANDIDATE='opt2-l28'
def verify_remote():
    evidence={}
...
def main():
    verify_remote()
    for arm,rt in [('CONTROL',CONTROL),('OPT2',CANDIDATE)]:
        execute(rt,'frontend-a01','warmup-'+arm,1,True)
    for phase,n in [('short',5),('formal',10)]:
        results=[]
        for cycle in range(n):
            row={}
            order=[('CONTROL',CONTROL),('OPT2',CANDIDATE)]
