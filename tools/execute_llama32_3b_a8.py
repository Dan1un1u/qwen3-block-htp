#!/usr/bin/env python3
"""L32-0046 no-performance-gate 3B optimization against frozen L32-0041 payloads."""
...
def stage(tag):
    ...
    d=R/tag;d.mkdir(exist_ok=False);remote='/data/local/tmp/llama32-htp/l32-0046/'+tag
...
def deploy(name):
    ...
    for n in names:
        ...
        target=remote+'/'+n;directory=target.rsplit('/',1)[0]
        if n in pm['files'] and pm['files'][n]['sha256']==mf['files'][n]['sha256']:
            links.append('mkdir -p '+shlex.quote(directory)+' && ln -s '+shlex.quote(parent['remote']+'/'+n)+' '+shlex.quote(target))
        else:
            adb('shell','mkdir -p '+shlex.quote(directory));adb('push',windows(p/n),target);pushed.append(n)
    ...
    save(R/('package-'+name+'.json'),dict(package=str(p),remote=remote,manifest_sha256=sha(p/'manifest.json'),layers=mf['layers'],sp2=sp2,pushed_files=pushed))
    if sp2:shutil.copyfile(R.parent/'l32-0044/frontend64-a01-teacher.json',R/(name+'-teacher.json'))
    print('DEPLOYED',name,flush=True)
def execute(runtime,name,tag,repeat=1,full=False):
    preflight();rt=read(R/runtime/'runtime.json');cfg=read(R/('package-'+name+'.json'));assert int(rt['seal']['layer_count'])==cfg['layers'];assert rt['seal']['model_size']=='3B';root=rt['remote'];p=Path(cfg['package']);d=R/tag;d.mkdir(exist_ok=False,parents=True)
    ...
    env.update(LD_LIBRARY_PATH=root,DSP_LIBRARY_PATH=root,ADSP_LIBRARY_PATH=root,QBH_LLAMA_SP2='8' if cfg.get('sp2') else '0',QBH_LLAMA_FP32_RESIDUAL='1',QBH_WIDE_SCORE='8',QBH_PAPER_FORMAT_DISABLE='0',QBH_PAPER_PIPELINE_DISABLE='0',QBH_DENSE_R3='0',QBH_DENSE_R4='0');env.pop('QBH_REPLAY_DUMP_DIR',None)
    ...
    if full:
        teacher=read(R/(name+'-teacher.json'));ids=teacher['prompt_ids'];steps=int(os.environ.get('QBH_3B_DECODE_COUNT','15'))+1;tokens=teacher['u8_generated_ids'][:steps];assert len(tokens)==steps
        if name.startswith('frontend64'):env.update(QBH_KV_CACHE_CAPACITY='128',QBH_GENERATION_EXPECTED_TOKENS='64')
        input_tokens=teacher.get('input_generated_ids',tokens)[:steps]
        row=[0,2 if os.environ.get('QBH_3B_GREEDY')=='1' else 3,steps]+ids+input_tokens;words=67+steps
