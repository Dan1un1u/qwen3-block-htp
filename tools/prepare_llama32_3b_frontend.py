...
def main(attempt):
    ...
    w,sc=_cached_w4_projection(str(dst.resolve()),'generation_lm_head',128256,H)
    ...
    old=Path('/mnt/d/llm_exp/models/llama32-htp/l32-0016/frontend-a02/generation_prompt_token_ids_u32.bin')
    ...
    gamma=np.fromfile(dst/'generation_final_norm_weight_f16.bin','<f2');cv=HmxU8Converter(ROOT/'build/l32-0003/qbh_hmx_u8_reference.so')
    ...
    for step in range(16):
        ...
        for i in range(28):
            ...
            print('FRONTEND_LAYER',step,i,flush=True)
    ...
    save(RES/(attempt+'-teacher.json'),dict(prompt_ids=ids.tolist(),u8_generated_ids=tokens,u8_selected_codes=codes,text=tokenizer.decode(tokens,skip_special_tokens=True),quality_accepted=False,recipe='fresh 3B GPTQ per-channel W4, minmax static A8, SP2 mode8, FP32 residual, no rotations',arithmetic='independent complete HMX conversion/integer attention/FP32 residual oracle',calibration_sha256=sha256(OUT/'calibration.json')))
    save(dst/'manifest.json',dict(experiment='L32-0041',model='Llama-3.2-3B-Instruct',layers=28,generation_tokens=16,prompt_tokens=64,cache_capacity=80,recipe='W4A8-SP2',rotation='OFF',fp32_residual=True,teacher_sha256=sha256(RES/(attempt+'-teacher.json')),files={str(p.relative_to(dst)):dict(bytes=p.stat().st_size,sha256=sha256(p)) for p in dst.rglob('*') if p.is_file()}))
