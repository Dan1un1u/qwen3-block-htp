...
def main():
    ...
    write(args.output/"reference_f16f16_block_output_f16.bin",x)
    ...
    manifest={"experiment":"L32-0001","recipe":"W16A16","layers":16,"generation_tokens":16,"prompt_tokens":64,"cache_capacity":80,"original":prov,"frontend_teacher_sha256":sha256(args.evidence/"teacher.json"),"dataset_freeze_sha256":sha256(args.evidence/"freeze.json"),"tied_head":"same original embedding tensor, separate row-major and HMX carriers","files":{str(f.relative_to(args.output)):{"bytes":f.stat().st_size,"sha256":sha256(f)} for f in sorted(args.output.rglob("*")) if f.is_file()}}
