"""L32-0033 fixed full-model factorial, independent frozen references."""
...
R=Path('/mnt/d/llm_exp/results/llama32-htp/l32-0033');OLD=R.parent/'l32-0018';M=Path('/mnt/d/llm_exp/models/llama32-htp/l32-0016')
ARMS={'LOG2':(0,0),'FP':(0,0)}
...
def preflight():
 ...
 assert 'ACTIVE_EXPERIMENT=L32-0033' in t

def prepare():
 preflight();prior=R.parent/'l32-0032/EVIDENCE_SHA256.json';assert sha(prior)=='a11ee12fd162617993b247e4b60074a815493205050517fa0643d8af0343f557'
 ...
 for key,name in [('l7','layer7-fp-a02'),('chain3','chain3-fp-a02'),('chain16','chain16-fp-a02'),('full','full-fp-a02')]:
  pkg=M.parent/'l32-0033'/name;manifest=read(pkg/'manifest.json');ref=read(R/(name+'-package.json'));assert sha(pkg/'manifest.json')==ref['manifest_sha256']
  parent=read(R/f'package-{key}.json');pm=read(Path(parent['package'])/'manifest.json');remote='/data/local/tmp/llama32-htp/l32-0033-models/'+name
  ...
  write(R/f'package-{key}-fp.json',dict(parent,package=str(pkg),manifest_sha256=sha(pkg/'manifest.json'),remote=remote,fresh_files=copies,immutable_links=len(links)))
...
def stage(count,trace=False):
 ...
 d=R/f'binaries-l{count}-a{attempt}';d.mkdir();remote=f'/data/local/tmp/llama32-htp/l32-0033-l{count}-a{attempt}'
 assert adb('shell','test ! -e '+remote,check=False).returncode==0;adb('shell','mkdir -p '+remote)
...
def execute(key,arm,tag,repeat=1,audit=False):
 ...
 env.update(QBH_LLAMA_SP2='8',QBH_WIDE_SCORE='7' if arm=='FP' else '0',LD_LIBRARY_PATH=root,DSP_LIBRARY_PATH=root,ADSP_LIBRARY_PATH=root,QBH_PAPER_FORMAT_DISABLE=str(ARMS[arm][0]),QBH_PAPER_PIPELINE_DISABLE=str(ARMS[arm][1]),QBH_DENSE_R3='0',QBH_DENSE_R4='0');env.pop('QBH_REPLAY_DUMP_DIR',None)
 ...
 if key=='full':
  ...
  if audit:
   for step in range(2):
    ...
    if arm=='LOG2':assert (d/name).read_bytes()==(Path(cfg['old'])/name).read_bytes(),(tag,step,'sealed output')
