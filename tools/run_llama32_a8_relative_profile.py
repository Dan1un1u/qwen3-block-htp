#!/usr/bin/env python3
"""L32-0005 matched three-arm A8 relative speed experiment."""
...
OLD=OUT.parent
REMOTE='/data/local/tmp/llama32-htp/l32-0005/profile-a01'
...
def main():
    ap=argparse.ArgumentParser(description=__doc__);ap.add_argument('stage',choices=['deploy','gate','formal','long']);args=ap.parse_args()
    if args.stage=='deploy':return deploy()
    preflight()
    if args.stage=='long':
        print(execute('w4a8','candidate','long-w4a8-candidate',long=True)['pass'],flush=True);return
    if args.stage=='gate':
    ...
    rng=np.random.default_rng(5005);idx=rng.integers(0,10,size=(20000,10));report={}
