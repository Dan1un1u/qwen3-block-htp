"""L32-0032 fixed full-model factorial, independent frozen references."""
...
R=Path('/mnt/d/llm_exp/results/llama32-htp/l32-0032');OLD=R.parent/'l32-0018';M=Path('/mnt/d/llm_exp/models/llama32-htp/l32-0016')
ARMS={'SP2':(0,0),'A8':(0,0)}
...
def preflight():
 ...
 assert 'ACTIVE_EXPERIMENT=L32-0032' in t

def prepare():
 preflight();prior=R.parent/'l32-0031/EVIDENCE_SHA256.json';assert sha(prior)=='660a885cbb7ea74b3533a40d9ad9c6a5dda71ed5724399b7c4e79c1a23696474'
 ...
 for key,name in [('l7','layer7-a8'),('chain3','chain3-a8'),('chain16','chain16-a8'),('full','full-a8')]:
  pkg=M.parent/'l32-0032'/name;manifest=read(pkg/'manifest.json');ref=read(R/(name+'-package.json'));assert sha(pkg/'manifest.json')==ref['manifest_sha256']
  parent=read(R/f'package-{key}.json');pm=read(Path(parent['package'])/'manifest.json');remote='/data/local/tmp/llama32-htp/l32-0032-models/'+name
  assert adb('shell','test ! -e '+remote,check=False).returncode==0;adb('shell','mkdir -p '+remote)
  names=list(manifest['files']);dirs=sorted({str(Path(n).parent) for n in names if str(Path(n).parent)!='.'});adb('shell','mkdir -p '+' '.join(shlex.quote(remote+'/'+n) for n in dirs))
  links=[];copies=[]
  ...
  names=[n for n in names if not n.endswith('.npy')]
  for i in range(0,len(names),32):
   lines=adb('shell','sha256sum '+' '.join(shlex.quote(remote+'/'+n) for n in names[i:i+32])).stdout.splitlines();assert len(lines)==len(names[i:i+32])
   for line in lines:
    h,n=line.split(None,1);assert h==manifest['files'][n.removeprefix(remote+'/')]['sha256'],n
  write(R/f'package-{key}-a8.json',dict(parent,package=str(pkg),manifest_sha256=sha(pkg/'manifest.json'),remote=remote,fresh_files=copies,immutable_links=len(links)))
  print('VERIFIED ordinary',key,flush=True)
 write(R/'oracle_freeze.json',{mode:sha(R/(mode+'-a8-teacher.json')) for mode in ['greedy','fixed']})
...
def stage(count,trace=False):
 ...
 d=R/f'binaries-l{count}-a{attempt}';d.mkdir();remote=f'/data/local/tmp/llama32-htp/l32-0032-l{count}-a{attempt}'
 assert adb('shell','test ! -e '+remote,check=False).returncode==0;adb('shell','mkdir -p '+remote)
...
def execute(key,arm,tag,repeat=1,audit=False):
 preflight();cfg=read(R/f"package-{key}{'-a8' if arm=='A8' else ''}.json");state=read(R/f"runtime-l{cfg['layers']}.json");root=state['remote'];assert audit or state['seal']['paper_trace'] is False;d=R/tag;d.mkdir(parents=True,exist_ok=False)
 ...
 env.update(QBH_LLAMA_SP2='8' if arm=='SP2' else '0',LD_LIBRARY_PATH=root,DSP_LIBRARY_PATH=root,ADSP_LIBRARY_PATH=root,QBH_PAPER_FORMAT_DISABLE=str(ARMS[arm][0]),QBH_PAPER_PIPELINE_DISABLE=str(ARMS[arm][1]),QBH_DENSE_R3='0',QBH_DENSE_R4='0');env.pop('QBH_REPLAY_DUMP_DIR',None)
 ...
 if key=='full':
  oracle=(read(R.parent/'l32-0016/frontend-a02-reference/teacher.json') if arm=='SP2' else dict(zip(['u8_generated_ids','u8_selected_codes'],(lambda z:(z['ids'],z['codes']))(read(R/(('fixed' if os.environ.get('QBH_PAPER_FIXED_TOKENS')=='1' else 'greedy')+'-a8-teacher.json'))))));steps=[v for v in rs if isinstance(v,dict) and 'selected_logit_half_bits' in v]
 ...
 for mode in ['greedy','fixed']:
  ...
  for phase,cycles in [('short',5),('formal',10)]:
   ...
   for c in range(cycles):
    for a in (keys if c%2==0 else keys[::-1]):out.append(dict(cycle=c,**execute('full',a,f'{mode}-{phase}/{c:02d}-{a}',repeat=10)))
