#!/usr/bin/env python3
"""L32-0063 vector FP intermediates: component/chain correctness and profiling."""
...
from probe_llama32_av_o_fullmodel import fnv,FIXTURE,BASE
...
R=Path('/mnt/d/llm_exp/results/llama32-htp/l32-0063')
P=Path('/mnt/d/llm_exp/models/llama32-htp/l32-0062/control')
REMOTE='/data/local/tmp/llama32-htp/l32-0063'
...
def component():
 preflight();binary=REMOTE+'/binaries-16';results=[]
 for i in range(16):
...
def reference():
 ...
 emb=np.memmap(P/'generation_embedding_weight_f16.bin','<f2','r',shape=(128256,2048));x=np.array(emb[ids],dtype='f4')
 co=np.concatenate([np.fromfile(P/'rope_cos_f16.bin','<f2').reshape(64,64),np.fromfile(P/'generation_decode_rope_cos_00_f16.bin','<f2').reshape(64,64)[:1]])
 si=np.concatenate([np.fromfile(P/'rope_sin_f16.bin','<f2').reshape(64,64),np.fromfile(P/'generation_decode_rope_sin_00_f16.bin','<f2').reshape(64,64)[:1]])
 ...
 for i in range(16):
  ...
  if i+1 in [1,3,16]:
   ...
   if hfile.exists():heads[str(i+1)]=read(hfile)
    ...
    a=ref.norm(x[63:],gamma,gq['generation_final_norm_output']);logits=project_w4u8(a,P,'generation_lm_head',128256,2048,gq['generation_final_norm_output'],gq['generation_lm_head_output'],cv)
...
def execute(tag,repeat=1,audit=False,nl=16,serial=False):
