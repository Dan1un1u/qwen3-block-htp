#!/usr/bin/env python3
"""Summarize exclusive L32-0064 phase fractions without adding worker timers."""
import csv,json,sys
from pathlib import Path
import numpy as np
from report_llama32_pipeline_profile import MODULES
from profile_fp_islands_3b import R,save

def main():
 result={};rows=[];rng=np.random.default_rng(63);idx=rng.integers(0,10,(10000,10))
 for arm,prefix in [('operator_boundaries','phases-')]:
  ...
  for step,mode in enumerate(['prefill','decode_one_step']):
   ...
   if arm=='operator_boundaries' and step==0:
    ...
    for name,value in plot.items():rows.append(dict(model='Llama3.2-3B',prompt=64,schedule=arm,component=name,us=value,percent=100*value/h))
 ...
 with (R/'figure-b-llama3b.csv').open('w',newline='') as f:
  w=csv.DictWriter(f,fieldnames=list(rows[0]));w.writeheader();w.writerows(rows)
 text=['# L32-0064 full additive modules','', 'Microseconds; percentages use complete Host wall. Prefill M64, one subsequent decode for correctness/diagnostics. Softmax worker work is overlapping, excluded from this ledger.','']
 text+=['| Module | Operator-boundary prefill |','|---|---:|']
 for name in result['operator_boundaries']['prefill']['modules_us']:
  cells=[]
  for arm in result:
   v=result[arm]['prefill'];cells.append(f"{v['modules_us'][name]:.2f} ({v['modules_percent'][name]:.2f}%)")
  text.append('| '+name+' | '+' | '.join(cells)+' |')
 text+=['','The column includes embedding, all 28 blocks, final norm, LM head, greedy and FastRPC. They are diagnostic schedules, not an optimized-vs-baseline speedup experiment.','']
