#!/usr/bin/env python3
"""L32-0062: paired full-model AV scale folding, independent actual arithmetic."""
...
M=Path('/mnt/d/llm_exp/models/llama32-htp/l32-0062')
R=Path('/mnt/d/llm_exp/results/llama32-htp/l32-0062')
REMOTE='/data/local/tmp/llama32-htp/l32-0062'
BASE=Path('/mnt/d/llm_exp/results/llama32-htp/l32-0060/1B/sp2/base.json')
FIXTURE=Path('/mnt/d/llm_exp/results/llama32-htp/l32-0057/1B/sp2/fixture.json')
...
def manifest(p,**extra):
    save(p/'manifest.json',dict(experiment='L32-0062',**extra,files={str(f.relative_to(p)):dict(bytes=f.stat().st_size,sha256=sha256(f)) for f in p.rglob('*') if f.is_file() and f.name!='manifest.json'}))

def prepare():
    preflight();parent=M/'recovered-parent';old=read(parent/'manifest.json')
    ...
    audits=[]
    for i in range(16):
        a=make_int16(parent/f'layer{i}',ctl/f'layer{i}');a['layer']=i;audits.append(a)
        print('INT16',i,flush=True)
    save(R/'int16-audit.json',audits)
    ...
    for i in range(16):
        ...
        w=unpack_w4_codes(p,'o',2048,2048).astype('i8')
...
def reference(arm):
    ...
    cs=[];ss=[]
    cs.append(np.fromfile(p/'rope_cos_f16.bin','<f2').reshape(64,64));ss.append(np.fromfile(p/'rope_sin_f16.bin','<f2').reshape(64,64))
    for j in range(42):
        cs.append(np.fromfile(p/f'generation_decode_rope_cos_{j:02d}_f16.bin','<f2').reshape(64,64)[:1])
        ss.append(np.fromfile(p/f'generation_decode_rope_sin_{j:02d}_f16.bin','<f2').reshape(64,64)[:1])
    ...
    for i in range(16):
        ...
        if info.exists():
            ...
            x,kv,diag=ref.layer(x,lp,q,co,si,None,sp2=True)
            np.save(out,x)
        ...
        if i+1 in [3,16]:
            ...
            if hf.exists():heads[str(i+1)]=read(hf)
                ...
                logits=project_w4u8(act,p,'generation_lm_head',128256,2048,gq['generation_final_norm_output'],gq['generation_lm_head_output'],cv)
...
def deploy():
    preflight();base=read(BASE);parent=read(M/'recovered-parent/manifest.json')
    for arm in ['control','folded']:
        ...
        for start in range(0,len(names),24):
            ...
            for n in batch:
                ...
                if mf[n]['sha256']==parent['files'][n]['sha256']:commands.append('ln -s '+shlex.quote(base['remote']+'/'+n)+' '+shlex.quote(remote+'/'+n))
...
def execute(arm,tag,nl=16,repeat=1,audit=False):
    d=R/tag
...
def run():
    preflight()
    for arm in ['control','folded']:execute(arm,'audit16-'+arm,audit=True)
    ...
    for stage_name,n in [('short',5),('formal',10)]:
        ...
        rng=np.random.default_rng(620062);idx=rng.integers(0,n,(20000,n));summary={}
