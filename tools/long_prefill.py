"""L32-0050 isolated long-prefill fixtures, build deployment and immutable runs."""
...
R=Path('/mnt/d/llm_exp/results/llama32-htp/l32-0050')
M=Path('/mnt/d/llm_exp/models/llama32-htp/l32-0050')
BASE=R.parent/'l32-0040/package-full.json'
REMOTE='/data/local/tmp/llama32-htp/l32-0050'
...
def preflight():
 ...
 assert 'ACTIVE_EXPERIMENT=L32-0050' in z
def prepare():
 preflight()
 import torch
 ...
 cfg=read(original/'config.json');cfg['head_dim']=64
 ...
 ids=ids[:768];fixed=read(R.parent/'l32-0016/frontend-a02-reference/teacher.json')['u8_generated_ids']
 fixed=(fixed*4)[:64]
 cos,sin=rope(cfg,torch.arange(832,device='cuda')[None],torch.float16)
 cos=cos.cpu().numpy().reshape(832,64);sin=sin.cpu().numpy().reshape(832,64)
 assert np.array_equal(cos[:64],np.fromfile(src/'rope_cos_f16.bin','<f2').reshape(64,64))
 assert np.array_equal(sin[:64],np.fromfile(src/'rope_sin_f16.bin','<f2').reshape(64,64))
 ...
 for length in [64,65,128,129,536,741]:
  ...
  for n,v in mf['files'].items():
   ...
   if 'kv_cache_' in Path(n).name:
    a=np.fromfile(src/n,'u1').reshape(8,80,64);z=np.zeros((8,832,64),'u1');z[:,:80]=a;z.tofile(target);changed.append(n)
  ...
  put(dst/'manifest.json',dict(experiment='L32-0050',parent=base,parent_sha256=base['manifest_sha256'],files=files,changed=changed))
