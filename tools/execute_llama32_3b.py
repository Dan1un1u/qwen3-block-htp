#!/usr/bin/env python3
"""Immutable L32-0041 staged native gates and full-model profiling."""
import sys,os,json,shlex,shutil,subprocess,struct,statistics
from pathlib import Path
import numpy as np
from prepare_llama32_3b import ROOT as S,OUT as M,RES as R,preflight,save
...
def stage(tag):
    ...
    d=R/tag;d.mkdir(exist_ok=False);remote='/data/local/tmp/llama32-htp/l32-0041/'+tag
...
def deploy(name):
    preflight();p=M/name;mf=read(p/'manifest.json');names=[n for n in mf['files'] if not n.endswith('.npy')]
    remote='/data/local/tmp/llama32-htp/l32-0041/models/'+name
...
def execute(runtime,name,tag,repeat=1,full=False):
    ...
    if full:
        teacher=read(R/(name+'-teacher.json'));ids=teacher['prompt_ids'];tokens=teacher['u8_generated_ids'];row=[0,2 if os.environ.get('QBH_3B_GREEDY')=='1' else 3,16]+ids+tokens
    ...
    ps=[v for v in rs if isinstance(v,dict) and v.get('record')==('generation_profile' if full else 'replay_profile')];assert len(ps)==repeat*(16 if full else 2),(tag,len(ps))
    for v in ps:
    ...
    if full:
        steps=[v for v in rs if isinstance(v,dict) and 'selected_logit_half_bits' in v];assert [v['selected_token_id'] for v in steps]==tokens*repeat;assert [v['selected_logit_half_bits'] for v in steps]==teacher['u8_selected_codes']*repeat
    else:
