#!/usr/bin/env python3
"""Reconcile the fixed matched L32-0005 three-arm Host/DSP ledgers."""
import json
from pathlib import Path
import numpy as np
from report_llama32_pipeline_profile import MODULES
OUT=Path('/mnt/d/llm_exp/results/llama32-htp/l32-0005')
...
def main():
    ...
    report={'experiment':'L32-0005','formal_cycles':10,'verified_token_boundaries':checked,'qtimer_ticks_per_us':19.2,'speed':speed,'modules':data,'quality':'Unchanged sealed weights/qparams. W4A16 prior PPL gate failed; A8 text unusable without quality gate. No PPL rerun or quality promotion.'}
    assert checked==240 and not (OUT/'profiling_summary.json').exists()
    (OUT/'profiling_summary.json').write_text(json.dumps(report,ensure_ascii=False,indent=2)+'\n')
    lines=['# L32-0005: native A8 relative speed','', 'SM8750 / HTP V79. Fixed ten ABC/BCA/CAB cycles; identical original M64 prompt and seven continuous decode tokens for all three arms. Quantized recipe outputs differ; input dimensions and KV lengths match.','']
    for mode in ['prefill','decode']:
    ...
    lines+=['','Complete warm Host wall includes embedding,16 layers,final norm,LM head,greedy and FastRPC. Model loading/session preparation and external tokenizer are excluded. No layer extrapolation. All240 additive ledgers reconcile.','']
    for mode,x in speed.items():
