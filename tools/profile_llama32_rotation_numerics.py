#!/usr/bin/env python3
"""L32-0022 fixed ten three-arm warmed single-layer Host-wall comparison.
...
def main():
 ...
 cfg={}
 for arm in ['off','both','oldboth']:
  base='both' if arm=='oldboth' else arm;origin=CONTROL if arm=='oldboth' else R
  expected_head='5adc46b774a23df9138f1070560fb68a28c165a1' if arm=='oldboth' else head
  suffix='a03' if arm=='oldboth' else 'a01'
  ...
  if arm=='oldboth':
   ledger=CONTROL/'evidence-ledger-checkpoint-a01.json';assert sha256(ledger)=='fd22398af6ddf91ce6fcf79a70601364b1f1bc59e173a9f49bec948a8ea2b1b9'
 ...
 save(out/'protocol.json',dict(experiment='L32-0022',source_head=head,cycles=cycles_n,auxiliary=a.short,arms=cfg,order='cyclic shift of OFF/both/oldboth',scope='single layer0, M64 prefill + one decode past64; complete Host wall, no audit dumps; loading outside timer; no E2E extrapolation',inner_replays=replays,warmup_replays=1,seed=22022,bootstrap_samples=20000,gate='paired bootstrap95 upper<=1.10; actual-tail/component audits required; ideal-exact CLI failures retained'))
 ...
 rng=np.random.default_rng(22022);idx=rng.integers(0,cycles_n,(20000,cycles_n));speed={};modules={}
 ...
 save(out/'summary.json',dict(improvement_vs_sealed0021=improvements,speed=speed,modules=modules,formal_cycles=0 if a.short else cycles_n,auxiliary=a.short,successful_rpc_boundaries=cycles_n*len(arms)*2*replays,e2e=None));print(json.dumps(speed),flush=True)
