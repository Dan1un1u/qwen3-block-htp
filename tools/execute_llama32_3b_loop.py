#!/usr/bin/env python3
"""L32-0043 exact 3B optimization against frozen L32-0041 payloads."""
...
def stage(tag):
    ...
    d=R/tag;d.mkdir(exist_ok=False);remote='/data/local/tmp/llama32-htp/l32-0043/'+tag
...
def execute(runtime,name,tag,repeat=1,full=False):
    preflight();rt=read(R/runtime/'runtime.json');cfg=read((R if name=='layer0-a01' else BASE)/('package-'+name+'.json'));assert int(rt['seal']['layer_count'])==cfg['layers'];assert rt['seal']['model_size']=='3B';root=rt['remote'];p=Path(cfg['package']);d=R/tag;d.mkdir(exist_ok=False,parents=True)
