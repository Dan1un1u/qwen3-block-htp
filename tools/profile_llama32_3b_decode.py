#!/usr/bin/env python3
"""L32-0044 fixed five short/ten paired formal cycles, frozen 3B trajectory."""
from execute_llama32_3b_decode import execute,read,save,R,BASE,adb,sha
...
def main():
    verify_remote()
    os.environ['QBH_3B_DECODE_COUNT']='42'
    for arm,rt in [('CONTROL',CONTROL),('OPT',CANDIDATE)]:
        os.environ['QBH_3B_HEAD_TILES']='32' if arm=='OPT' else '8'
        execute(rt,'frontend64-a01','warmup-'+arm,1,True)
    for phase,n in [('short',5),('formal',10),('supplement63',5)]:
        os.environ['QBH_3B_DECODE_COUNT']='63' if phase=='supplement63' else '42'
        results=[]
        for cycle in range(n):
            row={}
            order=[('CONTROL',CONTROL),('OPT',CANDIDATE)]
            if cycle%2:order.reverse()
            for arm,rt in order:
                os.environ['QBH_3B_HEAD_TILES']='32' if arm=='OPT' else '8'
                row[arm]=execute(rt,'frontend64-a01',f'{phase}/{cycle:02d}-{arm}',10,True)
            results.append(row)
