#!/usr/bin/env python3
"""Llama head64/GQA4 actual HMX reference; generic projection math derived from the frozen runtime oracle."""
...
def exact_rms_norm_u8(
    ...
    denominator = np.float32(mean_square + np.float32(1.0e-5))
    inverse = np.float32(
...
def exact_attention_prefill(
    ...
    if rows != 64 or head_dim != 64 or heads != 4 * kv_heads:
        raise ValueError(
...
def exact_attention_dynamic(
    ...
    ``[kv_heads, valid_tokens, 64]``.  Only valid logical columns are
