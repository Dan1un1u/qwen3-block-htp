#!/usr/bin/env python3
"""L32-0020 fixed ten four-arm warmed single-layer Host-wall comparison.
...
def main():
 ...
 for arm in ['off','r3','r4','both']:
  ...
  audit=R/('validate-layer0-'+arm+'-a01')
 ...
 save(out/'protocol.json',dict(experiment='L32-0020',source_head=head,cycles=cycles_n,auxiliary=a.short,arms=cfg,order='cyclic shift of OFF/R3/R4/both',scope='single layer0, M64 prefill + one decode past64; complete Host wall, no audit dumps; loading outside timer; no E2E extrapolation',inner_replays=replays,warmup_replays=1,seed=20020,bootstrap_samples=20000,gate='paired bootstrap95 upper<=1.10; actual-tail/component audits required; ideal-exact CLI failures retained'))
 ...
 rng=np.random.default_rng(20020);idx=rng.integers(0,cycles_n,(20000,cycles_n));speed={};modules={}
 ...
 save(out/'summary.json',dict(speed=speed,modules=modules,formal_cycles=0 if a.short else cycles_n,auxiliary=a.short,successful_rpc_boundaries=cycles_n*4*2*replays,e2e=None));print(json.dumps(speed),flush=True)
