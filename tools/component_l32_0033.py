...
S=Path('/home/daniuniu/work/llama32-htp');R=Path('/mnt/d/llm_exp/results/llama32-htp/l32-0033');F=R/'component-fixtures'
...
def preflight():
 x=subprocess.check_output(['python3',str(S)+'-project-memory/scripts/project_memory.py','preflight','--source-worktree',str(S)],text=True);assert 'ACTIVE_EXPERIMENT=L32-0033' in x
...
def stage(count):
 ...
 archive=R/f'binaries-l1-a{attempt}';archive.mkdir();remote=f'/data/local/tmp/llama32-htp/l32-0033-l1-a{attempt}';assert adb('shell','test ! -e '+remote,check=False).returncode==0;adb('shell','mkdir -p '+remote)
