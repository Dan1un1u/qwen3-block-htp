...
remote_root="${EXP0090_REMOTE_ROOT:-/data/local/tmp/qwen3-block-htp/exp0090}"
queue_mode="${1:-0}"
repeat_count="${2:-1}"
attribution_mode="${3:-on}"
audit_mode="${4:-off}"

if [[ "${queue_mode}" != "0" && "${queue_mode}" != "1" ]]; then
    printf 'usage: %s 0|1 [repeat] [attribution] [audit]\n' "$0" >&2
    exit 2
fi

runtime_args="2 32 rms_rope_softmax ${attribution_mode} ${audit_mode} fused_pool6 serial control hvx w4u8_streaming_persistent_mlp_hvx 3 64 u8_log2_gqa_qkv_overlap_vgather_vdeal_fused_qk_requant_hmx_batch_lut_templates_gqa_batch_dependency_stream 6 w4u8_mlp_io_qkv_o qkvo_batch4_qk_head_pairs hvx_tree_qk_batched_rsqrt_shared_rope_parallel_input control 4 4 ${queue_mode}"
