...
def main():
 ...
 histw4=read(R.parent/'exp0291/MODULES.json');histsp=read(R.parent/'exp0288/MODULES.json')['means']
 # Current W4A16 retained decode-best diagnostic, explicitly not a paired formal arm.
 w4key=next(k for k in histw4 if False) if False else None
 print('W4 historical keys',list(histw4))
 summary=dict(experiment='EXP-0292',execution_state='completed',shape='Qwen3-0.6B full28 batch1 M64+42 cache128',measured_source=read(R/'runtime-l28.json')['seal']['source_head'],short_rounds=5,formal_rounds=10,repeat=10,formal_profiles=8600,performance=perf,comparison=comparison,residual_adds_bit_exact=True,changed_norm_max_half_ulp=1,original_control_byte_exact_files=2451,repeat_candidate_byte_exact_files=2451,cache_files=4816,full_floating_alignment_pass=False,full_floating_nrmse_max={c:max(z['nrmse'] for z in read(R/('audit/'+c+'/validated.json'))['independent_output']) for c in ['fp16','fp32']},head_boundary_pass=read(R/'head-boundary-checks.json')['pass_all'],quality_claim=False,baseline_promoted=False)
...
"""
 ...
 report=['# EXP0292 FULL PROFILING REPORT',notes,et,json.dumps(summary,indent=2)]
 aux={}
 ...
 for mode in ['prefill','decode']:
  ...
  for k in sorted(set(a)&set(b)&set(x)&set(y)):
   ...
   rows.append([k,f'{x[k]:.6f}',f'{y[k]:.6f}',f'{a[k]:.6f}',f'{b[k]:.6f}',f'{am:.6f}',f'{bm:.6f}',f'{100*(bm/am-1):+.3f}%' if am else 'N/A zero denominator'])
  report+=['## '+mode,table(['field','FP16 residual R1','FP32 residual R1','FP16 R10 mean','FP32 R10 mean','FP16 R10 median','FP32 R10 median','median delta'],rows)]
