...
"${project_root}/scripts/build_exp0198.sh" > "${result_dir}/build.log" 2>&1
"${project_root}/scripts/check_exp0198_static.sh" > "${result_dir}/static_gate.json"
"${project_root}/scripts/deploy_exp0198.sh" > "${result_dir}/deploy.log" 2>&1

for cell in independent continuous; do
    QBH_EXP0173_GENERATION_STEPS=4 \
    QBH_GENERATION_BOUNDARY_AUDIT=1 \
    QBH_GENERATION_AUDIT_DIR="${result_dir}/audit/${cell}" \
        "${project_root}/scripts/run_exp0198.sh" "${cell}" \
...
for round in $(seq 1 5); do
    ...
    if ((round % 2 == 1)); then
        order=(independent continuous)
    else
        order=(continuous independent)
    fi
...
"${python_exe}" "${project_root}/scripts/summarize_exp0198.py" \
