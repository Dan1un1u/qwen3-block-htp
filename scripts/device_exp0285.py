...
REMOTE='/data/local/tmp/qwen3-block-htp/exp0285'
...
def run(package,tag,count=1,repeat=1,fp32=1,dump=False):
 ...
 if dump and fp32:
  errs=[]
  for step in range(2):
   rows=64 if not step else 1;a=np.fromfile(d/f'step{step:02d}_output.bin','<f2' if fp32==3 else '<f4').reshape(rows,2048);b=np.fromfile(package_path(package)/(('reference_w4u8_block_output_f16.bin' if not step else 'replay_decode_reference_00_f16.bin') if fp32==3 else ('reference_w4u8_block_output_f32.bin' if not step else 'replay_decode_reference_00_f32.bin')),'<f2' if fp32==3 else '<f4').reshape(64,2048)[:rows];delta=a.astype('f8')-b
