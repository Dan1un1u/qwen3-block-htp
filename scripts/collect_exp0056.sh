...
package="${QBH_EXP0056_PACKAGE:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0042/block_package_layer14_m64_integer_attention_parallel}"
result_root="${QBH_EXP0056_RESULT_ROOT:-/mnt/d/llm_exp/results/qwen3-block-htp/exp0056}"
artifact_root="${QBH_EXP0056_ARTIFACT_ROOT:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0056/artifacts}"
...
"${project_root}/scripts/build_exp0056.sh" \
    > "${result_dir}/build.log" 2>&1
"${project_root}/scripts/check_exp0056_static.sh" \
    > "${result_dir}/static_gate.json"
QBH_EXP0056_PACKAGE="${package}" \
    "${project_root}/scripts/deploy_exp0056_block.sh" \
...
for mode in control candidate; do
    "${project_root}/scripts/run_exp0056.sh" \
...
for round in 1 2 3 4 5 6 7; do
    printf 'EXP-0056 paired round %d/%d\n' "${round}" "${paired_rounds}"
    if (( round % 2 == 1 )); then
        repeats=(1 10)
...
python3 "${project_root}/scripts/validate_exp0056.py" \
...
{
    printf 'experiment=EXP-0056\n'
    printf 'source_branch=%s\n' "${source_branch}"
    printf 'source_head=%s\n' "${source_head}"
    printf 'execution_unit=qwen3_layer14_complete_block_m64\n'
    printf 'control=EXP0055-independent-softmax-rows\n'
    printf 'candidate=paired-rows-independent-normalization-banked-vlut32\n'
