...
remote_root="${REMOTE_ROOT:-/data/local/tmp/qwen3-block-htp/exp0001}"
vector_length="${1:-65536}"
addend="${2:-29}"
...
"${adb_exe}" push "$(wslpath -w "${host_executable}")" \
    "${remote_root}/qwen3_probe_cli" >/dev/null
...
"${adb_exe}" shell \
    "cd ${remote_root} && LD_LIBRARY_PATH=${remote_root} DSP_LIBRARY_PATH=${remote_root} ADSP_LIBRARY_PATH=${remote_root} ./qwen3_probe_cli ${vector_length} ${addend}"
