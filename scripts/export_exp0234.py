...
from data_exp0234 import RESULT,OUTPUT,MODEL,SOURCE,sha,write,preflight,verified

BASE_PACKAGE=OUTPUT.parent/'exp0224/A'
BASE_HASH='a5de4e6c4e02ac913e69fbddb0d4b0b9e12b5cfe88ff606cf1ea18842dc0c179'

def frozen():
    from data_exp0234 import frozen as inputs
    d=inputs();assert sha(BASE_PACKAGE/'manifest.json')==BASE_HASH
    return d

def prepare(v):
    preflight();frozen();data=json.loads(verified('exp0230','dataset.json').read_text());v='G64';root=OUTPUT/v;result=RESULT/v
    ...
    manifest=dict(experiment='EXP-0234',variant=v,format=group.FORMAT,groupsize=128,
        ...
        calibration_sha256=sha(verified('exp0230','inputs/C64_calibration_u32.bin')),calibration_tokens=65536,
    ...
    write(f'{v}/package.json',dict(manifest_sha256=sha(root/'manifest.json'),original_shards=origin,
        elapsed_s=time.monotonic()-started,calibration_tokens=65536,format=group.FORMAT,
        quantizer_source={n:sha(SOURCE/'scripts'/n) for n in ['gptq_exp0221.py','clipping_exp0223.py','group_exp0234.py','export_exp0234.py']},
...
if __name__=='__main__':
    ...
    p=argparse.ArgumentParser();p.add_argument('variant',nargs='?',default='G64',choices=['G64']);a=p.parse_args();prepare(a.variant)
