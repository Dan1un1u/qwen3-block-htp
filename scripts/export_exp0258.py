...
import json,os,shutil,copy,math,sys
import numpy as np
import torch
...
def main():
 preflight();R.mkdir(exist_ok=True);O.mkdir(exist_ok='--resume-prefix' in sys.argv)
 ...
 root=O/'r3'
 if '--resume-prefix' in sys.argv:
  assert (root/'manifest.json').exists() and not (R/'export_audit.json').exists()
  new=read(root/'manifest.json');diff=[n for n in old['files'] if old['files'][n]!=new['files'][n]]
  assert new['parent_manifest_sha256']==sha(OLD/'package/manifest.json') and new['parameters_sha256']==sha(P/'parameters/R3_ALL.json')
  for n,v in new['files'].items():assert sha(root/n)==v['sha256']
 else:
 ...
 # FP32 GEMM association may cross a FP16 midpoint; the stored U8 seed must match the independent Float64 reference exactly.
 fp16_rounding_differences=int(np.sum(rot!=oracle))
 payload=[]
 for i in range(28):
  for key,value in [('k_cache',rot[i]),('v_cache',raw[i,1])]:
   q=qp(b[f'L{i:02d}.{key}']['mse']);u=qdqcode(value,q);ref=np.clip(np.floor(value.astype(np.float32)/np.float32(q['scale'])+q['zero_point']+.5),0,255).astype('u1');assert np.array_equal(u,ref);
   if key=='k_cache':assert np.array_equal(u,qdqcode(oracle[i],q)),('stored prefix U8 independent reference',i)
   payload.append(u.tobytes())
