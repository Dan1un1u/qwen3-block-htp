...
set -u
result_root="${QBH_RESULT_ROOT:-/mnt/d/llm_exp/results/qwen3-block-htp/exp0004}"
artifact_root="${QBH_ARTIFACT_ROOT:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0004}"
...
"${project_root}/scripts/build_exp0004.sh" \
    > "${result_dir}/build.log" 2>&1
QBH_STATIC_OUTPUT_DIR="${result_dir}/static" \
    "${project_root}/scripts/check_exp0004_static.sh" \
...
for projection in gate_up down; do
    for pattern in identity signed structured boundary; do
        "${project_root}/scripts/run_exp0004.sh" \
            "${projection}" "${pattern}" 1 \
