...
result_root="${QBH_RESULT_ROOT:-/mnt/d/llm_exp/results/qwen3-block-htp/exp0032}"
artifact_root="${QBH_ARTIFACT_ROOT:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0032/artifacts}"
...
"${project_root}/scripts/build_exp0032.sh" > "${result_dir}/build.log" 2>&1
QBH_STATIC_OUTPUT_DIR="${result_dir}/static" \
    "${project_root}/scripts/check_exp0032_static.sh" \
...
configs=(f16_b2 f16_b4 w4_control w4_b4_dma4 w4_b4_dma8_cache)
...
set_config() {
    ...
    case "${config}" in
        ...
        f16_b4)
            variant=F16F16; workers=2; f16_mode=gate4; w4_mode=control ;;
        w4_control)
            variant=W4F16; workers=3; f16_mode=serial
            w4_mode=adaptive_down96_cross ;;
        w4_b4_dma4)
            variant=W4F16; workers=3; f16_mode=serial
            w4_mode=adaptive_down96_gate4_cross ;;
        w4_b4_dma8_cache)
            variant=W4F16; workers=3; f16_mode=serial
...
for round in 1 2 3 4 5; do
    if (( round % 2 == 1 )); then
        ...
        order=(w4_b4_dma8_cache w4_b4_dma4 w4_control f16_b4 f16_b2)
    fi
...
python3 "${project_root}/scripts/validate_exp0032_evidence.py" \
...

{
    ...
    printf 'gate_up_cache_bytes=98304\n'
