...
from experiment_exp0220 import RESULT,OUTPUT,BASE,VARIANTS
...
REMOTE='/data/local/tmp/qwen3-block-htp/exp0220-'
...
def run(variant,phase,index=None):
    ...
    write_json(path.with_suffix('.provenance.json'),dict(remote_root=REMOTE+variant,variant=variant,experiment='EXP-0220'))
...
def main():
    ...
    variants=[a.variant] if a.variant else (VARIANTS if a.phase in ['quick','full','repeat'] else ['original','r2_only'])
