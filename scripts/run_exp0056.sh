...
remote_root="${EXP0056_REMOTE_ROOT:-/data/local/tmp/qwen3-block-htp/exp0056}"
mode="${1:-control}"
repeat_count="${2:-1}"
...
case "${mode}" in
    control) attention_mode=u8_log2_gqa_qkv_overlap ;;
    candidate) attention_mode=u8_log2_gqa_qkv_overlap_paired_softmax ;;
    *)
