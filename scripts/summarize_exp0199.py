#!/usr/bin/env python3
"""Validate and summarize EXP-0199 direct-n QKV batch-sixteen evidence."""
...
def validate_run(run: dict[str, object], cell: str, steps: int) -> None:
    batch = 16 if cell == CANDIDATE else 8
    base.validate_run(
...
def main() -> None:
    ...
    report = [
        "# EXP-0199 direct-n QKV batch-sixteen report",
        "",
        f"Source commit: `{args.source_commit}`",
        "",
        "Both cells inherit EXP-0198. The candidate changes only logical-M1 "
        "direct-n QKV from batch eight with four slots to batch sixteen with "
        "two capacity-checked HMX-accessible ping-pong slots.",
        "",
        "## End-to-end gate",
        "",
        "| Metric | Batch8 control | Batch16 candidate | Change |",
        "|---|---:|---:|---:|",
        ...
        *pair_rows,
        ...
        "",
        "| Module | Batch8 control | Batch16 candidate | Candidate speed change |",
        "|---|---:|---:|---:|",
        *module_rows,
