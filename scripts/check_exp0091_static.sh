...
if [[ "${control_lut}" -ne 2 || "${control_loads}" -ne 1 || \
    ...
    printf '{"static_gate":"fail","control_vlut32":%s,"control_packed_loads":%s,"candidate_vlut32":%s,"candidate_packed_loads":%s,"candidate_stack_refs":%s}\n' \
        "${control_lut}" "${control_loads}" "${candidate_lut}" \
        "${candidate_loads}" "${candidate_stack}"
    exit 1
fi
printf '{"static_gate":"pass","control_vectors_per_loop":1,"control_vlut32":%s,"candidate_vectors_per_loop":2,"candidate_vlut32":%s,"candidate_stack_refs":0}\n' \
    "${control_lut}" "${candidate_lut}"
