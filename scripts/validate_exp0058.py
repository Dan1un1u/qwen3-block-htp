#!/usr/bin/env python3
"""Validate EXP-0058 HVX vdeal V-carrier transpose evidence."""
...
def validate_record(record: dict[str, object], repeat: int,
    ...
    fixed = {
        ...
        "attention_pipeline_mode": (
            ...
            if mode == "candidate"
            else "u8_log2_gqa_qkv_overlap_vgather"
        ),
    ...
    for field in previous.TARGETS + ("invocation_ticks",):
...
def build_summary(result_dir: Path, package_dir: Path) -> dict[str, object]:
    ...
    summary.update({
        "experiment": "EXP-0058",
        "control": "EXP-0057 vgather with scalar V-carrier transpose",
        "candidate": "exact five-vdeal V-carrier transpose",
    })
    return summary
...
def render_report(summary: dict[str, object]) -> str:
    lines = [
        ...
        "The candidate applies five byte-vdeal stages to map four contiguous "
        "32-byte rows into 32 adjacent four-row HMX words, then performs one "
        "vector store. The exact vgather table, all arithmetic and scheduling "
        "remain unchanged.", "",
    ]
    ...
    lines.extend([
        ...
        "hashes are byte-exact. VTCM is exactly 8 MiB; intermediate DDR "
        "and spill/fill remain zero; one FastRPC execution unit and one HMX "
        "owner are preserved.", "",
        "## Decision", "",
        f"EXP-0058 local gate: **{'PASS' if summary['local_gate_pass'] else 'FAIL'}**.", "",
    ])
    return "\n".join(lines)
