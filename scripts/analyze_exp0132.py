#!/usr/bin/env python3
"""Validate and report EXP-0132 W4F16 Gate/Up claim granularity."""
...
EXP0124_FORMAL = parent.EXP0124_FORMAL
...
def validate_record(record: dict, repeat: int, cell: str,
    ...
    parent.validate_record(compatible, repeat, "worker3", audit=audit)
    base.require(record, "experiment", "EXP-0132")
    base.require(record, "w4f16_group_fence_mode", "join_only")
    base.require(record, "w4f16_expand_claim_regions",
                 1 if cell == "claim1" else 2)
...
def build_summary(result_dir: Path, exp0109_dir: Path,
    ...
    if ((result_dir / "boot_id_before.txt").read_bytes() !=
        ...
        raise SystemExit("device boot ID changed during EXP-0132")
    ...
    if correctness["claim1"]["output_hash"] != \
            correctness["claim2"]["output_hash"]:
    ...
    return {
        ...
        "pc028": parent.baseline_pc028(
            exp0109_dir, exp0124_dir, records[10]["claim1"]),
        "pc028_provenance": {
            "f16f16": str(exp0109_dir),
            "w4f16": str(result_dir) + "/paired_claim1_r10.jsonl",
            "w4u8": str(exp0124_dir) + "/paired_control_r10.jsonl",
        },
        "repeat10_candidate_modules":
            exp109.module_medians(records[10]["claim2"]),
    }
...
def add_candidate_modules(lines: list[str], summary: dict) -> None:
    ...
    lines.extend([
        "## EXP-0132 repeat-ten W4F16 module wall-time", "",
        "| Module | Claim one | Claim two | Speed |",
        "|---|---:|---:|---:|",
    ])
    for name in control:
...
def render_report(summary: dict) -> str:
    lines = ["# EXP-0132 — W4F16 Gate/Up claim-granularity report", ""]
    exp107.add_pc028(lines, summary)
    lines.extend([
        "PC-028 provenance: F16F16 uses frozen EXP-0109, W4U8 uses "
        "EXP-0124 control, and W4F16 is this experiment's claim-one control.",
        "",
    ])
    ...
    lines.extend([
        ...
        f"| EXP-0132 local gate | {'PASS' if summary['local_gate_pass'] else 'FAIL'} |",
