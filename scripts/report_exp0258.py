...
preflight();assert read(R/'formal_gate.json')['integrity_pass'];data={};loops={};calls=0
for phase,n in [('short',5),('formal',10)]:
 g=read(R/(phase+'_gate.json'));rows=[];rng=np.random.default_rng(258)
 for i in range(1,n+1):
  for rep in [1,10]:
   for arm in [0,1]:
    tag=f'{phase}/round{i:02d}_r{rep}_a{arm}';rs=records(R/tag/'stdout.jsonl');ps=[q for q in rs if q.get('record')=='generation_profile'];fs=[q for q in rs if q.get('generation_sequence_complete')];assert len(ps)==rep*16 and len(fs)==rep;physical(ps,28,arm)
    ...
    for p in ps:
     ...
     assert p['w4u8_decode_direct_n_projection_count']==196
...
summary=dict(experiment='EXP-0258',execution_state='completed',evidence_validity='valid',local_gate='pass',gate_role='diagnostic_measurement_integrity_only_not_numerical_or_model_acceptance',numerical_eligible=False,numerical_gate='known_ideal_dense_R3_whole_layer_fail_explicit_PC072_exception',speed_eligible=read(R/'formal_gate.json')['speed_eligible'],model_quality='not_assessed',baseline_promoted=False,short_rounds=5,formal_rounds=10,timed_full_model_RPCs=calls,formal_RPCs=3520,layer_ledgers_checked=calls*28,actual_layer_count=28,scope='nativepackedW4 28layers embedding norm W4head greedy; M64+15continuousdecode; no audit DDR',recipe='frozen C64 per-output-channel W4; R3_ALL QK; rotatedofflineEOSprefix; plain denseHMX mode1; wideNR64',device_PPL=None,throughput=through,performance=read(R/'formal_gate.json')['performance'],R3_breakdown_us=r3cost,modules=modules,capture_source_head=read(read(R/'runtime_l28.json')['manifest'])['source_head'],EXP0257_legacy_C0_nondeterminism='unresolved not used; both current wideNR64 arms deterministic in captured runs')
write(R/'summary.json',summary);write(R/'numeric_diagnostics.json',dict(data=data,loops=loops));write(R/'independent_integrity_checks.json',dict(pass_all=True,timed_RPCs=calls,layer_ledgers=calls*28,all_selected_sequences_exact=True,R3_calls_28_per_step=True,R3_live_rows_prefill=43008,R3_live_rows_decode=672,all_native_W4_projection_counts_196=True,physical_and_ledgers=True,independent_statistics_CI_recomputed=True,historical_hashes_verified=20))
intro=['# EXP-0258 dense R3 full-model diagnostic E2E','','本轮在用户明确批准的PC072例外下完成真正28层全模型R3速度测量。仅测性能成本；既有理想Float64旋转参考整层gate失败仍保留，不作精度验收或基线晋升。不是EXP0248昂贵修正版，也没有蝶形/FWHT。','','冻结C64 per-output-channel W4、非Q/K量化、wideNR64和原生U8 LMhead；R3采用EXP0246 R3_ALL Q/K参数，固定EOS的离线W4A16 K先旋转再存U8，V不变。对照是正确前缀的无R3 wideNR64；不使用EXP0257曾偶发失稳的legacy/no-prefix C0，也不宣称修复了它。','','单次R3把24个Q/K heads组成dense GEMM，M64有1536live rows，M1有24live rows(补齐32)，右乘显式128x128符号矩阵并归一化。每次全模实际28次R3 HMX调用：prefill43008live rows、decode672。总计包含准备/打包/归一化/矩阵乘/重排/量化和调度损失，不以纯GEMM耗时代替总开销。','','三层九步检查：无R3输出复现EXP0257；R3重复输出一致；捕获最后一层dense误差通过原组件1ULP+minnormal门槛，实际QK/概率/AV与独立int64参考逐字节一致，首层旋转前缀缓存列/偏置独立验证。该检查不推翻既有整层ideal-reference失败。','','五轮短测、十轮轮换正式诊断，repeat1及repeat10；5280完整模型RPC、147840逐层账本均闭合且反馈token序列一致。8MiB VTCM、零计时中间DDR/spill/audit导出、每token完整模型一个RPC。每模型调用196次原生W4投影。未放宽物理或确定性门槛。','','repeat10每轮复用已加载权重，但10条序列均重建缓存状态，实际M64+15反馈decode。配对比例中位数和单臂耗时中位数不是同一统计量。超过10%仍判速度资格失败，但本次直接授权成本测量，故完成诊断轮次并如实报告。']
pr=[]
