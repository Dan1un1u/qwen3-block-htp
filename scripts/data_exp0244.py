...
from data_exp0229 import BASE,SOURCE,MEMORY,MODEL,CELLS,sha,verified as legacy_verified,preflight,hashed_grams
from data_exp0243 import prior_data as old_prior
from data_exp0233 import ids_of
from gptq_exp0221 import fetch
RESULT=BASE/'exp0244'
def verified(exp,name):
 if exp!='exp0243':return legacy_verified(exp,name)
 ledger=BASE/exp/'EVIDENCE_SHA256.json'
 assert sha(ledger)=='f570b447e6f7bd45f39d5786e447bceb53a23cf4b43202d1dc37fc18d857ff6b'
...
def prior_data():
 paths,ds,rows=old_prior();p=verified('exp0243','dataset.json');d=json.loads(p.read_text())
 return paths+[p],ds+[d],rows+d['samples']
def frozen():
 f=json.loads((RESULT/'dataset_freeze.json').read_text())
 assert sha(MEMORY/'docs/experiments/EXP-0244.md')==f['protocol_sha256']
...
def prepare():
 ...
 rng=np.random.default_rng(244);chosen={c:[] for c in CELLS};used=set();grams=set();hashes=set();sources=[]
 def select(cell,rows,path):
  if len(chosen[cell])==96:return
 ...
 for lang in ['en','zh']:
  ...
  select(cell,rows,p);assert len(chosen[cell])==96;print('DATA_CELL_READY',cell,96,flush=True)
 for lang in ['en','zh']:
  ...
  for p in sorted(pool.values()):
   ...
   if len(chosen[cell])==96:break
  if len(chosen[cell])<96:
   from urllib.parse import urlencode
   for off in range(74000,84000,100):
    ...
    if len(chosen[cell])==96:break
  assert len(chosen[cell])==96,(cell,len(chosen[cell]));print('DATA_CELL_READY',cell,96,flush=True)
 samples=[]
 for i in range(96):
  for c in CELLS:
   r=chosen[c][i];r.update(id=len(samples),split='development' if i<32 else 'final',prompt_ids=r['token_ids'][:64],target_ids=r['token_ids'][64:]);samples.append(r)
 write('dataset.json',dict(version='qbh-EXP244-prefix-ablation-v1',seed=244,cells=CELLS,samples=samples,provenance=sources,tokenizer_sha256=sha(MODEL/'qwen3-tokenizer.json'),calibration_reference='EXP230_C64_65536',development_targets=2048,final_targets=4096,prior_paths=[str(p) for p in paths]))
 ...
 write('dataset_freeze.json',dict(protocol_sha256=sha(MEMORY/'docs/experiments/EXP-0244.md'),files={'dataset.json':sha(RESULT/'dataset.json')},references={str(p):sha(p) for p in refs},frozen_before_export_and_scoring=True))
...
def audit():
 ...
 assert len(used)==384
 write('independent_data_audit.json',dict(pass_all=True,documents=384,development_targets=2048,final_targets=4096,direct_tuple32_disjoint=True,all_windows_reconstructed=True,dataset_sha256=sha(RESULT/'dataset.json')));print('DATA_AUDIT_PASS',flush=True)
