...
S=old.SOURCE;R=old.RESULT.parent/'exp0252';MODELS=old.MODELS/'exp0247'
REMOTE='/data/local/tmp/qwen3-block-htp/exp0252-layer0';ADB=old.ADB
ENV=dict(old.ENV,LD_LIBRARY_PATH=REMOTE,DSP_LIBRARY_PATH=REMOTE,ADSP_LIBRARY_PATH=REMOTE)
def sha(p):return hashlib.sha256(Path(p).read_bytes()).hexdigest()
def preflight():
    old.preflight()
    assert subprocess.check_output(['git','branch','--show-current'],cwd=S,text=True).strip()=='codex/exp-0252-normalization-r3'
...
def run(cell,repeat,tag,dump=False,steps=8):
    modes={
        'control':(0,1,'control'),'exact':(0,3,'control'),'nr64':(0,4,'control'),
        'control_scalar':(0,2,'control'),'exact_scalar':(0,5,'control'),'nr64_scalar':(0,6,'control'),
        'r3_control':(1,1,'r3'),'r3_exact':(1,3,'r3'),'r3_nr64':(1,4,'r3'),
        'scalar_control':(2,2,'r3'),'scalar_exact':(2,5,'r3'),'scalar_nr64':(2,6,'r3')}
    assert cell in modes and repeat in [1,10] and 1<=steps<=8
    dense,wide,package=modes[cell];env=dict(ENV);env['QBH_DENSE_R3']=str(dense);env['QBH_WIDE_SCORE']=str(wide);env['QBH_REPLAY_DECODE_STEPS']=str(steps)
    path=R/tag;path.mkdir(exist_ok=False)
    if dump:
    ...
    if cell.startswith('r3_'):assert all(d['hmx_calls']==1 and d['rows']==(1536 if d['step']==0 else 24) for d in extra)
