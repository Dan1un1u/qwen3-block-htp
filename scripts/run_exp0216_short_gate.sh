...
"${project_root}/scripts/build_exp0216.sh" > "${result_dir}/build.log" 2>&1
"${project_root}/scripts/check_exp0216_static.sh" > "${result_dir}/static_gate.json"
"${project_root}/scripts/deploy_exp0216.sh" > "${result_dir}/deploy.log" 2>&1
for cell in control direct_qkvo; do
    QBH_EXP0173_GENERATION_STEPS=4 \
    QBH_GENERATION_BOUNDARY_AUDIT=1 \
    QBH_GENERATION_AUDIT_DIR="${result_dir}/audit/${cell}" \
        "${project_root}/scripts/run_exp0216.sh" "${cell}" \
        > "${result_dir}/raw/audit_${cell}.log" 2>&1
done
for round in $(seq 1 5); do
    printf -v index '%02d' "${round}"
    if ((round % 2 == 1)); then
        order=(control direct_qkvo)
    else
        order=(direct_qkvo control)
    fi
...
"${python_exe}" "${project_root}/scripts/summarize_exp0216.py" \
    --result-dir "${result_dir}" --source-commit "${source_commit}" \
    --rounds 5 --steps 193 --audit-result-dir "${result_dir}" --gate
printf 'RESULT_DIR=%s\n' "${result_dir}"
