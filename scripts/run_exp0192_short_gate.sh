...
"${project_root}/scripts/build_exp0192.sh" > "${result_dir}/build.log" 2>&1
"${project_root}/scripts/check_exp0192_static.sh" > "${result_dir}/static_gate.json"
"${project_root}/scripts/deploy_exp0192.sh" > "${result_dir}/deploy.log" 2>&1

for cell in down2 down4; do
    QBH_EXP0173_GENERATION_STEPS=4 \
    QBH_GENERATION_BOUNDARY_AUDIT=1 \
    QBH_GENERATION_AUDIT_DIR="${result_dir}/audit/${cell}" \
        "${project_root}/scripts/run_exp0192.sh" "${cell}" \
...
"${python_exe}" "${project_root}/scripts/summarize_exp0190.py" \
    ...
    --experiment 192 --control-batch 2 --candidate-batch 4 --target down
printf 'RESULT_DIR=%s\n' "${result_dir}"
