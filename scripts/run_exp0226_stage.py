 ...
 **{s:[CPU,str(ROOT/'scripts/diagnose_exp0226.py'),s] for s in ['inherit','identity','controls']},
 ...
 'smoke':[GPU,str(ROOT/'scripts/learned_rotation_exp0226.py'),'smoke'],
 'train':[GPU,str(ROOT/'scripts/learned_rotation_exp0226.py'),'train'],
 **{s:[CPU,str(ROOT/'scripts/export_exp0226.py'),s] for s in ['step000','step050','step100','control','select','quality']},
 **{s:[CPU,str(ROOT/'scripts/measure_exp0226.py'),s] for s in ['quick','full','repeat','warmup','short','formal']},
 'quality-report':[CPU,str(ROOT/'scripts/summarize_exp0226.py'),'--quality-only'],
 'speed-report':[CPU,str(ROOT/'scripts/summarize_exp0226.py'),'--speed-only'],
}
def run(stage):
    ...
    command=COMMANDS[stage] if stage!='deploy' else [CPU,str(ROOT/'scripts/measure_exp0226.py'),'deploy','--variant',json.loads((RESULT/'selection.json').read_text())['selected']]
