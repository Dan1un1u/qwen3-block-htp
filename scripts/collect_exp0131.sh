...
u8_package="${QBH_EXP0131_U8_PACKAGE:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0042/block_package_layer14_m64_integer_attention_parallel}"
result_root="${QBH_EXP0131_RESULT_ROOT:-/mnt/d/llm_exp/results/qwen3-block-htp/exp0131}"
artifact_root="${QBH_EXP0131_ARTIFACT_ROOT:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0131/artifacts}"
...
if ! git -C "${project_root}" diff --quiet || \
   ...
   [[ -n "$(git -C "${project_root}" ls-files --others --exclude-standard)" ]]; then
    printf 'source worktree must be clean before EXP-0131 formal collection\n' >&2
    exit 1
fi
...
"${project_root}/scripts/check_exp0131_static.sh" \
    > "${result_dir}/static_gate.json"
QBH_EXP0124_U8_PACKAGE="${u8_package}" \
EXP0124_REMOTE_ROOT="${remote_root}" \
...
for cell in "${cells[@]}"; do
    ...
    EXP0131_REMOTE_ROOT="${remote_root}" \
        "${project_root}/scripts/run_exp0131.sh" \
        "${cell}" 1 on on > "${raw}"
...
for round in 1 2 3 4 5; do
    printf 'EXP-0131 round %d/5\n' "${round}"
    if ((round % 2 == 1)); then
        repeats=(1 10)
        order=(control main_drain)
    else
        repeats=(10 1)
        order=(main_drain control)
    fi
...
python3 "${project_root}/scripts/analyze_exp0131.py" "${result_dir}" \
    > "${result_dir}/gate_summary.json"
python3 "${project_root}/scripts/analyze_exp0131.py" "${result_dir}" --report \
...
{
    printf 'experiment=EXP-0131\nsource_head=%s\n' "${source_head}"
    printf 'parent=EXP-0124\ncells=%s\n' "${cells[*]}"
    printf 'recipe=W4U8\nvariable=post_dma_hmx_tail_main_qk_prep\n'
