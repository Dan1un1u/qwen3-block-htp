...
"${project_root}/scripts/build_exp0206.sh" > "${result_dir}/build.log" 2>&1
"${project_root}/scripts/check_exp0206_static.sh" > "${result_dir}/static_gate.json"
"${project_root}/scripts/deploy_exp0206.sh" > "${result_dir}/deploy.log" 2>&1
...
for cell in q16 q32; do
    ...
    QBH_GENERATION_AUDIT_DIR="${result_dir}/audit/${cell}" \
        "${project_root}/scripts/run_exp0206.sh" "${cell}" \
...
"${python_exe}" "${project_root}/scripts/summarize_exp0206.py" \
