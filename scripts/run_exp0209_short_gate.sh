...
"${project_root}/scripts/build_exp0209.sh" > "${result_dir}/build.log" 2>&1
"${project_root}/scripts/check_exp0209_static.sh" > "${result_dir}/static_gate.json"
"${project_root}/scripts/deploy_exp0209.sh" > "${result_dir}/deploy.log" 2>&1
for cell in q16 q32; do
    QBH_EXP0173_GENERATION_STEPS=4 \
    QBH_GENERATION_BOUNDARY_AUDIT=1 \
    QBH_GENERATION_AUDIT_DIR="${result_dir}/audit/${cell}" \
        "${project_root}/scripts/run_exp0209.sh" "${cell}" \
        > "${result_dir}/raw/audit_${cell}.log" 2>&1
done
for round in $(seq 1 5); do
    printf -v index '%02d' "${round}"
    if ((round % 2 == 1)); then order=(q16 q32); else order=(q32 q16); fi
...
"${python_exe}" "${project_root}/scripts/summarize_exp0209.py" \
