...
S=old.SOURCE;R=old.RESULT.parent/'exp0251';MODELS=old.MODELS/'exp0247'
REMOTE='/data/local/tmp/qwen3-block-htp/exp0251-layer0';ADB=old.ADB
ENV=dict(old.ENV,LD_LIBRARY_PATH=REMOTE,DSP_LIBRARY_PATH=REMOTE,ADSP_LIBRARY_PATH=REMOTE)
def sha(p):return hashlib.sha256(Path(p).read_bytes()).hexdigest()
def preflight():
    old.preflight()
    assert subprocess.check_output(['git','branch','--show-current'],cwd=S,text=True).strip()=='codex/exp-0251-wide-score-device'
...
def deploy():
    preflight();assert json.loads((R.parent/'exp0247'/'export_audit.json').read_text())['pass_all']
    ...
    attempt=1
    while (R/f'device_binary_manifest_attempt{attempt}.json').exists():attempt+=1
    manifest=R/f'device_binary_manifest_attempt{attempt}.json'
    import shutil
    saved=R/'binaries'/f'attempt{attempt}';saved.mkdir(parents=True)
    for binary in binaries():shutil.copy2(binary,saved/binary.name)
    with manifest.open('x') as f:json.dump(x,f,indent=2)
    print('DEPLOYED',flush=True)
def run(cell,repeat,tag,dump=False,steps=8):
    assert cell in ['control','wide','wide_scalar','r3_wide','scalar_wide','r3','scalar','identity'] and repeat in [1,10] and 1<=steps<=8
    env=dict(ENV);env['QBH_DENSE_R3']=str(dict(control=0,wide=0,wide_scalar=0,r3_wide=1,scalar_wide=2,r3=1,scalar=2,identity=3)[cell]);env['QBH_WIDE_SCORE']=str(2 if cell in ['wide_scalar','scalar_wide'] else 1 if cell in ['wide','r3_wide'] else 0);env['QBH_REPLAY_DECODE_STEPS']=str(steps)
    package='control' if cell in ['control','wide','wide_scalar'] else 'r3';path=R/tag;path.mkdir(exist_ok=False)
    if dump:
    ...
    assert all(d['wide_score_mode']==int(env['QBH_WIDE_SCORE']) for d in records)
    if cell in ['r3','r3_wide']:assert all(d['hmx_calls']==1 and d['rows']==(1536 if d['step']==0 else 24) for d in extra)
