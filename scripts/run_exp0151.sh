...
remote_root="${EXP0151_REMOTE_ROOT:-/data/local/tmp/qwen3-block-htp/exp0151}"
host_executable="${project_root}/android_ReleaseG_aarch64/ship/qwen3_rpcmem2_cli"
...
"${adb_exe}" push "$(wslpath -w "${host_executable}")" \
    "${remote_root}/qwen3_rpcmem2_cli" >/dev/null 2>&1
...
"${adb_exe}" shell \
    "chmod 755 ${remote_root}/qwen3_rpcmem2_cli ${remote_root}/libqwen3_probe.so ${remote_root}/libqwen3_probe_skel.so"
"${adb_exe}" shell \
    "cd ${remote_root} && LD_LIBRARY_PATH=${remote_root} DSP_LIBRARY_PATH=${remote_root} ADSP_LIBRARY_PATH=${remote_root} ./qwen3_rpcmem2_cli"
