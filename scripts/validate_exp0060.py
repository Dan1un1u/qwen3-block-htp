#!/usr/bin/env python3
"""Validate EXP-0060 exact HVX U8 RMSNorm reduction evidence."""
...
def validate_record(record: dict[str, object], repeat: int,
    ...
    fixed = {
        ...
        "u8_norm_reduction_mode": (
            "hvx_tree" if mode == "candidate" else "scalar"
        ),
...
def build_summary(result_dir: Path, package_dir: Path) -> dict[str, object]:
    ...
    summary.update({
        "experiment": "EXP-0060",
        "control": "EXP-0058 scalar stack lane reduction",
        "candidate": "exact combined-vector HVX word-tree reduction",
    })
    return summary
...
def render_report(summary: dict[str, object]) -> str:
    lines = [
        ...
        "The control spills four 32-word U8 RMSNorm square accumulators "
        "and sums 128 lanes with scalar code. The candidate adds the four "
        "vectors lane-wise and performs one exact HVX word-reduction tree. "
        "Square arithmetic, sqrtf, qparams, gamma and output encoding are "
        "unchanged.", "",
    ]
    for repeat in previous.REPEATS:
        ...
        for field in (
            ...
            "input_norm_ticks", "u8_attention_qk_norm_rope_ticks",
            "post_attention_residual_ticks", "qkv_projection_ticks",
        ...
        lines.extend([
            "",
            f"Four-target speed gate: **{'PASS' if result['three_target_speed_gate'] else 'FAIL'}**; "
    ...
    lines.extend([
        ...
        "## Decision", "",
        f"EXP-0060 local gate: **{'PASS' if summary['local_gate_pass'] else 'FAIL'}**.", "",
    ])
    return "\n".join(lines)
