...
result_root="${QBH_RESULT_ROOT:-/mnt/d/llm_exp/results/qwen3-block-htp/exp0041}"
artifact_root="${QBH_ARTIFACT_ROOT:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0041/artifacts}"
package="${QBH_EXP0041_PACKAGE:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0040/block_package_layer14_m64_stage_b}"
...
"${project_root}/scripts/build_exp0041.sh" \
    > "${result_dir}/build.log" 2>&1
"${project_root}/scripts/check_exp0041_static.sh" \
    > "${result_dir}/static_gate.json"
QBH_EXP0041_PACKAGE="${package}" \
    "${project_root}/scripts/deploy_exp0041_block.sh" \
...
run_control() {
    "${project_root}/scripts/run_exp0041_block.sh" \
        W4U8 "$1" scalar scalar on off
}

run_candidate() {
    "${project_root}/scripts/run_exp0041_block.sh" \
        W4U8 "$1" u8_all fused on off
}

for round in $(seq 1 "${paired_rounds}"); do
    printf 'EXP-0041 paired round %d/%d\n' \
        "${round}" "${paired_rounds}"
    if (( round % 2 == 1 )); then
        repeat_order=(1 10)
...
python3 "${project_root}/scripts/validate_exp0041_evidence.py" \
...
{
    printf 'experiment=EXP-0041\n'
    printf 'source_branch=%s\n' "$(git -C "${project_root}" branch --show-current)"
    printf 'source_head=%s\n' "${source_head}"
    printf 'execution_unit=qwen3_layer14_complete_block_m64\n'
    printf 'control=EXP0040_W4U8_scalar_common_ops\n'
    printf 'candidate=HVX_U8_RMSNorm_RoPE_bridge_residual_and_FP16_HVX_Softmax\n'
    ...
    printf 'package_reused_unchanged=%s\n' "${package}"
    printf 'result_dir=%s\n' "${result_dir}"
