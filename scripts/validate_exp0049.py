#!/usr/bin/env python3
"""Validate EXP-0049 paired evidence and render the PC-027 report."""
...
def validate_record(record: dict[str, object], repeat: int,
                    mode: str) -> None:
    fixed = {
        "experiment": "EXP-0048" if mode == "control" else "EXP-0049",
        ...
        "hmx_command_count": (1040 if mode == "control" else 320) * repeat,
...
def build_summary(result_dir: Path, package_dir: Path) -> dict[str, object]:
    ...
    for mode in MODES:
        ...
        require(record, "experiment",
                "EXP-0048" if mode == "control" else "EXP-0049")
    ...
    for repeat in REPEATS:
        ...
        command_gate = (
            metrics["hmx_command_count"]["control"] == 1040.0 and
            metrics["hmx_command_count"]["candidate"] == 320.0
        )
        ...
        repeat_results[f"repeat{repeat}"] = {
            ...
            "gate_up_control_commands_per_block": 768,
            "gate_up_candidate_commands_per_block": 48,
            "gate_up_candidate_batch_n_tiles": 8,
        }

    return {
        "experiment": "EXP-0049",
        "control": "W4U8-EXP0048-NATIVE-QKV-O",
        "candidate": "Gate/Up accumulate-store fusion plus batch8 HMX consume",
...
def render_report(summary: dict[str, object]) -> str:
    lines = [
        ...
        "The candidate changes only the W4U8 Gate/Up HMX consumer cadence. "
        "It fuses the final U8 output store into the streaming accumulate "
        "command and consumes eight consecutive output tiles per worker "
        "wake-up. W4 values/scales, U8 qparams, HMX tile arithmetic, DMA "
        "bytes/descriptors, Attention, Down, tensor boundaries and final "
    ...
    for repeat in REPEATS:
        ...
        add_table(
            lines, "Primary latency and Gate/Up target",
            (
                "host_wall_ns_per_block", "invocation_ticks", "total_ticks",
                "gate_up_ticks", "w4u8_mlp_gate_up_pipeline_ticks",
            ), metrics,
        )
        ...
        lines.extend([
            "Gate/Up HMX commands per block: **768 → 48**; total HMX "
            "commands per block: **1040 → 320**; HMX tile pairs remain "
            "**49,408**.", "",
    ...
    lines.extend([
        ...
        "## Decision", "",
        f"EXP-0049 local gate: **{'PASS' if summary['local_gate_pass'] else 'FAIL'}**. "
