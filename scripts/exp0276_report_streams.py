...
from device_exp0276 import records
from summarize_exp0217 import normalized
from report_exp0273 import GROUPS
R=Path('/mnt/d/llm_exp/results/qwen3-block-htp/exp0276')
ARMS=['ALL','SPLIT']
rng=np.random.default_rng(276);idx=rng.integers(0,10,(20000,10));out={'experiment':'EXP-0276','scopes':{},'precision':'FP32 residual, nativeW4/SP2mode8, no rotation; quality unassessed'}
for scope in [28]:
...
lines=['# EXP0276：SP2 decode 共装与两独立流','','固定原生W4/SP2mode8、FP32残差2、无旋转，权重码/尺度、LUT和所有非目标优化不变。SPLIT由SwiGLU直接输出独立high平面，Down对同一已搬入权重执行两次矩阵累积；ALL用空闲物理行共装一次矩阵流。Prefill合同不变。','','全模5short+10formal，交替顺序，repeat10主计时，repeat1辅助。95%CI按十个配对轮次bootstrap20000seed276，保留全部样本。']
for mode in ['prefill','decode']:
 lines += ['',f'## 完整28层 {mode} 模块表','','| 模块 | 共装 μs（Host占比） | 两独立流 μs（Host占比） |','|---|---:|---:|']
 ...
 v=out['scopes']['28']['effects'][mode]['SPLIT'];lines += ['',f"两流/共装 Host wall：{v['ratio']:.6f}，95%CI [{v['ci95'][0]:.6f}, {v['ci95'][1]:.6f}]。"]
...
lines += ['','M64+15、cache128、冻结EOS前缀。包含embedding/28层/finalnorm/head/greedy/FastRPC，排除tokenizer/冷加载/ADB。HMXcommand计数是worker提交次数；真正额外工作看tilepair计数，decode每层增加12288，28层增加344064。权重搬运不增加。结果只解释固定SP2合同的执行方式，不是SP2相对普通A8的成本（A5另测），也不代表模型质量。']
(R/'REPORT.md').write_text('\n'.join(lines)+'\n');print(json.dumps(out['scopes']['28']['effects'],indent=2))
