...
decode_direct_n_qkv_single_dma="${QBH_W4U8_DECODE_DIRECT_N_QKV_SINGLE_DMA:-0}"
...
case "${decode_direct_n_qkv_single_dma}" in
0|1) ;;
*) printf 'invalid direct-n QKV single-DMA mode: %s\n' "${decode_direct_n_qkv_single_dma}" >&2; exit 2 ;;
esac
if [[ "${decode_direct_n_qkv_single_dma}" == 1 &&
      ("${decode_projection_mode}" != direct_n ||
       "${decode_direct_n_qkv_batch_tiles}" != 16 ||
       $((decode_direct_n_mask & 1)) -eq 0) ]]; then
    printf 'QKV single-DMA requires direct-n QKV batch16\n' >&2
    exit 2
fi
