...
decode_direct_n_q_batch_tiles="${QBH_W4U8_DECODE_DIRECT_N_Q_BATCH_N_TILES:-0}"
decode_direct_n_next_q_prefetch="${QBH_W4U8_DECODE_DIRECT_N_NEXT_Q_PREFETCH:-0}"
decode_direct_n_down_batch_tiles="${QBH_W4U8_DECODE_DIRECT_N_DOWN_BATCH_N_TILES:-2}"
decode_direct_n_down_single_dma="${QBH_W4U8_DECODE_DIRECT_N_DOWN_SINGLE_DMA:-0}"
decode_direct_n_o_single_dma="${QBH_W4U8_DECODE_DIRECT_N_O_SINGLE_DMA:-0}"
...
case "${decode_direct_n_gate_up_continuous}" in
0|1) ;;
*) printf 'invalid direct-n Gate/Up continuous mode: %s\n' "${decode_direct_n_gate_up_continuous}" >&2; exit 2 ;;
esac
if [[ "${decode_direct_n_gate_up_continuous}" == 1 &&
      ("${decode_projection_mode}" != direct_n ||
       "${decode_direct_n_gate_up_batch_tiles}" != 32 ||
       $((decode_direct_n_mask & 4)) -eq 0) ]]; then
    printf 'continuous Gate/Up requires direct-n MLP batch32\n' >&2
    exit 2
fi
case "${decode_direct_n_o_gate_prefetch}" in
0|1) ;;
*) printf 'invalid direct-n O-to-Gate prefetch: %s\n' "${decode_direct_n_o_gate_prefetch}" >&2; exit 2 ;;
esac
if [[ "${decode_direct_n_o_gate_prefetch}" == 1 &&
      ("${decode_projection_mode}" != direct_n ||
       "${decode_direct_n_gate_up_batch_tiles}" != 32 ||
       "${decode_direct_n_gate_up_continuous}" != 1 ||
       $((decode_direct_n_mask & 4)) -eq 0) ]]; then
    printf 'O-to-Gate prefetch requires continuous direct-n MLP batch32\n' >&2
    exit 2
fi
case "${decode_direct_n_gate_up_swiglu_stream}" in
0|1) ;;
*) printf 'invalid direct-n Gate/Up SwiGLU stream: %s\n' "${decode_direct_n_gate_up_swiglu_stream}" >&2; exit 2 ;;
esac
if [[ "${decode_direct_n_gate_up_swiglu_stream}" == 1 &&
      ("${decode_projection_mode}" != direct_n ||
       "${decode_direct_n_gate_up_batch_tiles}" != 32 ||
       "${decode_direct_n_gate_up_continuous}" != 1 ||
       "${decode_direct_n_o_gate_prefetch}" != 1 ||
...
case "${decode_direct_n_q_batch_tiles}" in
0|32) ;;
*) printf 'invalid direct-n Q-only batch tiles: %s\n' "${decode_direct_n_q_batch_tiles}" >&2; exit 2 ;;
esac
if [[ "${decode_direct_n_q_batch_tiles}" != 0 &&
      ("${decode_projection_mode}" != direct_n ||
       "${decode_direct_n_qkv_batch_tiles}" != 16 ||
       $((decode_direct_n_mask & 1)) -eq 0) ]]; then
    printf 'Q-only batch32 requires direct-n QKV with K/V batch16\n' >&2
    exit 2
fi
case "${decode_direct_n_next_q_prefetch}" in
0|1) ;;
*) printf 'invalid direct-n next-Q prefetch mode: %s\n' "${decode_direct_n_next_q_prefetch}" >&2; exit 2 ;;
esac
if [[ "${decode_direct_n_next_q_prefetch}" == 1 &&
      ("${decode_projection_mode}" != direct_n ||
       "${decode_direct_n_qkv_batch_tiles}" != 16 ||
       "${decode_direct_n_q_batch_tiles}" != 32 ||
       "${decode_direct_n_down_batch_tiles}" != 8 ||
       "${decode_direct_n_down_single_dma}" != 1 ||
       $((decode_direct_n_mask & 5)) -ne 5) ]]; then
    printf 'next-Q prefetch requires Q32/KV16 and direct-n Down8\n' >&2
    exit 2
fi
...
0|1) ;;
*) printf 'invalid direct-n Down single-DMA mode: %s\n' "${decode_direct_n_down_single_dma}" >&2; exit 2 ;;
esac
...
0|1) ;;
*) printf 'invalid direct-n O single-DMA mode: %s\n' "${decode_direct_n_o_single_dma}" >&2; exit 2 ;;
esac
if [[ "${decode_direct_n_o_single_dma}" == 1 &&
      ("${decode_projection_mode}" != direct_n ||
       "${o_batch_tiles}" != 16 ||
       $((decode_direct_n_mask & 2)) -eq 0) ]]; then
    printf 'O single-DMA requires direct-n O batch16\n' >&2
    exit 2
fi
