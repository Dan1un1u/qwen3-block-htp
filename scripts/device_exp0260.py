...
import subprocess,json,hashlib,shutil,shlex,statistics,argparse,struct
...
def run(count,arm,rep,tag,dump=False,audit=False):
 ...
 if count==28:
  e.update(QBH_GENERATION_SEQUENCE='7',QBH_GENERATION_STEPS='16')
  ids=list(struct.unpack('<64I',(C/'generation_prompt_token_ids_u32.bin').read_bytes()));row=[0,2,16]+ids+[0]*16
  path=R/(tag.replace('/','_')+'_sequence.bin');path.parent.mkdir(parents=True,exist_ok=True);path.write_bytes(struct.pack('<4I',0x51424556,1,rep,83)+b''.join(struct.pack('<83I',i,*row[1:]) for i in range(rep)));remote=root+'/'+path.name;adb('push',win(path),remote);e['QBH_EVAL_FILE']=remote
 ...
 cmd='cd '+root+' && '+' '.join(k+'='+shlex.quote(v) for k,v in e.items())+f' ./qwen3_block_cli {PKG} W4F16 {1 if count==28 else rep} {args}'
 ...
 result=dict(tag=tag,arm=arm,repeat=rep,count=count,output_hashes=[q['output_hash'] for q in ps],prefill_ns=statistics.mean(q['host_wall_ns'] for q in ps if q['mode']=='prefill'),decode_ns=statistics.mean(q['host_wall_ns'] for q in ps if q['mode']=='decode'))
 if count==28:
  rs=records(p/'stdout.jsonl');fs=[q for q in rs if q.get('generation_sequence_complete')];assert len(fs)==rep and all(q['all_steps_pass'] for q in fs)
  result['token_sequences']=[q['token_ids'] for q in fs];result['selected_codes']=[q['selected_logit_half_bits'] for q in rs if 'selected_logit_half_bits' in q];assert len(result['selected_codes'])==16*rep
 write(p/'validated.json',result);print('PASS',tag,flush=True)
