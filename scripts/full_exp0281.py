...
from device_exp0281 import adb,win,records
from measure_exp0218 import LEDGER
from summarize_exp0217 import normalized
import shlex,struct,statistics,numpy as np,os
STEPS=int(os.environ.get('QBH_GENERAL_STEPS','64'))
SEED='/data/local/tmp/qwen3-block-htp/exp0257-prefix/prefix_kv_u8.bin'

def full(fp,repeat,tag,audit=False):
 preflight();assert read(R/'inherited_gates.json')['pass_all'];state=read(R/'runtime-l28.json');root=state['remote'];assert state['seal']['paper_trace'] is False;d=R/tag;d.mkdir(parents=True,exist_ok=False)
 import exp0240_device as old
 sp2=int(os.environ.get('QBH_SP2','8'));pkg=read(R/'deployment-sp2-fp32.json')['remote'];e=dict(old.ENV,LD_LIBRARY_PATH=root,DSP_LIBRARY_PATH=root,ADSP_LIBRARY_PATH=root,QBH_SP2=str(sp2),QBH_U8_PREFILL_OPT=os.environ.get('QBH_U8_PREFILL_OPT','0'),QBH_FP32_RESIDUAL=str(fp),QBH_PAPER_FORMAT_DISABLE=os.environ.get('QBH_PAPER_FORMAT_DISABLE','0'),QBH_PAPER_PIPELINE_DISABLE=os.environ.get('QBH_PAPER_PIPELINE_DISABLE','0'),QBH_WIDE_SCORE=os.environ.get('QBH_WIDE_SCORE','4'),QBH_DENSE_R3='0',QBH_DENSE_R4='0',QBH_KV_CACHE_CAPACITY='128',QBH_GENERATION_SEQUENCE='9',QBH_GENERATION_STEPS=str(STEPS),QBH_PREFIX_KV='1',QBH_PREFIX_FILE=SEED)
 assert adb('shell','sha256sum '+SEED).stdout.split()[0]=='7683237318d42ac5cc80052fb53619205a3d82a0d1377bcbbaed78d7c7683b91'
 ids=read(R/'prompts.json')[os.environ['QBH_GENERAL_PROMPT']]['ids'];assert len(ids)==64
 fixed=read(R/'fixed_tokens.json')['ids'];row=[0,2,STEPS]+ids+fixed;f=d/'eval.bin';f.write_bytes(struct.pack('<4I',0x51424556,1,repeat,83)+b''.join(struct.pack('<83I',i,*row[1:]) for i in range(repeat)));remote_eval=root+'/'+tag.replace('/','_')+'.bin';adb('push',win(f),remote_eval);e['QBH_EVAL_FILE']=remote_eval
 if audit:
...
def validate(d,fp,repeat,audit=False):
 ...
 for step,v in enumerate(ps):
  assert v['generation_step']==step%STEPS and v['valid_length']==64+step%STEPS and v['logical_m']==(64 if step%STEPS==0 else 1)
  assert v['scan_cache_append_mismatch_count']==0
  ...
  assert v['block_invocation_count']==28 and v['dense_r3_mode']==v['dense_r4_mode']==0 and v['wide_score_mode']==int(os.environ.get('QBH_WIDE_SCORE','4'))
