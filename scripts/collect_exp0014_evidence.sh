...
set -u
result_root="${QBH_RESULT_ROOT:-/mnt/d/llm_exp/results/qwen3-block-htp/exp0014}"
artifact_root="${QBH_ARTIFACT_ROOT:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0014}"
...
"${project_root}/scripts/build_exp0014.sh" \
...
storages=(
    expanded_s8_control expanded_s8_control
    packed_w4_hmx_postscale packed_w4_hmx_postscale
    expanded_s8_control expanded_s8_control
    packed_w4_hmx_postscale packed_w4_hmx_postscale
)
projections=(
    gate_up gate_up gate_up gate_up
    down down down down
)
plans=(
    expanded_s8_dma_chain2 expanded_s8_dma_chain2
    slots8e7_chunk64_dma_chain4 slots8e7_chunk64_dma_chain4
    exp0005_full_bundle_control exp0005_full_bundle_control
    slots4_chunk96_dma_batch2 slots4_chunk96_dma_batch2
)
output_modes=(
    scalar_memcpy linked_2d_dma scalar_memcpy linked_2d_dma
    scalar_memcpy linked_2d_dma scalar_memcpy linked_2d_dma
)
...
run_one() {
    ...
    local output="$4"
    if [[ "${deployed}" == "0" ]]; then
        "${project_root}/scripts/run_exp0014.sh" \
            "${storages[index]}" "${projections[index]}" "${pattern}" \
            "${repeat_count}" "${plans[index]}" \
            "${output_modes[index]}" | tee -a "${output}"
        deployed=1
    else
        QBH_SKIP_DEPLOY=1 "${project_root}/scripts/run_exp0014.sh" \
            "${storages[index]}" "${projections[index]}" "${pattern}" \
            "${repeat_count}" "${plans[index]}" \
            "${output_modes[index]}" | tee -a "${output}"
    fi
...
python3 "${project_root}/scripts/validate_exp0014_evidence.py" \
