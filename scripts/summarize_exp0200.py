#!/usr/bin/env python3
"""Validate and summarize EXP-0200 O-to-Gate cross-prefetch evidence."""
...
def main() -> None:
    ...
    report = [
        "# EXP-0200 O-to-Gate direct-n cross-prefetch report",
        "",
        f"Source commit: `{args.source_commit}`",
        "",
        "Both cells inherit EXP-0199. The candidate starts the first Gate "
        "batch32 packed-W4 DMA after O and consumes it after unchanged "
        "post-attention residual/RMSNorm.",
        "",
        "## End-to-end gate",
        "",
        "| Metric | No-prefetch control | Cross-prefetch candidate | Change |",
        "|---|---:|---:|---:|",
        ...
        *pair_rows,
        ...
        "",
        "| Module | No-prefetch control | Cross-prefetch candidate | Candidate speed change |",
        "|---|---:|---:|---:|",
        *module_rows,
        ...
        f"{gate_up[CANDIDATE]:.1f} us/token. Prefetch: "
        f"`{json.dumps(prefetch, sort_keys=True)}`.",
        "",
