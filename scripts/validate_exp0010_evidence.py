...
PLAN_CONFIG = {
    "exp0005_full_bundle_control": (1, 2, 32, 2),
    "slots4_chunk32": (6, 4, 32, 2),
    "slots4_chunk64": (6, 4, 64, 1),
}
...
def assert_runtime_valid(record, repeat):
    ...
    if record["experiment"] != "EXP-0010":
        raise SystemExit(f"wrong experiment for {record_key}")
    if record["projection"] != "gate_up":
        raise SystemExit(f"non-gate projection in {record_key}")
    ...
    if record["weight_storage"] == "packed_w4_hmx_postscale":
        ...
        raise SystemExit(f"scaled-S8 carrier mismatch for {record_key}")
...
def main() -> int:
    ...
    for grouped in (correctness_groups, timing_groups):
        for pattern in PATTERNS:
            peers = [
                ...
                if group_key[2:] == ("gate_up", pattern)
                for record in records
            ]
            reference = peers[0]
            for record in peers[1:]:
                for field in comparable:
                    if record[field] != reference[field]:
                        raise SystemExit(
                            f"gate_up/{pattern}: variants differ at {field}"
                        )
    ...
    for storage, plan in VARIANTS:
        ...
        metrics[f"{storage}:{plan}"] = median_metrics(records)
    ...
    inherited_best = min(prescale, postscale32)
    improves_inherited = candidate < inherited_best
    beats_expanded = candidate < expanded

    packed_sample = correctness_groups[
        ("packed_w4_hmx_postscale", "slots4_chunk64", "gate_up", "identity")
    ][0]
        ...
        json.dumps(
            {
                ...
                "selected_candidate_plan": "slots4_chunk64",
                "local_performance_gate": improves_inherited,
                "beats_expanded_s8": beats_expanded,
                "inherited_best_host_wall_ns_median": inherited_best,
                "candidate_host_wall_ns_median": candidate,
                "expanded_s8_host_wall_ns_median": expanded,
                "candidate_vs_inherited_improvement_percent":
                    (inherited_best - candidate) * 100.0 / inherited_best,
                "candidate_vs_expanded_improvement_percent":
                    (expanded - candidate) * 100.0 / expanded,
                "metrics": metrics,
            },
            separators=(",", ":"),
        )
