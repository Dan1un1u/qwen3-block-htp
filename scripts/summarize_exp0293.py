...
def main():
 ...
 comparison={};rng=np.random.default_rng(293)
 for mode in ['prefill','decode']:
 ...
 summary=dict(experiment='EXP-0293',execution_state='completed',shape='Qwen3-0.6B full28 batch1 M64+42 cache128',measured_source=read(R/'runtime-l28.json')['seal']['source_head'],short_rounds=5,formal_rounds=10,repeat=10,formal_profiles=8600,performance=perf,comparison=comparison,local_gate='fail',gate_role='full_floating_alignment_remains_failed; new_residual_boundaries_pass',residual_adds_bit_exact=True,changed_norm_max_half_ulp=1,original_control_byte_exact_files=2451,prior_fp32_byte_exact_files=2451,cache_files=4816,full_floating_alignment_pass=False,full_floating_nrmse_max={c:max(z['nrmse'] for z in read(R/('audit/'+c+'/validated.json'))['independent_output']) for c in ['fp16','fp32']},head_boundary_pass=read(R/'head-boundary-checks.json')['pass_all'],quality_claim=False,baseline_promoted=False)
