#!/usr/bin/env python3
"""Validate and summarize EXP-0210 direct-W4 O16 single-DMA evidence."""
...
EXPERIMENT = 210
LAYERS = 28
DOWN_BATCHES = {CONTROL: 4, CANDIDATE: 4}
DOWN_COMMANDS = {CONTROL: 448, CANDIDATE: 448}
Q_BATCHES = {CONTROL: 32, CANDIDATE: 32}
QKV_COMMANDS = {CONTROL: 168, CANDIDATE: 168}
O_BATCHES = {CONTROL: 8, CANDIDATE: 16}
O_COMMANDS = {CONTROL: 224, CANDIDATE: 112}
O_SINGLE_DMA = {CONTROL: 0, CANDIDATE: 1}
...
def validate_run(run: dict[str, object], cell: str, steps: int) -> None:
    ...
    for index, profile in enumerate(profiles):
        ...
        base.require_equal(
            ...
            1, context + " O-to-Gate selector")
        base.require_equal(
            int(profile[
                "w4u8_decode_direct_n_gate_up_swiglu_stream"]),
            1, context + " Gate/Up SwiGLU stream selector")
        base.require_equal(
        ...
        base.require_equal(
            ...
            O_BATCHES[cell], context + " requested O batch")
        expected_batch = 4 if index == 0 else O_BATCHES[cell]
        expected_commands = 448 if index == 0 else O_COMMANDS[cell]
        base.require_equal(
            int(profile["w4u8_o_batch_n_tiles_observed"]),
            expected_batch, context + " observed O batch")
        base.require_equal(
            int(profile["w4u8_o_batch_count"]),
            expected_commands, context + " O command count")
        base.require_equal(
            int(profile["w4u8_decode_direct_n_down_batch_n_tiles"]),
            batch, context + " requested Down batch")
        base.require_equal(
            int(profile["w4u8_decode_direct_n_down_single_dma"]),
            1, context + " inherited Down single-DMA selector")
        base.require_equal(
            int(profile["w4u8_decode_direct_n_o_single_dma"]),
            O_SINGLE_DMA[cell], context + " O single-DMA selector")
        base.require_equal(
            int(profile["w4u8_decode_direct_n_q_batch_n_tiles"]),
            Q_BATCHES[cell], context + " Q-only batch selector")
        if index > 0:
            base.require_equal(
                int(profile["w4u8_mlp_down_hmx_command_count"]),
                DOWN_COMMANDS[cell], context + " Down command count")
...
def main() -> None:
    ...
    o_projection = {
        CONTROL: base.median_module_us(
            runs[CONTROL], ("o_projection_ticks",)),
        CANDIDATE: base.median_module_us(
            runs[CANDIDATE], ("o_projection_ticks",)),
    }
    prefetch = {
    ...
    gates = {
        ...
        "o_projection_wall_faster":
            o_projection[CANDIDATE] < o_projection[CONTROL],
        ...
        "direct_hmx_command_delta_exact":
            ...
            physical["candidate_direct_n_hmx_commands"] == 112,
        ...
        "dma_descriptor_delta_exact":
            ...
            physical["candidate_dma_descriptors"] == 224,
        "qkv_commands_preserved_at_168":
            physical["control_qkv_commands"] == QKV_COMMANDS[CONTROL] and
            physical["candidate_qkv_commands"] == QKV_COMMANDS[CANDIDATE],
        "o_commands_224_to_112":
            physical["control_o_commands"] == O_COMMANDS[CONTROL] and
            physical["candidate_o_commands"] == O_COMMANDS[CANDIDATE],
        "down_commands_preserved":
            physical["control_down_commands"] == DOWN_COMMANDS[CONTROL] and
            physical["candidate_down_commands"] == DOWN_COMMANDS[CANDIDATE],
    ...
    report = [
        "# EXP-0210 direct-W4 O16 single-DMA report",
        "",
        f"Source commit: `{args.source_commit}`",
        "",
        "Both cells inherit the evidence-valid EXP-0209 enabling candidate, "
        "including Q32/KV16 and Down batch4 single-DMA. The only runtime "
        "difference is O batch8 split-DMA versus O batch16 with one complete "
        "packed-W4 descriptor chained to one bias descriptor.",
        "",
        "## End-to-end gate",
        "",
        "| Metric | O8 control | O16 single-DMA candidate | Change |",
        "|---|---:|---:|---:|",
        ...
        *pair_rows,
        ...
        "",
        "| Module | O8 control | O16 single-DMA candidate | Candidate speed change |",
        "|---|---:|---:|---:|",
        *module_rows,
        ...
        "",
        f"O projection wall: {o_projection[CONTROL]:.1f} -> "
        f"{o_projection[CANDIDATE]:.1f} us/token. Prefetch: "
        f"`{json.dumps(prefetch, sort_keys=True)}`. Stream: "
        f"`{json.dumps(stream, sort_keys=True)}`.",
        "",
