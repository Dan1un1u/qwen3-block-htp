...
result_root="${QBH_EXP0145_RESULT_ROOT:-/mnt/d/llm_exp/results/qwen3-block-htp/exp0145}"
artifact_root="${QBH_EXP0145_ARTIFACT_ROOT:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0145/artifacts}"
...
if ! git -C "${project_root}" diff --quiet || \
   ...
   [[ -n "$(git -C "${project_root}" ls-files --others --exclude-standard)" ]]; then
    printf 'source worktree must be clean before EXP-0145 formal collection\n' >&2
    exit 1
fi
...
"${project_root}/scripts/build_exp0145.sh" > "${result_dir}/build.log" 2>&1
"${project_root}/scripts/check_exp0145_static.sh" > "${result_dir}/static_gate.json"
QBH_EXP0145_PACKAGE="${package}" \
    "${project_root}/scripts/deploy_exp0145_block.sh" \
...
for cell in "${cells[@]}"; do
    ...
    "${project_root}/scripts/run_exp0145.sh" "${cell}" 1 on on > "${raw}"
...
for round in 1 2 3 4 5; do
    ...
    for repeat in "${repeats[@]}"; do
        for cell in "${order[@]}"; do
            ...
            "${project_root}/scripts/run_exp0145.sh" \
...
python3 "${project_root}/scripts/analyze_exp0145.py" "${result_dir}" \
    > "${result_dir}/gate_summary.json"
python3 "${project_root}/scripts/analyze_exp0145.py" "${result_dir}" --report \
...
{
    printf 'experiment=EXP-0145\nsource_head=%s\n' "${source_head}"
    printf 'parent_source=EXP-0144-candidate\ncells=%s\n' "${cells[*]}"
    printf 'recipe=W4U8\nvariable=Down_nonstreaming_HMX_batch_width_only\n'
