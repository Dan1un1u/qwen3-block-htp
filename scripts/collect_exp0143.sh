...
package="${QBH_EXP0143_PACKAGE:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0042/block_package_layer14_m64_integer_attention_parallel}"
result_root="${QBH_EXP0143_RESULT_ROOT:-/mnt/d/llm_exp/results/qwen3-block-htp/exp0143}"
artifact_root="${QBH_EXP0143_ARTIFACT_ROOT:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0143/artifacts}"
...
if ! git -C "${project_root}" diff --quiet || \
   ...
   [[ -n "$(git -C "${project_root}" ls-files --others --exclude-standard)" ]]; then
    printf 'source worktree must be clean before EXP-0143 formal collection\n' >&2
    exit 1
fi
...
"${project_root}/scripts/build_exp0143.sh" > "${result_dir}/build.log" 2>&1
"${project_root}/scripts/check_exp0143_static.sh" > "${result_dir}/static_gate.json"
QBH_EXP0143_PACKAGE="${package}" \
    "${project_root}/scripts/deploy_exp0143_block.sh" \
...
for cell in "${cells[@]}"; do
    ...
    "${project_root}/scripts/run_exp0143.sh" "${cell}" 1 on on > "${raw}"
...
for round in 1 2 3 4 5; do
    ...
    for repeat in "${repeats[@]}"; do
        for cell in "${order[@]}"; do
            ...
            "${project_root}/scripts/run_exp0143.sh" \
...
python3 "${project_root}/scripts/analyze_exp0143.py" "${result_dir}" \
    > "${result_dir}/gate_summary.json"
python3 "${project_root}/scripts/analyze_exp0143.py" "${result_dir}" --report \
...
{
    printf 'experiment=EXP-0143\nsource_head=%s\n' "${source_head}"
    printf 'parent_source=EXP-0124-ring3\ncells=%s\n' "${cells[*]}"
    printf 'recipe=W4U8\nvariable=QK_Norm_RoPE_row_cache_pack_only\n'
