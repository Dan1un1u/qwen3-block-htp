...
case "${mode}" in
    control) layout=hmx_native_u8 ;;
    candidate) layout=hmx_native_u8_delta ;;
    *) exit 2 ;;
esac
case "${stage}" in
...
remote_root="${EXP0159_REMOTE_ROOT:-/data/local/tmp/qwen3-block-htp/exp0159-${mode}}"
runtime_env="QBH_W4U8_STREAM_FENCE=single_fence QBH_W4U8_GATE_UP_RING_SLOTS=16 QBH_W4U8_QKV_RING_EXPAND_WORKERS=3 QBH_KV_CACHE_LAYOUT=${layout} QBH_W4U8_PREFILL_CACHE_MODE=reuse"
...
if [[ "${QBH_EXP0159_DEPLOY:-0}" == 1 ]]; then
    EXP0159_REMOTE_ROOT="${remote_root}" \
        "${project_root}/scripts/deploy_exp0159.sh" "${mode}"
fi
