...
set -u
result_root="${QBH_RESULT_ROOT:-/mnt/d/llm_exp/results/qwen3-block-htp/exp0009}"
artifact_root="${QBH_ARTIFACT_ROOT:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0009}"
...
"${project_root}/scripts/build_exp0009.sh" \
...
run_one() {
    ...
    local output="$5"
    if [[ "${deployed}" == "0" ]]; then
        "${project_root}/scripts/run_exp0009.sh" \
            "${storage}" down "${pattern}" "${repeat_count}" "${plan}" \
            | tee -a "${output}"
        deployed=1
    else
        QBH_SKIP_DEPLOY=1 "${project_root}/scripts/run_exp0009.sh" \
            "${storage}" down "${pattern}" "${repeat_count}" "${plan}" \
            | tee -a "${output}"
    fi
}
...
python3 "${project_root}/scripts/validate_exp0009_evidence.py" \
