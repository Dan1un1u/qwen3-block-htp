...
result_root="${QBH_RESULT_ROOT:-/mnt/d/llm_exp/results/qwen3-block-htp/exp0026}"
artifact_root="${QBH_ARTIFACT_ROOT:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0026/artifacts}"
...
"${project_root}/scripts/build_exp0026.sh" > "${result_dir}/build.log" 2>&1
QBH_STATIC_OUTPUT_DIR="${result_dir}/static" \
    "${project_root}/scripts/check_exp0026_static.sh" \
...
for mode in scalar hvx fused; do
    for variant in F16F16 W4F16; do
        "${project_root}/scripts/run_exp0026_block.sh" \
            "${variant}" 1 on on "${mode}" hvx \
...
for round in 1 2 3 4 5; do
    if (( round % 2 == 1 )); then
        ...
        modes=(fused hvx scalar)
        variants=(W4F16 F16F16)
    fi
    for variant in "${variants[@]}"; do
        for mode in "${modes[@]}"; do
            "${project_root}/scripts/run_exp0026_block.sh" \
                "${variant}" 1 on off "${mode}" hvx \
                >> "${result_dir}/timing_${mode}_${variant}_repeat1.jsonl"
            "${project_root}/scripts/run_exp0026_block.sh" \
                "${variant}" 10 on off "${mode}" hvx \
                >> "${result_dir}/timing_${mode}_${variant}_repeat10.jsonl"
        done
    done
done
...
python3 "${project_root}/scripts/validate_exp0026_evidence.py" \
