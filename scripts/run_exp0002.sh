...
remote_root="${REMOTE_ROOT:-/data/local/tmp/qwen3-block-htp/exp0002}"
pattern="${1:-identity}"
repeat_count="${2:-1}"
...
if [[ "${QBH_DETACH:-0}" == "1" ]]; then
    ...
    "${adb_exe}" shell \
        "cd ${remote_root} && (LD_LIBRARY_PATH=${remote_root} DSP_LIBRARY_PATH=${remote_root} ADSP_LIBRARY_PATH=${remote_root} ./qwen3_probe_cli ${pattern} ${repeat_count} > ${remote_log} 2>&1 < /dev/null &)"
    printf 'DETACHED_LOG=%s\n' "${remote_log}"
    exit 0
fi

"${adb_exe}" shell \
    "cd ${remote_root} && LD_LIBRARY_PATH=${remote_root} DSP_LIBRARY_PATH=${remote_root} ADSP_LIBRARY_PATH=${remote_root} ./qwen3_probe_cli ${pattern} ${repeat_count}"
