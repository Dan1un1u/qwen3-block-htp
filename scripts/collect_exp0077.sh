...
package="${QBH_EXP0077_PACKAGE:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0042/block_package_layer14_m64_integer_attention_parallel}"
result_root="${QBH_EXP0077_RESULT_ROOT:-/mnt/d/llm_exp/results/qwen3-block-htp/exp0077}"
artifact_root="${QBH_EXP0077_ARTIFACT_ROOT:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0077/artifacts}"
...
"${project_root}/scripts/build_exp0077.sh" > "${result_dir}/build.log" 2>&1
"${project_root}/scripts/check_exp0077_static.sh" > "${result_dir}/static_gate.json"
QBH_EXP0077_PACKAGE="${package}" \
    "${project_root}/scripts/deploy_exp0077_block.sh" \
...
for mode in control candidate; do
    "${project_root}/scripts/run_exp0077.sh" "${mode}" 1 on on \
...
for round in 1 2 3 4 5 6 7; do
    printf 'EXP-0077 paired round %d/%d\n' "${round}" "${paired_rounds}"
    if (( round % 2 == 1 )); then
        repeats=(1 10)
    ...
    for repeat in "${repeats[@]}"; do
        for mode in "${modes[@]}"; do
            "${project_root}/scripts/run_exp0077.sh" \
                "${mode}" "${repeat}" on off \
                >> "${result_dir}/paired_${mode}_r${repeat}.jsonl"
        done
    done
...
python3 "${project_root}/scripts/validate_exp0077.py" \
...
{
    printf 'experiment=EXP-0077\nsource_branch=%s\nsource_head=%s\n' \
        "${source_branch}" "${source_head}"
    printf 'execution_unit=qwen3_layer14_complete_block_m64\n'
    printf 'control=exp0075_gate_up_internal_linked_dma_start\n'
    printf 'candidate=first_chain4_gate_up_dma_overlaps_post_residual\n'
