...
result_root="${QBH_RESULT_ROOT:-/mnt/d/llm_exp/results/qwen3-block-htp/exp0036}"
artifact_root="${QBH_ARTIFACT_ROOT:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0036/artifacts}"
...
run_config() {
    ...
    local mlp_mode
    case "${config}" in
        control) mlp_mode=streaming ;;
        candidate) mlp_mode=crouton_native ;;
...
python3 "${project_root}/scripts/validate_exp0036_evidence.py" \
...

{
    ...
    printf 'configs=control,candidate\n'
