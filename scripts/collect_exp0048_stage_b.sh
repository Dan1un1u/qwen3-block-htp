...
"${project_root}/scripts/check_exp0048_stage_b_static.sh" \
...
REMOTE_ROOT="${remote_root}" QBH_EXP0048_PACKAGE="${package}" \
    ...
    w4u8_mlp_io_qkv_o > "${result_dir}/attention_audit.log"
...
for round in 1 2 3 4 5 6 7; do
    printf 'EXP-0048 Stage B paired round %d/%d\n' "${round}" "${paired_rounds}"
    if (( round % 2 == 1 )); then
        repeat_order=(1 10)
        mode_order=(o_native qkv_native)
    else
        repeat_order=(10 1)
        mode_order=(qkv_native o_native)
    fi
...
python3 "${project_root}/scripts/validate_exp0048_stage_b.py" \
...

{
    ...
    printf 'control=EXP-0048-Stage-A-QKV-native\n'
    printf 'candidate=O-native-HMX-output-to-post-attention-residual\n'
