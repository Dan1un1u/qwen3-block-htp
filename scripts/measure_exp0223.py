...
from experiment_exp0223 import RESULT,OUTPUT,VARIANTS
from clipping_exp0223 import PREVIOUS_MODELS
...
REMOTE='/data/local/tmp/qwen3-block-htp/exp0223-'
CONTROLS=['A0','B0','C0']
ALL=['A0','A','B0','B','C0','C']

def remote(v):return '/data/local/tmp/qwen3-block-htp/exp0221-'+v[0] if v in CONTROLS else REMOTE+v

def package_path(v):return PREVIOUS_MODELS/v[0] if v in CONTROLS else OUTPUT/v

def deploy(variant):
    ev.dataset();root=remote(variant)
    assert variant in VARIANTS
    FROZEN_REMOTE='/data/local/tmp/qwen3-block-htp/exp0221-'+variant
...
def run(variant,phase,index=None):
    ...
    write_json(path.with_suffix('.provenance.json'),dict(remote_root=remote(variant),variant=variant,experiment='EXP-0223'))
