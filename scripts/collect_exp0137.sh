...
u8_package="${QBH_EXP0137_U8_PACKAGE:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0042/block_package_layer14_m64_integer_attention_parallel}"
result_root="${QBH_EXP0137_RESULT_ROOT:-/mnt/d/llm_exp/results/qwen3-block-htp/exp0137}"
artifact_root="${QBH_EXP0137_ARTIFACT_ROOT:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0137/artifacts}"
...
if ! git -C "${project_root}" diff --quiet || \
   ...
   [[ -n "$(git -C "${project_root}" ls-files --others --exclude-standard)" ]]; then
    printf 'source worktree must be clean before EXP-0137 formal collection\n' >&2
    exit 1
fi
...
"${project_root}/scripts/build_exp0137.sh" > "${result_dir}/build.log" 2>&1
"${project_root}/scripts/check_exp0137_static.sh" > "${result_dir}/static_gate.json"
QBH_EXP0137_U8_PACKAGE="${u8_package}" \
    "${project_root}/scripts/deploy_exp0137_block.sh" \
...
for cell in "${cells[@]}"; do
    ...
    "${project_root}/scripts/run_exp0137.sh" "${cell}" 1 on on > "${raw}"
...
for round in 1 2 3 4 5; do
    printf 'EXP-0137 round %d/5\n' "${round}"
    if ((round % 2 == 1)); then
        repeats=(1 10)
        order=(cap0 cap8 cap16)
    else
        repeats=(10 1)
        order=(cap16 cap8 cap0)
    fi
    for repeat in "${repeats[@]}"; do
        for cell in "${order[@]}"; do
            raw="${result_dir}/round_${round}_${cell}_r${repeat}_raw.jsonl"
            "${project_root}/scripts/run_exp0137.sh" \
...
python3 "${project_root}/scripts/analyze_exp0137.py" "${result_dir}" \
    > "${result_dir}/gate_summary.json"
python3 "${project_root}/scripts/analyze_exp0137.py" "${result_dir}" --report \
...
{
    printf 'experiment=EXP-0137\nsource_head=%s\n' "${source_head}"
    printf 'logical_parent=EXP-0120\nsource_parent=EXP-0136\ncells=%s\n' "${cells[*]}"
    printf 'recipe=W4U8\nvariable=gate_up_adaptive_hvx_lead_credit\n'
