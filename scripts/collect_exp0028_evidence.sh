...
result_root="${QBH_RESULT_ROOT:-/mnt/d/llm_exp/results/qwen3-block-htp/exp0028}"
artifact_root="${QBH_ARTIFACT_ROOT:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0028/artifacts}"
...
"${project_root}/scripts/build_exp0028.sh" > "${result_dir}/build.log" 2>&1
QBH_STATIC_OUTPUT_DIR="${result_dir}/static" \
    "${project_root}/scripts/check_exp0028_static.sh" \
...
configs=(f16_batch2 w4_exp27 w4_tuned32 w4_adaptive96)
for config in "${configs[@]}"; do
    case "${config}" in
        f16_batch2)
            variant=F16F16; workers=2; region=32
            f16_mode=batch2; w4_mode=control ;;
        w4_exp27)
            variant=W4F16; workers=2; region=16
            f16_mode=serial; w4_mode=control ;;
        w4_tuned32)
            variant=W4F16; workers=2; region=32
            f16_mode=serial; w4_mode=control ;;
        w4_adaptive96)
            variant=W4F16; workers=3; region=32
            f16_mode=serial; w4_mode=adaptive_down96_cross ;;
    esac
    "${project_root}/scripts/run_exp0028_block.sh" \
        "${variant}" 1 on on fused "${f16_mode}" hvx \
        "${workers}" "${region}" "${w4_mode}" \
...
for round in 1 2 3 4 5; do
    if (( round % 2 == 1 )); then
        order=(f16_batch2 w4_exp27 w4_tuned32 w4_adaptive96)
    else
        order=(w4_adaptive96 w4_tuned32 w4_exp27 f16_batch2)
    fi
    for config in "${order[@]}"; do
        ...
        "${project_root}/scripts/run_exp0028_block.sh" \
            "${variant}" 1 on off fused "${f16_mode}" hvx \
            "${workers}" "${region}" "${w4_mode}" \
            >> "${result_dir}/timing_${config}_repeat1.jsonl"
        "${project_root}/scripts/run_exp0028_block.sh" \
            "${variant}" 10 on off fused "${f16_mode}" hvx \
            "${workers}" "${region}" "${w4_mode}" \
            >> "${result_dir}/timing_${config}_repeat10.jsonl"
    done
done
...
python3 "${project_root}/scripts/validate_exp0028_evidence.py" \
...
{
    printf 'experiment=EXP-0028\n'
    printf 'source_branch=%s\n' "$(git -C "${project_root}" branch --show-current)"
    printf 'source_head=%s\n' "${source_head}"
    printf 'execution_unit=qwen3_layer14_complete_block_m64\n'
    printf 'configs=f16_batch2,w4_exp27,w4_tuned32,w4_adaptive96\n'
    printf 'correctness_numerical_audit=on\n'
    printf 'performance_numerical_audit=off\n'
    printf 'common_ops_mode=hvx_fp16\n'
    printf 'residual_mode=hvx_fused_post_norm\n'
