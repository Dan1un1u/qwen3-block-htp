...
result_root="${QBH_EXP0100_RESULT_ROOT:-/mnt/d/llm_exp/results/qwen3-block-htp/exp0100}"
artifact_root="${QBH_EXP0100_ARTIFACT_ROOT:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0100/artifacts}"
...
if ! git -C "${project_root}" diff --quiet || \
   ...
   [[ -n "$(git -C "${project_root}" ls-files --others --exclude-standard)" ]]; then
    printf 'source worktree must be clean before EXP-0100 formal collection\n' >&2
    exit 1
fi
...
if [[ -e "${result_dir}" || -e "${artifact_dir}" ]]; then
    printf 'EXP-0100 evidence destination already exists\n' >&2
    exit 1
fi
mkdir -p "${result_dir}" "${artifact_dir}"
"${project_root}/scripts/build_exp0100.sh" > "${result_dir}/build.log" 2>&1
"${project_root}/scripts/check_exp0100_static.sh" \
    > "${result_dir}/static_gate.json"
QBH_EXP0100_PACKAGE="${package}" \
    "${project_root}/scripts/deploy_exp0100_block.sh" \
...
for mode in "${modes[@]}"; do
    ...
    "${project_root}/scripts/run_exp0100.sh" "${mode}" 1 on on > "${raw}"
...
for ((round = 1; round <= rounds; ++round)); do
    ...
    for repeat in "${repeats[@]}"; do
        for mode in "${order[@]}"; do
            ...
            "${project_root}/scripts/run_exp0100.sh" \
...
python3 "${project_root}/scripts/analyze_exp0100.py" \
...
cp "${project_root}/hexagon_ReleaseG_toolv19_v79/ship/libqwen3_probe_skel.so" \
    "${artifact_dir}/"
{
    printf 'experiment=EXP-0100\nsource_head=%s\n' "${source_head}"
    printf 'control=o_projection_four_output_tiles_per_hmx_command\n'
    printf 'candidate=o_projection_eight_output_tiles_per_hmx_command\n'
