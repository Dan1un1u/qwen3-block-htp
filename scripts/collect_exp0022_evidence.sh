...
result_root="${QBH_RESULT_ROOT:-/mnt/d/llm_exp/results/qwen3-block-htp/exp0022}"
artifact_root="${QBH_ARTIFACT_ROOT:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0022/artifacts}"
...
for variant in F16F16 W4F16 W4U8; do
    "${project_root}/scripts/run_exp0022_block.sh" "${variant}" 1 \
        > "${result_dir}/correctness_${variant}.jsonl"
    : > "${result_dir}/timing_${variant}_repeat1.jsonl"
    : > "${result_dir}/timing_${variant}_repeat10.jsonl"
    for round in 1 2 3 4 5; do
        "${project_root}/scripts/run_exp0022_block.sh" "${variant}" 1 \
            >> "${result_dir}/timing_${variant}_repeat1.jsonl"
        "${project_root}/scripts/run_exp0022_block.sh" "${variant}" 10 \
            >> "${result_dir}/timing_${variant}_repeat10.jsonl"
    done
done
...
if [[ -f "${package_dir}/export_summary.json" ]]; then
    cp "${package_dir}/export_summary.json" "${result_dir}/"
fi
python3 "${project_root}/scripts/validate_exp0022_evidence.py" \
...

{
    ...
    printf 'variants=F16F16,W4F16,W4U8\n'
