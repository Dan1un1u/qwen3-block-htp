...
package="${QBH_EXP0088_PACKAGE:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0042/block_package_layer14_m64_integer_attention_parallel}"
remote_root="${EXP0088_REMOTE_ROOT:-/data/local/tmp/qwen3-block-htp/exp0088}"
...
for artifact in "${artifacts[@]}"; do
    if [[ ! -f "${artifact}" ]]; then
        printf 'missing EXP-0088 artifact: %s\n' "${artifact}" >&2
        exit 1
    fi
done
