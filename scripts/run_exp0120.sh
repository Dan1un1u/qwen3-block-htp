...
remote_root="${EXP0120_REMOTE_ROOT:-/data/local/tmp/qwen3-block-htp/exp0120-u8}"
cell="${1:-ring8}"
repeat_count="${2:-1}"
...
case "${cell}" in
    ...
    ring16)
        ...
        ;;
    *)
        printf 'usage: %s ring8|ring16 [repeat] [attribution] [audit]\n' "$0" >&2
        exit 2
        ;;
esac
...
"${adb_exe}" shell \
    "cd ${remote_root} && QBH_W4U8_STREAM_FENCE=single_fence QBH_W4U8_GATE_UP_RING_SLOTS=${ring_slots} LD_LIBRARY_PATH=${remote_root} DSP_LIBRARY_PATH=${remote_root} ADSP_LIBRARY_PATH=${remote_root} ./qwen3_block_cli ${remote_root}/block_package_layer14_m64 W4U8 ${repeat_count} ${runtime_args}"
