...
case "${cell}" in
    control)
        ...
        ;;
    quartet)
        cache_layout=hmx_native_u8_segmented_quartet_v5
        qk_padding_poison=0
        ;;
    quartet_poison)
        cache_layout=hmx_native_u8_segmented_quartet_v5
        qk_padding_poison=1
        ;;
    *) printf 'invalid EXP-0180 cell: %s\n' "${cell}" >&2; exit 2 ;;
esac

EXP0173_REMOTE_ROOT=/data/local/tmp/qwen3-block-htp/exp0173-exp0180-w4u8 \
...
QBH_EXP0180_KV_CACHE_LAYOUT="${cache_layout}" \
