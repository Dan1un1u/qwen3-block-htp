...
result_root="${QBH_RESULT_ROOT:-/mnt/d/llm_exp/results/qwen3-block-htp/exp0042}"
artifact_root="${QBH_ARTIFACT_ROOT:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0042/artifacts}"
package="${QBH_EXP0042_PACKAGE:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0042/block_package_layer14_m64_integer_attention_parallel}"
...
"${project_root}/scripts/build_exp0042.sh" \
    > "${result_dir}/build.log" 2>&1
"${project_root}/scripts/check_exp0042_static.sh" \
    > "${result_dir}/static_gate.json"
QBH_EXP0042_PACKAGE="${package}" \
...
for round in $(seq 1 "${paired_rounds}"); do
    printf 'EXP-0042 paired round %d/%d\n' \
        "${round}" "${paired_rounds}"
    if (( round % 2 == 1 )); then
        repeat_order=(1 10)
    ...
    for repeat in "${repeat_order[@]}"; do
        for mode in "${mode_order[@]}"; do
            "${project_root}/scripts/run_exp0042_block.sh" \
                "${mode}" "${repeat}" on off \
...
python3 "${project_root}/scripts/validate_exp0042_evidence.py" \
...
{
    printf 'experiment=EXP-0042\n'
    printf 'source_branch=%s\n' "$(git -C "${project_root}" branch --show-current)"
    printf 'source_head=%s\n' "${source_head}"
    printf 'execution_unit=qwen3_layer14_complete_block_m64\n'
    printf 'control=EXP0041_W4U8_FP16_Attention_island\n'
    printf 'candidate=integer_QK_log2_U8_Softmax_integer_AV\n'
