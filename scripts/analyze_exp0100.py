#!/usr/bin/env python3
"""Validate EXP-0100 and render its PC-027/PC-028 closure."""
...
TARGETS = ("o_projection_ticks", "host_wall_ns_per_block")
...
PHYSICAL = tuple(dict.fromkeys((
    *base.COUNTERS, *base.RESOURCES, "block_invocation_count",
    "w4u8_o_batch_n_tiles", "w4u8_o_batch_n_tiles_observed",
    "w4u8_o_batch_count",
)))
...
def validate_record(record: dict[str, object], repeat: int,
    ...
    fixed = {
        ...
        "weight_dma_descriptor_count": (
            512 if mode == "control" else 496
        ) * repeat,
        "hmx_command_count": (176 if mode == "control" else 168) * repeat,
        "hmx_u8s8_tile_pair_count": 49_408 * repeat,
        "w4u8_qkv_batch_n_tiles": 4,
        "w4u8_qkv_batch_count": 32 * repeat,
        "w4u8_qkvo_prefetch_count": (
            44 if mode == "control" else 36
        ) * repeat,
        "w4u8_qkvo_overlap_schedule_count": (
            44 if mode == "control" else 36
        ) * repeat,
        "w4u8_o_batch_count": (16 if mode == "control" else 8) * repeat,
...
def build_summary(result_dir: Path, package_dir: Path) -> dict[str, object]:
    if (result_dir / "boot_id_before.txt").read_bytes() != \
        ...
        raise SystemExit("device boot ID changed during EXP-0100")
    ...
    for mode in MODES:
        ...
        correctness[mode] = {
            ...
            "o_batch_tiles": record["w4u8_o_batch_n_tiles_observed"],
            "o_commands": record["w4u8_o_batch_count"],
    ...
    return {
        "experiment": "EXP-0100",
        "control": "four O output tiles per HMX command",
        "candidate": "eight O output tiles per HMX command",
        ...
        "pc028": {
            ...
            "w4u8_provenance": (
                "EXP-0100 batch8" if local_gate else "EXP-0100 control"
            ),
...
def render_report(summary: dict[str, object]) -> str:
    lines = ["# EXP-0100 — Complete profiling report", ""]
    add_pc028(lines, summary)
    lines.extend([
        "The candidate doubles only O-projection output batching from four "
        "to eight tiles. QKV stays at batch four. Both cells use the same "
        "enlarged VTCM plan, while the candidate reduces O commands 16→8, "
        "total HMX commands 176→168 and DMA descriptors 512→496 per block.", "",
    ])
    for repeat in REPEATS:
        ...
        lines.extend([
            "O projection and complete Host speed gate: "
            f"**{'PASS' if result['o_and_host_speed_gate'] else 'FAIL'}**.", "",
        ])
    lines.extend([
        ...
        "| Weight bytes | 25,444,352 per block |",
        "| U8xS8 tile pairs | 49,408 per block |",
        "| O / total HMX commands | 16→8 / 176→168 per block |",
        "| DMA descriptors | 512→496 per block |", "",
        "## Decision", "",
        f"EXP-0100 local gate: **{'PASS' if summary['local_gate_pass'] else 'FAIL'}**. "
