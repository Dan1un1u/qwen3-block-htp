...
remote_root="${EXP0112_REMOTE_ROOT:-/data/local/tmp/qwen3-block-htp/exp0112-u8}"
cell="${1:-control}"
repeat_count="${2:-1}"
attribution_mode="${3:-on}"
audit_mode="${4:-off}"
stream_fence=control
...
case "${cell}" in
    control)
        ;;
    single_fence)
        stream_fence=single_fence
        ;;
    release_only)
        stream_fence=release_only
        ;;
    *)
        printf 'usage: %s control|single_fence|release_only [repeat] [attribution] [audit]\n' "$0" >&2
        exit 2
        ;;
esac
...
"${adb_exe}" shell \
    "cd ${remote_root} && QBH_W4U8_STREAM_FENCE=${stream_fence} LD_LIBRARY_PATH=${remote_root} DSP_LIBRARY_PATH=${remote_root} ADSP_LIBRARY_PATH=${remote_root} ./qwen3_block_cli ${remote_root}/block_package_layer14_m64 W4U8 ${repeat_count} ${runtime_args}"
