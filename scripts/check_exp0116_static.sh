...
grep -q 'QBH_BLOCK_EXPERIMENT UINT32_C(116)' \
    "${project_root}/include/block_protocol.h"
grep -q 'QBH_BLOCK_W4F16_GROUP_FENCE_JOIN_ONLY_DOWN' \
    "${project_root}/include/block_protocol.h"
grep -q 'strcmp(group_fence, "join_only_down")' \
    "${project_root}/src/host/block_main.c"
grep -q 'relaxed_down_group_fence' \
    "${project_root}/src/dsp/block_imp.c"
grep -q 'desc == &header->projections\[QBH_BLOCK_PROJ_DOWN\]' \
    "${project_root}/src/dsp/block_imp.c"
...
if grep -RInE 'QnnGraph|QnnContext|QnnBackend|libQnn' \
    ...
    printf 'QNN dependency found in EXP-0116 source\n' >&2
    exit 1
fi
printf '%s\n' '{"experiment":"EXP-0116","static_gate":"pass","recipe":"W4F16","control_join_only_selectable":true,"candidate_join_only_down_selectable":true,"down_only_relaxed_selector":true,"ordered_unpack_has_barrier":true,"relaxed_unpack_has_barrier":false,"pool_join_barrier_retained":true,"one_hmx_owner":true,"qnn_dependency":false}'
