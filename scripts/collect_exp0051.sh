...
package="${QBH_EXP0051_PACKAGE:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0042/block_package_layer14_m64_integer_attention_parallel}"
result_root="${QBH_EXP0051_RESULT_ROOT:-/mnt/d/llm_exp/results/qwen3-block-htp/exp0051}"
artifact_root="${QBH_EXP0051_ARTIFACT_ROOT:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0051/artifacts}"
...
"${project_root}/scripts/build_exp0051.sh" \
    > "${result_dir}/build.log" 2>&1
"${project_root}/scripts/check_exp0051_static.sh" \
    > "${result_dir}/static_gate.json"
QBH_EXP0051_PACKAGE="${package}" \
    "${project_root}/scripts/deploy_exp0051_block.sh" \
...
for mode in control candidate; do
    ...
    "${project_root}/scripts/run_exp0051.sh" \
        "${run_mode}" 1 on on > "${result_dir}/correctness_${mode}.jsonl"
done
...
for round in 1 2 3 4 5 6 7; do
    printf 'EXP-0051 paired round %d/%d\n' "${round}" "${paired_rounds}"
    if (( round % 2 == 1 )); then
        repeats=(1 10)
...
python3 "${project_root}/scripts/validate_exp0051.py" \
...
{
    printf 'experiment=EXP-0051\n'
    printf 'source_branch=%s\n' "${source_branch}"
    printf 'source_head=%s\n' "${source_head}"
    printf 'control_source_head=%s\n' "${source_head}"
    printf 'execution_unit=qwen3_layer14_complete_block_m64\n'
    printf 'control=W4U8-EXP0050-DOWN-COMMAND-FUSION\n'
    printf 'candidate=Q-generation-safe-head-pair-command\n'
