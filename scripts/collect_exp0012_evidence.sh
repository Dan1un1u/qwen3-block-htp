...
set -u
result_root="${QBH_RESULT_ROOT:-/mnt/d/llm_exp/results/qwen3-block-htp/exp0012}"
artifact_root="${QBH_ARTIFACT_ROOT:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0012}"
...
"${project_root}/scripts/build_exp0012.sh" \
...
storages=(
    expanded_s8_control expanded_s8_control expanded_s8_control
...
projections=(
    gate_up gate_up gate_up gate_up gate_up gate_up gate_up
...
run_one() {
    ...
    local output="$4"
    if [[ "${deployed}" == "0" ]]; then
        "${project_root}/scripts/run_exp0012.sh" \
            "${storages[index]}" "${projections[index]}" "${pattern}" \
            "${repeat_count}" "${plans[index]}" | tee -a "${output}"
        deployed=1
    else
        QBH_SKIP_DEPLOY=1 "${project_root}/scripts/run_exp0012.sh" \
            "${storages[index]}" "${projections[index]}" "${pattern}" \
            "${repeat_count}" "${plans[index]}" | tee -a "${output}"
    fi
}
...
python3 "${project_root}/scripts/validate_exp0012_evidence.py" \
