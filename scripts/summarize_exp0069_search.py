#!/usr/bin/env python3
"""Select the EXP-0069 active GQA Attention context candidate."""
...
MODES = ("control", "attention4", "attention5")
FIELDS = (
...
def main() -> None:
    ...
    expected_active = {"control": 6, "attention4": 4, "attention5": 5}
    for mode in MODES:
        ...
        for record in records:
            if record.get("experiment") != "EXP-0069":
                raise SystemExit(f"wrong experiment in {path}")
            if record.get("attention_hvx_contexts") != 6:
                raise SystemExit(f"wrong persistent context count in {path}")
            if record.get("attention_active_contexts") != expected_active[mode]:
                raise SystemExit(f"wrong active context count in {path}")
            if record.get("attention_hvx_workers_created") != 5 or \
                    record.get("attention_hvx_workers_locked") != 5:
                raise SystemExit(f"wrong persistent worker count in {path}")
        ...
        summary: dict[str, object] = {
            "samples": len(records),
            "qk_prep_contexts": 6,
            "attention_active_contexts": expected_active[mode],
            "persistent_hvx_workers": 5,
        }
        for field in FIELDS:
    ...
    print(json.dumps({
        ...
        "selected_attention_active_contexts": expected_active[selected_mode],
        "qk_prep_contexts": 6,
        "modes": modes,
