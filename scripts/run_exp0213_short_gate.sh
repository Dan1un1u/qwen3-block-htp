...
"${project_root}/scripts/build_exp0213.sh" > "${result_dir}/build.log" 2>&1
"${project_root}/scripts/check_exp0213_static.sh" > "${result_dir}/static_gate.json"
"${project_root}/scripts/deploy_exp0213.sh" > "${result_dir}/deploy.log" 2>&1
for cell in control next_q; do
    ...
    QBH_GENERATION_AUDIT_DIR="${result_dir}/audit/${cell}" \
        "${project_root}/scripts/run_exp0213.sh" "${cell}" \
...
"${python_exe}" "${project_root}/scripts/summarize_exp0213.py" \
