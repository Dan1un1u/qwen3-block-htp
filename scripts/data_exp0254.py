...
from data_exp0253 import prior_data as old_prior, verified as previous_verified
from data_exp0233 import ids_of
from gptq_exp0221 import fetch
RESULT=BASE/'exp0254'
def verified(exp,name):
 if exp!='exp0253':return previous_verified(exp,name)
 ledger=BASE/exp/'EVIDENCE_SHA256.json'
 assert sha(ledger)=='f8dbb4d50965546f69c2ad853bcdd05d49d3fa8c36ce79f0c64d4f967d5bec1d'
...
def prior_data():
 paths,ds,rows=old_prior();p=verified('exp0253','dataset.json');d=json.loads(p.read_text())
 return paths+[p],ds+[d],rows+d['samples']
def frozen():
 f=json.loads((RESULT/'dataset_freeze.json').read_text())
 assert sha(MEMORY/'docs/experiments/EXP-0254.md')==f['protocol_sha256']
...
def prepare():
 ...
 rng=np.random.default_rng(254);chosen={c:[] for c in CELLS};used=set();grams=set();hashes=set();sources=[]
 ...
 for lang in ['en','zh']:
  ...
  if len(chosen[cell])<64:
   ...
   for off in range(144000,154000,100):
 ...
 for i in range(64):
  for c in CELLS:
   r=chosen[c][i];r.update(id=len(samples),split='triage' if i<32 else 'confirmation',prompt_ids=r['token_ids'][:64],target_ids=r['token_ids'][64:]);samples.append(r)
 write('dataset.json',dict(version='qbh-EXP254-ordered-joint-two-panels-v1',seed=254,cells=CELLS,samples=samples,provenance=sources,tokenizer_sha256=sha(MODEL/'qwen3-tokenizer.json'),calibration_reference='EXP230_C64_65536',final_targets=4096,prior_paths=[str(p) for p in paths]))
 ...
 write('dataset_freeze.json',dict(protocol_sha256=sha(MEMORY/'docs/experiments/EXP-0254.md'),files={'dataset.json':sha(RESULT/'dataset.json')},references={str(p):sha(p) for p in refs},frozen_before_export_and_scoring=True))
