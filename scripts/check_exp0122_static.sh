...
grep -q 'QBH_BLOCK_EXPERIMENT UINT32_C(122)' \
    "${project_root}/include/block_protocol.h"
grep -q 'QBH_PHYSICAL_PLAN_STREAMING_E7_DMA_CHAIN8 = 16' \
    "${project_root}/include/probe_protocol.h"
grep -q 'linked_descriptors\[8\]' \
    "${project_root}/src/dsp/w4_parallel_pipeline.c"
grep -q 'bundle_count > 8U' \
    "${project_root}/src/dsp/w4_parallel_pipeline.c"
grep -q 'QBH_W4U8_GATE_UP_DMA_CHAIN' \
...
if grep -RInE 'QnnGraph|QnnContext|QnnBackend|libQnn' \
    ...
    printf 'QNN dependency found in EXP-0122 source\n' >&2
    exit 1
fi
printf '%s\n' '{"experiment":"EXP-0122","static_gate":"pass","recipe":"W4U8","ring_slots":16,"dma_chain4_runtime_selectable":true,"dma_chain8_runtime_selectable":true,"persistent_hvx_workers":3,"hmx_batch_outputs":8,"stream_fence":"single_fence","one_hmx_owner":true,"qnn_dependency":false}'
