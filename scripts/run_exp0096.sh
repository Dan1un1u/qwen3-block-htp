...
remote_root="${EXP0096_REMOTE_ROOT:-/data/local/tmp/qwen3-block-htp/exp0096-u8}"
cell="${1:-control}"
repeat_count="${2:-1}"
...
case "${cell}" in
    control) mlp_mode="w4u8_streaming_persistent_mlp_hvx" ;;
    candidate)
        mlp_mode="w4u8_streaming_persistent_mlp_hvx_arithmetic_activation"
        ;;
    *)
...
runtime_args="2 32 rms_rope_softmax ${attribution_mode} ${audit_mode} fused_pool6 serial control hvx ${mlp_mode} 3 64 ${attention_pipeline} 6 w4u8_mlp_io_qkv_o qkvo_batch4_qk_head_pairs hvx_tree_qk_batched_rsqrt_shared_rope_parallel_input control 4 4"
