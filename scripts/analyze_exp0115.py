#!/usr/bin/env python3
"""Validate and report EXP-0115 W4F16 unmasked-low decode."""
...
def validate_record(record: dict, repeat: int, cell: str,
    ...
    base.require(record, "experiment", "EXP-0115")
    base.require(
        record, "w4f16_group_fence_mode",
        "join_only" if cell == "control" else "join_only_unmasked",
    )
...
def build_summary(result_dir: Path, exp0109_dir: Path) -> dict:
    if ((result_dir / "boot_id_before.txt").read_bytes() !=
        ...
        raise SystemExit("device boot ID changed during EXP-0115")
...
def render_report(summary: dict) -> str:
    lines = ["# EXP-0115 — Complete profiling report", ""]
    ...
    lines.extend([
        ...
        f"| EXP-0115 local gate | {'PASS' if summary['local_gate_pass'] else 'FAIL'} |",
