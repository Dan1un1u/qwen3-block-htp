"""EXP-0295 isolated long-prefill fixtures, build deployment and immutable runs."""
...
SIZE=os.environ.get('QBH_QWEN_MODEL_SIZE','0.6B')
...
R=Path('/mnt/d/llm_exp/results/qwen3-block-htp/exp0295')/SIZE
M=Path('/mnt/d/llm_exp/models/qwen3-block-htp/exp0295')/SIZE
BASE=R/'base.json'
REMOTE='/data/local/tmp/qwen3-block-htp/exp0295/'+SIZE
...
def preflight():
 ...
 assert 'EXPERIMENT=EXP-0295' in z
def prepare():
 preflight()
 import torch
 ...
 for length in [64,65,128,129,536,741]:
  ...
  put(dst/'manifest.json',dict(experiment='EXP-0295',parent=base,parent_sha256=base['manifest_sha256'],files=files,changed=changed))
...
def run(length,tag,decode=3,audit=False,repeat=1,legacy=False):
 ...
 if not legacy:env.update(QBH_LONG_PREFILL_TOKENS=str(length),QBH_LONG_DECODE_STEPS=str(decode),QBH_LONG_REPEATS=str(repeat),QBH_KV_CACHE_CAPACITY='832',QBH_GENERATION_EXPECTED_TOKENS='64')
  ...
  env['QBH_GENERATION_STEPS']='4'
  # Same frozen short-cache layout, ten independent trajectories in one process.
  import struct
  fixture=read(R/'fixture.json');words=[0x51424556,2,repeat,71]
  for sample in range(repeat):words += [sample,3,4]+fixture['prompt_ids'][:64]+fixture['fixed'][:4]
