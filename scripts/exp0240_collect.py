...
def main():
 ...
 (d.RESULT/'formal_provenance.json').write_text(json.dumps(before,indent=2)+'\n')
 for phase,rounds in [('short',5),('formal',10)]:
  for round in range(rounds):
   for repeat in [1,10]:
    order=['control','lpbq32'] if (round+repeat)%2 else ['lpbq32','control']
    for cell in order:
     tag=f'{phase}_{round:02d}_r{repeat}_{cell}'
 ...
 for i in range(5):
  x=d.run('matched',10,f'schedule_{i:02d}_matched');assert all(v['output_hash']==gold['control'][v['replay_step']] for v in x)
