...
package="${QBH_EXP0045_PACKAGE:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0042/block_package_layer14_m64_integer_attention_parallel}"
result_root="${QBH_EXP0045_RESULT_ROOT:-/mnt/d/llm_exp/results/qwen3-block-htp/exp0045}"
artifact_root="${QBH_EXP0045_ARTIFACT_ROOT:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0045/artifacts}"
...
"${project_root}/scripts/build_exp0045.sh" \
    > "${result_dir}/build.log" 2>&1
"${project_root}/scripts/check_exp0045_stage_b_static.sh" \
    > "${result_dir}/static_gate.json"
QBH_EXP0045_PACKAGE="${package}" \
...
QBH_EXP0045_PACKAGE="${package}" \
    "${project_root}/scripts/audit_exp0045_attention.sh" \
    "${result_dir}/attention_implementation_audit" qkvo_batch4 \
...
for round in $(seq 1 "${paired_rounds}"); do
    printf 'EXP-0045 Stage B paired round %d/%d\n' \
        "${round}" "${paired_rounds}"
    if (( round % 2 == 1 )); then
        repeat_order=(1 10)
        mode_order=(qkv_batch4 qkvo_batch4 serial)
    else
        repeat_order=(10 1)
        mode_order=(serial qkvo_batch4 qkv_batch4)
    fi
...
python3 "${project_root}/scripts/validate_exp0045_stage_b.py" \
...
{
    printf 'experiment=EXP-0045\n'
    printf 'stage=B\n'
    printf 'source_branch=%s\n' "$(git -C "${project_root}" branch --show-current)"
    printf 'source_head=%s\n' "${source_head}"
    printf 'execution_unit=qwen3_layer14_complete_block_m64\n'
    printf 'control=qkv_batch4_selected_stage_a\n'
    printf 'candidate=qkvo_batch4\n'
    printf 'context_control=serial_W4U8_QKVO_projection\n'
