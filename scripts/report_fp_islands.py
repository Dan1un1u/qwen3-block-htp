#!/usr/bin/env python3
"""EXP0305 exclusive phase ledger; no summation of overlapping worker times."""
import json,csv,sys
...
for step,mode in enumerate(['prefill','decode_one_step']):
 ...
 if step==0:
  ...
  for name,v in plot.items():csvrows.append(dict(model='Qwen3-1.7B',prompt=64,schedule='operator_boundaries',component=name,us=v,percent=100*v/h))
