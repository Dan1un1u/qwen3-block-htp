#!/usr/bin/env python3
"""Validate EXP-0180 quartet-native mutable V-cache-tail evidence."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--source-branch",
        default="codex/exp-0180-w4u8-decode-quartet-native-v-tail",
    )
...
def load_run(path: Path, expected_steps: int) -> list[dict[str, object]]:
    ...
    for index, (step, profile) in enumerate(zip(steps, profiles)):
        if not (int(step.get("experiment", -1)) == 180
                and int(profile.get("experiment", -1)) == 180
...
def validate_layout(run: list[dict[str, object]], cell: str) -> bool:
    ...
    for index, profile in enumerate(run):
        if not (
            ...
            and int(profile["kv_cache_v_format"]) == (10 if candidate else 9)
...
def main() -> None:
    ...
    lines = [
        "# EXP-0180 quartet-native mutable V-cache tail", "",
    ...
    lines += [
        ...
        "| Module | Segmented-v4 control | Quartet-v5 candidate | "
        "Candidate speed |",
        "|---|---:|---:|---:|",
    ]
    for name in ROWS:
    ...
    lines += ["", "## Cache and Attention diagnostics", "",
              "| Field | Segmented-v4 | Quartet-v5 | Delta |",
              "|---|---:|---:|---:|"]
    for field in DIAGNOSTICS:
    ...
    lines += ["", "## Conclusion", "", conclusion, "",
              "The candidate changes only the W4U8 mutable V-cache tail. "
              "Complete four-row groups are persisted in their final HMX "
              "carrier; Attention repacks at most three rows. K cache, "
    ...
    if not gates["all_required"]:
        raise SystemExit("EXP-0180 required gate failed")
