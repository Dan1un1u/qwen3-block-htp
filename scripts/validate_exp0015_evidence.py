...
LIFETIMES = ("transient_resources", "prepared_session")


def plans(projection):
    ...
    return (
        ("expanded_s8_control", "exp0005_full_bundle_control",
         1, 2, 8, 32, 1, 6, False),
        ("packed_w4_hmx_postscale", "slots4_chunk96_dma_batch2",
         6, 4, 8, 96, 2, 2, False),
    )
...
def key(record):
    return (record["weight_storage"], record["physical_plan"],
            record["resource_lifetime_mode"], record["projection"],
            record["pattern"])


def plan_config(record):
    for plan in plans(record["projection"]):
        if (record["weight_storage"], record["physical_plan"]) == \
                plan[:2]:
            return plan
    raise SystemExit(f"unknown plan: {key(record)}")


def assert_runtime_valid(record, repeat):
    record_key = key(record)
    if record["experiment"] != "EXP-0015":
    ...
    if record["mismatches"] != 0 or record["warmup_output_checksum"] != \
            record["measured_output_checksum"]:
        raise SystemExit(f"warm-up/measured correctness failure for {record_key}")
    if record["output_assembly_mode"] != "linked_2d_dma":
        raise SystemExit(f"wrong output mode for {record_key}")
    ...
    prepared = record["resource_lifetime_mode"] == "prepared_session"
    ...
    expected_fields = {
        ...
        "output_dma_descriptor_count": config["n_tiles"],
        "output_dma_chain_count": 1,
        "output_dma_descriptor_completion_count": config["n_tiles"],
        "output_dma_descriptor_timeout_count": 0,
        "resource_setup_in_run": 0 if prepared else 1,
        "resource_release_in_run": 0 if prepared else 1,
        "warmup_prepared_session_run_index": 1 if prepared else 0,
        "prepared_session_run_index": 2 if prepared else 0,
    ...
    if record["resource_vtcm_address"] == 0 or \
            record["resource_hmx_context_id"] == 0:
        raise SystemExit(f"missing resource identity for {record_key}")
    if prepared and (
        ...
        raise SystemExit(f"prepared resource identity changed for {record_key}")
    if prepared and (record["prepare_wall_ns"] == 0 or
                     record["release_wall_ns"] == 0):
        raise SystemExit(f"missing prepared lifecycle timing for {record_key}")
    if not prepared and (record["prepare_wall_ns"] != 0 or
                         record["release_wall_ns"] != 0):
        raise SystemExit(f"transient mode reports prepare/release for {record_key}")
...
def expected_keys():
    return {
        (plan[0], plan[1], lifetime, projection, pattern)
        for projection in PROJECTION_CONFIG
        for plan in plans(projection)
...
def assert_equivalent(groups):
    comparable = (
        ...
        "output_dma_descriptor_count", "output_dma_chain_count",
    )
    for projection in PROJECTION_CONFIG:
        for pattern in PATTERNS:
                    ...
                    for field in comparable:
                        if record[field] != reference[field]:
                            raise SystemExit(
                                ...
                                f"lifetime modes differ at {field}")


def median_metrics(records):
    fields = (
        "session_open_wall_ns", "prepare_wall_ns", "warmup_host_wall_ns",
        "host_wall_ns", "release_wall_ns", "session_close_wall_ns",
        "dsp_total_ticks",
        "pipeline_ticks", "output_assembly_ticks", "activation_stage_ticks",
        "weight_stage_ticks", "weight_expand_ticks", "hmx_compute_ticks",
        "hmx_ready_wait_ticks", "producer_slot_wait_ticks",
        "expanded_slot_wait_ticks", "hvx_max_active_workers",
        "vtcm_plan_bytes",
    )
    metrics = {
    ...
    metrics["session_resource_overhead_ns_median"] = statistics.median(
        record["prepare_wall_ns"] + record["release_wall_ns"]
        for record in records)
    return metrics
...
def summarize(groups):
    ...
    for projection in PROJECTION_CONFIG:
        for plan in plans(projection):
            storage, physical_plan = plan[:2]
            for lifetime in LIFETIMES:
                records = [record for group_key, group in groups.items()
                           if group_key[:4] == (
                               storage, physical_plan, lifetime, projection)
                           for record in group]
                metrics[
                    f"{projection}:{storage}:{physical_plan}:{lifetime}"] = \
                    median_metrics(records)
    return metrics


def compare_lifetimes(metrics, repeat_name):
    comparisons = {}
    w4_gate = True
    for projection in PROJECTION_CONFIG:
        for storage in ("expanded_s8_control",
                        "packed_w4_hmx_postscale"):
            physical_plan = next(plan[1] for plan in plans(projection)
                                 if plan[0] == storage)
            prefix = f"{projection}:{storage}:{physical_plan}:"
            transient = metrics[prefix + "transient_resources"]
            prepared = metrics[prefix + "prepared_session"]
            transient_wall = transient["host_wall_ns_median"]
            prepared_wall = prepared["host_wall_ns_median"]
            overhead = prepared["session_resource_overhead_ns_median"]
            saving = transient_wall - prepared_wall
            break_even = math.ceil(overhead / saving) if saving > 0 else None
            comparison = {
                "transient_host_wall_ns_median": transient_wall,
                "prepared_host_wall_ns_median": prepared_wall,
                "hot_run_improvement_percent":
                    saving * 100.0 / transient_wall,
                "prepare_plus_release_ns_median": overhead,
                "cold_one_run_ns": prepared_wall + overhead,
                "cold_one_run_improvement_percent":
                    (transient_wall - prepared_wall - overhead) * 100.0 /
                    transient_wall,
                "break_even_run_count": break_even,
                "transient_dsp_total_ticks_median":
                    transient["dsp_total_ticks_median"],
                "prepared_dsp_total_ticks_median":
                    prepared["dsp_total_ticks_median"],
            }
            comparisons[f"{projection}:{storage}"] = comparison
            if storage == "packed_w4_hmx_postscale":
                if repeat_name == "repeat1":
                    w4_gate &= prepared_wall < transient_wall
                else:
                    w4_gate &= prepared_wall <= transient_wall

        s8_plan = next(plan[1] for plan in plans(projection)
                       if plan[0] == "expanded_s8_control")
        w4_plan = next(plan[1] for plan in plans(projection)
                       if plan[0] == "packed_w4_hmx_postscale")
        s8 = metrics[
            f"{projection}:expanded_s8_control:{s8_plan}:prepared_session"]
        w4 = metrics[
            f"{projection}:packed_w4_hmx_postscale:{w4_plan}:prepared_session"]
        s8_wall = s8["host_wall_ns_median"]
        w4_wall = w4["host_wall_ns_median"]
        comparisons[f"{projection}:fair_storage_comparison"] = {
            "w4_prepared_host_wall_ns_median": w4_wall,
            "s8_prepared_host_wall_ns_median": s8_wall,
...
def main():
    ...
    repeat1_comparisons, repeat1_gate = compare_lifetimes(
        repeat1_metrics, "repeat1")
    repeat10_comparisons, repeat10_gate = compare_lifetimes(
    ...
    print(json.dumps({
        ...
        "warmup_and_measured_outputs_identical": True,
        "lifetime_modes_projection_equivalent": True,
        ...
        "session_open_excluded_from_both_run_modes": True,
