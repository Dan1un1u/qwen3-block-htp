...
result_root="${QBH_RESULT_ROOT:-/mnt/d/llm_exp/results/qwen3-block-htp/exp0033}"
artifact_root="${QBH_ARTIFACT_ROOT:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0033/artifacts}"
...
"${project_root}/scripts/build_exp0033.sh" > "${result_dir}/build.log" 2>&1
QBH_STATIC_OUTPUT_DIR="${result_dir}/static" \
    "${project_root}/scripts/check_exp0033_static.sh" \
...
configs=(f16_control w4_control w4_dynamic)
...
for round in 1 2 3 4 5; do
    if (( round % 2 == 1 )); then
        order=(f16_control w4_control w4_dynamic)
    else
        order=(w4_dynamic w4_control f16_control)
    fi
...
python3 "${project_root}/scripts/validate_exp0033_evidence.py" \
...

{
    ...
    printf 'dynamic_scope=qkv_worker_ownership_only\n'
