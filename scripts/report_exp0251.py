...
R=Path('/mnt/d/llm_exp/results/qwen3-block-htp/exp0251')
...
def main():
 ...
 allruns={};allrecords=[];perf={};modules={};rng=np.random.default_rng(251)
 for rep in [1,10]:
  for mode in ['prefill','decode']:
   runs={c:[] for c in ['control','wide']}
   for c in runs:
    for i in range(10):
   ...
   a=np.array([x['host_wall_ns']/1000 for x in runs['control']]);b=np.array([x['host_wall_ns']/1000 for x in runs['wide']]);ratios=b/a
   sampled=np.median(ratios[rng.integers(0,10,(10000,10))],axis=1);ci=np.quantile(sampled,[.025,.975])
   key=f'repeat{rep}_{mode}';perf[key]=dict(control_us=float(np.median(a)),wide_us=float(np.median(b)),paired_regression_percent=float((np.median(ratios)-1)*100),paired_ratio_ci95=ci.tolist(),stable_over10percent=bool(ci[0]>1.1),round_control_us=a.tolist(),round_wide_us=b.tolist());allruns[key]=runs
   if rep==10:
    modules[mode]={}
    for c in runs:
 ...
 details={'r3_wide_eligible':False,'r3_wide_whole_gate':numerical['r3_wide_whole']}
 summary=dict(experiment='EXP-0251',source_head=subprocess.check_output(['git','rev-parse','HEAD'],cwd=S,text=True).strip(),runtime_source_head=json.loads((R/'formal_provenance.json').read_text())['source_head'],execution_state='completed',evidence_validity='valid',local_gate='pass' if speed_pass else 'fail',adoption_status='not_applicable',numerical_gate='pass',physical_gate='pass',speed_gate='pass' if speed_pass else ('fail' if speed_fail else 'uncertain'),short_rounds=5,formal_rounds=10,timed_layer_RPCs=2970,audit_runs=6,audit_layer_RPCs=54,performance=perf,module_centers=modules,dense_r3=details,baseline_promoted=False,device_PPL=None,E2E_tokens_per_second=None,reason='Incremental no-R3 wide-score repair measured. Plain dense HMX R3 combination remains numerically ineligible. No full model authorized by PC065.')
 ...
 for key,runs in allruns.items():
  ...
  for k in runs['control'][0]:
   ...
   a=statistics.median(x[k] for x in runs['control']);b=statistics.median(x[k] for x in runs['wide']);delta=f'{100*(b/a-1):+.2f}%' if a else 'N/A zero denominator'
   text.append(f'|{k}|{a:.4f}|{b:.4f}|{delta}|')
 text.extend(['','## R3 diagnostics','','Untimed plain dense HMX R3 numerical audit; not valid formal timing evidence.',json.dumps(details,indent=2),'','E2E prefill/decode token/s: N/A. Device PPL: N/A. Full-model work is outside PC065. The score repair does not establish model quality acceptance.'])
