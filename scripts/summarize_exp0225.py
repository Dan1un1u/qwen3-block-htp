...
from measure_exp0225 import ALL,VARIANTS,SELECTED
...
def quality():
    ...
    entries={'control_A':old['samples']['selected_A']};determinism={};software={}
    for v in VARIANTS:
    ...
    out=dict(experiment='EXP-0225',summary=summaries,samples=entries,software_diagnostic={k:score(rows,teacher) for k,rows in software.items()},
    ...
    labels=['A EXP0224（冻结）','learned R1R2 '+SELECTED]
...
def speed():
    ...
    write_json(RESULT/'speed_summary.json',dict(data=data,times=times,paired_speed_percent=paired,profile_candidate=chosen,formal_invocations=320,formal_layer_ledgers=8960))
    ...
    module=table(['模块','F16A16 冻结 EXP-0218',f'W4A16 learned R1R2 GPTQ {chosen} EXP-0225','W4A8 冻结 EXP-0218','W4A8 相对 W4A16 增速'],rows);(RESULT/'module_table.md').write_text(module)
