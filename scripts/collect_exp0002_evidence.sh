...
set -u
result_root="${QBH_RESULT_ROOT:-/mnt/d/llm_exp/results/qwen3-block-htp/exp0002}"
artifact_root="${QBH_ARTIFACT_ROOT:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0002}"
...
"${project_root}/scripts/build_exp0002.sh" \
    > "${result_dir}/build.log" 2>&1
QBH_STATIC_OUTPUT_DIR="${result_dir}/static" \
    "${project_root}/scripts/check_exp0002_static.sh" \
...
for pattern in identity signed structured boundary; do
    "${project_root}/scripts/run_exp0002.sh" "${pattern}" 1 \
        | tee -a "${result_dir}/correctness_repeat1.jsonl"
done
for pattern in identity signed structured boundary; do
    "${project_root}/scripts/run_exp0002.sh" "${pattern}" 1000 \
