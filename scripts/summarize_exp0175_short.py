#!/usr/bin/env python3
"""Validate the EXP-0175 five-cell continuous-decode short gate."""
...
MODES = {cell: index for index, cell in enumerate(CELLS)}
BATCHES = {
    "post_batch8": 8,
    "pair_ready_batch8": 8,
    "pair_ready_batch16": 16,
    "pair_ready2_batch8": 8,
    "pair_ready2_batch16": 16,
}
READY_WORKERS = {
    "post_batch8": 0,
    "pair_ready_batch8": 1,
    "pair_ready_batch16": 1,
    "pair_ready2_batch8": 2,
    "pair_ready2_batch16": 2,
}
ROWS = (
    "Token embedding",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--source-branch",
        default="codex/exp-0175-w4u8-decode-gate-up-pair-ready",
    )
...
def load_cell(result_dir: Path, cell: str) -> list[list[dict[str, object]]]:
    ...
    for path in paths:
        ...
        for index, (step, profile) in enumerate(zip(steps, profiles)):
            if not (int(step.get("experiment", -1)) == 175
                    and int(profile.get("experiment", -1)) == 175
...
def main() -> None:
    ...
    for cell, cell_runs in runs.items():
        for run in cell_runs:
            ...
            for index, profile in enumerate(run):
                ...
                decode = index > 0
                ready_workers = READY_WORKERS[cell] if decode else 0
                expected_ready = 5376 if ready_workers else 0
                expected_batch = BATCHES[cell] if decode else 8
                schedule &= bool(
                    int(profile["w4u8_decode_gate_up_mode"]) == MODES[cell]
                    and int(profile["w4u8_mlp_gate_up_hmx_batch_n_tiles"])
                        == expected_batch
                    and int(profile["w4u8_mlp_gate_up_hvx_workers"])
                        == 3 + ready_workers
                    and int(profile[
                    ...
                    and int(profile[
                        ...
                        == expected_ready
                    and int(profile["w4u8_mlp_pair_publish_count"]) == 5376
                    and int(profile["w4u8_mlp_pair_consume_count"]) == 5376
                )
                physical &= bool(
    ...
    gates = {
        "all_25_sessions_physical_contract": physical,
        "requested_and_observed_pair_ready_geometry": schedule,
        "all_cache_lengths_synchronous": cache,
        "all_25_sessions_byte_exact_output_hashes": len(set(hashes)) == 1,
        "all_25_sessions_identical_token_sequence": len(set(sequences)) == 1,
        "any_pair_ready_cell_beats_control_gate_up_and_complete_decode_wall":
            any(eligible.values()),
    }
    selected = min(
    ...
    conclusion = (
        ...
        else "reject_pair_ready_publication_without_193_step_formal_run"
    )
    summary = {
        "experiment": "EXP-0175",
        ...
        "formal_193_step_sections": (
            "not_run_by_approved_short_gate_stop_condition"
            if not any(eligible.values()) else "pending"
        ),
        "provenance": {
    ...
    lines = [
        "# EXP-0175 decode Gate/Up pair-ready publication short gate", "",
        f"Source: `{args.source_branch}` @ `{args.source_commit}`", "",
        "The approved stop condition skips the 193-step formal run when no "
        "candidate improves both Gate/Up and complete decode wall.", "",
    ...
    for name in ROWS:
        lines.append(
            f"| {name} | " + " | ".join(
                fmt(rows[cell]["decode"][name], walls[cell])
                for cell in CELLS) + " |")
    ...
    for field in DIAGNOSTICS:
        ...
        lines.append(
            f"| {name} | " + " | ".join(
                f"{diagnostics[cell][name]:.3f}" for cell in CELLS) + " |")
    lines += ["", "## Gates", ""]
