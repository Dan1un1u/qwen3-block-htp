...
from device_exp0277 import run,records
...
def audit(package,tag):
 ...
 for step in range(2):
  ...
  mid=np.load(p/f'fp32_step{step:02d}_middle.npy');a=unpack_u8_hmx_activation(b[BASE+8*CAP:BASE+9*CAP],6144)
  sp2='QBH_SP2=8 ' in read(d/'protocol.json')['command']
  assert np.array_equal(a[:rows],((mid+32768)&255).astype('u1') if sp2 else mid.astype('u1'))
  if step and sp2:assert np.array_equal(a[4:5],((mid+32768)>>8).astype('u1'))
