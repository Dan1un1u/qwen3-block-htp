#!/usr/bin/env python3
"""Validate EXP-0093 and render its PC-027/PC-028 closure."""
...
PHYSICAL = tuple(dict.fromkeys((
    *base.COUNTERS, *base.RESOURCES,
    "block_invocation_count", "w4u8_residual_active_contexts",
...
def validate_record(record: dict[str, object], repeat: int,
    ...
    fixed = {
        "experiment": "EXP-0093",
        "residual_mode": (
            "hvx_fused_post_norm_pool6_shuffle4"
            if mode == "candidate"
            else "hvx_fused_post_norm_pool6"
        ),
...
def build_summary(result_dir: Path, package_dir: Path) -> dict[str, object]:
    ...
    selected_records = records_by_repeat[10][
        "candidate" if local_gate else "control"
    ]
    return {
        "experiment": "EXP-0093",
        "control": "EXP-0084 six-context per-row vgather residual",
        "candidate": "four-row two-level vshuff residual load",
        ...
        "static_gate": json.loads(
            (result_dir / "static_gate.json").read_text()
        ),
        ...
        "pc028": {
            ...
            "w4u8_provenance": (
                "EXP-0093 candidate" if local_gate else "EXP-0084 control"
            ),
...
def add_table(lines: list[str], title: str, fields: tuple[str, ...],
    ...
    lines.extend([
        f"### {title}", "",
        "| Metric | EXP-0084 control | EXP-0093 candidate | Delta | Paired delta |",
        "|---|---:|---:|---:|---:|",
    ])
    for field in fields:
...
def render_report(summary: dict[str, object]) -> str:
    lines = ["# EXP-0093 — Complete profiling report", ""]
    add_pc028(lines, summary)
    lines.extend([
        "The candidate changes only the native integer-HMX output load "
        "topology at both residual boundaries. Four aligned tile loads and "
        "two shuffle levels replace the per-row gather and scratch reload; "
        "Q14 arithmetic, qparams, tasks, buffers and all HMX work remain "
        "unchanged.", "",
    ])
    for repeat in REPEATS:
    ...
    lines.extend([
        ...
        "| Gate | Result |", "|---|---:|",
        "| Static hot loop | 4 vshuff, 0 vgather, 0 vector stack spill |",
        ...
        "## Decision", "",
        f"EXP-0093 local gate: **{'PASS' if summary['local_gate_pass'] else 'FAIL'}**. "
