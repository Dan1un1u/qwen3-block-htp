...
"${project_root}/scripts/check_exp0180_static.sh" \
    > "${result_dir}/static_gate.json"
"${project_root}/scripts/deploy_exp0180.sh" \
    > "${result_dir}/deploy.log" 2>&1

for cell in control quartet; do
    QBH_EXP0173_GENERATION_STEPS=34 \
        "${project_root}/scripts/run_exp0180.sh" "${cell}" \
...
QBH_GENERATION_AUDIT_DIR="${result_dir}/audit" \
    "${project_root}/scripts/run_exp0180.sh" quartet \
...
"${python_exe}" "${project_root}/scripts/verify_exp0167_generation.py" \
    ...
    --steps 193 --experiment-record 180 --experiment-label EXP-0180 \
    > "${result_dir}/audit/verify.log" 2>&1

QBH_EXP0173_GENERATION_STEPS=193 \
    "${project_root}/scripts/run_exp0180.sh" quartet_poison \
...
"${python_exe}" "${project_root}/scripts/summarize_exp0180.py" \
