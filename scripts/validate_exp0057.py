#!/usr/bin/env python3
"""Validate EXP-0057 V-recenter vgather evidence."""
...
def build_summary(result_dir: Path, package_dir: Path) -> dict[str, object]:
    ...
    for mode in MODES:
        ...
        correctness[mode] = {
            ...
            "v_recenter_saturation_count":
                record["u8_attention_v_recenter_saturation_count"],
    ...
    if (correctness["control"]["v_recenter_saturation_count"] !=
            correctness["candidate"]["v_recenter_saturation_count"]):
        raise SystemExit("candidate changed V recenter saturation count")
    ...
    return {
        "experiment": "EXP-0057",
        "control": "EXP-0055 scalar V recenter lookup",
        "candidate": "two-half-vector exact V recenter vgather",
        ...
        "v_recenter_saturation_equivalence_gate": True,
...
def render_report(summary: dict[str, object]) -> str:
    lines = [
        ...
        "The candidate replaces 128 scalar V-recenter table accesses per "
        "carrier vector with two 64-lane halfword HVX vgathers. The exact "
        "256-entry table, output S8 carrier, AV HMX math and schedule remain "
        "unchanged.", "",
    ]
    for repeat in REPEATS:
        ...
        for field in (
            ...
            "u8_attention_v_pack_ticks", "attention_ticks",
            "qkv_projection_ticks", "gate_up_ticks", "down_ticks",
        ):
    ...
    lines.extend([
        ...
        "Final output, V saturation count, and QK/probability/AV audit "
        "hashes are byte-exact. Requested/acquired VTCM is exactly 8 MiB; "
        "intermediate DDR and spill/fill remain zero; one FastRPC execution "
        "unit and one HMX owner are preserved.", "",
        "## Decision", "",
        f"EXP-0057 local gate: **{'PASS' if summary['local_gate_pass'] else 'FAIL'}**.", "",
    ])
    return "\n".join(lines)
