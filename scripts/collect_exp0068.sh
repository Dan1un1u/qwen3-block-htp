...
package="${QBH_EXP0068_PACKAGE:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0042/block_package_layer14_m64_integer_attention_parallel}"
result_root="${QBH_EXP0068_RESULT_ROOT:-/mnt/d/llm_exp/results/qwen3-block-htp/exp0068}"
artifact_root="${QBH_EXP0068_ARTIFACT_ROOT:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0068/artifacts}"
selection_path="${1:?usage: collect_exp0068.sh SELECTION_JSON}"
...
contexts = r.get("selected_attention_hvx_contexts")
if r.get("experiment") != "EXP-0068" or mode not in ("context5", "context6"):
    raise SystemExit("invalid EXP-0068 selection")
if contexts != {"context5": 5, "context6": 6}[mode]:
    raise SystemExit("selection mode/context mismatch")
print(mode)
PY
)"
...
QBH_EXP0068_PACKAGE="${package}" "${project_root}/scripts/deploy_exp0068_block.sh" \
...
"${project_root}/scripts/run_exp0068.sh" control 1 on on \
    > "${result_dir}/correctness_control.jsonl"
"${project_root}/scripts/run_exp0068.sh" "${candidate_mode}" 1 on on \
...
for round in 1 2 3 4 5 6 7; do
    printf 'EXP-0068 paired round %d/%d\n' "${round}" "${paired_rounds}"
    if (( round % 2 == 1 )); then
        repeats=(1 10)
    ...
    for repeat in "${repeats[@]}"; do
        for mode in "${modes[@]}"; do
            ...
            "${project_root}/scripts/run_exp0068.sh" "${run_mode}" "${repeat}" on off \
                >> "${result_dir}/paired_${mode}_r${repeat}.jsonl"
        done
    done
done
...
python3 "${project_root}/scripts/validate_exp0068.py" "${result_dir}" "${package}" \
    > "${result_dir}/gate_summary.json"
python3 "${project_root}/scripts/validate_exp0068.py" "${result_dir}" "${package}" --report \
...
{
    printf 'experiment=EXP-0068\nsource_branch=%s\nsource_head=%s\n' "${source_branch}" "${source_head}"
    printf 'execution_unit=qwen3_layer14_complete_block_m64\n'
    printf 'control=exp0065_main_plus_three_attention_hvx_workers\n'
    printf 'candidate=%s\n' "${candidate_mode}"
    printf 'selection=%s\n' "${selection_path}"
