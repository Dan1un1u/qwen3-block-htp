...
set -u
result_root="${QBH_RESULT_ROOT:-/mnt/d/llm_exp/results/qwen3-block-htp/exp0008}"
artifact_root="${QBH_ARTIFACT_ROOT:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0008}"
...
"${project_root}/scripts/build_exp0008.sh" \
...
run_one() {
    ...
    local output="$6"
    if [[ "${deployed}" == "0" ]]; then
        "${project_root}/scripts/run_exp0008.sh" \
            "${storage}" "${projection}" "${pattern}" \
            "${repeat_count}" "${plan}" | tee -a "${output}"
        deployed=1
    else
        QBH_SKIP_DEPLOY=1 "${project_root}/scripts/run_exp0008.sh" \
            "${storage}" "${projection}" "${pattern}" \
            "${repeat_count}" "${plan}" | tee -a "${output}"
    fi
}
...
python3 "${project_root}/scripts/validate_exp0008_evidence.py" \
