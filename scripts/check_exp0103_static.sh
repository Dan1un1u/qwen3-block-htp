...
candidate_symbol="qbh_qk_norm_rope_one_head_u8_hoisted_constants"
control_symbol="qbh_qk_norm_rope_one_head_u8_preconverted_coefficient"

grep -q 'QBH_BLOCK_ABI_VERSION UINT32_C(43)' \
    "${project_root}/include/block_protocol.h"
grep -q 'QBH_BLOCK_EXPERIMENT UINT32_C(103)' \
    "${project_root}/include/block_protocol.h"
grep -q 'QBH_BLOCK_W4U8_QK_PAIR_INTERLEAVED' \
...
control_line="$(${tools_root}/hexagon-nm -S "${skel}" | grep " ${control_symbol}$")"
candidate_start="0x$(awk '{print $1}' <<<"${candidate_line}")"
candidate_size="$((16#$(awk '{print $2}' <<<"${candidate_line}")))"
control_size="$((16#$(awk '{print $2}' <<<"${control_line}")))"
...
disassembly="$(${tools_root}/hexagon-llvm-objdump -d \
    ...
    "${skel}")"
if grep -q 'allocframe' <<<"${disassembly}"; then
    printf 'candidate helper contains a stack frame\n' >&2
    exit 1
fi
if grep -Eq 'vmem\(r29|vmem\(r30|memd\(r29|memd\(r30' <<<"${disassembly}"; then
    printf 'candidate helper contains a vector stack access\n' >&2
    exit 1
fi
printf '{"experiment":"EXP-0103","static_gate":"pass","candidate_symbol_bytes":%u,"control_symbol_bytes":%u,"candidate_allocframe":false,"candidate_vector_stack_access":false,"qnn_dependency":false}\n' \
