...
package="${QBH_EXP0050_PACKAGE:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0042/block_package_layer14_m64_integer_attention_parallel}"
result_root="${QBH_EXP0050_RESULT_ROOT:-/mnt/d/llm_exp/results/qwen3-block-htp/exp0050}"
artifact_root="${QBH_EXP0050_ARTIFACT_ROOT:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0050/artifacts}"
control_artifacts="${QBH_EXP0050_CONTROL_ARTIFACTS:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0049/artifacts/b37d208d262d/formal/candidate}"
...
mkdir -p "${result_dir}" "${artifact_dir}/candidate" \
    "${artifact_dir}/control"

"${project_root}/scripts/build_exp0050.sh" \
    > "${result_dir}/build.log" 2>&1
"${project_root}/scripts/check_exp0050_static.sh" \
    > "${result_dir}/static_gate.json"
QBH_EXP0050_PACKAGE="${package}" \
QBH_EXP0050_CONTROL_ARTIFACTS="${control_artifacts}" \
    "${project_root}/scripts/deploy_exp0050_block.sh" \
...
for mode in control candidate; do
    "${project_root}/scripts/run_exp0050.sh" \
...
for round in 1 2 3 4 5 6 7; do
    printf 'EXP-0050 paired round %d/%d\n' "${round}" "${paired_rounds}"
    if (( round % 2 == 1 )); then
        repeats=(1 10)
...
python3 "${project_root}/scripts/validate_exp0050.py" \
...
cp "${project_root}/hexagon_ReleaseG_toolv19_v79/ship/libqwen3_probe_skel.so" \
    "${artifact_dir}/candidate/"
cp "${control_artifacts}/qwen3_block_cli" \
    "${artifact_dir}/control/"
cp "${control_artifacts}/libqwen3_probe.so" \
    "${artifact_dir}/control/"
cp "${control_artifacts}/libqwen3_probe_skel.so" \
    "${artifact_dir}/control/"

{
    printf 'experiment=EXP-0050\n'
    printf 'source_branch=%s\n' "${source_branch}"
    printf 'source_head=%s\n' "${source_head}"
    printf 'control_source_head=b37d208d262d5a45365d6efe32c03786a5c3a2d3\n'
    printf 'execution_unit=qwen3_layer14_complete_block_m64\n'
    printf 'control=W4U8-EXP0049-GATE-UP-BATCH8\n'
    printf 'candidate=down_two_chunk_generation_safe_hmx_command\n'
