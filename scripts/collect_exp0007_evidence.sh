...
set -u
result_root="${QBH_RESULT_ROOT:-/mnt/d/llm_exp/results/qwen3-block-htp/exp0007}"
artifact_root="${QBH_ARTIFACT_ROOT:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0007}"
...
"${project_root}/scripts/build_exp0007.sh" \
...
plans=(
    ...
    exp0006_slots2_chunk32_control
    slots3_chunk32
    slots4_chunk32
    slots2_chunk16
    slots3_chunk16
...
run_one() {
    ...
    local output="$6"
    if [[ "${deployed}" == "0" ]]; then
        "${project_root}/scripts/run_exp0007.sh" \
            "${storage}" "${projection}" "${pattern}" \
            "${repeat_count}" "${plan}" | tee -a "${output}"
        deployed=1
    else
        QBH_SKIP_DEPLOY=1 "${project_root}/scripts/run_exp0007.sh" \
            "${storage}" "${projection}" "${pattern}" \
            "${repeat_count}" "${plan}" | tee -a "${output}"
    fi
}
...
for projection in "${projections[@]}"; do
    for pattern in "${patterns[@]}"; do
        for index in "${!plans[@]}"; do
            run_one "${storages[index]}" "${projection}" "${pattern}" 1 \
                "${plans[index]}" \
                "${result_dir}/correctness_repeat1.jsonl"
        done
    done
...
for round in 0 1 2; do
    for projection in "${projections[@]}"; do
        for pattern_index in "${!patterns[@]}"; do
            start=$(((round * 2 + pattern_index) % ${#plans[@]}))
            for step in "${!plans[@]}"; do
                index=$(((start + step) % ${#plans[@]}))
                run_one "${storages[index]}" "${projection}" \
                    "${patterns[pattern_index]}" 10 "${plans[index]}" \
                    "${result_dir}/timing_repeat10.jsonl"
            done
        done
    done
...
python3 "${project_root}/scripts/validate_exp0007_evidence.py" \
