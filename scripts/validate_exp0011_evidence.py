...
def variants(projection):
    ...
    return (
        ("expanded_s8_control", "exp0005_full_bundle_control", 1),
        ("expanded_s8_control", "expanded_s8_dma_batch2", 2),
        ("packed_w4_hmx_postscale", f"slots4_chunk{chunk}", 1),
        ("packed_w4_hmx_postscale",
         f"slots4_chunk{chunk}_dma_batch2", 2),
        ("packed_w4_hmx_postscale",
         f"slots4_chunk{chunk}_dma_batch4", 4),
    )
...
def plan_config(record):
    ...
    for storage, plan, batch in variants(projection):
        if (record["weight_storage"], record["physical_plan"]) == (
                storage, plan):
            if storage == "expanded_s8_control":
                return 1, 2, 32, batch, PROJECTION_CONFIG[projection][
                    "k_tiles"] // 32
            return (6, 4, PROJECTION_CONFIG[projection]["w4_chunk"],
                    batch, PROJECTION_CONFIG[projection]["w4_chunks"])
    raise SystemExit(f"unknown variant: {key(record)}")


def assert_runtime_valid(record, repeat):
    record_key = key(record)
    if record["experiment"] != "EXP-0011":
    ...
    workers, slots, chunk, batch, chunks = plan_config(record)
    logical_bundles = config["n_tiles"] * repeat
    expected_weight_dmas = logical_bundles // batch
    ...
    if record["dma_submit_count"] != config["k_tiles"] + expected_weight_dmas:
        raise SystemExit(f"wrong DMA submit count for {record_key}")
    if record["dma_wait_count"] != 2 * (
            config["k_tiles"] + expected_weight_dmas):
...
def median_metrics(records):
    fields = (
        ...
        "hvx_max_active_workers", "vtcm_plan_bytes",
    )
    metrics = {
...
def main():
    ...
    for projection in PROJECTION_CONFIG:
        ...
        for storage, plan, batch in variants(projection):
            ...
            label = f"{projection}:{storage}:{plan}"
            projection_metrics[(storage, plan, batch)] = median_metrics(records)
            metrics[label] = projection_metrics[(storage, plan, batch)]
        w4 = [(key, value) for key, value in projection_metrics.items()
              if key[0] == "packed_w4_hmx_postscale"]
        s8 = [(key, value) for key, value in projection_metrics.items()
              if key[0] == "expanded_s8_control"]
        selected_w4 = min(w4, key=lambda item: item[1]["host_wall_ns_median"])
        selected_s8 = min(s8, key=lambda item: item[1]["host_wall_ns_median"])
        parent = next(value for key, value in w4 if key[2] == 1)
        ...
        selections[projection] = {
            "selected_w4_plan": selected_w4[0][1],
            "selected_w4_dma_batch": selected_w4[0][2],
            "selected_s8_plan": selected_s8[0][1],
            "selected_s8_dma_batch": selected_s8[0][2],
            "parent_w4_host_wall_ns_median": parent_wall,
            "selected_w4_host_wall_ns_median": w4_wall,
