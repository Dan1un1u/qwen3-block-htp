...
grep -q 'EXP-0032' "${output_dir}/host.strings.txt"
...
required = (
    "QBH_BLOCK_EXPERIMENT UINT32_C(32)",
    "QBH_BLOCK_ABI_VERSION UINT32_C(16)",
    "QBH_BLOCK_W4F16_PIPELINE_ADAPTIVE_DOWN96_GATE4_DMA8_CROSS_PREFETCH",
    "qbh_prepare_gate_up_scale_cache(",
    "w4f16_gate_up_scale_cache_bytes",
...
for fragment in required:
    if fragment not in implementation and fragment not in protocol and fragment not in host:
        raise SystemExit(f"missing EXP-0032 contract fragment: {fragment}")
...
printf '{"experiment":"EXP-0032","gate_up_batch4":true,'
printf '"gate_up_dma8":true,"preencoded_scale_cache":true,'
printf '"scale_cache_bytes":98304,"fixed_vtcm_request_bytes":8388608,'
