...
def preflight():
    data.preflight()
    assert subprocess.check_output(['git','branch','--show-current'],cwd=data.SOURCE,text=True).strip()=='codex/exp-0250-raw-score-max'
def freeze():
    ...
    write('freeze.json',dict(files={'inputs.json':data.sha(ROOT/'inputs.json'),'dataset.json':data.sha(ROOT/'dataset.json')},references=refs,
        protocol_sha256=data.sha(data.MEMORY/'docs/experiments/EXP-0250.md'),frozen_before_inference=True))
def frozen():
    data.frozen();f=read('freeze.json')
    assert f['protocol_sha256']==data.sha(data.MEMORY/'docs/experiments/EXP-0250.md')
...
def check_arrays(q,k,v,valid,c,mode,expected=None,scale=.002):
    ...
    if mode in ['wide_exact','wide_sole']:
        mask=np.broadcast_to(valid,ref['raw'].shape)
        for n in ['delta','exponent']:
            assert np.array_equal(gpu[n].cpu().numpy()[mask],ref[n][mask]),(mode,n)
    return {n:int(ref[n].size) for n in ['raw','probability','av']}

def numerical():
    rng=np.random.default_rng(250);c=read('inputs.json')['configs']['R3_ALL'][0];proofs=[]
    ...
    for params in read('inputs.json')['configs'].values():
        for c in params:
            ...
            for mode in MODES:check_arrays(q,k,v,valid,c,mode)
    ...
    for tag in ['chain_r3_01','chain_scalar_01','chain_refined_02']:
        ...
        for g,fields in enumerate(au.configs):
            ...
            for mode in ['unbounded','wide_float','wide_exact','wide_sole']:check_arrays(q,k,v,np.tril(np.ones((64,64),bool)),c,mode)
    ...
    wide_checks=ia.wide_safety_checks()
    write('checks/numerical.json',dict(pass_all=True,wide_checks=wide_checks,random_cases=proofs,captures=captures,EXP0042='scaled_QK_probability_AV_exact',negative_ties_and_saturation=True,
...
def run(phase):
    ...
    for recipe in ['C64','F']:
        ...
        for name in NAMES:
            ...
            if phase=='development' and (name in ['F','C64'] or name.split('_',1)[-1] in ['carrier','score','sole']):
                p=data.verified('exp0249',f'scores/development_{name}.json');old=json.loads(p.read_text())
