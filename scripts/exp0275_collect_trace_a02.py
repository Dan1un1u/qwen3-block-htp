...
e.write(r/'trace_log_transport_repair.json',dict(cause='SDK static FARF requires process-name .farf in DSP_LIBRARY_PATH to route to logcat',remote_farf=farf,configuration='empty file; diagnostic runtime only',production_timing_affected=False))
for a in e.ARMS:
 e.setarm(a);tag=f'trace-{a}-a02';log=r/f'{tag}-logcat.txt'
