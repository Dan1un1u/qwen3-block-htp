...
result_root="${QBH_RESULT_ROOT:-/mnt/d/llm_exp/results/qwen3-block-htp/exp0024}"
artifact_root="${QBH_ARTIFACT_ROOT:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0024/artifacts}"
...
"${project_root}/scripts/build_exp0024.sh" > "${result_dir}/build.log" 2>&1
QBH_STATIC_OUTPUT_DIR="${result_dir}/static" \
    "${project_root}/scripts/check_exp0024_static.sh" \
...
for mode in scalar hvx; do
    for variant in F16F16 W4F16; do
        "${project_root}/scripts/run_exp0024_block.sh" \
            "${variant}" 1 "${mode}" \
            > "${result_dir}/correctness_${mode}_${variant}.jsonl"
        : > "${result_dir}/timing_${mode}_${variant}_repeat1.jsonl"
        : > "${result_dir}/timing_${mode}_${variant}_repeat10.jsonl"
    done
done

for operator_mode in rms rope softmax silu; do
    for variant in F16F16 W4F16; do
        "${project_root}/scripts/run_exp0024_block.sh" \
            "${variant}" 1 "${operator_mode}" \
            > "${result_dir}/microgate_${operator_mode}_${variant}.jsonl"
    done
done
...
python3 "${project_root}/scripts/validate_exp0024_evidence.py" \
...
{
    printf 'experiment=EXP-0024\n'
    printf 'source_branch=%s\n' "$(git -C "${project_root}" branch --show-current)"
    printf 'source_head=%s\n' "${source_head}"
    printf 'execution_unit=qwen3_layer14_complete_block_m64\n'
    printf 'variants=F16F16,W4F16\n'
    printf 'common_ops_modes=scalar,hvx_fp16\n'
    printf 'common_ops=rmsnorm,qk_norm_rope,stable_causal_softmax,silu_by_up\n'
    printf 'softmax_algorithm=standard_stable_natural_exp\n'
