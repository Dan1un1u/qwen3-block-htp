...
package="${QBH_EXP0169_OVERLAY:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0169/w4u8_greedy193_overlay}"
transformer_package="${QBH_EXP0169_TRANSFORMER_PACKAGE:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0163/candidate_segmented_capacity257}"

mkdir -p "${result_dir}/raw" "${result_dir}/audit"
"${project_root}/scripts/check_exp0169_static.sh" \
    > "${result_dir}/static_gate.json"

QBH_EXP0169_DEPLOY=1 \
QBH_GENERATION_BOUNDARY_AUDIT=1 \
QBH_GENERATION_AUDIT_DIR="${result_dir}/audit" \
    "${project_root}/scripts/run_exp0169.sh" \
...
"${python_exe}" "${project_root}/scripts/verify_exp0167_generation.py" \
    ...
    --steps 193 --experiment-record 169 --experiment-label EXP-0169 \
...
"${python_exe}" "${project_root}/scripts/summarize_exp0169.py" \
