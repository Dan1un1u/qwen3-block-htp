...
from reference_exp0278 import layer
from reference_w4u8_hmx import load_qparams_bin
M=Path('/mnt/d/llm_exp/models/qwen3-block-htp');R=Path('/mnt/d/llm_exp/results/qwen3-block-htp/exp0278');O=M/'exp0278'
def sha(p):
 h=hashlib.sha256()
...
def verify(p,expected):
 ...
 return m
def main():
 out=subprocess.check_output(['python3','/home/daniuniu/work/qwen3-block-htp-project-memory/scripts/project_memory.py','preflight','--source-worktree',str(S)],text=True);assert 'EXPERIMENT=EXP-0278' in out
 base=M/'exp0257/package';bm=verify(base,'ab2442027f315797dc7ff8f9c49b0d6bf27a3de3c958e831eddf4e1953f915bb')
 O.mkdir(exist_ok=False)
 for key,src,count,origlayer in [('layer14-fp',M/'exp0269/layer14-fp32-a01',1,14),('full-fp',M/'exp0271/sp2-fp32',28,0)]:
  ...
  write(dst/'manifest.json',dict(experiment='EXP-0278',recipe='nativeW4 SP2 continuousFPsoftmax FP32residual2',parent=str(src),parent_sha256=sha(src/'manifest.json'),unchanged_original_parent=str(base),unchanged_original_parent_sha256=sha(base/'manifest.json'),changed_metadata=changed,new_weight_codes=False,reference_scope='selectedlayer14' if count==1 else 'first3 layers only; full frontend separately checked',files=files))
