...
u8_package="${QBH_EXP0117_U8_PACKAGE:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0042/block_package_layer14_m64_integer_attention_parallel}"
result_root="${QBH_EXP0117_RESULT_ROOT:-/mnt/d/llm_exp/results/qwen3-block-htp/exp0117}"
artifact_root="${QBH_EXP0117_ARTIFACT_ROOT:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0117/artifacts}"
...
if ! git -C "${project_root}" diff --quiet || \
   ...
   [[ -n "$(git -C "${project_root}" ls-files --others --exclude-standard)" ]]; then
    printf 'source worktree must be clean before EXP-0117 formal collection\n' >&2
    exit 1
fi
...
"${project_root}/scripts/build_exp0117.sh" > "${result_dir}/build.log" 2>&1
QBH_EXP0117_LUT="${u8_package}/silu_up_lut_u16.bin" \
    "${project_root}/scripts/check_exp0117_static.sh" \
    > "${result_dir}/static_gate.json"
QBH_EXP0117_U8_PACKAGE="${u8_package}" \
    "${project_root}/scripts/deploy_exp0117_block.sh" \
...
for cell in "${cells[@]}"; do
    ...
    "${project_root}/scripts/run_exp0117.sh" "${cell}" 1 on on > "${raw}"
...
for round in 1 2 3 4 5; do
    printf 'EXP-0117 round %d/5\n' "${round}"
    if ((round % 2 == 1)); then
        repeats=(1 10)
        order=(control packed_pair)
    else
        repeats=(10 1)
        order=(packed_pair control)
    fi
    for repeat in "${repeats[@]}"; do
        for cell in "${order[@]}"; do
            raw="${result_dir}/round_${round}_${cell}_r${repeat}_raw.jsonl"
            "${project_root}/scripts/run_exp0117.sh" \
...
python3 "${project_root}/scripts/analyze_exp0117.py" "${result_dir}" \
    > "${result_dir}/gate_summary.json"
python3 "${project_root}/scripts/analyze_exp0117.py" "${result_dir}" --report \
...
{
    printf 'experiment=EXP-0117\nsource_head=%s\n' "${source_head}"
    printf 'parent=EXP-0112\ncells=%s\n' "${cells[*]}"
    printf 'recipe=W4U8\nvariable=exact_packed_pair_swiglu_lut\n'
    printf 'repeat_contract=repeat1,repeat10\nrounds=5\n'
    printf 'lut_contract=65536_exact_mappings_128kib_reserved_64kib_addressed\n'
