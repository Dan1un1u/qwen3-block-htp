...
QBH_GENERATION_AUDIT_DIR="${result_dir}/audit" \
    "${project_root}/scripts/run_exp0176.sh" o_batch8 \
...
"${python_exe}" "${project_root}/scripts/verify_exp0167_generation.py" \
    ...
    --steps 193 --experiment-record 176 --experiment-label EXP-0176 \
...
"${python_exe}" "${project_root}/scripts/summarize_exp0176.py" \
