...
from device_exp0277 import records
from summarize_exp0217 import normalized
from report_exp0273 import GROUPS
R=Path('/mnt/d/llm_exp/results/qwen3-block-htp/exp0277')
ID='EXP-0277';LAYERS=28;SEED=277;DELTA=344064
STDOUT='stdout.jsonl'
...
out.update(measured_source=next(iter(measured)),timed_profiles=count,paired_profiles=count//2,physical_gate=True,numerical_gate_scope='Qwen selected layer14 and chain3 independent exact; full28 physical/front-end finalNorm and full-vocabulary head oracle; no full28 CPU transformer equivalence claim',precision='FP32 residual2; frozen nativeW4; middle ordinaryA8 versus SP2; no rotation',weight_bytes_identical=True,worker_dispatches_identical=True,prefill_extra_pairs=DELTA,decode_extra_pairs=0,first_formal_steps=physical)
