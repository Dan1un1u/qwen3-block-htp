...
fp16_package="${QBH_EXP0136_FP16_PACKAGE:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0022/block_package_layer14_m64}"
result_root="${QBH_EXP0136_RESULT_ROOT:-/mnt/d/llm_exp/results/qwen3-block-htp/exp0136}"
artifact_root="${QBH_EXP0136_ARTIFACT_ROOT:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0136/artifacts}"
...
if ! git -C "${project_root}" diff --quiet || \
   ...
   [[ -n "$(git -C "${project_root}" ls-files --others --exclude-standard)" ]]; then
    printf 'source worktree must be clean before EXP-0136 formal collection\n' >&2
    exit 1
fi
...
"${project_root}/scripts/build_exp0136.sh" > "${result_dir}/build.log" 2>&1
"${project_root}/scripts/check_exp0136_static.sh" > "${result_dir}/static_gate.json"
QBH_EXP0136_FP16_PACKAGE="${fp16_package}" \
    "${project_root}/scripts/deploy_exp0136_block.sh" > "${result_dir}/deploy.log" 2>&1
...
for cell in "${cells[@]}"; do
    ...
    "${project_root}/scripts/run_exp0136.sh" "${cell}" 1 on on > "${raw}"
...
for round in 1 2 3 4 5; do
    ...
    for repeat in "${repeats[@]}"; do
        for cell in "${order[@]}"; do
            ...
            "${project_root}/scripts/run_exp0136.sh" \
...
python3 "${project_root}/scripts/analyze_exp0136.py" "${result_dir}" \
    > "${result_dir}/gate_summary.json"
python3 "${project_root}/scripts/analyze_exp0136.py" "${result_dir}" --report \
...
{
    printf 'experiment=EXP-0136\nsource_head=%s\n' "${source_head}"
    printf 'parent_source=EXP-0135\ncells=%s\n' "${cells[*]}"
    printf 'recipe=W4F16\nvariable=gate_up_stream_subgroup_tiles\n'
