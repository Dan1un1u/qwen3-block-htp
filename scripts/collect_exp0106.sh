...
result_root="${QBH_EXP0106_RESULT_ROOT:-/mnt/d/llm_exp/results/qwen3-block-htp/exp0106}"
artifact_root="${QBH_EXP0106_ARTIFACT_ROOT:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0106/artifacts}"
...
rounds=5
cells=(
    fair_f16f16 fair_w4f16 fair_w4u8 down4
    down4_softmax down4_softmax_residual integrated
)

if ! git -C "${project_root}" diff --quiet || \
   ! git -C "${project_root}" diff --cached --quiet || \
   [[ -n "$(git -C "${project_root}" ls-files --others --exclude-standard)" ]]; then
    printf 'source worktree must be clean before EXP-0106 formal collection\n' >&2
    exit 1
fi
...
"${project_root}/scripts/build_exp0106.sh" \
    > "${result_dir}/build.log" 2>&1
"${project_root}/scripts/check_exp0106_static.sh" \
    > "${result_dir}/static_gate.json"
QBH_EXP0106_FP16_PACKAGE="${fp16_package}" \
QBH_EXP0106_U8_PACKAGE="${u8_package}" \
    "${project_root}/scripts/deploy_exp0106_block.sh" \
...
for ((round = 1; round <= rounds; ++round)); do
    ...
    if ((round % 2 == 1)); then
        ...
        repeats=(10 1)
        order=(
            integrated down4_softmax_residual down4_softmax down4
            fair_w4u8 fair_w4f16 fair_f16f16
        )
    fi
...
cmp "${result_dir}/boot_id_before.txt" \
    "${result_dir}/boot_id_after.txt"
python3 "${project_root}/scripts/analyze_exp0106.py" \
    "${result_dir}" > "${result_dir}/gate_summary.json"
python3 "${project_root}/scripts/analyze_exp0106.py" \
    "${result_dir}" --report > "${result_dir}/full_profiling_report.md"
...
cp "${project_root}/hexagon_ReleaseG_toolv19_v79/ship/libqwen3_probe_skel.so" \
    "${artifact_dir}/"
{
    printf 'experiment=EXP-0106\nsource_head=%s\n' "${source_head}"
    printf 'fair_parent=EXP-0084\nfast_parent=EXP-0099\n'
    printf 'integrated_candidates=EXP-0095,EXP-0093,EXP-0104\n'
    printf 'cells=%s\n' "${cells[*]}"
