...
import numpy as np
from data_exp0232 import RESULT,CELLS,write,sha,preflight
...
def summarize(combined=False):
    ...
    base=statistics(rows,dict(F=losses['F'],A0=losses['C64'],A=losses['E64'],C8=losses['C8']),seed=232)
    extra=statistics(rows,dict(F=losses['F'],A0=losses['C8'],A=losses['E64']),seed=232)
    ...
    for name,g in base.items():
        e=extra[name];mapping={'F':'F','A0':'C64','A':'E64','C8':'C8'}
        result[name]=dict(nll={mapping[k]:n for k,n in g['nll'].items()},ppl={mapping[k]:n for k,n in g['ppl'].items()},
            vs_F16={'C8':e['vs_F16']['A0'],'C64':g['vs_F16']['A0'],'E64':g['vs_F16']['A']},
            E64_vs_C64=g['A_vs_A0'],E64_vs_C8=e['A_vs_A0'])
    ...
    gates=[g['vs_F16']['E64'] for g in result.values()]
    ...
    out=dict(experiment='EXP-0232',phase='combined' if combined else 'primary',role='packed_FP16_GPU_software_only',
