...
package="${QBH_EXP0059_PACKAGE:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0042/block_package_layer14_m64_integer_attention_parallel}"
result_root="${QBH_EXP0059_RESULT_ROOT:-/mnt/d/llm_exp/results/qwen3-block-htp/exp0059}"
artifact_root="${QBH_EXP0059_ARTIFACT_ROOT:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0059/artifacts}"
...
"${project_root}/scripts/build_exp0059.sh" > "${result_dir}/build.log" 2>&1
"${project_root}/scripts/check_exp0059_static.sh" > "${result_dir}/static_gate.json"
QBH_EXP0059_PACKAGE="${package}" "${project_root}/scripts/deploy_exp0059_block.sh" \
...
for mode in control candidate; do
    "${project_root}/scripts/run_exp0059.sh" "${mode}" 1 on on \
...
for round in 1 2 3 4 5 6 7; do
    printf 'EXP-0059 paired round %d/%d\n' "${round}" "${paired_rounds}"
    ...
    for repeat in "${repeats[@]}"; do for mode in "${modes[@]}"; do
        "${project_root}/scripts/run_exp0059.sh" "${mode}" "${repeat}" on off \
            >> "${result_dir}/paired_${mode}_r${repeat}.jsonl"
    done; done
done
...
python3 "${project_root}/scripts/validate_exp0059.py" "${result_dir}" "${package}" \
    > "${result_dir}/gate_summary.json"
python3 "${project_root}/scripts/validate_exp0059.py" "${result_dir}" "${package}" --report \
...
{
    printf 'experiment=EXP-0059\nsource_branch=%s\nsource_head=%s\n' "${source_branch}" "${source_head}"
    printf 'execution_unit=qwen3_layer14_complete_block_m64\n'
    printf 'control=EXP0058-vgather-vdeal-independent-softmax\n'
    printf 'candidate=EXP0058-vgather-vdeal-paired-softmax\n'
