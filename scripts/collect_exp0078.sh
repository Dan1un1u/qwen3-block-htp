...
package="${QBH_EXP0078_PACKAGE:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0042/block_package_layer14_m64_integer_attention_parallel}"
result_root="${QBH_EXP0078_RESULT_ROOT:-/mnt/d/llm_exp/results/qwen3-block-htp/exp0078}"
artifact_root="${QBH_EXP0078_ARTIFACT_ROOT:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0078/artifacts}"
...
"${project_root}/scripts/build_exp0078.sh" > "${result_dir}/build.log" 2>&1
"${project_root}/scripts/check_exp0078_static.sh" > "${result_dir}/static_gate.json"
QBH_EXP0078_PACKAGE="${package}" "${project_root}/scripts/deploy_exp0078_block.sh" \
...
for mode in control candidate; do
    "${project_root}/scripts/run_exp0078.sh" "${mode}" 1 on on \
...
for round in 1 2 3 4 5 6 7; do
    printf 'EXP-0078 paired round %d/%d\n' "${round}" "${paired_rounds}"
    if (( round % 2 == 1 )); then
        repeats=(1 10)
    ...
    for repeat in "${repeats[@]}"; do
        for mode in "${modes[@]}"; do
            "${project_root}/scripts/run_exp0078.sh" "${mode}" "${repeat}" on off \
                >> "${result_dir}/paired_${mode}_r${repeat}.jsonl"
        done
    done
done
...
python3 "${project_root}/scripts/validate_exp0078.py" "${result_dir}" "${package}" \
    > "${result_dir}/gate_summary.json"
python3 "${project_root}/scripts/validate_exp0078.py" "${result_dir}" "${package}" --report \
...
{
    printf 'experiment=EXP-0078\nsource_branch=%s\nsource_head=%s\n' "${source_branch}" "${source_head}"
    printf 'execution_unit=qwen3_layer14_complete_block_m64\n'
    printf 'control=three_transient_gate_up_hvx_workers\n'
    printf 'candidate=three_persistent_gate_up_hvx_workers\n'
