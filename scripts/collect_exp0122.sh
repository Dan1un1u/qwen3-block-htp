...
u8_package="${QBH_EXP0122_U8_PACKAGE:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0042/block_package_layer14_m64_integer_attention_parallel}"
result_root="${QBH_EXP0122_RESULT_ROOT:-/mnt/d/llm_exp/results/qwen3-block-htp/exp0122}"
artifact_root="${QBH_EXP0122_ARTIFACT_ROOT:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0122/artifacts}"
...
if ! git -C "${project_root}" diff --quiet || \
   ...
   [[ -n "$(git -C "${project_root}" ls-files --others --exclude-standard)" ]]; then
    printf 'source worktree must be clean before EXP-0122 formal collection\n' >&2
    exit 1
fi
...
"${project_root}/scripts/build_exp0122.sh" > "${result_dir}/build.log" 2>&1
"${project_root}/scripts/check_exp0122_static.sh" > "${result_dir}/static_gate.json"
QBH_EXP0122_U8_PACKAGE="${u8_package}" \
    "${project_root}/scripts/deploy_exp0122_block.sh" \
...
for cell in "${cells[@]}"; do
    ...
    "${project_root}/scripts/run_exp0122.sh" "${cell}" 1 on on > "${raw}"
...
for round in 1 2 3 4 5; do
    ...
    for repeat in "${repeats[@]}"; do
        for cell in "${order[@]}"; do
            ...
            "${project_root}/scripts/run_exp0122.sh" \
...
python3 "${project_root}/scripts/analyze_exp0122.py" "${result_dir}" \
    > "${result_dir}/gate_summary.json"
python3 "${project_root}/scripts/analyze_exp0122.py" "${result_dir}" --report \
...
{
    printf 'experiment=EXP-0122\nsource_head=%s\n' "${source_head}"
    printf 'parent=EXP-0120-ring16-worker3\ncells=%s\n' "${cells[*]}"
    printf 'recipe=W4U8\nvariable=gate_up_linked_dma_chain_length\n'
