#!/usr/bin/env python3
"""Transformer-only final-output-selected GPTQ A/C export with unchanged W4F16 DSP runtime."""
import argparse
import gc
import json
import shutil
...
from output_scale_exp0224 import RESULT,OUTPUT,PREVIOUS,PREVIOUS_MODELS,CLIPPED,control_package,select_output
...
def prepare(v):
    ...
    if v=='C':
        assert pack.sha256_file(CLIPPED/'closure.json')=='5355f49db8b8d887ca0737a5e0a7468cfab1c4b9f9ec6ae030b5ea4518cf0d41'
        closure=json.loads((CLIPPED/'closure.json').read_text())
    ...
    manifest.update(experiment='EXP-0224',variant=v,transform={'A':'original','B':'fresh_gamma_fold','C':'fresh_gamma_fold_H2048_R1_H128_R2'}[v],
        quantizer='GPTQ_final_output_3range_per_row_signed_minus7_to7_no_groups',calibration_sha256=ev.digest(PREVIOUS/'calibration.json'),frozen_head_from='EXP0221_'+v,
