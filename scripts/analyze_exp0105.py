#!/usr/bin/env python3
"""Validate EXP-0105 and render its PC-027/PC-028 closure."""
...
PHYSICAL = tuple(dict.fromkeys((
    *base.COUNTERS, *base.RESOURCES, *exp84.PHYSICAL,
    "block_invocation_count", "w4u8_softmax_reduction_mode",
    "w4u8_softmax_reduction_mode_observed",
    "w4u8_softmax_dual_head_row_count",
...
def validate_record(record: dict[str, object], repeat: int,
    ...
    fixed = {
        "experiment": "EXP-0105",
        "w4u8_softmax_reduction_mode": 1 if candidate else 0,
        "w4u8_softmax_reduction_mode_observed": 1 if candidate else 0,
        "w4u8_softmax_dual_head_row_count": 512 * repeat if candidate else 0,
...
def build_summary(result_dir: Path, package_dir: Path) -> dict[str, object]:
    if (result_dir / "boot_id_before.txt").read_bytes() != \
        ...
        raise SystemExit("device boot ID changed during EXP-0105")
    ...
    for mode in MODES:
        ...
        correctness[mode] = {
            ...
            "mode_observed": record["w4u8_softmax_reduction_mode_observed"],
            "candidate_row_count": record["w4u8_softmax_dual_head_row_count"],
        }
    ...
    for repeat in REPEATS:
        ...
        metrics = metric_set(records["control"], records["dual"])
        speed = all(
        ...
        repeat_results[f"repeat{repeat}"] = {
            "metrics": metrics,
            "softmax_attention_host_speed_gate": speed,
        }
        records_by_repeat[repeat] = records
    local_gate = all(gates)
    selected = "dual" if local_gate else "control"
    return {
        "experiment": "EXP-0105",
        "control": "EXP-0084 serial per-head Softmax reductions",
        "candidate": "dual-head interleaved Softmax reductions",
        ...
        "pc028": {
            ...
            "w4u8_candidate": module_medians(records_by_repeat[10]["dual"]),
            "w4u8_provenance": (
                "EXP-0105 candidate" if local_gate else "EXP-0105 control"
            ),
...
def render_report(summary: dict[str, object]) -> str:
    lines = ["# EXP-0105 — Complete profiling report", ""]
    add_pc028(lines, summary)
    lines.extend([
        "The candidate leaves the paired-head carrier and all Softmax math "
        "unchanged. It advances the two independent max, log2-weight-sum "
        "and probability-sum trees together at every HVX reduction level. "
        "All three candidate helpers are explicit symbols with no vector "
        "stack access.", "",
    ])
    for repeat in REPEATS:
        ...
        lines.extend([
            "Softmax work, Attention ledger and complete Host speed gate: "
            f"**{'PASS' if result['softmax_attention_host_speed_gate'] else 'FAIL'}**.", "",
        ])
    lines.extend([
        "## Correctness and physical gates", "",
        "| Gate | Result |", "|---|---:|",
        "| Final block and audited boundaries | byte-exact, 0 mismatch, 0 LSB |",
        "| Candidate static helpers | 3 dual-head trees, 0 vector stack access |",
        "| Candidate dual-head rows | 512 per block |",
        ...
        "## Decision", "",
        f"EXP-0105 local gate: **{'PASS' if summary['local_gate_pass'] else 'FAIL'}**. "
