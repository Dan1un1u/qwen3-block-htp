#!/usr/bin/env python3
"""Validate and summarize EXP-0212 Down8-tail next-Q32 prefetch."""
...
def main() -> None:
    ...
    report = [
        "# EXP-0212 Down8-tail next-Q32 prefetch report",
        "",
        f"Source commit: `{args.source_commit}`",
        "",
        "Both cells inherit the EXP-0211 fastest enabling parent. The "
        "candidate moves each next layer's first Q32 packed-W4 weight DMA "
        "under the current layer's final Down8 HMX command; the next QKV "
        "entry fetches only bias. Math, bytes, descriptors, HMX commands and "
        "tile pairs remain unchanged.",
        "",
        "## End-to-end gate",
        "",
