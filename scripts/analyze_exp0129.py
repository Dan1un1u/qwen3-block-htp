#!/usr/bin/env python3
"""Validate and report EXP-0129 W4F16 Gate/Up worker count."""
...
W4F16_PIPELINE = (
    ...
    "w4f16_requested_hvx_workers", "w4f16_hvx_workers_created",
    "w4f16_hvx_workers_locked", "w4f16_active_worker_min",
    "w4f16_active_worker_max",
)
...
def validate_record(record: dict, repeat: int, cell: str,
    ...
    compatible["w4f16_requested_hvx_workers"] = 3
    compatible["w4f16_hvx_workers_created"] = 3
    compatible["w4f16_hvx_workers_locked"] = 3
    exp110.validate_record(compatible, repeat, "carrier", audit=audit)
    base.require(record, "experiment", "EXP-0129")
    base.require(record, "w4f16_group_fence_mode", "join_only")
    expected_workers = 3 if cell == "worker3" else 4
    base.require(record, "w4f16_requested_hvx_workers", expected_workers)
    base.require(record, "w4f16_hvx_workers_created", expected_workers)
    base.require(record, "w4f16_hvx_workers_locked", expected_workers)
...
def baseline_pc028(exp0109_dir: Path, exp0124_dir: Path,
...
def build_summary(result_dir: Path, exp0109_dir: Path,
    ...
    if ((result_dir / "boot_id_before.txt").read_bytes() !=
        ...
        raise SystemExit("device boot ID changed during EXP-0129")
    ...
    for repeat in REPEATS:
        ...
        for cell in CELLS:
            ...
            sides[cell] = rows
        values = metrics(sides["worker3"], sides["worker4"])
...
def add_candidate_modules(lines: list[str], summary: dict) -> None:
    ...
    lines.extend([
        "## EXP-0129 repeat-ten W4F16 module wall-time", "",
        "| Module | Three workers | Four workers | Speed |",
        "|---|---:|---:|---:|",
    ])
...
def render_report(summary: dict) -> str:
    lines = ["# EXP-0129 — W4F16 Gate/Up worker-count report", ""]
    exp107.add_pc028(lines, summary)
    lines.append(
        "PC-028 provenance: F16F16 uses frozen EXP-0109, W4U8 uses "
        "EXP-0124 control, and W4F16 is this experiment's three-worker "
        "control."
    )
    lines.append("")
    ...
    lines.extend([
        ...
        f"| EXP-0129 local gate | {'PASS' if summary['local_gate_pass'] else 'FAIL'} |",
