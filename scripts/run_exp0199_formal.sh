...
"${project_root}/scripts/build_exp0199.sh" > "${result_dir}/build.log" 2>&1
"${project_root}/scripts/check_exp0199_static.sh" > "${result_dir}/static_gate.json"
"${project_root}/scripts/deploy_exp0199.sh" > "${result_dir}/deploy.log" 2>&1
...
for round in $(seq 1 10); do
    ...
    for cell in "${order[@]}"; do
        ...
        QBH_GENERATION_BOUNDARY_AUDIT=0 \
            "${project_root}/scripts/run_exp0199.sh" "${cell}" \
            > "${result_dir}/raw/pair_${index}_${cell}.log" 2>&1
    done
done

"${python_exe}" "${project_root}/scripts/summarize_exp0199.py" \
