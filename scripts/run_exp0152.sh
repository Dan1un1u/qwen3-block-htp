...
model_root="${QBH_EXP0152_MODEL_ROOT:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0152}"
recipe="${1:?f16f16, w4f16, or w4u8 required}"
mode="${2:-replay}"
remote_root="${EXP0152_REMOTE_ROOT:-/data/local/tmp/qwen3-block-htp/exp0152-${recipe}}"
package="${QBH_EXP0152_PACKAGE:-${model_root}/${recipe}}"
capture_root="${QBH_REPLAY_CAPTURE_ROOT:-}"
numerical_audit="${QBH_EXP0152_NUMERICAL_AUDIT:-off}"
...
if [[ "${QBH_EXP0152_DEPLOY:-0}" == 1 ]]; then
