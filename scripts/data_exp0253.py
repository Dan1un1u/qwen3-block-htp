...
from data_exp0252 import prior_data as old_prior, verified as previous_verified
from data_exp0233 import ids_of
from gptq_exp0221 import fetch
RESULT=BASE/'exp0253'
def verified(exp,name):
 if exp!='exp0252':return previous_verified(exp,name)
 ledger=BASE/exp/'EVIDENCE_SHA256.json'
 assert sha(ledger)=='c2491088eefd8f96f943be73c63898815697f906a8eab6e898d48639169ccd9c'
...
def prior_data():
 paths,ds,rows=old_prior();p=verified('exp0252','dataset.json');d=json.loads(p.read_text())
 return paths+[p],ds+[d],rows+d['samples']
def frozen():
 f=json.loads((RESULT/'dataset_freeze.json').read_text())
 assert sha(MEMORY/'docs/experiments/EXP-0253.md')==f['protocol_sha256']
...
def prepare():
 ...
 rng=np.random.default_rng(253);chosen={c:[] for c in CELLS};used=set();grams=set();hashes=set();sources=[]
 def select(cell,rows,path):
  if len(chosen[cell])==32:return
 ...
 for lang in ['en','zh']:
  ...
  select(cell,rows,p);assert len(chosen[cell])==32;print('DATA_CELL_READY',cell,32,flush=True)
 for lang in ['en','zh']:
  ...
  for p in sorted(pool.values()):
   ...
   if len(chosen[cell])==32:break
  if len(chosen[cell])<32:
   from urllib.parse import urlencode
   for off in range(134000,144000,100):
    ...
    if len(chosen[cell])==32:break
  assert len(chosen[cell])==32,(cell,len(chosen[cell]));print('DATA_CELL_READY',cell,32,flush=True)
 samples=[]
 for i in range(32):
  for c in CELLS:
   r=chosen[c][i];r.update(id=len(samples),split='final',prompt_ids=r['token_ids'][:64],target_ids=r['token_ids'][64:]);samples.append(r)
 write('dataset.json',dict(version='qbh-EXP253-floating-attention-final-v1',seed=253,cells=CELLS,samples=samples,provenance=sources,tokenizer_sha256=sha(MODEL/'qwen3-tokenizer.json'),calibration_reference='EXP230_C64_65536',final_targets=2048,prior_paths=[str(p) for p in paths]))
 ...
 write('dataset_freeze.json',dict(protocol_sha256=sha(MEMORY/'docs/experiments/EXP-0253.md'),files={'dataset.json':sha(RESULT/'dataset.json')},references={str(p):sha(p) for p in refs},frozen_before_export_and_scoring=True))
...
def audit():
 ...
 assert len(used)==128
 write('independent_data_audit.json',dict(pass_all=True,documents=128,final_targets=2048,direct_tuple32_disjoint=True,all_windows_reconstructed=True,dataset_sha256=sha(RESULT/'dataset.json')));print('DATA_AUDIT_PASS',flush=True)
