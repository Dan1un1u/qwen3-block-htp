...
package_dir="${QBH_EXP0047_PACKAGE:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0042/block_package_layer14_m64_integer_attention_parallel}"
stamp="$(date -u +%Y%m%dT%H%M%SZ)"
source_head="$(git -C "${project_root}" rev-parse --short=12 HEAD)"
result_dir="${1:-/mnt/d/llm_exp/results/qwen3-block-htp/exp0047/${stamp}_${source_head}_attention_audit}"
...
"${adb_exe}" -s "${serial}" shell \
    "mkdir -p ${remote_dump} && cd ${remote_root} && QBH_DUMP_ATTENTION_DIR=${remote_dump} LD_LIBRARY_PATH=${remote_root} DSP_LIBRARY_PATH=${remote_root} ADSP_LIBRARY_PATH=${remote_root} ./qwen3_block_cli ${remote_root}/block_package_layer14_m64 W4U8 1 2 32 rms_rope_softmax on on fused serial control hvx w4u8_streaming 3 64 u8_log2_gqa_qkv_overlap 4 w4u8_mlp_io qkvo_batch4" \
