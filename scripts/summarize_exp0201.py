#!/usr/bin/env python3
"""Validate and summarize EXP-0201 interleaved Gate/Up asynchronous SwiGLU evidence."""
...
def validate_run(run: dict[str, object], cell: str, steps: int) -> None:
    ...
    for index, profile in enumerate(profiles):
        ...
        base.require_equal(
            ...
            enabled, context + " O-to-Gate selector")
        base.require_equal(
            int(profile[
                "w4u8_decode_direct_n_gate_up_swiglu_stream"]),
            enabled, context + " Gate/Up SwiGLU stream selector")
        base.require_equal(
...
def main() -> None:
    ...
    gates = {
        ...
        "gate_up_swiglu_wall_faster":
            gate_up_swiglu[CANDIDATE] < gate_up_swiglu[CONTROL],
    ...
    report = [
        "# EXP-0201 interleaved Gate/Up asynchronous SwiGLU report",
        "",
        f"Source commit: `{args.source_commit}`",
        "",
        "Both cells inherit EXP-0199. The candidate combines O-to-Gate "
        "prefetch with Gate/Up group interleaving and a persistent HVX "
        "consumer that computes the unchanged U8 LUT SwiGLU.",
        "",
        "## End-to-end gate",
        "",
        "| Metric | EXP-0199 control | Stream candidate | Change |",
        "|---|---:|---:|---:|",
        ...
        *pair_rows,
        ...
        "",
        "| Module | EXP-0199 control | Stream candidate | Candidate speed change |",
        "|---|---:|---:|---:|",
        *module_rows,
