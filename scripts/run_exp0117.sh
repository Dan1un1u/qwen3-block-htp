...
remote_root="${EXP0117_REMOTE_ROOT:-/data/local/tmp/qwen3-block-htp/exp0117-u8}"
cell="${1:-control}"
repeat_count="${2:-1}"
attribution_mode="${3:-on}"
audit_mode="${4:-off}"
activation_lut=control
...
case "${cell}" in
    control)
        ;;
    packed_pair)
        activation_lut=packed_pair
        ;;
    *)
        printf 'usage: %s control|packed_pair [repeat] [attribution] [audit]\n' "$0" >&2
        exit 2
        ;;
esac
...
"${adb_exe}" shell \
    "cd ${remote_root} && QBH_W4U8_STREAM_FENCE=single_fence QBH_W4U8_ACTIVATION_LUT=${activation_lut} LD_LIBRARY_PATH=${remote_root} DSP_LIBRARY_PATH=${remote_root} ADSP_LIBRARY_PATH=${remote_root} ./qwen3_block_cli ${remote_root}/block_package_layer14_m64 W4U8 ${repeat_count} ${runtime_args}"
