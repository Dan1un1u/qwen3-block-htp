...
"${project_root}/scripts/build_exp0188.sh" > "${result_dir}/build.log" 2>&1
"${project_root}/scripts/check_exp0188_static.sh" > "${result_dir}/static_gate.json"
"${project_root}/scripts/deploy_exp0188.sh" > "${result_dir}/deploy.log" 2>&1

for round in $(seq 1 10); do
    printf -v index '%02d' "${round}"
    if ((round % 2 == 1)); then
        order=(expanded_s8 direct_n)
    else
        order=(direct_n expanded_s8)
    fi
    for cell in "${order[@]}"; do
        QBH_EXP0173_GENERATION_STEPS=193 \
        QBH_GENERATION_BOUNDARY_AUDIT=0 \
        QBH_EXP0188_DIRECT_N_MASK=7 \
            "${project_root}/scripts/run_exp0188.sh" "${cell}" \
            > "${result_dir}/raw/pair_${index}_${cell}.log" 2>&1
    done
done

"${python_exe}" "${project_root}/scripts/summarize_exp0188.py" \
