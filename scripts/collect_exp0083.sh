...
package="${QBH_EXP0083_PACKAGE:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0042/block_package_layer14_m64_integer_attention_parallel}"
result_root="${QBH_EXP0083_RESULT_ROOT:-/mnt/d/llm_exp/results/qwen3-block-htp/exp0083}"
artifact_root="${QBH_EXP0083_ARTIFACT_ROOT:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0083/artifacts}"
...
"${project_root}/scripts/build_exp0083.sh" > "${result_dir}/build.log" 2>&1
"${project_root}/scripts/check_exp0083_static.sh" > "${result_dir}/static_gate.json"
QBH_EXP0083_PACKAGE="${package}" "${project_root}/scripts/deploy_exp0083_block.sh" \
...
for mode in control candidate diagnostic32; do
    "${project_root}/scripts/run_exp0083.sh" "${mode}" 1 on on \
        > "${result_dir}/correctness_${mode}.jsonl"
done

for repeat in 1 10; do
    for mode in control candidate diagnostic32; do
        : > "${result_dir}/search_${mode}_r${repeat}.jsonl"
    done
done
for round in 1 2 3; do
    if (( round % 2 == 1 )); then
        modes=(control candidate diagnostic32)
    else
        modes=(diagnostic32 candidate control)
    fi
    for repeat in 1 10; do
        for mode in "${modes[@]}"; do
            "${project_root}/scripts/run_exp0083.sh" "${mode}" "${repeat}" on off \
                >> "${result_dir}/search_${mode}_r${repeat}.jsonl"
        done
    done
done
...
for round in 1 2 3 4 5 6 7; do
    printf 'EXP-0083 paired round %d/%d\n' "${round}" "${paired_rounds}"
    if (( round % 2 == 1 )); then
        repeats=(1 10)
...
python3 "${project_root}/scripts/validate_exp0083.py" "${result_dir}" "${package}" \
    > "${result_dir}/gate_summary.json"
python3 "${project_root}/scripts/validate_exp0083.py" "${result_dir}" "${package}" --report \
...
{
    printf 'experiment=EXP-0083\nsource_branch=%s\nsource_head=%s\n' "${source_branch}" "${source_head}"
    printf 'execution_unit=qwen3_layer14_complete_block_m64\n'
    printf 'control=gate_up_hmx_batch8\ncandidate=gate_up_hmx_batch16\ndiagnostic=gate_up_hmx_batch32\n'
