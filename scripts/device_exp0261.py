...
REMOTE=base.REMOTE+'-r4';PARENT='/data/local/tmp/qwen3-block-htp/exp0252-layer0/r3'
...
def physical(ps,arm,dump):
 for p in ps:
  ...
  assert p['dense_r4_hmx_calls']==((5 if p['logical_m']==64 else 2) if arm else 0)
...
if __name__=='__main__':
 p=argparse.ArgumentParser();p.add_argument('action');p.add_argument('--arm',type=int,default=0);p.add_argument('--repeat',type=int,default=1);p.add_argument('--tag',default='smoke');p.add_argument('--dump',action='store_true');a=p.parse_args()
