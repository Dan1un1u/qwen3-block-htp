#!/usr/bin/env python3
"""Pre-freeze calibration/development/untouched testing for PC051 group128 software."""
...
RESULT=BASE/'exp0231'
OUTPUT=Path('/mnt/d/llm_exp/models/qwen3-block-htp/exp0231')
...
def prior_data():
    paths=[verified(e,n) for e,n in [
        ...
        ('exp0229','dataset.json'),('exp0230','dataset.json')]]
...
def prepare():
    ...
    rng=np.random.default_rng(231);chosen={c:[] for c in CELLS}
    ...
    for lang in ['en','zh']:
        cell=lang+'_wiki'
        for offset in range(26000,32000,100):
    ...
    write('dataset.json',dict(version='qbh-group128-independent-v1',seed=231,cells=CELLS,samples=samples,
        ...
        calibration_reference='EXP0230_C8_8192',development_reference='EXP0230_128docs_descriptive_only',primary_targets=8192,reserve_targets=8192,
    ...
    write('dataset_freeze.json',dict(files={str(p.relative_to(RESULT)):sha(p) for p in [RESULT/'dataset.json',*sorted(inputs.glob('*.bin'))]},
        frozen_before_scoring=True,source_roles='new final primary/final reserve; prior230calibration and development references',protocol_sha256=sha(MEMORY/'docs/experiments/EXP-0231.md')))
