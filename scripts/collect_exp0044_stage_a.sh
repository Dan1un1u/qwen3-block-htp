...
for repeat in 1 10; do
    ...
    : > "${result_dir}/paired_candidate_r${repeat}.jsonl"
done

for round in $(seq 1 "${paired_rounds}"); do
    printf 'EXP-0044 Stage A paired round %d/%d\n' \
        "${round}" "${paired_rounds}"
    if (( round % 2 == 1 )); then
        repeat_order=(1 10)
        mode_order=(control candidate)
    else
        repeat_order=(10 1)
        mode_order=(candidate control)
    fi
...
python3 "${project_root}/scripts/validate_exp0044_stage_a.py" \
...

{
    ...
    printf 'control=EXP0042_integer_Attention\n'
    printf 'candidate=fused_K_Norm_RoPE_carrier_bias\n'
