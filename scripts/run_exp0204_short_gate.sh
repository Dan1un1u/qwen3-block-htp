...
"${project_root}/scripts/build_exp0204.sh" > "${result_dir}/build.log" 2>&1
"${project_root}/scripts/check_exp0204_static.sh" > "${result_dir}/static_gate.json"
"${project_root}/scripts/deploy_exp0204.sh" > "${result_dir}/deploy.log" 2>&1
for cell in o4 o8; do
    QBH_EXP0173_GENERATION_STEPS=4 \
    QBH_GENERATION_BOUNDARY_AUDIT=1 \
    QBH_GENERATION_AUDIT_DIR="${result_dir}/audit/${cell}" \
        "${project_root}/scripts/run_exp0204.sh" "${cell}" \
        > "${result_dir}/raw/audit_${cell}.log" 2>&1
done
for round in $(seq 1 5); do
    printf -v index '%02d' "${round}"
    if ((round % 2 == 1)); then order=(o4 o8); else order=(o8 o4); fi
...
"${python_exe}" "${project_root}/scripts/summarize_exp0204.py" \
