#!/usr/bin/env python3
"""Validate EXP-0107 and render its PC-027/PC-028 closure."""
...
OVERLAP = tuple(dict.fromkeys((
    *base.OVERLAP,
    "f16f16_prefetch_count", "f16f16_prefetch_wait_ticks",
    ...
    "w4f16_gate_up_hmx_wait_ticks", "w4f16_gate_up_stream_work_ticks",
...
def validate(record: dict, repeat: int, recipe: str,
    ...
    exp84.validate_record(compatible, repeat, expected_variant)
    base.require(record, "experiment", "EXP-0107")
    if recipe == "f16f16":
        base.require(
            record, "f16f16_projection_mode",
            "gate_up_batch8_interleaved" if candidate else
            "gate_up_batch8",
        )
...
def metrics(left: list[dict], right: list[dict]) -> dict:
    fields = tuple(dict.fromkeys((
        ...
        "down_ticks", "total_ticks", *LEDGER, *OVERLAP, *PHYSICAL,
    )))
    return {field: summarize(left, right, field) for field in fields}


def modules(records: list[dict]) -> dict[str, float]:
    rows = [dict(exp84.module_us(record)) for record in records]
    return {
        name: float(statistics.median(row[name] for row in rows))
        for name in rows[0]
    }
...
def build_summary(result_dir: Path) -> dict:
    if (result_dir / "boot_id_before.txt").read_bytes() != \
        ...
        raise SystemExit("device boot ID changed during EXP-0107")
    ...
    for recipe in RECIPES:
        ...
        for side in ("control", "candidate"):
            row = load(
                result_dir / f"correctness_{side}_{recipe}.jsonl", 1
            )[0]
        ...
        if pair["control"]["output_hash"] != pair["candidate"]["output_hash"]:
            raise SystemExit(f"{recipe}: control/candidate hash mismatch")
        correctness[recipe] = pair
    ...
    for repeat in REPEATS:
        ...
        for recipe in RECIPES:
            ...
            repeat_result[recipe] = {"metrics": values}
        ...
        results[f"repeat{repeat}"] = repeat_result

    f16_gate = all(value < 0.0 for value in f16_speed_cells)
    parity_gate = all(value <= 1.0 for value in parity_cells)
    physical_gate = all(physical_cells)
    selected = {
        "f16f16": "candidate" if f16_gate else "control",
        "w4f16": "control", "w4u8": "control",
    }
    pc028 = {
        recipe: modules(records[10][recipe][selected[recipe]])
        for recipe in RECIPES
    }
    return {
        "experiment": "EXP-0107",
        ...
        "f16f16_speed_gate": f16_gate,
        "w4f16_w4u8_parity_gate": parity_gate,
        "local_gate_pass": f16_gate and parity_gate and physical_gate,
        "selected": selected,
        "pc028": pc028,
    }
...
def render_report(summary: dict) -> str:
    lines = ["# EXP-0107 — Complete profiling report", ""]
    ...
    for repeat in REPEATS:
        ...
        for recipe in RECIPES:
            ...
            add_table(lines, "Primary targets", (
                ...
                "activation_ticks", "down_ticks", "total_ticks",
            ), values)
    ...
    lines.extend([
        "## Gates", "", "| Gate | Result |", "|---|---:|",
        f"| F16F16 Gate/Up and Host speed | {'PASS' if summary['f16f16_speed_gate'] else 'FAIL'} |",
        f"| W4F16/W4U8 <=1% parity | {'PASS' if summary['w4f16_w4u8_parity_gate'] else 'FAIL'} |",
        ...
        f"| EXP-0107 local gate | {'PASS' if summary['local_gate_pass'] else 'FAIL'} |",
