...
def preflight():
    data.preflight()
    assert subprocess.check_output(['git','branch','--show-current'],cwd=data.SOURCE,text=True).strip()=='codex/exp-0252-normalization-r3'
def freeze():
    ...
    write('freeze.json',dict(files={'inputs.json':data.sha(ROOT/'inputs.json'),'dataset.json':data.sha(ROOT/'dataset.json')},references=refs,
        protocol_sha256=data.sha(data.MEMORY/'docs/experiments/EXP-0252.md'),frozen_before_inference=True))
def frozen():
    data.frozen();f=read('freeze.json')
    assert f['protocol_sha256']==data.sha(data.MEMORY/'docs/experiments/EXP-0252.md')
...
def numerical():
    rng=np.random.default_rng(252);c=read('inputs.json')['configs']['R3_ALL'][0];proofs=[]
...
def run(phase):
    ...
    for recipe in ['C64','F']:
        ...
        for name in NAMES:
            ...
            if phase=='development' and (name in ['F','C64'] or name.split('_',1)[-1] in ['wide_sole','wide_exact']):
                p=data.verified('exp0250',f'scores/development_{name}.json');old=json.loads(p.read_text())
