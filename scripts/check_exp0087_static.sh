...
grep -q '#define QBH_BLOCK_ABI_VERSION UINT32_C(45)' include/block_protocol.h
grep -q '#define QBH_BLOCK_EXPERIMENT UINT32_C(87)' include/block_protocol.h
grep -q 'EXP-0087' src/host/block_main.c
grep -q 'QBH_BLOCK_W4U8_SOFTMAX_CONTROL_ROW_SLICES UINT32_C(2)' src/dsp/block_imp.c
grep -q 'w4u8_attention_softmax_row_slices_requested' src/dsp/block_imp.c
...
printf '%s\n' \
    '{"experiment":"EXP-0087","static_gate":"pass","block_abi":45,"softmax_row_slice_grid":[2,4],"timeline_diagnostic_isolated_from_numerical_audit":true,"qnn_dependency":false,"vtcm_request_bytes":8388608,"single_hmx_owner":true}'
