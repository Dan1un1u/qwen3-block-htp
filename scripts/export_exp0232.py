#!/usr/bin/env python3
"""Fresh AWQ-folded FP32 originals, unchanged C64-budget CPU GPTQ and runtime format."""
...
from data_exp0232 import RESULT,OUTPUT,MODEL,SOURCE,sha,write,preflight,verified

BASE_PACKAGE=OUTPUT.parent/'exp0224/A'
BASE_HASH='a5de4e6c4e02ac913e69fbddb0d4b0b9e12b5cfe88ff606cf1ea18842dc0c179'

from awq_exp0232 import frozen,fold_layer,load_scales,inputs,C64_HASH

def prepare(v):
    preflight();frozen();inputs();assert sha(BASE_PACKAGE/'manifest.json')==BASE_HASH;data=json.loads(verified('exp0230','dataset.json').read_text());v='E64';root=OUTPUT/v;result=RESULT/v
    ...
    rows=[r for r in data['samples'] if r['split']=='calibration']
    ids=torch.tensor([r['token_ids'] for r in rows],dtype=torch.long)
    assert ids.numel()==65536 and ids.shape[1]==128
    ...
    for i,layer in enumerate(model.model.layers):
        fold_layer(layer,load_scales(i))
        ...
        for short,long in [('input','input_layernorm'),('post','post_attention_layernorm')]:
            name=f'layer{i}/{short}_norm_weight_f16.bin'
            pack.atomic_write_bytes(root/name,layer.get_submodule(long).weight.numpy().tobytes());changed.append(name)
        incoming=hidden;normed=layer.input_layernorm(incoming);f=factor(normed)
        for name in ['q','k','v']:
    ...
    manifest.update(experiment='EXP-0232',variant=v,calibration_sha256=sha(verified('exp0230','inputs/C64_calibration_u32.bin')),
        ...
        coordinates='fresh_AWQ_input_diagonals_with_legal_norm_and_Up_compensation',search_sha256=sha(RESULT/'search.json'),
        invariance_sha256=sha(RESULT/'invariance.json'),diagonal_scale_files={f'layer{i}.npz':sha(OUTPUT/'scales'/f'layer{i}.npz') for i in range(28)},
    ...
    write(f'{v}/package.json',dict(manifest_sha256=sha(root/'manifest.json'),changed_files=changed,
        ...
        quantizer_source={n:sha(SOURCE/'scripts'/n) for n in ['gptq_exp0221.py','output_scale_exp0224.py','experiment_exp0224.py','awq_exp0232.py','export_exp0232.py']},
        control_manifest_sha256=C64_HASH,inherited_base_manifest_sha256=BASE_HASH,source_head=manifest['source_head']))
...
if __name__=='__main__':
    ...
    p=argparse.ArgumentParser();p.add_argument('variant',nargs='?',default='E64',choices=['E64']);a=p.parse_args();prepare(a.variant)
