...
RESULT=BASE/'exp0233'
OUTPUT=Path('/mnt/d/llm_exp/models/qwen3-block-htp/exp0233')
...
def prior_data():
    paths=[verified(e,n) for e,n in [
        ...
        ('exp0229','dataset.json'),('exp0230','dataset.json'),('exp0231','dataset.json'),('exp0232','dataset.json')]]
...
def prepare():
    ...
    rng=np.random.default_rng(233);chosen={c:[] for c in CELLS}
    ...
    for lang in ['en','zh']:
        cell=lang+'_wiki'
        for offset in range(38000,46000,100):
    ...
    write('dataset.json',dict(version='qbh-PC052-independent-v1',seed=233,cells=CELLS,samples=samples,
    ...
    write('dataset_freeze.json',dict(files={str(p.relative_to(RESULT)):sha(p) for p in [RESULT/'dataset.json',*sorted(inputs.glob('*.bin'))]},
        frozen_before_scoring=True,source_roles='new final primary/final reserve; prior230calibration and development references',protocol_sha256=sha(MEMORY/'docs/experiments/EXP-0233.md')))
