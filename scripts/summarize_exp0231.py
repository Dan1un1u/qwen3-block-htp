#!/usr/bin/env python3
"""Fixed group128 software PPL acceptance, matched controls and bounded reserve."""
import argparse,json,math
import numpy as np
from data_exp0231 import RESULT,CELLS,write,sha,preflight
from export_exp0231 import frozen
from summarize_exp0229 import statistics, test_statistics

VARIANTS=['F','C8','C64','G128']
...
def summarize(combined=False):
    ...
    base=statistics(rows,dict(F=losses['F'],A0=losses['C8'],A=losses['G128'],C64=losses['C64']),seed=231)
    extra=statistics(rows,dict(F=losses['F'],A0=losses['C64'],A=losses['G128']),seed=231)
    ...
    for name,g in base.items():
        e=extra[name];mapping={'F':'F','A0':'C8','A':'G128','C64':'C64'}
        result[name]=dict(nll={mapping[k]:n for k,n in g['nll'].items()},ppl={mapping[k]:n for k,n in g['ppl'].items()},
            vs_F16={'C8':g['vs_F16']['A0'],'C64':e['vs_F16']['A0'],'G128':g['vs_F16']['A']},
            G128_vs_C8=g['A_vs_A0'],G128_vs_C64=e['A_vs_A0'])
    ...
    gates=[g['vs_F16']['G128'] for g in result.values()]
    ...
    out=dict(experiment='EXP-0231',phase='combined' if combined else 'primary',role='packed_FP16_GPU_software_only',
        documents=len(rows),targets=len(rows)*16,statistics=result,status=status,reserve_trigger=trigger,
        bootstrap=dict(unit='paired_document_stratified_four_cells',replicates=5000,seed=231),
