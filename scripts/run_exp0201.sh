...
cell="${1:?EXP-0201 cell required}"
case "${cell}" in
    control) prefetch=0; stream=0 ;;
    stream) prefetch=1; stream=1 ;;
    *) printf 'invalid EXP-0201 cell: %s\n' "${cell}" >&2; exit 2 ;;
esac
EXP0173_REMOTE_ROOT=/data/local/tmp/qwen3-block-htp/exp0173-exp0201-gate-up-swiglu-stream \
...
QBH_W4U8_DECODE_DIRECT_N_O_GATE_PREFETCH="${prefetch}" \
QBH_W4U8_DECODE_DIRECT_N_GATE_UP_SWIGLU_STREAM="${stream}" \
