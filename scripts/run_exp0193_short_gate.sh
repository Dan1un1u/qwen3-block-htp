...
"${project_root}/scripts/build_exp0193.sh" > "${result_dir}/build.log" 2>&1
"${project_root}/scripts/check_exp0193_static.sh" > "${result_dir}/static_gate.json"
"${project_root}/scripts/deploy_exp0193.sh" > "${result_dir}/deploy.log" 2>&1

for cell in batch2 batch4; do
    QBH_EXP0173_GENERATION_STEPS=4 \
    QBH_GENERATION_BOUNDARY_AUDIT=1 \
    QBH_GENERATION_AUDIT_DIR="${result_dir}/audit/${cell}" \
        "${project_root}/scripts/run_exp0193.sh" "${cell}" \
...
"${python_exe}" "${project_root}/scripts/summarize_exp0190.py" \
    ...
    --experiment 193 --control-batch 2 --candidate-batch 4 --target down-dma
printf 'RESULT_DIR=%s\n' "${result_dir}"
