...
result_root="${QBH_RESULT_ROOT:-/mnt/d/llm_exp/results/qwen3-block-htp/exp0019}"
artifact_root="${QBH_ARTIFACT_ROOT:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0019}"
...
"${project_root}/scripts/build_exp0019.sh" > "${result_dir}/build.log" 2>&1
...
# Every row is an equivalent-scope candidate.  Array order is deliberately
# interleaved by shape during timing rotations so no plan owns one thermal slot.
storages=(
    expanded_s8_control
    packed_w4_hmx_postscale packed_w4_hmx_postscale
    packed_w4_hmx_postscale packed_w4_hmx_postscale
    packed_w4_hmx_postscale packed_w4_hmx_postscale
    expanded_s8_control
    packed_w4_hmx_postscale packed_w4_hmx_postscale
...
projections=(
    gate_up_pair gate_up_pair gate_up_pair gate_up_pair gate_up_pair
...
plans=(
    ...
    stream32_gate_hvx2 stream32_gate_hvx3 stream32_gate_hvx4
    stream32_gate_hvx6 stream32_gate_hvx6_cap2
    exp0005_full_bundle_control
    slots4_chunk96_dma_batch2
    stream32_down_hvx2 stream32_down_hvx3 stream32_down_hvx4
    stream32_down_hvx6 stream32_down_hvx6_cap2
)
...
run_one() {
    ...
    local output="$4"
    if [[ "${deployed}" == "0" ]]; then
        "${project_root}/scripts/run_exp0019.sh" \
            "${storages[index]}" "${projections[index]}" "${pattern}" \
            "${repeat_count}" "${plans[index]}" linked_2d_dma \
            prepared_session single_invocation >> "${output}"
        deployed=1
    else
        QBH_SKIP_DEPLOY=1 "${project_root}/scripts/run_exp0019.sh" \
            "${storages[index]}" "${projections[index]}" "${pattern}" \
            "${repeat_count}" "${plans[index]}" linked_2d_dma \
            prepared_session single_invocation >> "${output}"
    fi
...
run_timing_matrix() {
    ...
    for round in 0 1 2 3 4; do
        for pattern_index in "${!patterns[@]}"; do
            local start=$(((round + pattern_index * 3) % ${#plans[@]}))
            for step in "${!plans[@]}"; do
                local index=$(((start + step) % ${#plans[@]}))
                run_one "${index}" "${patterns[pattern_index]}" \
                    "${repeat_count}" "${output}"
            done
        done
    done
...
run_timing_matrix 1 "${result_dir}/timing_repeat1.jsonl"
run_timing_matrix 10 "${result_dir}/timing_repeat10.jsonl"
...
python3 "${project_root}/scripts/validate_exp0019_evidence.py" \
    "${result_dir}" > "${result_dir}/matrix_gate.json"

cp "${project_root}/android_ReleaseG_aarch64/ship/qwen3_probe_cli" "${artifact_dir}/"
