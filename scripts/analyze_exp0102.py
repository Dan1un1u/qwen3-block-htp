#!/usr/bin/env python3
"""Validate EXP-0102 and render its PC-027/PC-028 closure."""
...
PHYSICAL = tuple(dict.fromkeys((
    *base.COUNTERS, *base.RESOURCES, "block_invocation_count",
    "w4u8_qkv_batch_n_tiles", "w4u8_qkv_batch_n_tiles_config",
    "w4u8_qkv_batch_n_tiles_observed", "w4u8_qkv_batch_count",
...
def validate_record(record: dict[str, object], repeat: int,
    ...
    batch = 4 if mode == "control" else 8
    fixed = {
        "experiment": "EXP-0102",
        "w4u8_qkv_batch_n_tiles": batch,
        "w4u8_qkv_batch_n_tiles_config": batch,
        "w4u8_qkv_batch_n_tiles_observed": batch,
        "block_invocation_count": repeat,
        "vtcm_requested_bytes": 8_388_608,
        "vtcm_acquired_bytes": 8_388_608,
        "vtcm_peak_plan_bytes": 5_570_272,
        ...
        "weight_dma_descriptor_count": (
            512 if mode == "control" else 480
        ) * repeat,
        "hmx_command_count": (176 if mode == "control" else 160) * repeat,
        "hmx_u8s8_tile_pair_count": 49_408 * repeat,
        "w4u8_qkv_batch_count": (32 if mode == "control" else 16) * repeat,
        "w4u8_qkvo_prefetch_count": (
            44 if mode == "control" else 28
        ) * repeat,
        "w4u8_qkvo_overlap_schedule_count": (
            44 if mode == "control" else 28
        ) * repeat,
        "w4u8_q_ready_publish_count": 16 * repeat,
        "w4u8_k_ready_publish_count": 8 * repeat,
...
def per_block(record: dict[str, object], field: str) -> float:
    unscaled = {
        ...
        "w4u8_qkv_batch_n_tiles", "w4u8_qkv_batch_n_tiles_config",
        "w4u8_qkv_batch_n_tiles_observed",
...
def build_summary(result_dir: Path, package_dir: Path) -> dict[str, object]:
    if (result_dir / "boot_id_before.txt").read_bytes() != \
        ...
        raise SystemExit("device boot ID changed during EXP-0102")
    ...
    for mode in MODES:
        ...
        correctness[mode] = {
            ...
            "qkv_batch_tiles": record["w4u8_qkv_batch_n_tiles_observed"],
            "qkv_commands": record["w4u8_qkv_batch_count"],
            "total_commands": record["hmx_command_count"],
            "weight_dma_descriptors": record["weight_dma_descriptor_count"],
            "q_ready_publishes": record["w4u8_q_ready_publish_count"],
            "k_ready_publishes": record["w4u8_k_ready_publish_count"],
        }
    ...
    for repeat in REPEATS:
        ...
        metrics = metric_set(records["control"], records["batch8"])
        speed = all(
        ...
        repeat_results[f"repeat{repeat}"] = {
            "metrics": metrics,
            "qkv_and_host_speed_gate": speed,
        }
        records_by_repeat[repeat] = records
    local_gate = all(gates)
    selected = "batch8" if local_gate else "control"
    return {
        "experiment": "EXP-0102",
        "control": "QKV batch four: one head per HMX command",
        "candidate": "QKV batch eight: two heads per HMX command",
        ...
        "head_readiness_publish_gate": True,
        ...
        "pc028": {
            ...
            "w4u8_candidate": module_medians(records_by_repeat[10]["batch8"]),
            "w4u8_provenance": (
                "EXP-0102 batch8" if local_gate else "EXP-0102 control"
            ),
...
def render_report(summary: dict[str, object]) -> str:
    lines = ["# EXP-0102 — Complete profiling report", ""]
    add_pc028(lines, summary)
    lines.extend([
        "The candidate doubles only Q/K/V projection batching from four to "
        "eight output tiles, aligning one HMX command with two complete heads. "
        "O stays at batch four. Both cells use the same enlarged VTCM plan. "
        "The candidate publishes both completed heads separately and reduces "
        "QKV commands 32→16, total HMX commands 176→160 and DMA descriptors "
        "512→480 per block without changing tiles, bytes, or math.", "",
    ])
    for repeat in REPEATS:
        ...
        lines.extend([
            "QKV projection and complete Host speed gate: "
            f"**{'PASS' if result['qkv_and_host_speed_gate'] else 'FAIL'}**.", "",
        ])
    lines.extend([
        "## Correctness and physical gates", "",
        "| Gate | Result |", "|---|---:|",
        "| Final block and audited boundaries | byte-exact, 0 mismatch, 0 LSB |",
        "| Q/K readiness publication | 16 / 8 heads per block in both cells |",
        "| Requested/acquired VTCM | 8,388,608 / 8,388,608 bytes |",
        "| Common peak plan | 5,570,272 bytes |",
        "| Intermediate DDR / spill-fill | 0 / 0 |",
        "| FastRPC / HMX ownership | one execution unit / one owner |",
        "| Weight bytes / U8xS8 tile pairs | 25,444,352 / 49,408 per block |",
        "| QKV / total HMX commands | 32→16 / 176→160 per block |",
        "| DMA descriptors | 512→480 per block |", "",
        "## Decision", "",
        f"EXP-0102 local gate: **{'PASS' if summary['local_gate_pass'] else 'FAIL'}**. "
