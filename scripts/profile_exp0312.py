...
def preflight():
 ...
 assert 'EXPERIMENT=EXP-0312' in z
def stage(a):
...
def execute(a,tag,arm,repeat=1,audit=False):
 ...
 base=read(OLD/'formal-00/protocol.json')['command']
 ...
 fixture=read(OLD/'fixture.json');words=[0x51424556,2,repeat,69]
 ...
 save(d/'protocol.json',dict(command=cmd,layers=a.layers,repeat=repeat,arm=arm,audit=audit,seal_sha256=sha(R/f'binaries-{a.layers}-{a.version}/seal.json'),old_protocol_sha256=sha(OLD/'formal-00/protocol.json'),reference_sha256=sha(OLD/'reference/summary-runtime-hash.json')))
 adb('push',win(ef),er)
 ...
 rr=records(d/'stdout.txt');pp=[x for x in rr if x.get('record')=='generation_profile']
 ...
 gold=read(OLD/'reference/summary-runtime-hash.json');checks=[]
 ...
 for j,(x,t) in enumerate(zip(pp,ss)):
  ...
  norm=normalized([x]);assert sum(norm[k] for _,k in LEDGER)==x['invocation_ticks']
...
if __name__=='__main__':
 p=argparse.ArgumentParser();p.add_argument('action',choices=['stage','audit','run']);p.add_argument('--model',choices=['1.7B','0.6B'],default='1.7B');p.add_argument('--layers',type=int,default=28);p.add_argument('--version',default='a01');p.add_argument('--arm',choices=['fused','split'],default='split');p.add_argument('--tag',default='audit');a=p.parse_args()
 R=Path('/mnt/d/llm_exp/results/qwen3-block-htp/exp0312')/a.model;REMOTE='/data/local/tmp/qwen3-block-htp/exp0312/'+a.model
 OLD=Path('/mnt/d/llm_exp/results/qwen3-block-htp')/('exp0305' if a.model=='1.7B' else 'exp0306')
