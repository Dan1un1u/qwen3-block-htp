...
result_root="${QBH_RESULT_ROOT:-/mnt/d/llm_exp/results/qwen3-block-htp/exp0030}"
artifact_root="${QBH_ARTIFACT_ROOT:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0030/artifacts}"
...
"${project_root}/scripts/build_exp0030.sh" > "${result_dir}/build.log" 2>&1
QBH_STATIC_OUTPUT_DIR="${result_dir}/static" \
    "${project_root}/scripts/check_exp0030_static.sh" \
...
configs=(
    f16_control f16_parallel f16_streaming
    w4_control w4_parallel w4_streaming
)
...
set_config() {
    ...
    case "${config}" in
        f16_control)
            variant=F16F16; workers=2; f16_mode=batch2
            w4_mode=control; mlp_mode=control; contexts=1 ;;
        f16_parallel)
            variant=F16F16; workers=2; f16_mode=batch2
            w4_mode=control; mlp_mode=parallel_silu; contexts=4 ;;
        f16_streaming)
            variant=F16F16; workers=2; f16_mode=batch2
            w4_mode=control; mlp_mode=streaming; contexts=4 ;;
        w4_control)
            variant=W4F16; workers=3; f16_mode=serial
            w4_mode=adaptive_down96_cross; mlp_mode=control; contexts=1 ;;
        w4_parallel)
            variant=W4F16; workers=3; f16_mode=serial
            w4_mode=adaptive_down96_cross; mlp_mode=parallel_silu; contexts=4 ;;
        w4_streaming)
            variant=W4F16; workers=3; f16_mode=serial
            w4_mode=adaptive_down96_cross; mlp_mode=streaming; contexts=4 ;;
        *)
...
for round in 1 2 3 4 5; do
    if (( round % 2 == 1 )); then
        ...
        order=(
            w4_streaming w4_parallel w4_control
            f16_streaming f16_parallel f16_control
        )
    fi
...
python3 "${project_root}/scripts/validate_exp0030_evidence.py" \
...

{
    ...
    printf 'mlp_modes=control,multi_worker_silu,streaming\n'
