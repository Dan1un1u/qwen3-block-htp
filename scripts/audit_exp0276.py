...
def chain(tag, repeat_tag):
 preflight();p=O/'sp2-fp32';d=R/tag;a=read(d/'validated.json');r=read(R/repeat_tag/'validated.json');assert a['numerical_pass'] and r['output_hashes']==a['output_hashes']*10
 for step in range(2):
  ...
  elif step:assert np.array_equal(actual[4:5],((mid+32768)>>8).astype('u1'))
