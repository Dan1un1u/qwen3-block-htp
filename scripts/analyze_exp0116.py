#!/usr/bin/env python3
"""Validate and report EXP-0116 W4F16 Down publication fences."""
...
def validate_record(record: dict, repeat: int, cell: str,
    ...
    base.require(record, "experiment", "EXP-0116")
    base.require(
...
def metrics(left: list[dict], right: list[dict]) -> dict:
    fields = tuple(dict.fromkeys((
        ...
        *exp111.W4F16_PIPELINE, *exp107.PHYSICAL,
    )))
...
def build_summary(result_dir: Path, exp0109_dir: Path) -> dict:
    if ((result_dir / "boot_id_before.txt").read_bytes() !=
        ...
        raise SystemExit("device boot ID changed during EXP-0116")
...
def add_candidate_modules(lines: list[str], summary: dict) -> None:
    ...
    lines.extend([
        "## EXP-0116 repeat-ten W4F16 module wall-time", "",
        "| Module | Ordered-Down control | Relaxed-Down candidate | Speed |",
        "|---|---:|---:|---:|",
    ])
    for name in control:
...
def render_report(summary: dict) -> str:
    lines = ["# EXP-0116 — Complete profiling report", ""]
    exp107.add_pc028(lines, summary)
    lines.append(
        "PC-028 provenance: F16F16 and W4U8 use frozen EXP-0109 formal "
        "evidence; W4F16 is this experiment's join-only control cell."
    )
    lines.append("")
    ...
    lines.extend([
        "## Gates", "", "| Gate | Result |", "|---|---:|",
        f"| Down and Host speed | {'PASS' if summary['speed_gate'] else 'FAIL'} |",
        ...
        f"| EXP-0116 local gate | {'PASS' if summary['local_gate_pass'] else 'FAIL'} |",
