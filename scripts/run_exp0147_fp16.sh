...
set +e
"${adb_exe}" shell \
    "cd ${remote_root} && ${remote_dump_env} QBH_SCAN_MODE=${scan_mode} QBH_LOGICAL_M=${logical_m} QBH_KV_CACHE_LENGTH=${past} QBH_KV_CACHE_CAPACITY=${capacity} LD_LIBRARY_PATH=${remote_root} DSP_LIBRARY_PATH=${remote_root} ADSP_LIBRARY_PATH=${remote_root} ./qwen3_block_cli ${remote_root}/block_package_layer14_m64 ${variant} ${repeat_count} ${runtime_args}"
run_status=$?
set -e
