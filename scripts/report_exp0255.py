...
def audit_zero_points():
 params=[read(f'parameters/{n}.json')['parameters'] for n in ['R_MSE','R_FAN']];vectors=np.load(R/'calibration_vectors.npz');fit=read('calibration.json')['sites'];different=inside_diff=count=zeros=0
 for n in params[0]:
  if not n.endswith(('residual_mid','residual_out')):continue
  pa,pb=[p[n]['mse'] for p in params]
  assert pa==fit[n]['candidates'][fit[n]['mse_index']] and pb==fit[n]['candidates'][fit[n]['fanout_index']]
  assert pa['scale']==pb['scale'] and pa['inv_scale']==pb['inv_scale'];zeros+=pa['zero']!=pb['zero'];x=vectors[n].astype(np.float32)
  def q(p):return ((np.clip(np.floor(x*np.float32(p['inv_scale'])+np.float32(p['zero'])+np.float32(.5)),0,255)-np.float32(p['zero']))*np.float32(p['scale'])).astype(np.float16)
  diff=q(pa)!=q(pb);inside=(x>=max(pa['lo'],pb['lo']))&(x<=min(pa['hi'],pb['hi']));different+=int(diff.sum());inside_diff+=int((diff&inside).sum());count+=x.size
 z=read('calibration_zero_point_attribution.json');assert (zeros,count,different,inside_diff)==(z['sites_different_zero'],z['input_values'],z['different_decoded_values'],z['differences_inside_common_range'])
 for n,p in read('parameters/S_MSE.json')['parameters'].items():
  if n.endswith('swiglu'):assert p['mse']==fit[n]['candidates'][fit[n]['mse_index']]
 return z

def main():
 subprocess.run(['python3',str(M/'scripts/project_memory.py'),'preflight','--source-worktree',str(S)],check=True)
 zero=audit_zero_points();checked=audit();sel=read('selection.json');selected=sel['selected'];names=read('final_complete.json')['names'];scores={n:read(f'scores/final_{n}.json') for n in names}
 ...
 lines+=['','参数范围/零点仍为每层边界一个静态U8量化器；仅残差或SwiGLU指定边界改变，所有其它参数、权重、R3实现及设备二进制冻结。没有恢复FP16激活。校准使用128篇既有校准文档、10112个实际执行token；每边界固定1024个完整通道向量用于拟合，非全token拟合。范围极值统计覆盖全部执行token。','', '| 候选 | 参数改变边界数 | 开发集 float PPL | 开发集 integer PPL |','|---|---:|---:|---:|']
 for c in ['B','R_MSE','R_FAN','S_MSE']:
  ds=sel['development_scores'][c];lines.append(f"| {c} | {checked['parameter_change_counts'].get(c,0)} | {math.exp(ds['float_core']):.6f} | {math.exp(ds['wide_nr64']):.6f} |")
 ...
 lines+=['', '两种残差目标的scale/inverse在全部56个残差边界上完全相同，43个边界零点不同。在固定校准向量117440512个值中，10074个解码值不同，全部发生在共同可表示区间之外。该分析仅定位两量化器的裁剪差异，不证明PPL差异的完整因果机制。', '', '本轮规则选择B即保留原参数。未选中的R_MSE/R_FAN没有在当前确认集追加测试；其开发集的单路径收益不等同于独立确认收益。双路径候选筛选规则与原5%/10%模型验收门槛是两个不同层次。']
