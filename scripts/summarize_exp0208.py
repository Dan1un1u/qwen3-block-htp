#!/usr/bin/env python3
"""Validate and summarize EXP-0208 direct-W4 QKV single-DMA evidence."""
...
QKV_SINGLE_DMA = {CONTROL: 0, CANDIDATE: 1}
QKV_GROUPS = 224
...
def validate_run(run: dict[str, object], cell: str, steps: int) -> None:
    ...
    for index, profile in enumerate(profiles):
        ...
        base.require_equal(
            int(profile["w4u8_decode_direct_n_qkv_single_dma"]),
            QKV_SINGLE_DMA[cell], context + " QKV single-DMA selector")
        if index > 0:
            base.require_equal(
...
def main() -> None:
    ...
    gates = {
        ...
        "dma_descriptors_reduced_by_one_per_qkv_group":
            physical["control_dma_descriptors"] -
            physical["candidate_dma_descriptors"] == QKV_GROUPS,
    ...
    report = [
        "# EXP-0208 direct-W4 QKV single-DMA report",
        "",
        ...
        "QKV batch16 split three-descriptor transfer versus one complete "
        "weight descriptor plus one bias descriptor.",
        "",
        "## End-to-end gate",
        "",
        "| Metric | QKV split3 control | QKV single2 candidate | Change |",
        "|---|---:|---:|---:|",
        ...
        *pair_rows,
        ...
        "",
        "| Module | QKV split3 control | QKV single2 candidate | Candidate speed change |",
        "|---|---:|---:|---:|",
        *module_rows,
