...
"${project_root}/scripts/check_exp0178_static.sh" \
    > "${result_dir}/static_gate.json"
"${project_root}/scripts/deploy_exp0178.sh" \
...
QBH_GENERATION_AUDIT_DIR="${result_dir}/audit" \
    "${project_root}/scripts/run_exp0178.sh" row4 \
...
"${python_exe}" "${project_root}/scripts/verify_exp0167_generation.py" \
    ...
    --steps 193 --experiment-record 178 --experiment-label EXP-0178 \
    > "${result_dir}/audit/verify.log" 2>&1

QBH_EXP0173_GENERATION_STEPS=193 \
QBH_GENERATION_BOUNDARY_AUDIT=0 \
    "${project_root}/scripts/run_exp0178.sh" row4_poison \
...
for round in $(seq 1 10); do
    ...
    for cell in "${order[@]}"; do
        QBH_EXP0173_GENERATION_STEPS=193 \
        QBH_GENERATION_BOUNDARY_AUDIT=0 \
            "${project_root}/scripts/run_exp0178.sh" "${cell}" \
...
"${python_exe}" "${project_root}/scripts/summarize_exp0178.py" \
