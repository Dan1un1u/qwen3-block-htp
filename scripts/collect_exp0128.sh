...
u8_package="${QBH_EXP0128_U8_PACKAGE:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0042/block_package_layer14_m64_integer_attention_parallel}"
result_root="${QBH_EXP0128_RESULT_ROOT:-/mnt/d/llm_exp/results/qwen3-block-htp/exp0128}"
artifact_root="${QBH_EXP0128_ARTIFACT_ROOT:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0128/artifacts}"
...
if ! git -C "${project_root}" diff --quiet || \
   ...
   [[ -n "$(git -C "${project_root}" ls-files --others --exclude-standard)" ]]; then
    printf 'source worktree must be clean before EXP-0128 formal collection\n' >&2
    exit 1
fi
...
"${project_root}/scripts/build_exp0128.sh" > "${result_dir}/build.log" 2>&1
"${project_root}/scripts/check_exp0128_static.sh" > "${result_dir}/static_gate.json"
QBH_EXP0128_U8_PACKAGE="${u8_package}" \
    "${project_root}/scripts/deploy_exp0128_block.sh" \
...
for cell in "${cells[@]}"; do
    ...
    "${project_root}/scripts/run_exp0128.sh" "${cell}" 1 on on > "${raw}"
...
for round in 1 2 3 4 5; do
    printf 'EXP-0128 round %d/5\n' "${round}"
    if ((round % 2 == 1)); then
        repeats=(1 10)
        order=(stream32 stream16)
    else
        repeats=(10 1)
        order=(stream16 stream32)
    fi
    for repeat in "${repeats[@]}"; do
        for cell in "${order[@]}"; do
            raw="${result_dir}/round_${round}_${cell}_r${repeat}_raw.jsonl"
            "${project_root}/scripts/run_exp0128.sh" \
...
python3 "${project_root}/scripts/analyze_exp0128.py" "${result_dir}" \
    > "${result_dir}/gate_summary.json"
python3 "${project_root}/scripts/analyze_exp0128.py" "${result_dir}" --report \
...
{
    printf 'experiment=EXP-0128\nsource_head=%s\n' "${source_head}"
    printf 'parent=EXP-0120-ring16\ncells=%s\n' "${cells[*]}"
    printf 'recipe=W4U8\nvariable=gate_up_stream_region_32_vs_16_k_tiles\n'
