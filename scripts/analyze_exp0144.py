#!/usr/bin/env python3
"""Validate and report EXP-0144 Q/K Norm-RoPE SIMD output stores."""
...
import analyze_exp0143 as parent
...
EXPECTED_MODE = {"control": 2, "candidate": 3}
...
def validate_record(record: dict, repeat: int, cell: str,
    ...
    compatible["experiment"] = "EXP-0143"
    compatible["w4u8_qk_pair_kernel_mode"] = 2
    compatible["w4u8_qk_pair_kernel_mode_observed"] = 2
    parent.validate_record(compatible, repeat, "candidate", audit=audit)
    base.require(record, "experiment", "EXP-0144")
...
def build_summary(result_dir: Path, exp0109_dir: Path,
    ...
    if ((result_dir / "boot_id_before.txt").read_bytes() !=
        ...
        raise SystemExit("device boot ID changed during EXP-0144")
    ...
    for repeat in REPEATS:
        ...
        values = parent.metrics(sides["control"], sides["candidate"])
        comparisons[f"repeat{repeat}"] = values
        physical_values.append(parent.physical_equal(
...
def add_candidate_modules(lines: list[str], summary: dict) -> None:
    ...
    lines.extend([
        "## EXP-0144 W4U8 module wall-time (repeat10)", "",
        "| Module | memcpy output | SIMD output | Speed |",
        "|---|---:|---:|---:|",
    ])
    for name in control:
...
def render_report(summary: dict) -> str:
    lines = ["# EXP-0144 — Complete profiling report", ""]
    ...
    lines.extend([
        ...
        f"| EXP-0144 local gate | {'PASS' if summary['local_gate_pass'] else 'FAIL'} |",
