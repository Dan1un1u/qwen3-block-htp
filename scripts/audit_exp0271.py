...
def chain():
 preflight();p=O/'sp2-fp32';d=R/'slice-a01';a=read(d/'validated.json');r=read(R/'slice-repeat10/validated.json');assert a['numerical_pass'] and r['output_hashes']==a['output_hashes']*10
 for step in range(2):
  ...
  mid=np.load(p/f'chain_step{step}_last_middle.npy');actual=unpack_u8_hmx_activation(b[BASE+8*CAP:BASE+9*CAP],6144);assert np.array_equal(actual[:rows],((mid+32768)&255).astype('u1'))
  if step:assert np.array_equal(actual[4:5],((mid+32768)>>8).astype('u1'))
 write(R/'slice_gate.json',dict(pass_all=True,layers=3,fp32_output_exact=True,last_layer_QK_AV_post_gate_up_SP2_exact=True,repeat10_exact=True,independent_fp32_values=65*2048,physical_pass=True))
 print('CHAIN_PASS',flush=True)

def head(tag):
 preflight();d=R/tag;p=O/'sp2-fp32';v=read(d/'validated.json');assert v['audit'] and v['fp32']==1
