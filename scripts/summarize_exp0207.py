#!/usr/bin/env python3
"""Validate and summarize EXP-0207 direct-W4 Down single-DMA evidence."""
...
SINGLE_DMA = {CONTROL: 0, CANDIDATE: 1}
...
def validate_run(run: dict[str, object], cell: str, steps: int) -> None:
    ...
    for index, profile in enumerate(profiles):
        ...
        base.require_equal(
            ...
            SINGLE_DMA[cell], context + " Down single-DMA selector")
        if index > 0:
            base.require_equal(
...
def main() -> None:
    ...
    gates = {
        ...
        "dma_descriptors_reduced_by_one_per_down_batch":
            physical["control_dma_descriptors"] -
            physical["candidate_dma_descriptors"] == LAYERS * 16,
    ...
    report = [
        "# EXP-0207 direct-W4 Down single-DMA report",
        "",
        f"Source commit: `{args.source_commit}`",
        "",
        "Both cells inherit the evidence-valid EXP-0205 candidate and use "
        "direct-W4 decode Down batch4. The only runtime difference is the "
        "existing split three-descriptor weight transfer versus one complete "
        "weight descriptor plus one bias descriptor.",
        "",
        "## End-to-end gate",
        "",
        "| Metric | Split3 control | Single2 candidate | Change |",
        "|---|---:|---:|---:|",
        ...
        *pair_rows,
        ...
        "",
        "| Module | Split3 control | Single2 candidate | Candidate speed change |",
        "|---|---:|---:|---:|",
        *module_rows,
