#!/usr/bin/env python3
"""Validate and summarize EXP-0204 direct-W4 O batch8 evidence."""
...
EXPERIMENT = 204
LAYERS = 28
O_BATCHES = {CONTROL: 4, CANDIDATE: 8}
O_COMMANDS = {CONTROL: 448, CANDIDATE: 224}
...
def validate_run(run: dict[str, object], cell: str, steps: int) -> None:
    ...
    for index, profile in enumerate(profiles):
        ...
        base.require_equal(
            ...
            batch, context + " requested O batch")
        expected_batch = 4 if index == 0 else batch
...
def main() -> None:
    ...
    report = [
        "# EXP-0204 direct-W4 O batch8 report",
        "",
        f"Source commit: `{args.source_commit}`",
        "",
        "Both cells inherit the evidence-valid EXP-0201 candidate. The only "
        "runtime difference is direct-W4 decode O projection batch4 versus "
        "batch8 command granularity.",
        "",
        "## End-to-end gate",
        "",
        "| Metric | O batch4 control | O batch8 candidate | Change |",
        "|---|---:|---:|---:|",
        ...
        *pair_rows,
        ...
        "",
        "| Module | O batch4 control | O batch8 candidate | Candidate speed change |",
        "|---|---:|---:|---:|",
        *module_rows,
