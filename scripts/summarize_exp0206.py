#!/usr/bin/env python3
"""Validate and summarize EXP-0206 direct-W4 QKV batch32 evidence."""
...
def validate_run(run: dict[str, object], cell: str, steps: int) -> None:
    batch = 32 if cell == CANDIDATE else 16
    base.validate_run(
    ...
    for index, profile in enumerate(profiles):
        ...
        base.require_equal(
            ...
            1, context + " O-to-Gate prefetch parent")
        base.require_equal(
            int(profile["w4u8_decode_direct_n_gate_up_swiglu_stream"]),
            1, context + " asynchronous SwiGLU parent")
        base.require_equal(
            int(profile["w4u8_decode_direct_n_down_batch_n_tiles"]),
            4, context + " Down batch4 parent")
...
def main() -> None:
    ...
    expected_control_batches = LAYERS * (
        Q_N_TILES // 16 + 2 * (KV_N_TILES // 16))
    expected_candidate_batches = LAYERS * (
        Q_N_TILES // 32 + 2 * (KV_N_TILES // 32))
    physical = {
        "control_qkv_batches": base.median_counter(
            runs[CONTROL], "w4u8_qkv_ring_batch_count"),
        "candidate_qkv_batches": base.median_counter(
            runs[CANDIDATE], "w4u8_qkv_ring_batch_count"),
        "expected_control_qkv_batches": expected_control_batches,
        "expected_candidate_qkv_batches": expected_candidate_batches,
    ...
    batch_delta = expected_control_batches - expected_candidate_batches
    gates = {
        ...
        "qkv_wall_faster": target[CANDIDATE] < target[CONTROL],
        "qkv_batches_224_to_112":
            physical["control_qkv_batches"] == expected_control_batches and
            physical["candidate_qkv_batches"] == expected_candidate_batches,
        "direct_hmx_command_delta_exact":
            physical["control_direct_n_hmx_commands"] -
            physical["candidate_direct_n_hmx_commands"] == batch_delta,
        "dma_descriptor_delta_exact":
            physical["control_dma_descriptors"] -
            physical["candidate_dma_descriptors"] == 2 * batch_delta,
    ...
    report = [
        "# EXP-0206 direct-W4 QKV batch32 report",
        "",
        f"Source commit: `{args.source_commit}`",
        "",
        "Both cells inherit the evidence-valid EXP-0205 candidate. The only "
        "runtime difference is logical-M1 direct-W4 QKV batch16 versus "
        "batch32 using two capacity-checked HMX-accessible ping-pong slots.",
        "",
        "## End-to-end gate",
        "",
        "| Metric | QKV batch16 control | QKV batch32 candidate | Change |",
        "|---|---:|---:|---:|",
        ...
        *pair_rows,
        ...
        "",
        "| Module | QKV batch16 control | QKV batch32 candidate | Candidate speed change |",
        "|---|---:|---:|---:|",
        *module_rows,
        ...
        "",
        f"QKV plus Q/K preparation wall: {target[CONTROL]:.1f} -> "
        f"{target[CANDIDATE]:.1f} us/token.",
        "",
