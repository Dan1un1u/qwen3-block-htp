#!/usr/bin/env python3
"""Validate and report EXP-0128 W4U8 Gate/Up stream granularity."""
...
REPEATS = (1, 10)
SAMPLES = 5
CELLS = ("stream32", "stream16")
EXPECTED_REGIONS = {"stream32": 32, "stream16": 16}
EXPECTED_STREAMS = {"stream32": 768, "stream16": 1536}
...
EXP0127_FORMAL = Path(
    "/mnt/d/llm_exp/results/qwen3-block-htp/exp0127/"
    "20260901T000359Z_b3afdc39a5b5_formal"
)
TARGETS = (
...
PIPELINE = tuple(dict.fromkeys((
    *exp112.W4U8_PIPELINE,
    ...
    "w4u8_mlp_gate_up_hmx_stream_count",
)))
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--exp0127-formal", type=Path,
                        default=EXP0127_FORMAL)
...
def validate_record(record: dict, repeat: int, cell: str,
    ...
    base.require(record, "experiment", "EXP-0128")
    base.require(record, "w4u8_stream_fence_mode", "single_fence")
    base.require(
        record, "w4u8_mlp_gate_up_expanded_slot_count",
        16,
    )
    base.require(record, "w4u8_gate_up_stream_region_tiles",
                 EXPECTED_REGIONS[cell])
    base.require(record, "w4u8_mlp_gate_up_hmx_stream_count",
                 EXPECTED_STREAMS[cell] * repeat)
...
def build_summary(result_dir: Path, exp0109_dir: Path,
    ...
    if len({value["output_hash"] for value in correctness.values()}) != 1:
        raise SystemExit("stream-granularity cells produced different output hashes")
    ...
    physical_gate = all(
        physical_equal(records[repeat]["stream32"],
                       records[repeat]["stream16"])
        for repeat in REPEATS
    )
    plan_gate = all(
    ...
    pc028 = baseline_pc028(exp0109_dir, exp0127_dir)
    pc028["w4u8"] = exp109.module_medians(records[10]["stream16"])
    return {
        "experiment": "EXP-0128",
        "source_commit":
        ...
        "selected_cell": "stream16" if speed_gate else "stream32",
        "comparisons": comparisons,
        "pc028": pc028,
        "pc028_provenance": {
            "f16f16": str(exp0109_dir),
            "w4f16": str(exp0127_dir) + "/paired_control_r10.jsonl",
            "w4u8": str(result_dir) + "/paired_stream16_r10.jsonl",
        },
    }
...
def add_pc028(lines: list[str], summary: dict) -> None:
    ...
    lines.extend([
        ...
        "| Module | W16A16 | W4A16 specialized | W4A8 stream16 | A8 vs A16 speed |",
        "|---|---:|---:|---:|---:|",
    ])
...
def render_report(summary: dict) -> str:
    lines = ["# EXP-0128 — Complete profiling report", ""]
    add_pc028(lines, summary)
    for repeat in REPEATS:
        values = summary["comparisons"][f"repeat{repeat}"]
        lines.extend([f"## Repeat {repeat}: stream32 versus stream16", ""])
    ...
    lines.extend([
        ...
        f"| Physical equality except declared stream granularity | {'PASS' if summary['physical_equality_gate'] else 'FAIL'} |",
        ...
        f"| EXP-0128 local gate | {'PASS' if summary['local_gate_pass'] else 'FAIL'} |",
...
def main() -> None:
    ...
    summary = build_summary(
        args.result_dir, args.exp0109_formal, args.exp0127_formal
    )
