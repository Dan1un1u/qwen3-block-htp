...
result_root="${QBH_RESULT_ROOT:-/mnt/d/llm_exp/results/qwen3-block-htp/exp0029}"
artifact_root="${QBH_ARTIFACT_ROOT:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0029/artifacts}"
...
"${project_root}/scripts/build_exp0029.sh" > "${result_dir}/build.log" 2>&1
QBH_STATIC_OUTPUT_DIR="${result_dir}/static" \
    "${project_root}/scripts/check_exp0029_static.sh" \
...
configs=(
    f16_control f16_qk f16_av f16_combined
    w4_control w4_qk w4_av w4_combined
)
...
set_config() {
    ...
    case "${config}" in
        f16_control)
            ...
            f16_mode=batch2; w4_mode=control; pack_mode=control ;;
        f16_qk)
            variant=F16F16; workers=2; region=32
            f16_mode=batch2; w4_mode=control; pack_mode=qk_hvx ;;
        f16_av)
            variant=F16F16; workers=2; region=32
            f16_mode=batch2; w4_mode=control; pack_mode=av_hvx ;;
        f16_combined)
            variant=F16F16; workers=2; region=32
            f16_mode=batch2; w4_mode=control; pack_mode=hvx ;;
        w4_control)
...
run_config() {
    ...
    local audit="$3"
    set_config "${config}"
    "${project_root}/scripts/run_exp0029_block.sh" \
        "${variant}" "${repeat}" on "${audit}" fused \
        "${f16_mode}" hvx "${workers}" "${region}" \
        "${w4_mode}" "${pack_mode}"
}
...
for round in 1 2 3 4 5; do
    if (( round % 2 == 1 )); then
        ...
        order=(
            w4_combined w4_av w4_qk w4_control
            f16_combined f16_av f16_qk f16_control
        )
    fi
...
python3 "${project_root}/scripts/validate_exp0029_evidence.py" \
...

{
    ...
    printf 'attention_pack_modes=control,qk_hvx,av_hvx,combined_hvx\n'
