...
def single():
 preflight();proof=[]
 for l in [0,14,27]:
  p=f'layer{l}-fp32-a01';tag=f'final-audit-layer{l}';f=R/tag/'validated.json'
  if not f.exists():run(p,tag,dump=True)
  af=R/tag/'independent_gate.json';proof.append(read(af) if af.exists() else audit(p,tag))
  rz=run(p,f'final-repeat-layer{l}',repeat=10);assert rz['output_hashes']==read(f)['output_hashes']*10
 write(R/'single_gate.json',dict(pass_all=True,layers=proof,repeat10_exact=True))

def profile(phase):
 preflight();assert read(R/'single_gate.json')['pass_all']
 if phase=='formal':assert read(R/'layer-short.json')['speed_pass']
 n=5 if phase=='short' else 10;rows=[]
 for i in range(n):
  for rep in [1,10]:
   for fp in ([0,1] if i%2==0 else [1,0]):
    tag=f'layer-{phase}/round{i:02d}-r{rep}-fp{fp}';f=R/tag/'validated.json';z=read(f) if f.exists() else run('layer0-fp32-a01',tag,repeat=rep,fp32=fp)
    gold=read(R/('final-audit-layer0' if fp else 'diag-control-a01')/'validated.json')['output_hashes'][:2];assert z['output_hashes']==gold*rep;rows.append(dict(round=i,repeat=rep,fp32=fp,**z))
...
if __name__=='__main__':
 ...
 elif sys.argv[1] in ['short','formal']:profile(sys.argv[1])
