...
package="${QBH_EXP0074_PACKAGE:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0042/block_package_layer14_m64_integer_attention_parallel}"
result_root="${QBH_EXP0074_RESULT_ROOT:-/mnt/d/llm_exp/results/qwen3-block-htp/exp0074}"
artifact_root="${QBH_EXP0074_ARTIFACT_ROOT:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0074/artifacts}"
...
"${project_root}/scripts/build_exp0074.sh" > "${result_dir}/build.log" 2>&1
"${project_root}/scripts/check_exp0074_static.sh" > "${result_dir}/static_gate.json"
QBH_EXP0074_PACKAGE="${package}" "${project_root}/scripts/deploy_exp0074_block.sh" \
...
for mode in control candidate; do
    "${project_root}/scripts/run_exp0074.sh" "${mode}" 1 on on \
...
for round in 1 2 3 4 5 6 7; do
    printf 'EXP-0074 paired round %d/%d\n' "${round}" "${paired_rounds}"
    if (( round % 2 == 1 )); then
        repeats=(1 10)
    ...
    for repeat in "${repeats[@]}"; do
        for mode in "${modes[@]}"; do
            "${project_root}/scripts/run_exp0074.sh" "${mode}" "${repeat}" on off \
                >> "${result_dir}/paired_${mode}_r${repeat}.jsonl"
        done
    done
done
...
python3 "${project_root}/scripts/validate_exp0074.py" "${result_dir}" "${package}" \
    > "${result_dir}/gate_summary.json"
python3 "${project_root}/scripts/validate_exp0074.py" "${result_dir}" "${package}" --report \
