...
recipe="${1:-f16f16}"
remote_root="${EXP0217_REMOTE_ROOT:-/data/local/tmp/qwen3-block-htp/exp0217-${recipe}}"
case "${recipe}" in
    f16f16)
        variant=F16F16
        recipe_env=""
...
if [[ "$recipe" == f16f16 ]]; then mode=10; else mode=7; fi
"${adb_exe}" get-state >/dev/null
"${adb_exe}" shell "cd ${remote_root} && ${recipe_env} QBH_GENERATION_SEQUENCE=${mode} QBH_KV_CACHE_LAYOUT=hmx_native_f16 QBH_VERTICAL_SLICE=1 QBH_REPLAY_SEQUENCE=1 QBH_SCAN_MODE=prefill QBH_LOGICAL_M=64 QBH_KV_CACHE_LENGTH=0 QBH_KV_CACHE_CAPACITY=80 LD_LIBRARY_PATH=${remote_root} DSP_LIBRARY_PATH=${remote_root} ADSP_LIBRARY_PATH=${remote_root} ./qwen3_block_cli ${remote_root}/block_package_layer14_m64 ${variant} 1 ${runtime_args}"
