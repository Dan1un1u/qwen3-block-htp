#!/usr/bin/env python3
"""Seal EXP0261 after source/report closure and immutable remote verification."""
...
def main():
 ...
 z=read(R/'summary.json');assert z['stop_before_fullmodel'] and not z['speed_eligible'] and z['timed_RPCs']==2970
 ...
 for file,report in [('EXP-0261-RESULTS.md','REPORT.md'),('EXP-0261-PROFILE.md','FULL_PROFILING_REPORT.md')]:assert (S/'docs/experiments'/file).read_bytes()==(R/report).read_bytes()
 native={str(p.relative_to(S)):sha(p) for d in ['src','include'] for p in sorted((S/d).rglob('*')) if p.is_file()}
 provenance=dict(experiment='EXP-0261',source_branch=subprocess.check_output(['git','branch','--show-current'],cwd=S,text=True).strip(),source_head=head,runtime=runtime,native_file_hashes=native,packages=manifests,boot=boot,ABI=123,parent_exp0260_head='0210d840497a134f7256defd7c7c8244e271482a',parent_exp0259_evidence='2f891f9aa35e7fa5bece5180e8e3a81fe4fd1253dc3749565dd226b462541a07',source_and_remote_verified=True,device_PPL=None,E2E=None,baseline_promoted=False,closed_utc=datetime.now(timezone.utc).isoformat())
 ...
 write(R/'EVIDENCE_SHA256.json',dict(experiment='EXP-0261',files=files))
