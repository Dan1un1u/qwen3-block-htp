"""EXP-0296 isolated long-prefill fixtures, build deployment and immutable runs."""
...
R=Path('/mnt/d/llm_exp/results/qwen3-block-htp/exp0296')/SIZE
M=Path('/mnt/d/llm_exp/models/qwen3-block-htp/exp0296')/SIZE
BASE=R/'base.json'
REMOTE='/data/local/tmp/qwen3-block-htp/exp0296/'+SIZE
...
def preflight():
 ...
 assert 'EXPERIMENT=EXP-0296' in z
def prepare():
 preflight()
 import torch
 ...
 for length in [64,65,128,129,536,741]:
  ...
  put(dst/'manifest.json',dict(experiment='EXP-0296',parent=base,parent_sha256=base['manifest_sha256'],files=files,changed=changed))
