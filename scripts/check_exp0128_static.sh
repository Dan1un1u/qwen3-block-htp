...
grep -q 'QBH_BLOCK_EXPERIMENT UINT32_C(128)' \
    "${project_root}/include/block_protocol.h"
grep -q 'QBH_W4_MAX_COMPRESSED_SLOT_COUNT UINT32_C(16)' \
    "${project_root}/include/probe_protocol.h"
grep -q 'QBH_W4_MAX_EXPANDED_CHUNK_SLOT_COUNT UINT32_C(16)' \
    "${project_root}/include/probe_protocol.h"
grep -q 'QBH_W4U8_GATE_UP_RING_SLOTS' \
    "${project_root}/src/host/block_main.c"
grep -q 'QBH_W4U8_GATE_UP_STREAM_REGION_TILES' \
    "${project_root}/src/host/block_main.c"
grep -q 'layout->expanded_slot_count = ring_slots' \
    "${project_root}/src/dsp/block_imp.c"
grep -q 'QBH_W4_STREAM_FENCE_SINGLE' \
...
if grep -RInE 'QnnGraph|QnnContext|QnnBackend|libQnn' \
    ...
    printf 'QNN dependency found in EXP-0128 source\n' >&2
    exit 1
fi
printf '%s\n' '{"experiment":"EXP-0128","static_gate":"pass","recipe":"W4U8","ring_slots":16,"stream32_runtime_selectable":true,"stream16_runtime_selectable":true,"hmx_batch_outputs":8,"persistent_hvx_workers":3,"stream_fence":"single_fence","one_hmx_owner":true,"qnn_dependency":false}'
