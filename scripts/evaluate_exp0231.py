...
import numpy as np
import torch
from data_exp0231 import RESULT,OUTPUT,MODEL,SOURCE,CELLS,write,sha,preflight,verified
from export_exp0231 import frozen as new_frozen
from group_exp0231 import read_weight,FORMAT
...
def load(v,device):
    if v!='G128':
        ...
        return model,h
    root=OUTPUT/'G128';expected=json.loads((RESULT/'G128/package.json').read_text())['manifest_sha256']
    ...
    for i,layer in enumerate(model.model.layers):
        for name,long in rot.PROJECTIONS.items():
            p=layer.get_submodule(long).weight;n,k=p.shape
            p.copy_(torch.from_numpy(read_weight(root/f'layer{i}',name,n,k)))
...
def evaluate(phase,v,cpu=False,output_prefix=''):
    ...
    torch.set_num_threads(8);torch.manual_seed(231);torch.set_grad_enabled(False)
    ...
    regression=None
    if phase=='development' and v!='G128' and not cpu:
...
if __name__=='__main__':
    ...
    p.add_argument('variant',choices=['F','C8','C64','G128']);p.add_argument('--cpu-check',action='store_true');a=p.parse_args()
