...
CONFIGS = {
    "f16_b2": {
        "variant": "F16F16", "f16_mode": "double_buffer_batch2",
        "w4_mode": "not_applicable", "peak": 7_072_512,
        "commands": 352, "descriptors": 320, "prefetch": 313,
        "streamed": 0, "gate_commands": 0, "cache": 0,
    },
    "f16_b4": {
        "variant": "F16F16", "f16_mode": "gate_up_batch4",
        "w4_mode": "not_applicable", "peak": 7_072_512,
        "commands": 256, "descriptors": 224, "prefetch": 217,
        "streamed": 0, "gate_commands": 0, "cache": 0,
    },
    "w4_control": {
        "variant": "W4F16", "f16_mode": "not_applicable",
        "w4_mode": "adaptive_down96_cross_prefetch", "peak": 7_745_024,
        "commands": 352, "descriptors": 160, "prefetch": 153,
        "streamed": 320, "gate_commands": 192, "cache": 0,
    },
    "w4_b4_dma4": {
        "variant": "W4F16", "f16_mode": "not_applicable",
        "w4_mode": "adaptive_down96_gate4_cross_prefetch",
        "peak": 7_745_024, "commands": 256, "descriptors": 160,
        "prefetch": 153, "streamed": 224, "gate_commands": 96,
        "cache": 0,
    },
    "w4_b4_dma8_cache": {
        "variant": "W4F16", "f16_mode": "not_applicable",
        "w4_mode": "adaptive_down96_gate4_dma8_cross_prefetch",
        "peak": 7_843_328, "commands": 256, "descriptors": 112,
        "prefetch": 105, "streamed": 224, "gate_commands": 96,
        "cache": 98_304,
    },
}
...
def expected_weight_bytes(config, repeat):
    ...
    setup = 131_072 if config["cache"] else 81_920
    return 25_165_824 * repeat + setup
...
def validate_record(record, name, repeat, audit_mode):
    ...
    fixed = {
        ...
        "w4f16_gate_up_scale_cache_bytes": config["cache"],
        "release_result": 0,
        "close_result": 0,
    }
    ...
    elif not is_f16 and record["w4f16_gate_up_scale_init_ticks"] <= 0:
        raise SystemExit("control Gate/Up scale initialization was not observed")
...
def medians(records):
    ...
    fields = (
        ...
        "down_ticks", "w4f16_gate_up_prefetch_wait_ticks",
...
def main():
    ...
    comparisons = {}
    selected = "w4_b4_dma8_cache"
    for family in ("repeat1", "repeat10"):
        candidate = timing[family][selected]
        w4_control = timing[family]["w4_control"]
        f16_control = timing[family]["f16_b4"]
        if candidate["gate_up_ticks_median_per_block"] >= \
                w4_control["gate_up_ticks_median_per_block"]:
            raise SystemExit("selected W4 Gate/Up did not improve its W4 control")
        if candidate["gate_up_ticks_median_per_block"] >= \
                f16_control["gate_up_ticks_median_per_block"]:
            raise SystemExit("selected W4 Gate/Up is not faster than fastest F16")
        if candidate["host_wall_ns_per_block_median"] >= \
                w4_control["host_wall_ns_per_block_median"]:
            raise SystemExit("selected W4 block did not improve its W4 control")
        if candidate["host_wall_ns_per_block_median"] >= \
                f16_control["host_wall_ns_per_block_median"]:
            raise SystemExit("selected W4 block is not faster than fastest F16")
        comparisons[family] = {
            "gate_up_percent_vs_w4_control": pct(
                candidate["gate_up_ticks_median_per_block"],
                w4_control["gate_up_ticks_median_per_block"]),
            "gate_up_percent_vs_f16_b4": pct(
                candidate["gate_up_ticks_median_per_block"],
                f16_control["gate_up_ticks_median_per_block"]),
            "host_percent_vs_w4_control": pct(
                candidate["host_wall_ns_per_block_median"],
                w4_control["host_wall_ns_per_block_median"]),
            "host_percent_vs_f16_b4": pct(
                candidate["host_wall_ns_per_block_median"],
                f16_control["host_wall_ns_per_block_median"]),
        }
    ...
    print(json.dumps({
        ...
        "correctness": {
            "w4_candidates_byte_exact": True,
            "w4_output_hash": "f18b9abbe1487231",
            "f16_controls_byte_exact": True,
            "f16_output_hash": "704252c89780e695",
        },
        "physical_contract": {
            "fixed_vtcm_bytes": 8_388_608,
            "selected_peak_vtcm_bytes": 7_843_328,
            "gate_up_scale_cache_bytes": 98_304,
