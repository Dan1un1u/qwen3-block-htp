...
u8_package="${QBH_EXP0124_U8_PACKAGE:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0042/block_package_layer14_m64_integer_attention_parallel}"
result_root="${QBH_EXP0124_RESULT_ROOT:-/mnt/d/llm_exp/results/qwen3-block-htp/exp0124}"
artifact_root="${QBH_EXP0124_ARTIFACT_ROOT:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0124/artifacts}"
...
if ! git -C "${project_root}" diff --quiet || \
   ...
   [[ -n "$(git -C "${project_root}" ls-files --others --exclude-standard)" ]]; then
    printf 'source worktree must be clean before EXP-0124 formal collection\n' >&2
    exit 1
fi
...
"${project_root}/scripts/build_exp0124.sh" > "${result_dir}/build.log" 2>&1
"${project_root}/scripts/check_exp0124_static.sh" > "${result_dir}/static_gate.json"
QBH_EXP0124_U8_PACKAGE="${u8_package}" \
    "${project_root}/scripts/deploy_exp0124_block.sh" \
...
for round in 1 2 3 4 5; do
    ...
    for repeat in "${repeats[@]}"; do
        for cell in "${order[@]}"; do
            ...
            "${project_root}/scripts/run_exp0124.sh" \
...
python3 "${project_root}/scripts/analyze_exp0124.py" "${result_dir}" \
    > "${result_dir}/gate_summary.json"
python3 "${project_root}/scripts/analyze_exp0124.py" "${result_dir}" --report \
...
{
    printf 'experiment=EXP-0124\nsource_head=%s\n' "${source_head}"
    printf 'parent=EXP-0120\ncorrectness_cells=%s\n' "${correctness_cells[*]}"
    printf 'formal_cells=%s\n' "${formal_cells[*]}"
    printf 'recipe=W4U8\nvariable=qkv_dma_hvx_expand_hmx_qk_prep_ring\n'
