...
remote_root="${EXP0136_REMOTE_ROOT:-/data/local/tmp/qwen3-block-htp/exp0136-w4f16}"
cell="${1:-group8}"
repeat_count="${2:-1}"
attribution_mode="${3:-on}"
audit_mode="${4:-off}"
stream_group_tiles=8

case "${cell}" in
    group8)
        ;;
    group4)
        stream_group_tiles=4
        ;;
    *)
        printf 'usage: %s group8|group4 [repeat] [attribution] [audit]\n' "$0" >&2
        exit 2
        ;;
esac
...
"${adb_exe}" shell \
    "cd ${remote_root} && QBH_QKV_SCHEDULE=control QBH_W4F16_GROUP_FENCE=join_only QBH_W4F16_EXPAND_CLAIM_REGIONS=1 QBH_W4F16_GATE_UP_EXTRA_EXPAND_WORKER=1 QBH_W4F16_GATE_UP_EXTRA_STREAM_WORKER=1 QBH_W4F16_GATE_UP_STREAM_GROUP_TILES=${stream_group_tiles} LD_LIBRARY_PATH=${remote_root} DSP_LIBRARY_PATH=${remote_root} ADSP_LIBRARY_PATH=${remote_root} ./qwen3_block_cli ${remote_root}/block_package_layer14_m64 W4F16 ${repeat_count} ${runtime_args}"
