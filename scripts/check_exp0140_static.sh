...
grep -q 'QBH_BLOCK_EXPERIMENT UINT32_C(140)' \
    "${project_root}/include/block_protocol.h"
...
grep -q 'desc == &header->projections\[QBH_BLOCK_PROJ_DOWN\]' \
    "${project_root}/src/dsp/block_imp.c"
grep -q 'header->w4f16_group_fence_mode !=' \
    "${project_root}/src/dsp/block_imp.c"
grep -q 'QBH_BLOCK_W4F16_GROUP_FENCE_CONTROL' \
    "${project_root}/src/dsp/block_imp.c"
...
if grep -RInE 'QnnGraph|QnnContext|QnnBackend|libQnn' \
    ...
    printf 'QNN dependency found in EXP-0140 source\n' >&2
    exit 1
fi
printf '%s\n' '{"experiment":"EXP-0140","static_gate":"pass","recipe":"W4F16","control_join_only_selectable":true,"candidate_join_only_down_selectable":true,"gate_up_subgroup4_preserved":true,"down_only_relaxed_selector":true,"ordered_unpack_has_barrier":true,"relaxed_unpack_has_barrier":false,"pool_join_barrier_retained":true,"one_hmx_owner":true,"qnn_dependency":false}'
