...
from reference_qhl_exp0294 import exact_qk_norm_rope_u8 as sdk_qk
...
from profile_fp_islands_06b import fp_table
...
def layer(x,p,cos,sin,past=None,sp2=True,seed=None):
 ...
 qr=sdk_qk(qr,16,q['q_projection'],q['q_rope'],np.fromfile(p/'q_norm_weight_f16.bin','<f2'),cos,sin).reshape(-1,16,128)
 kr=sdk_qk(kr,8,q['k_projection'],q['k_rope'],np.fromfile(p/'k_norm_weight_f16.bin','<f2'),cos,sin).reshape(-1,8,128)
 ...
 av=av.reshape(-1,2048);o=project(av,p,'o',1024,q['attention_concat'],None,True);res=x+o
 post=norm(res,np.fromfile(p/'post_norm_weight_f16.bin','<f2'),q['post_attention_norm'])
 g=project(post,p,'gate',3072,q['post_attention_norm'],q['gate']);u=project(post,p,'up',3072,q['post_attention_norm'],q['up'])
 lut,_=fp_table(q);mid=lut[g,u]
 down=project(mid,p,'down',1024,q['middle'],None,True);y=res+down
