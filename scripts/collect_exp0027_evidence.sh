...
result_root="${QBH_RESULT_ROOT:-/mnt/d/llm_exp/results/qwen3-block-htp/exp0027}"
artifact_root="${QBH_ARTIFACT_ROOT:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0027/artifacts}"
...
"${project_root}/scripts/build_exp0027.sh" > "${result_dir}/build.log" 2>&1
QBH_STATIC_OUTPUT_DIR="${result_dir}/static" \
    "${project_root}/scripts/check_exp0027_static.sh" \
...
configs=(f16_serial f16_async f16_batch2 w4)
for config in "${configs[@]}"; do
    case "${config}" in
        f16_serial) variant=F16F16; mode=serial ;;
        f16_async) variant=F16F16; mode=async ;;
        f16_batch2) variant=F16F16; mode=batch2 ;;
        w4) variant=W4F16; mode=serial ;;
    esac
    "${project_root}/scripts/run_exp0027_block.sh" \
        "${variant}" 1 on on fused "${mode}" hvx \
...
for round in 1 2 3 4 5; do
    if (( round % 2 == 1 )); then
        order=(f16_serial f16_async f16_batch2 w4)
    else
        order=(w4 f16_batch2 f16_async f16_serial)
    fi
    for config in "${order[@]}"; do
        ...
        "${project_root}/scripts/run_exp0027_block.sh" \
            "${variant}" 1 on off fused "${mode}" hvx \
            >> "${result_dir}/timing_${config}_repeat1.jsonl"
        "${project_root}/scripts/run_exp0027_block.sh" \
            "${variant}" 10 on off fused "${mode}" hvx \
            >> "${result_dir}/timing_${config}_repeat10.jsonl"
    done
done
...
python3 "${project_root}/scripts/validate_exp0027_evidence.py" \
...
{
    printf 'experiment=EXP-0027\n'
    printf 'source_branch=%s\n' "$(git -C "${project_root}" branch --show-current)"
    printf 'source_head=%s\n' "${source_head}"
    printf 'execution_unit=qwen3_layer14_complete_block_m64\n'
    printf 'configs=f16_serial,f16_async,f16_batch2,w4\n'
