...
u8_package="${QBH_EXP0119_U8_PACKAGE:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0042/block_package_layer14_m64_integer_attention_parallel}"
result_root="${QBH_EXP0119_RESULT_ROOT:-/mnt/d/llm_exp/results/qwen3-block-htp/exp0119}"
artifact_root="${QBH_EXP0119_ARTIFACT_ROOT:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0119/artifacts}"
...
if ! git -C "${project_root}" diff --quiet || \
   ...
   [[ -n "$(git -C "${project_root}" ls-files --others --exclude-standard)" ]]; then
    printf 'source worktree must be clean before EXP-0119 formal collection\n' >&2
    exit 1
fi
...
"${project_root}/scripts/build_exp0119.sh" > "${result_dir}/build.log" 2>&1
"${project_root}/scripts/check_exp0119_static.sh" > "${result_dir}/static_gate.json"
QBH_EXP0119_U8_PACKAGE="${u8_package}" \
    "${project_root}/scripts/deploy_exp0119_block.sh" \
...
for cell in "${cells[@]}"; do
    ...
    "${project_root}/scripts/run_exp0119.sh" "${cell}" 1 on on > "${raw}"
...
for round in 1 2 3 4 5; do
    printf 'EXP-0119 paired round %d/%d\n' "${round}" "${rounds}"
    if ((round % 2 == 1)); then
        repeats=(1 10)
        order=(control assist16 assist8 assist4)
    else
        repeats=(10 1)
        order=(assist4 assist8 assist16 control)
    fi
    for repeat in "${repeats[@]}"; do
        for cell in "${order[@]}"; do
            raw="${result_dir}/round_${round}_${cell}_r${repeat}_raw.jsonl"
            "${project_root}/scripts/run_exp0119.sh" \
...
python3 "${project_root}/scripts/analyze_exp0119.py" "${result_dir}" \
    > "${result_dir}/gate_summary.json"
python3 "${project_root}/scripts/analyze_exp0119.py" "${result_dir}" --report \
...
{
    printf 'experiment=EXP-0119\nsource_branch=%s\nsource_head=%s\n' \
        "${source_branch}" "${source_head}"
    printf 'parent=EXP-0112\nimplementation_parent=EXP-0118\nrecipe=W4U8\n'
    printf 'cells=%s\n' "${cells[*]}"
    printf 'variable=q_worker_assistance_region_k_tiles\n'
