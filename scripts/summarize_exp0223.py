#!/usr/bin/env python3
"""Frozen absmax GPTQ versus transformer-clipped GPTQ candidates, actual DSP evidence."""
import argparse
import json
import math
...
from experiment_exp0223 import RESULT,VARIANTS
from clipping_exp0223 import PREVIOUS
from measure_exp0223 import ALL
...
def quality():
    ...
    entries={'absmax_'+v:old['samples']['GPTQ_'+v] for v in VARIANTS};determinism={};software={}
    for v in VARIANTS:
        ...
        for s in full['samples']:
            ...
            rows.append(r)
        entries['clipped_'+v]=rows;mismatches=[]
    ...
    chosen=min(VARIANTS,key=lambda v:(summaries['clipped_'+v]['nll'],-summaries['clipped_'+v]['tasks_correct'],v))
    out=dict(experiment='EXP-0223',summary=summaries,samples=entries,software_diagnostic={k:score(rows,teacher) for k,rows in software.items()},
        determinism=determinism,effectiveness={v:improved(summaries['clipped_'+v],summaries['absmax_'+v]) for v in VARIANTS},
        C_incremental_rotation=improved(summaries['clipped_C'],summaries['clipped_A']),profile_candidate=chosen,
    ...
    labels=['absmax GPTQ A（冻结）','absmax GPTQ B（冻结）','absmax GPTQ C（冻结）','clipping GPTQ A','clipping GPTQ B','clipping GPTQ C']
...
def speed():
    ...
    write_json(RESULT/'speed_summary.json',dict(data=data,times=times,paired_speed_percent=paired,profile_candidate=chosen,formal_invocations=960,formal_layer_ledgers=26880))
    ...
    for v in VARIANTS:
        for mode in ['prefill','decode']:
            ...
            report.extend([f'## Absmax {v}0 versus clipped {v} {mode}',table(['Field','R1 original','R1 candidate','R1 change','R10 original median','R10 candidate median','R10 change'],rows)])
