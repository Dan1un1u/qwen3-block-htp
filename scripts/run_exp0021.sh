...
remote_root="${REMOTE_ROOT:-/data/local/tmp/qwen3-block-htp/exp0021}"
repeat_count="${1:-1}"
self_test="${2:-1}"
...
if [[ "${QBH_SKIP_DEPLOY:-0}" != "1" ]]; then
    ...
    "${adb_exe}" push "$(wslpath -w "${host_executable}")" \
        "${remote_root}/qwen3_mlp_cli" >/dev/null 2>&1
    ...
    "${adb_exe}" shell \
        "chmod 755 ${remote_root}/qwen3_mlp_cli ${remote_root}/libqwen3_probe.so ${remote_root}/libqwen3_probe_skel.so"
    "${adb_exe}" shell "rm -f ${remote_root}/qwen3_mlp_cli.farf"
fi

"${adb_exe}" shell \
    "cd ${remote_root} && LD_LIBRARY_PATH=${remote_root} DSP_LIBRARY_PATH=${remote_root} ADSP_LIBRARY_PATH=${remote_root} ./qwen3_mlp_cli ${repeat_count} ${self_test}"
