#!/usr/bin/env python3
"""EXP0306 Qwen0.6 vector floating intermediates, own-reference profiling."""
import json,os,shlex,struct,subprocess,hashlib,shutil,argparse
from pathlib import Path
import numpy as np
S=Path(__file__).resolve().parents[1]
R=Path('/mnt/d/llm_exp/results/qwen3-block-htp/exp0306')
P=Path('/mnt/d/llm_exp/models/qwen3-block-htp/exp0306/recovered-int16')
REMOTE='/data/local/tmp/qwen3-block-htp/exp0306'
...
def preflight():
 z=subprocess.check_output(['python3',str(S)+'-project-memory/scripts/project_memory.py','preflight','--source-worktree',str(S)],text=True);assert 'EXPERIMENT=EXP-0306' in z
 print(z,flush=True)
def fnv(x):
...
def fp_table(q):
 ...
 return result,truth
def stage(nl):
 preflight();seal=read(S/'build/qwen3-sp2-build-seal.json');assert seal['fp_islands'] and seal['model_size']=='0.6B'
 assert seal['source_head']==subprocess.check_output(['git','-C',str(S),'rev-parse','HEAD'],text=True).strip()
 d=R/f'binaries-{nl}-a01';d.mkdir(exist_ok=False);remote=REMOTE+'/'+d.name;adb('shell','mkdir -p '+remote)
...
def component():
 ...
 preflight();binary=REMOTE+'/binaries-28-a01';results=[]
 for i in range(28):
...
def reference():
 import reference_fp_islands_06b as ref
 ...
 prompt=np.fromfile(P/'generation_prompt_token_ids_u32.bin','<u4').tolist();fixed=read(R.parent/'exp0304/fixture.json')['fixed'];assert len(prompt)==64
 emb=np.memmap(P/'generation_embedding_weight_f16.bin','<f2','r',shape=(151936,1024));x=np.array(emb[prompt+fixed[:1]],dtype='f4')
 ...
 for i in range(28):
  ...
  if f.exists():x=np.load(f)
   ...
   x,kv,diag=ref.layer(x,P/f'layer{i}',co,si,seed=None);np.save(f,x)
   if i==0:
...
def execute(tag,repeat=1,audit=False,nl=28):
 ...
 base=read(R.parent/'exp0304/packages.json')['sp2'];prefix,args=base['command'].split(' ./qwen3_block_cli ',1)
 env=dict(t.split('=',1) for t in shlex.split(prefix.split(' && ')[1]));argv=shlex.split(args);argv[0]=REMOTE+'/recovered-int16';binary=REMOTE+f'/binaries-{nl}-a01'
 ...
 if not (d/'protocol.json').exists():save(d/'protocol.json',dict(command=cmd,layers=nl,repeat=repeat,audit=audit,seal_sha256=sha(R/f'binaries-{nl}-a01/seal.json'),package_manifest_sha256=sha(P/'manifest.json')))
