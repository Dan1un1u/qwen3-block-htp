...
"${project_root}/scripts/check_exp0174_static.sh" \
    > "${result_dir}/static_gate.json"
"${project_root}/scripts/deploy_exp0174.sh" \
    > "${result_dir}/deploy.log" 2>&1

for round in $(seq 1 5); do
    printf -v index '%02d' "${round}"
    case $((round % 3)) in
        1) order=(post_batch8 inline_batch8 inline_batch16) ;;
        2) order=(inline_batch16 post_batch8 inline_batch8) ;;
        0) order=(inline_batch8 inline_batch16 post_batch8) ;;
    esac
    for cell in "${order[@]}"; do
        QBH_EXP0173_GENERATION_STEPS=17 \
            "${project_root}/scripts/run_exp0174.sh" "${cell}" \
            > "${result_dir}/raw/pair_${index}_${cell}.log" 2>&1
    done
done

python3 "${project_root}/scripts/summarize_exp0174_short.py" \
