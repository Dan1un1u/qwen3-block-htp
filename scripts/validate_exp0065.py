#!/usr/bin/env python3
"""Validate EXP-0065 per-group SOLE probability-LUT templates."""
...
import validate_exp0064 as previous
...
def validate_record(record: dict[str, object], repeat: int,
    ...
    compatibility["experiment"] = "EXP-0064"
    ...
    base.require(record, "experiment", "EXP-0065")
...
def build_summary(result_dir: Path, package_dir: Path) -> dict[str, object]:
    ...
    summary.update({
        "experiment": "EXP-0065",
        "control": "EXP-0064 scalar SOLE LUT construction on every row",
        "candidate": "fourteen SOLE LUT templates precomputed per GQA group",
...
def render_report(summary: dict[str, object]) -> str:
    lines = [
        ...
        "The control rebuilds two sixteen-entry SOLE probability LUT banks "
        "on every paired-head row. The candidate builds the fourteen "
        "reachable `(leading, next)` banks once in each GQA group's reused "
        "VTCM scratch and selects them per row. QK requantization, log2 "
        "codes, sums, probability bytes, HMX commands and all other block "
        "work are unchanged.", "",
    ]
    for repeat in (1, 10):
        result = summary["repeat_results"][f"repeat{repeat}"]
    ...
    lines.extend([
        ...
        "## Decision", "",
        f"EXP-0065 local gate: **{'PASS' if summary['local_gate_pass'] else 'FAIL'}**. "
