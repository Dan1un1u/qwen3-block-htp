...
def prepare():
 ...
 rng=np.random.default_rng(243);chosen={c:[] for c in CELLS};used=set();grams=set();hashes=set();sources=[]
 ...
 for lang in ['en','zh']:
  ...
  select(cell,rows,p);assert len(chosen[cell])==128;print('DATA_CELL_READY',cell,128,flush=True)
 for lang in ['en','zh']:
  ...
  if len(chosen[cell])<256:
   ...
   for off in range(64000,74000,100):
    ...
    if len(chosen[cell])==128:break
  assert len(chosen[cell])==128,(cell,len(chosen[cell]));print('DATA_CELL_READY',cell,128,flush=True)
 samples=[]
 for i in range(128):
  for c in CELLS:
   r=chosen[c][i];r.update(id=len(samples),split='final',prompt_ids=r['token_ids'][:64],target_ids=r['token_ids'][64:]);samples.append(r)
 write('dataset.json',dict(version='qbh-EXP243-prefix-final-v1',seed=243,cells=CELLS,samples=samples,provenance=sources,tokenizer_sha256=sha(MODEL/'qwen3-tokenizer.json'),calibration_reference='EXP230_C64_65536',final_targets=8192,prior_paths=[str(p) for p in paths]))
 ...
 write('dataset_freeze.json',dict(protocol_sha256=sha(MEMORY/'docs/experiments/EXP-0243.md'),files={'dataset.json':sha(RESULT/'dataset.json')},references={str(p):sha(p) for p in refs},frozen_before_export_and_scoring=True))
...
def audit():
 ...
 assert len(used)==512
 write('independent_data_audit.json',dict(pass_all=True,documents=512,targets=8192,direct_tuple32_disjoint=True,all_windows_reconstructed=True,dataset_sha256=sha(RESULT/'dataset.json')));print('DATA_AUDIT_PASS',flush=True)
