...
from device_exp0278 import records
import numpy as np
...
def chain(tag, repeat_tag):
 preflight();p=package_path('full-fp' if 'FP' in tag else 'sp2-fp32');d=R/tag;a=read(d/'validated.json');r=read(R/repeat_tag/'validated.json');assert a['numerical_pass'] and r['output_hashes']==a['output_hashes']*10
 for step in range(2):
