#!/usr/bin/env python3
"""Pre-freeze calibration/development/untouched testing for PC051."""
...
RESULT=BASE/'exp0230'
OUTPUT=Path('/mnt/d/llm_exp/models/qwen3-block-htp/exp0230')
...
def prepare():
    ...
    rng=np.random.default_rng(230);chosen={c:[] for c in CELLS}
    ...
    def select(cell,rows,path):
        for index in rng.permutation(len(rows)):
            ...
            width=128 if len(chosen[cell])<128 else 80
    ...
    for lang in ['en','zh']:
        ...
        select(cell,rows,p);assert len(chosen[cell])==416,(cell,len(chosen[cell]))
        print('DATA_CELL_READY',cell,416,flush=True)
    for lang in ['en','zh']:
        cell=lang+'_wiki'
        for offset in range(20000,26000,100):
            ...
            if len(chosen[cell])==416:break
        assert len(chosen[cell])==416
    samples=[]
    for split,start,count in [('calibration',0,128),('development',128,32),('primary',160,128),('reserve',288,128)]:
        for i in range(count):
            for cell in CELLS:
                ...
                samples.append(row)
    assert len(samples)==len(used)==1664 and not forbidden & used_grams
    write('dataset.json',dict(version='qbh-calibration-coverage-v1',seed=230,cells=CELLS,samples=samples,
        ...
        calibration_tokens={'C8':8192,'C64':65536},development_targets=2048,primary_targets=8192,reserve_targets=8192,
    ...
    for name in ['C8','C64']:
        rows=[s for s in samples if s['split']=='calibration' and (name=='C64' or s['in_C8'])]
        a=np.asarray([s['token_ids'] for s in rows],dtype='<u4')
        with (inputs/(name+'_calibration_u32.bin')).open('xb') as f:f.write(a.tobytes())
    for split in ['development','primary','reserve']:
        rows=[s for s in samples if s['split']==split]
        for i in range(len(rows)//32):binary(inputs/f'{split}_{i:02d}.bin',rows[i*32:(i+1)*32])
    sentinels=[s for s in samples if s['split']=='primary'][:8]
    binary(inputs/'sentinel.bin',sentinels+sentinels)
    dev=[s for s in samples if s['split']=='development'][:8]
    binary(inputs/'development_sentinel.bin',dev+dev)
    old=[s for s in datasets[0]['samples'] if s['id'] in [0,20]]
    assert {s['language'] for s in old}=={'en','zh'} and all(s['split']=='full' for s in old)
    ...
    write('dataset_freeze.json',dict(files={str(p.relative_to(RESULT)):sha(p) for p in [RESULT/'dataset.json',*sorted(inputs.glob('*.bin'))]},
        frozen_before_scoring=True,source_roles='calibration/development/final primary/final reserve'))
...
def audit():
    ...
    tok=AutoTokenizer.from_pretrained(MODEL,local_files_only=True)
    _,_,prior=prior_data();forbidden={tuple(ids_of(r)[i:i+32]) for r in prior for i in range(len(ids_of(r))-31)}
    ...
    for row in d['samples']:
        ...
        key=(row['cell'],row['document']);assert key not in used and row['text_sha256'] not in hashes
    ...
    assert len(used)==1664
    write('independent_data_audit.json',dict(pass_all=True,source_windows=1664,
        direct_tuple32_disjoint=True,source_token_offset_reconstruction=True,dataset_sha256=sha(RESULT/'dataset.json')))
    print('INDEPENDENT_DATA_AUDIT_PASS',1664,flush=True)
