...
remote_root="${EXP0082_REMOTE_ROOT:-/data/local/tmp/qwen3-block-htp/exp0082}"
mode="${1:-control}"
repeat_count="${2:-1}"
...
"${adb_exe}" shell \
    "cd ${remote_root} && LD_LIBRARY_PATH=${remote_root} DSP_LIBRARY_PATH=${remote_root} ADSP_LIBRARY_PATH=${remote_root} ./qwen3_block_cli ${remote_root}/block_package_layer14_m64 W4U8 ${repeat_count} 2 32 rms_rope_softmax ${attribution_mode} ${audit_mode} fused_pool6 serial control hvx w4u8_streaming_persistent_mlp_hvx 3 64 ${attention_mode} 6 w4u8_mlp_io_qkv_o qkvo_batch4_qk_head_pairs hvx_tree_qk_batched_rsqrt_shared_rope_parallel_input ${decode_mode}"
