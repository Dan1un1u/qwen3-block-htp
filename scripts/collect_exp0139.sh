...
u8_package="${QBH_EXP0139_U8_PACKAGE:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0042/block_package_layer14_m64_integer_attention_parallel}"
result_root="${QBH_EXP0139_RESULT_ROOT:-/mnt/d/llm_exp/results/qwen3-block-htp/exp0139}"
artifact_root="${QBH_EXP0139_ARTIFACT_ROOT:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0139/artifacts}"
...
if ! git -C "${project_root}" diff --quiet || \
   ...
   [[ -n "$(git -C "${project_root}" ls-files --others --exclude-standard)" ]]; then
    printf 'source worktree must be clean before EXP-0139 formal collection\n' >&2
    exit 1
fi
...
"${project_root}/scripts/build_exp0139.sh" > "${result_dir}/build.log" 2>&1
"${project_root}/scripts/check_exp0139_static.sh" > "${result_dir}/static_gate.json"
QBH_EXP0139_U8_PACKAGE="${u8_package}" \
    "${project_root}/scripts/deploy_exp0139_block.sh" > "${result_dir}/deploy.log" 2>&1
...
for cell in "${cells[@]}"; do
    ...
    "${project_root}/scripts/run_exp0139.sh" "${cell}" 1 on on > "${raw}"
...
for round in 1 2 3 4 5; do
    printf 'EXP-0139 round %d/5\n' "${round}"
    case "${round}" in
        1|4)
            repeats=(1 10)
            order=(sequential ring3 assist)
            ;;
        2|5)
            repeats=(10 1)
            order=(assist ring3 sequential)
            ;;
        *)
            repeats=(1 10)
            order=(ring3 assist sequential)
            ;;
    esac
    for repeat in "${repeats[@]}"; do
        for cell in "${order[@]}"; do
            raw="${result_dir}/round_${round}_${cell}_r${repeat}_raw.jsonl"
            "${project_root}/scripts/run_exp0139.sh" \
...
python3 "${project_root}/scripts/analyze_exp0139.py" "${result_dir}" \
    > "${result_dir}/gate_summary.json"
python3 "${project_root}/scripts/analyze_exp0139.py" "${result_dir}" --report \
...
{
    printf 'experiment=EXP-0139\nsource_head=%s\n' "${source_head}"
    printf 'parent=EXP-0124\ncells=%s\n' "${cells[*]}"
    printf 'recipe=W4U8\nvariable=qkv_main_context_qk_prep_assist\n'
