...
import os,sys,shlex
from common_exp0277 import *
from device_exp0277 import adb,win,stage,run
from selected_audit_exp0277 import audit
from audit_exp0277 import chain,head
from full_exp0277 import full
ARMS=['SP2','A8'];PRIOR=R.parent/'exp0276'
def setarm(a):
 os.environ.update(QBH_SP2='8' if a=='SP2' else '0',QBH_U8_PREFILL_OPT='0' if a=='SP2' else '3',QBH_PAPER_FORMAT_DISABLE='0',QBH_PAPER_PIPELINE_DISABLE='0')
def prepare():
 preflight();ledger=PRIOR/'EVIDENCE_SHA256.json';assert sha(ledger)=='07fa8eb7877a063519f0b450fcd087e33506ae5931591644cae68bcc93077814'
 ...
 for name,old in [('layer14-a8','layer14-fp32-a01'),('full-a8','sp2-fp32')]:
  p=package_path(name);m=read(p/'manifest.json');a=read(R/(name+'-package.json'));assert a['manifest_sha256']==sha(p/'manifest.json');parent=read(R/f'deployment-{old}.json');pm=read(package_path(old)/'manifest.json');remote='/data/local/tmp/qwen3-block-htp/exp0277-models/'+name
 ...
 write(R/'fixed_tokens.json',dict(ids=[v[0] for v in read(PRIOR/'full-ALL-audit/validated.json')['selected_codes']],source=str(PRIOR/'full-ALL-audit/validated.json'),mode='forced replay mode3; no scoring work'))
def selected():
 out=[]
 for a in ARMS:
  setarm(a);name='layer14-fp32-a01' if a=='SP2' else 'layer14-a8';tag=f'selected-{a}-a01';run(name,tag,fp32=2,dump=True);out.append(audit(name,tag))
 write(R/'single_gate.json',dict(pass_all=True,runs=out))
def slices():
 assert read(R/'single_gate.json')['pass_all'];out=[]
 for a in ARMS:
  setarm(a);name='sp2-fp32' if a=='SP2' else 'full-a8';tag=f'slice-{a}-a01';rep=f'slice-{a}-r10';run(name,tag,count=3,fp32=2,dump=True);run(name,rep,count=3,fp32=2,repeat=10);chain(tag,rep);out.append(read(R/tag/'chain_gate.json'))
...
def fullgates():
 ...
 for forced in [False,True]:
  ...
  for a in ARMS:
   ...
   if a=='SP2':assert z['selected_codes']==read(PRIOR/'full-ALL-audit/validated.json')['selected_codes']
