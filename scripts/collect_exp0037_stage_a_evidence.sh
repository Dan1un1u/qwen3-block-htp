...
result_root="${QBH_RESULT_ROOT:-/mnt/d/llm_exp/results/qwen3-block-htp/exp0037/stage_a}"
artifact_root="${QBH_ARTIFACT_ROOT:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0037/stage_a_artifacts}"
...
"${project_root}/scripts/build_exp0037.sh" > "${result_dir}/build.log" 2>&1
QBH_STATIC_OUTPUT_DIR="${result_dir}/static" \
    "${project_root}/scripts/check_exp0037_static.sh" \
...
run_config() {
    ...
    local qkv_mode
    case "${config}" in
        control) qkv_mode=batch2 ;;
        q_batch4) qkv_mode=q_batch4 ;;
        *) printf 'unknown config: %s\n' "${config}" >&2; return 1 ;;
    esac
    "${project_root}/scripts/run_exp0037_block.sh" \
        W4F16 "${repeat}" on "${audit}" fused serial hvx 3 32 \
        adaptive_down96_gate4_dma8_cross hvx \
        crouton_native_batch8 4 64 gqa_qkv_overlap 4 "${qkv_mode}"
}
...
for round in 1 2 3 4 5; do
    if (( round % 2 == 1 )); then
        order=(control q_batch4)
    else
        order=(q_batch4 control)
    fi
...
python3 "${project_root}/scripts/validate_exp0037_stage_a_evidence.py" \
