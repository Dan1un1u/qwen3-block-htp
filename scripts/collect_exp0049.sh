...
package="${QBH_EXP0049_PACKAGE:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0042/block_package_layer14_m64_integer_attention_parallel}"
result_root="${QBH_EXP0049_RESULT_ROOT:-/mnt/d/llm_exp/results/qwen3-block-htp/exp0049}"
artifact_root="${QBH_EXP0049_ARTIFACT_ROOT:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0049/artifacts}"
...
"${project_root}/scripts/build_exp0049.sh" \
    > "${result_dir}/build.log" 2>&1
"${project_root}/scripts/check_exp0049_static.sh" \
    > "${result_dir}/static_gate.json"
QBH_EXP0049_PACKAGE="${package}" \
QBH_EXP0049_CONTROL_ARTIFACTS="${control_artifacts}" \
    "${project_root}/scripts/deploy_exp0049_block.sh" \
...
for mode in control candidate; do
    "${project_root}/scripts/run_exp0049.sh" \
...
for round in 1 2 3 4 5 6 7; do
    printf 'EXP-0049 paired round %d/%d\n' "${round}" "${paired_rounds}"
    if (( round % 2 == 1 )); then
        repeats=(1 10)
...
python3 "${project_root}/scripts/validate_exp0049.py" \
...
{
    printf 'experiment=EXP-0049\n'
    printf 'source_branch=%s\n' "${source_branch}"
    printf 'source_head=%s\n' "${source_head}"
    printf 'control_source_head=61cd86fdabc3c3cf535bf13bfd7f5693fc502b8e\n'
    printf 'execution_unit=qwen3_layer14_complete_block_m64\n'
    printf 'control=W4U8-EXP0048-NATIVE-QKV-O\n'
    printf 'candidate=gate_up_accumulate_store_fused_batch8\n'
