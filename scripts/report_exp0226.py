...
def report():
    ...
    quality=json.loads((RESULT/'quality_summary.json').read_text());selected=quality['profile_candidate']
    oldq={r['id']:r for r in quality['samples']['control_old100']};newq=quality['samples']['selected_'+selected]
    cases=[dict(id=r['id'],language=r['language'],old_nll=float(np.mean(oldq[r['id']]['nll'])),new_nll=float(np.mean(r['nll'])),delta=float(np.mean(r['nll'])-np.mean(oldq[r['id']]['nll']))) for r in newq if r['kind']=='nll']
    out['qbh_selected_vs_old100']=dict(selected_checkpoint=selected,role='final evaluation after selection; distinct from primary fixed-step100 validation attribution',cases=cases,language={lang:dict(improved_cases=sum(r['delta']<0 for r in cases if r['language']==lang),total=16,nll_change=float(np.mean([r['delta'] for r in cases if r['language']==lang]))) for lang in ['en','zh']})
    out['frozen_GPTQ_calibration_coverage']=json.loads((RESULT/'frozen_calibration_coverage.json').read_text())
