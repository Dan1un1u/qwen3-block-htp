...
remote_root="${REMOTE_ROOT:-/data/local/tmp/qwen3-block-htp/exp0041}"
variant="${1:-W4U8}"
repeat_count="${2:-1}"
common_ops_mode="${3:-scalar}"
residual_mode="${4:-scalar}"
attribution_mode="${5:-on}"
audit_mode="${6:-off}"

if [[ "${DEPLOY_EXP0041:-0}" == "1" ]]; then
...
if [[ "${variant}" == "W4U8" ]]; then
    ...
    printf 'unsupported EXP-0041 variant: %s\n' "${variant}" >&2
    exit 2
fi
