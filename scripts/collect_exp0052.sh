...
package="${QBH_EXP0052_PACKAGE:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0042/block_package_layer14_m64_integer_attention_parallel}"
result_root="${QBH_EXP0052_RESULT_ROOT:-/mnt/d/llm_exp/results/qwen3-block-htp/exp0052}"
artifact_root="${QBH_EXP0052_ARTIFACT_ROOT:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0052/artifacts}"
...
"${project_root}/scripts/build_exp0052.sh" \
    > "${result_dir}/build.log" 2>&1
"${project_root}/scripts/check_exp0052_static.sh" \
    > "${result_dir}/static_gate.json"
QBH_EXP0052_PACKAGE="${package}" \
    "${project_root}/scripts/deploy_exp0052_block.sh" \
...
for mode in control candidate; do
    "${project_root}/scripts/run_exp0052.sh" \
...
for round in 1 2 3 4 5 6 7; do
    printf 'EXP-0052 paired round %d/%d\n' "${round}" "${paired_rounds}"
    if (( round % 2 == 1 )); then
        repeats=(1 10)
...
python3 "${project_root}/scripts/validate_exp0052.py" \
