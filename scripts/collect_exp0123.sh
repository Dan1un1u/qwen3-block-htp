...
fp16_package="${QBH_EXP0123_FP16_PACKAGE:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0022/block_package_layer14_m64}"
result_root="${QBH_EXP0123_RESULT_ROOT:-/mnt/d/llm_exp/results/qwen3-block-htp/exp0123}"
artifact_root="${QBH_EXP0123_ARTIFACT_ROOT:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0123/artifacts}"
...
if ! git -C "${project_root}" diff --quiet || \
   ...
   [[ -n "$(git -C "${project_root}" ls-files --others --exclude-standard)" ]]; then
    printf 'source worktree must be clean before EXP-0123 formal collection\n' >&2
    exit 1
fi
...
"${project_root}/scripts/build_exp0123.sh" > "${result_dir}/build.log" 2>&1
"${project_root}/scripts/check_exp0123_static.sh" > "${result_dir}/static_gate.json"
QBH_EXP0123_FP16_PACKAGE="${fp16_package}" \
    "${project_root}/scripts/deploy_exp0123_block.sh" \
...
for cell in "${cells[@]}"; do
    ...
    "${project_root}/scripts/run_exp0123.sh" "${cell}" 1 on on > "${raw}"
...
for round in 1 2 3 4 5; do
    printf 'EXP-0123 round %d/5\n' "${round}"
    if ((round % 2 == 1)); then
        repeats=(1 10)
        order=(join_only persistent)
    else
        repeats=(10 1)
        order=(persistent join_only)
    fi
...
python3 "${project_root}/scripts/analyze_exp0123.py" "${result_dir}" \
    > "${result_dir}/gate_summary.json"
python3 "${project_root}/scripts/analyze_exp0123.py" "${result_dir}" --report \
...
{
    printf 'experiment=EXP-0123\nsource_head=%s\n' "${source_head}"
    printf 'parent=EXP-0120-with-EXP-0111-W4F16\ncells=%s\n' "${cells[*]}"
    printf 'recipe=W4F16\nvariable=gate_up_expansion_worker_lifetime\n'
