...
import numpy as np
S=Path('/home/daniuniu/work/qwen3-block-htp');R=Path('/mnt/d/llm_exp/results/qwen3-block-htp/exp0285')
...
for phase,n in [('short',5),('formal',10)]:
 ...
 for cycle in range(n):
  ...
  for a in arms:
   ...
   assert v['selected_codes']==read(f'full-{a}-audit-a01/validated.json')['selected_codes']
   for p in ps:
...
assert len(native)==1
out=dict(experiment='EXP-0285',model='Qwen3-1.7B',recipe='native W4A8 SP2 no rotation',measured_source=next(iter(native)),profiles_checked=profiles_checked,quality_evaluated=False,baseline_promoted=False,arms={},effects={},auxiliary=read('auxiliary.json'),reserved_hole_bytes=262144,live_residual_bytes={'FP32':524288,'FP16':262144},vtcm_peak_plan_bytes=max(v['vtcm_peak_plan_bytes'] for v in raw['FP16']))
...
lines=['# EXP-0285: FP16 residual storage speed comparison','',
'Qwen3-1.7B latest no-rotation native-W4 SP2 baseline, parent EXP0284 SP2 arm. Mode2 FP32 control versus mode3 FP16 storage; projection scale/add and ordered RMSNorm remain FP32. Norm still produces A8. Embedding begins in FP16; both residual additions round to FP16 at storage in candidate. No rotations, quantizer fitting, weight changes or model-quality/PPL evaluation. No baseline promotion.','',
