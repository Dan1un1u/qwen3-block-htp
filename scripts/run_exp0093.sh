...
remote_root="${EXP0093_REMOTE_ROOT:-/data/local/tmp/qwen3-block-htp/exp0093}"
cell="${1:-control}"
repeat_count="${2:-1}"
...
case "${cell}" in
    control) residual_mode=fused_pool6 ;;
    candidate) residual_mode=fused_pool6_shuffle4 ;;
    *)
...
runtime_args="2 32 rms_rope_softmax ${attribution_mode} ${audit_mode} ${residual_mode} serial control hvx w4u8_streaming_persistent_mlp_hvx 3 64 u8_log2_gqa_qkv_overlap_vgather_vdeal_fused_qk_requant_hmx_batch_lut_templates_gqa_batch_dependency_stream 6 w4u8_mlp_io_qkv_o qkvo_batch4_qk_head_pairs hvx_tree_qk_batched_rsqrt_shared_rope_parallel_input control 4 4"
