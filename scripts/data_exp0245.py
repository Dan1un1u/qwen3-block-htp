...
from data_exp0244 import prior_data as old_prior, verified as previous_verified
from data_exp0233 import ids_of
from gptq_exp0221 import fetch
RESULT=BASE/'exp0245'
def verified(exp,name):
 if exp!='exp0244':return previous_verified(exp,name)
 ledger=BASE/exp/'EVIDENCE_SHA256.json'
 assert sha(ledger)=='7f37673a6acfa9355819a0faeae47c1b41988e3f48214015144186e4387ee74e'
...
def prior_data():
 paths,ds,rows=old_prior();p=verified('exp0244','dataset.json');d=json.loads(p.read_text())
 return paths+[p],ds+[d],rows+d['samples']
def frozen():
 f=json.loads((RESULT/'dataset_freeze.json').read_text())
 assert sha(MEMORY/'docs/experiments/EXP-0245.md')==f['protocol_sha256']
...
def prepare():
 ...
 rng=np.random.default_rng(245);chosen={c:[] for c in CELLS};used=set();grams=set();hashes=set();sources=[]
 ...
 for lang in ['en','zh']:
  ...
  if len(chosen[cell])<64:
   ...
   for off in range(84000,94000,100):
 ...
 write('dataset.json',dict(version='qbh-EXP245-QKV-final-v1',seed=245,cells=CELLS,samples=samples,provenance=sources,tokenizer_sha256=sha(MODEL/'qwen3-tokenizer.json'),calibration_reference='EXP230_C64_65536',final_targets=4096,prior_paths=[str(p) for p in paths]))
 ...
 write('dataset_freeze.json',dict(protocol_sha256=sha(MEMORY/'docs/experiments/EXP-0245.md'),files={'dataset.json':sha(RESULT/'dataset.json')},references={str(p):sha(p) for p in refs},frozen_before_export_and_scoring=True))
