#!/usr/bin/env python3
"""Validate EXP-0075 dependency-driven row-sliced W4U8 Attention."""
...
def pipeline(mode: str) -> str:
    ...
    return stem + ("_dependency_stream" if mode == "candidate" else "")
...
def build_summary(result_dir: Path, package_dir: Path) -> dict[str, object]:
    ...
    if static_gate.get("global_attention_stage_barriers") != 0:
        raise SystemExit("candidate contains a global Attention stage barrier")
    ...
    for mode in MODES:
        ...
        correctness[mode] = {
            ...
            "softmax_task_count": record["attention_softmax_task_count"],
        }
    ...
    for repeat in REPEATS:
        ...
        dependency_gate = (
            metrics["attention_softmax_task_count"]["control"] == 0.0
            and metrics["attention_softmax_task_count"]["candidate"]
                == 16.0
        )
        passed = speed_gate and invariant_gate and dependency_gate
        ...
        repeat_results[f"repeat{repeat}"] = {
            ...
            "dependency_stream_execution_gate": dependency_gate,
        }

    return {
        "experiment": "EXP-0075",
        "control": "EXP-0072 eight coarse full-GQA Attention tasks",
        "candidate": "dependency-driven QK / row Softmax / AV stream",
        ...
        "zero_global_attention_stage_barrier_gate": True,
...
def add_table(lines: list[str], title: str, fields: tuple[str, ...],
    ...
    lines.extend([
        f"### {title}", "",
        "| Metric | coarse control | dependency candidate | Delta | Paired delta |",
        "|---|---:|---:|---:|---:|",
    ])
    for field in fields:
...
def add_pc028_table(lines: list[str], summary: dict[str, object]) -> None:
    ...
    used = "EXP-0075" if summary["local_gate_pass"] else "EXP-0072"
    lines.extend([
        "## PC-028 current-best three-variant module wall-time", "",
        f"{used} supplies the W4U8 column. F16F16 and W4F16 reuse their "
        "latest valid formal evidence. Percentages are shares of complete "
        "Host wall; positive speed means W4U8 is faster than W4F16.", "",
...
def render_report(summary: dict[str, object]) -> str:
    lines = ["# EXP-0075 — Complete profiling report", ""]
    add_pc028_table(lines, summary)
    lines.extend([
        "The control runs eight coarse full-GQA tasks. The candidate starts "
        "the six-context pool once and lets each context progress from QK to "
        "sixteen 32-row Softmax tasks and then AV through per-group generation "
        "tags. There is no whole-pool QK/Softmax/AV stage barrier. Arithmetic, "
        "tensor bytes, HMX commands and tile pairs remain unchanged.", "",
    ])
    for repeat in REPEATS:
        ...
        lines.extend([
            ...
            f"dependency stream execution: **{'PASS' if result['dependency_stream_execution_gate'] else 'FAIL'}**.",
            "",
        ])
    lines.extend([
        ...
        "| Global QK/Softmax/AV stage barriers | 0 |",
        ...
        "## Decision", "",
        f"EXP-0075 local gate: **{'PASS' if summary['local_gate_pass'] else 'FAIL'}**. "
