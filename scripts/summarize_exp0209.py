#!/usr/bin/env python3
"""Validate and summarize EXP-0209 direct-W4 Q32/KV16 evidence."""
...
Q_BATCHES = {CONTROL: 0, CANDIDATE: 32}
QKV_COMMANDS = {CONTROL: 224, CANDIDATE: 168}
...
def validate_run(run: dict[str, object], cell: str, steps: int) -> None:
    ...
    for index, profile in enumerate(profiles):
        ...
        base.require_equal(
            ...
            8, context + " inherited O batch")
        expected_batch = 4 if index == 0 else 8
        expected_commands = 448 if index == 0 else 224
        base.require_equal(
...
def main() -> None:
    ...
    gates = {
        ...
        "qkv_projection_wall_faster":
            qkv_projection[CANDIDATE] < qkv_projection[CONTROL],
        ...
        "direct_hmx_command_delta_exact":
            ...
            physical["candidate_direct_n_hmx_commands"] == 56,
        ...
        "dma_descriptor_delta_exact":
            ...
            physical["candidate_dma_descriptors"] == 112,
        "qkv_commands_224_to_168":
    ...
    summary = {
        ...
        "qkv_plus_qk_prep_us": qkv_projection,
        "prefetch": prefetch,
        "stream": stream,
        "physical": physical,
        "audit": audit,
    ...
    report = [
        "# EXP-0209 direct-W4 Q32/KV16 report",
        "",
        f"Source commit: `{args.source_commit}`",
        "",
        "Both cells inherit the evidence-valid EXP-0207 enabling candidate, "
        "including Down batch4 single-DMA. The only runtime difference is "
        "uniform Q/K/V batch16 versus Q batch32 with K/V batch16.",
        "",
        "## End-to-end gate",
        "",
        "| Metric | Uniform QKV16 control | Q32/KV16 candidate | Change |",
        "|---|---:|---:|---:|",
        ...
        *pair_rows,
        ...
        "",
        "| Module | Uniform QKV16 control | Q32/KV16 candidate | Candidate speed change |",
        "|---|---:|---:|---:|",
        *module_rows,
        ...
        "",
        f"QKV plus Q/K preparation wall: {qkv_projection[CONTROL]:.1f} -> "
        f"{qkv_projection[CANDIDATE]:.1f} us/token. Prefetch: "
