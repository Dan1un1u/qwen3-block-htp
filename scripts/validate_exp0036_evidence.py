...
POSITIVE_TIMING_FIELDS = (
    ...
    "hmx_compute_ticks", "mlp_stream_worker_work_ticks",
)
...
def validate_record(record, name, repeat, audit_mode):
    ...
    fixed = {
        ...
        "vtcm_peak_plan_bytes": 7_843_328,
        ...
        "w4f16_streamed_command_count": 224 * repeat,
        ...
        "w4f16_gate_up_hmx_command_count": 96 * repeat,
    ...
    if candidate:
        require(record, "w4f16_gate_up_unpack_ticks", 0)
    elif record["w4f16_gate_up_unpack_ticks"] <= 0:
        raise SystemExit("control did not materialize row-major Gate/Up")
    ...
    if audit_mode == "off":
        ...
        for field in STAGE_AUDIT_FIELDS:
            if candidate and field in ("gate_up_audit_ticks", "activation_audit_ticks"):
                require(record, field, 0)
            elif record[field] <= 0:
...
def main():
    ...
    print(json.dumps({
        ...
        "selected_candidate": "crouton_native",
        "timing": timing,
