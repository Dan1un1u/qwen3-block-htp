...
package="${QBH_EXP0141_PACKAGE:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0042/block_package_layer14_m64_integer_attention_parallel}"
result_root="${QBH_EXP0141_RESULT_ROOT:-/mnt/d/llm_exp/results/qwen3-block-htp/exp0141}"
artifact_root="${QBH_EXP0141_ARTIFACT_ROOT:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0141/artifacts}"
...
if ! git -C "${project_root}" diff --quiet || \
   ...
   [[ -n "$(git -C "${project_root}" ls-files --others --exclude-standard)" ]]; then
    printf 'source worktree must be clean before EXP-0141 formal collection\n' >&2
    exit 1
fi
...
"${project_root}/scripts/build_exp0141.sh" > "${result_dir}/build.log" 2>&1
"${project_root}/scripts/check_exp0141_static.sh" > "${result_dir}/static_gate.json"
QBH_EXP0141_PACKAGE="${package}" \
    "${project_root}/scripts/deploy_exp0141_block.sh" > "${result_dir}/deploy.log" 2>&1
...
for cell in "${cells[@]}"; do
    ...
    "${project_root}/scripts/run_exp0141.sh" "${cell}" 1 on on > "${raw}"
...
for round in 1 2 3 4 5; do
    ...
    for repeat in "${repeats[@]}"; do
        for cell in "${order[@]}"; do
            ...
            "${project_root}/scripts/run_exp0141.sh" \
...
python3 "${project_root}/scripts/analyze_exp0141.py" "${result_dir}" \
    > "${result_dir}/gate_summary.json"
python3 "${project_root}/scripts/analyze_exp0141.py" "${result_dir}" --report \
    > "${result_dir}/full_profiling_report.md"

cp "${package}/manifest.json" "${result_dir}/w4u8_package_manifest.json"
...
{
    printf 'experiment=EXP-0141\nsource_head=%s\n' "${source_head}"
    printf 'parent_source=EXP-0124-ring3\ncells=%s\n' "${cells[*]}"
    printf 'recipe=W4U8\nvariable=O_projection_HMX_batch_width_only\n'
