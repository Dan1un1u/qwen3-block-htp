...
from data_exp0230 import RESULT,write,sha,preflight,verified
from export_exp0230 import frozen
from evaluate_exp0230 import package
from device_exp0230 import variants,deployed,REMOTE
...
def summarize():
    ...
    module=table(['模块','F16A16 历史 EXP0218',f'W4A16 {chosen} EXP0230','W4A8 历史 EXP0218','W4A8 相对 W4A16 增速'],rows)
    (RESULT/'module_table.md').write_text(module)
    report=['# EXP0230 complete profiling',
        'Unchanged ABI108/runtime and per-channel format. Both weight packages have independent expected tokens. '
        'One warmup,5short,10rotated A0/candidate formal pairs;M64+15 feedback steps. '
    ...
    for mode in ['prefill','decode']:
        ...
        report.extend([f'## {mode}',table(['Field','R1 A0','R1 candidate','change','R10 A0 median','R10 candidate median','change'],rows)])
