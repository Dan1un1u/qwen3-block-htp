...
for phase,n in [('short',5),('formal',10)]:
 for c in range(n):
  ...
  for a,mask in [('ALL',0),('SPLIT',4)]:
   d=R/f'l28-{phase}/{c:02d}-{a}';cfg=json.loads((d/'protocol.json').read_text());seal=cfg['runtime']['seal'];assert not seal['paper_trace']
   ...
   ps=[v for v in records(d/'stdout.jsonl') if v.get('record')=='generation_profile'];assert len(ps)==160
   for v in ps:
  ...
  for x,y in zip(*arm):
   ...
   delta=y['hmx_u8s8_tile_pair_count']-x['hmx_u8s8_tile_pair_count'];assert delta==(344064 if x['mode']=='decode' else 0),(x['mode'],delta)
...
z=dict(pass_all=True,timed_profiles=count,paired_profiles=count//2,extra_decode_pairs_per_layer=12288,extra_decode_pairs_full=344064,weight_bytes_identical=True,worker_commands_identical=True,physical_gate=True,independent_numerical_exact=True,quality_claim=False,first_formal_steps=steps)
