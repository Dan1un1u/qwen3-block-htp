...
fp16_package="${QBH_EXP0135_FP16_PACKAGE:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0022/block_package_layer14_m64}"
result_root="${QBH_EXP0135_RESULT_ROOT:-/mnt/d/llm_exp/results/qwen3-block-htp/exp0135}"
artifact_root="${QBH_EXP0135_ARTIFACT_ROOT:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0135/artifacts}"
...
if ! git -C "${project_root}" diff --quiet || \
   ...
   [[ -n "$(git -C "${project_root}" ls-files --others --exclude-standard)" ]]; then
    printf 'source worktree must be clean before EXP-0135 formal collection\n' >&2
    exit 1
fi
...
"${project_root}/scripts/build_exp0135.sh" > "${result_dir}/build.log" 2>&1
"${project_root}/scripts/check_exp0135_static.sh" > "${result_dir}/static_gate.json"
QBH_EXP0135_FP16_PACKAGE="${fp16_package}" \
    "${project_root}/scripts/deploy_exp0135_block.sh" > "${result_dir}/deploy.log" 2>&1
...
for cell in "${cells[@]}"; do
    ...
    "${project_root}/scripts/run_exp0135.sh" "${cell}" 1 on on > "${raw}"
...
for round in 1 2 3 4 5; do
    printf 'EXP-0135 round %d/5\n' "${round}"
    if ((round % 2 == 1)); then
        repeats=(1 10)
        order=(stream3 stream4)
    else
        repeats=(10 1)
        order=(stream4 stream3)
    fi
    for repeat in "${repeats[@]}"; do
        for cell in "${order[@]}"; do
            raw="${result_dir}/round_${round}_${cell}_r${repeat}_raw.jsonl"
            "${project_root}/scripts/run_exp0135.sh" \
...
python3 "${project_root}/scripts/analyze_exp0135.py" "${result_dir}" \
    > "${result_dir}/gate_summary.json"
python3 "${project_root}/scripts/analyze_exp0135.py" "${result_dir}" --report \
...
{
    printf 'experiment=EXP-0135\nsource_head=%s\n' "${source_head}"
    printf 'parent_source=EXP-0134\ncells=%s\n' "${cells[*]}"
    printf 'recipe=W4F16\nvariable=gate_up_worker3_final_stream_drain\n'
