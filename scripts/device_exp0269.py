"""EXP0269 provenance-sealed no-rotation FP32 residual device checks."""
import argparse,subprocess,shutil,shlex,statistics,json,struct
import numpy as np
...
REMOTE='/data/local/tmp/qwen3-block-htp/exp0269'
...
def run(package,tag,count=1,repeat=1,fp32=1,dump=False):
 ...
 env=dict(old.ENV,LD_LIBRARY_PATH=root,DSP_LIBRARY_PATH=root,ADSP_LIBRARY_PATH=root,QBH_SP2='8',QBH_U8_PREFILL_OPT='0',QBH_FP32_RESIDUAL=str(fp32),QBH_WIDE_SCORE='4',QBH_DENSE_R3='0',QBH_DENSE_R4='0',QBH_KV_CACHE_CAPACITY='128',QBH_REPLAY_DECODE_STEPS='1')
 if dump:
 ...
 if dump and fp32:
  errs=[]
  for step in range(2):
   rows=64 if not step else 1;a=np.fromfile(d/f'step{step:02d}_output.bin','<f4').reshape(rows,2048);b=np.fromfile(O/package/('reference_w4u8_block_output_f32.bin' if not step else 'replay_decode_reference_00_f32.bin'),'<f4').reshape(64,2048)[:rows];delta=a.astype('f8')-b
