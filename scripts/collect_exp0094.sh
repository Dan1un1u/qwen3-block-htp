...
package="${QBH_EXP0094_PACKAGE:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0042/block_package_layer14_m64_integer_attention_parallel}"
result_root="${QBH_EXP0094_RESULT_ROOT:-/mnt/d/llm_exp/results/qwen3-block-htp/exp0094}"
artifact_root="${QBH_EXP0094_ARTIFACT_ROOT:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0094/artifacts}"
...
if ! git -C "${project_root}" diff --quiet || \
   ...
   [[ -n "$(git -C "${project_root}" ls-files --others --exclude-standard)" ]]; then
    printf 'source worktree must be clean before EXP-0094 formal collection\n' >&2
    exit 1
fi
...
"${project_root}/scripts/build_exp0094.sh" > "${result_dir}/build.log" 2>&1
"${project_root}/scripts/check_exp0094_static.sh" \
    > "${result_dir}/static_gate.json"
QBH_EXP0094_PACKAGE="${package}" \
    "${project_root}/scripts/deploy_exp0094_block.sh" \
...
for mode in "${modes[@]}"; do
    ...
    "${project_root}/scripts/run_exp0094_pair.sh" \
        "${mode}" 1 on on > "${raw}"
...
python3 "${project_root}/scripts/analyze_exp0094.py" \
...
cp "${project_root}/hexagon_ReleaseG_toolv19_v79/ship/libqwen3_probe_skel.so" \
    "${artifact_dir}/"
{
    printf 'experiment=EXP-0094\nsource_head=%s\n' "${source_head}"
    printf 'control=EXP-0084 per-block V metadata rebuild\n'
    printf 'candidate=Prepared-session 12KiB VTCM V metadata cache\n'
