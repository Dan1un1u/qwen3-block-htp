"""EXP-0297 isolated long-prefill fixtures, build deployment and immutable runs."""
...
SIZE=os.environ.get('QBH_QWEN_MODEL_SIZE','1.7B')
assert SIZE in ['0.6B','1.7B']
...
R=Path('/mnt/d/llm_exp/results/qwen3-block-htp/exp0297')
M=Path('/mnt/d/llm_exp/models/qwen3-block-htp/exp0296')/SIZE
BASE=R/'base.json'
REMOTE='/data/local/tmp/qwen3-block-htp/exp0297'
...
def preflight():
 ...
 assert 'EXPERIMENT=EXP-0297' in z
def prepare():
 preflight()
 import torch
 from transformers import AutoTokenizer
 from prepare_exp0288 import rope
 ...
 for length in [64,65,128,129,536,741]:
  ...
  for n,v in mf['files'].items():
   ...
   if 'kv_cache_' in Path(n).name and n.endswith('_u8.bin'):
    np.zeros((8,832,128),'u1').tofile(target);changed.append(n)
  ...
  put(dst/'manifest.json',dict(experiment='EXP-0297',parent=base,parent_sha256=base['manifest_sha256'],files=files,changed=changed))
  ...
  if os.environ.get('QBH_LOCAL_PREP'):
   print('PREPARED_LOCAL',length,flush=True);continue
...
def deploy():
 preflight();base=read(BASE);mf=read(Path(base['package'])/'manifest.json')
 for length in [64,65,128,129,536,741]:
  cfg=read(R/f'package-{length}.json');dst=Path(cfg['package']);remote=cfg['remote'];files=read(dst/'manifest.json')['files']
  assert adb('shell','test ! -e '+remote,check=False).returncode==0
  names=list(files);dirs=sorted({str(Path(n).parent) for n in names})
  adb('shell','mkdir -p '+' '.join(shlex.quote(remote+'/'+n) for n in dirs))
  links=[n for n in names if n in mf['files'] and files[n]['sha256']==mf['files'][n]['sha256']]
  for i in range(0,len(links),24):adb('shell',' && '.join('ln -s '+shlex.quote(base['remote']+'/'+n)+' '+shlex.quote(remote+'/'+n) for n in links[i:i+24]))
  for n in set(names)-set(links):adb('push',windows(dst/n),remote+'/'+n)
  for i in range(0,len(names),32):
   lines=adb('shell','sha256sum '+' '.join(shlex.quote(remote+'/'+n) for n in names[i:i+32])).stdout.splitlines()
   assert len(lines)==len(names[i:i+32])
   for line in lines:
    h,n=line.split(None,1);assert h==files[n.removeprefix(remote+'/')]['sha256'],n
  print('DEPLOYED',length,flush=True)
def stage():
 preflight();seal=read(S/'build/qwen3-sp2-build-seal.json')
 assert seal['source_head']==subprocess.check_output(['git','-C',str(S),'rev-parse','HEAD'],text=True).strip()
 assert seal['model_size']==SIZE
 i=1
...
def run(length,tag,decode=3,audit=False,repeat=1,legacy=False):
 ...
 env.pop('QBH_GENERATION_AUDIT_DIR',None);env.pop('QBH_GENERATION_BOUNDARY_AUDIT',None)
 if not legacy:env.update(QBH_KV_CACHE_LAYOUT='row_major',QBH_W4U8_PREFILL_CACHE_MODE='duplicate')
 env.update(QBH_GENERATION_SEQUENCE='9',QBH_SP2='8',QBH_LONG_OPT=os.environ.get('QBH_LONG_OPT','31'),LD_LIBRARY_PATH=root,DSP_LIBRARY_PATH=root,ADSP_LIBRARY_PATH=root,QBH_WIDE_SCORE='4',QBH_PAPER_FORMAT_DISABLE='0',QBH_PAPER_PIPELINE_DISABLE='0',QBH_DENSE_R3='0',QBH_DENSE_R4='0')
 if not legacy:env.update(QBH_LONG_PREFILL_TOKENS=str(length),QBH_LONG_DECODE_STEPS=str(decode),QBH_LONG_REPEATS=str(repeat),QBH_KV_CACHE_CAPACITY='832',QBH_GENERATION_EXPECTED_TOKENS='64')
  ...
  env['QBH_GENERATION_STEPS']=str(decode+1)
  # Same frozen short-cache layout, ten independent trajectories in one process.
  import struct
  fixture=read(R/'fixture.json');words=[0x51424556,2,repeat,68+decode]
  for sample in range(repeat):words += [sample,3,decode+1]+fixture['prompt_ids'][:64]+fixture['fixed'][:decode+1]
  ef=d/'short-control.bin';ef.write_bytes(struct.pack('<'+'I'*len(words),*words))
  er=root+'/'+tag.replace('/','_')+'-short-control.bin';adb('push',windows(ef),er)
  env['QBH_EVAL_FILE']=er
