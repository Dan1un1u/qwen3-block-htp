...
result_root="${QBH_RESULT_ROOT:-/mnt/d/llm_exp/results/qwen3-block-htp/exp0025}"
artifact_root="${QBH_ARTIFACT_ROOT:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0025/artifacts}"
...
"${project_root}/scripts/build_exp0025.sh" > "${result_dir}/build.log" 2>&1
QBH_STATIC_OUTPUT_DIR="${result_dir}/static" \
    "${project_root}/scripts/check_exp0025_static.sh" \
...
for mode in off on; do
    for variant in F16F16 W4F16; do
        "${project_root}/scripts/run_exp0025_block.sh" \
            "${variant}" 1 "${mode}" hvx \
...
python3 "${project_root}/scripts/validate_exp0025_evidence.py" \
