...
generation_steps="${QBH_EXP0171_GENERATION_STEPS:-193}"
swiglu_mode="${QBH_EXP0171_SWIGLU_MODE:-decode_row4}"
audit="${QBH_GENERATION_BOUNDARY_AUDIT:-0}"
audit_dir="${QBH_GENERATION_AUDIT_DIR:-}"
remote_audit_dir="${remote_root}/generation-audit-${swiglu_mode}"
runtime_env="QBH_W4U8_STREAM_FENCE=single_fence QBH_W4U8_GATE_UP_RING_SLOTS=16 QBH_W4U8_QKV_RING_EXPAND_WORKERS=3 QBH_KV_CACHE_LAYOUT=hmx_native_u8_segmented_v4 QBH_W4U8_PREFILL_CACHE_MODE=reuse QBH_W4U8_DELTA_RECONSTRUCTION=serial QBH_W4U8_DECODE_SOFTMAX=hvx_tile4 QBH_W4U8_DECODE_SWIGLU=${swiglu_mode}"
...
case "${swiglu_mode}" in
full_tile|decode_row4) ;;
*) printf 'invalid EXP-0171 SwiGLU mode: %s\n' "${swiglu_mode}" >&2; exit 2 ;;
esac
if [[ "${QBH_EXP0171_DEPLOY:-0}" == 1 ]]; then
    "${project_root}/scripts/deploy_exp0171.sh"
fi
