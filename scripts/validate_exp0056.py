#!/usr/bin/env python3
"""Validate EXP-0056 paired-row Softmax evidence."""
...
def validate_record(record: dict[str, object], repeat: int,
    ...
    fixed = {
        ...
        "attention_pipeline_mode": (
            "u8_log2_gqa_qkv_overlap_paired_softmax"
            if mode == "candidate"
...
def build_summary(result_dir: Path, package_dir: Path) -> dict[str, object]:
    ...
    return {
        "experiment": "EXP-0056",
        "control": "EXP-0055 independent 64-lane Softmax rows",
        "candidate": "paired rows with independent normalization and one banked vlut32",
...
def render_report(summary: dict[str, object]) -> str:
    lines = [
        ...
        "The candidate packs corresponding 64-element Softmax rows from "
        "the two Q heads of one GQA group into one 128-byte HVX vector. "
        "Max, sum, reciprocal and output remain independent per head. "
        "Two 16-entry probability tables share one 32-entry vlut32 call; "
        "QK and AV HMX work is unchanged.", "",
    ]
    for repeat in REPEATS:
    ...
    lines.extend([
        ...
        "Requested/acquired VTCM is exactly 8 MiB; intermediate DDR and "
        "spill/fill remain zero; one FastRPC execution unit and one HMX "
        "owner are preserved.", "",
        "## Decision", "",
        f"EXP-0056 local gate: **{'PASS' if summary['local_gate_pass'] else 'FAIL'}**.", "",
    ])
    return "\n".join(lines)
