...
"${project_root}/scripts/check_exp0045_static.sh" \
...
QBH_EXP0045_PACKAGE="${package}" \
    ...
    "${result_dir}/attention_implementation_audit" qkv_batch2 \
...
for round in $(seq 1 "${paired_rounds}"); do
    printf 'EXP-0045 Stage A paired round %d/%d\n' \
        "${round}" "${paired_rounds}"
    if (( round % 2 == 1 )); then
        repeat_order=(1 10)
...
python3 "${project_root}/scripts/validate_exp0045_stage_a.py" \
...

{
    ...
    printf 'control=serial_W4U8_QKV_projection\n'
    printf 'candidates=qkv_batch2,qkv_batch4\n'
