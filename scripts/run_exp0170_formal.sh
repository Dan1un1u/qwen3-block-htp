...
package="${QBH_EXP0170_OVERLAY:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0169/w4u8_greedy193_overlay}"
transformer_package="${QBH_EXP0170_TRANSFORMER_PACKAGE:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0163/candidate_segmented_capacity257}"

mkdir -p "${result_dir}/raw" "${result_dir}/audit"
"${project_root}/scripts/check_exp0170_static.sh" \
    > "${result_dir}/static_gate.json"
"${project_root}/scripts/deploy_exp0170.sh" \
    > "${result_dir}/deploy.log" 2>&1

QBH_EXP0170_SOFTMAX_MODE=hvx_tile4 \
QBH_GENERATION_BOUNDARY_AUDIT=1 \
QBH_GENERATION_AUDIT_DIR="${result_dir}/audit" \
    "${project_root}/scripts/run_exp0170.sh" \
...
"${python_exe}" "${project_root}/scripts/verify_exp0167_generation.py" \
    ...
    --steps 193 --experiment-record 170 --experiment-label EXP-0170 \
    > "${result_dir}/audit/verify.log" 2>&1

for round in $(seq 1 10); do
    printf -v index '%02d' "${round}"
    if (( round % 2 == 1 )); then
        order=(scalar hvx_tile4)
    else
        order=(hvx_tile4 scalar)
    fi
    for mode in "${order[@]}"; do
        QBH_EXP0170_SOFTMAX_MODE="${mode}" \
...
"${python_exe}" "${project_root}/scripts/summarize_exp0170.py" \
