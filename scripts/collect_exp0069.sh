...
result_root="${QBH_EXP0069_RESULT_ROOT:-/mnt/d/llm_exp/results/qwen3-block-htp/exp0069}"
artifact_root="${QBH_EXP0069_ARTIFACT_ROOT:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0069/artifacts}"
selection_path="${1:?usage: collect_exp0069.sh SELECTION_JSON}"
...
candidate_mode="$(python3 - "${selection_path}" <<'PY'
import json, sys
r = json.load(open(sys.argv[1]))
mode = r.get("selected_mode")
active = r.get("selected_attention_active_contexts")
if r.get("experiment") != "EXP-0069" or mode not in ("attention4", "attention5"):
    raise SystemExit("invalid EXP-0069 selection")
if r.get("qk_prep_contexts") != 6 or active != {"attention4": 4, "attention5": 5}[mode]:
    raise SystemExit("selection context mismatch")
print(mode)
PY
)"
mkdir -p "${result_dir}" "${artifact_dir}"
cp "${selection_path}" "${result_dir}/selection.json"
"${project_root}/scripts/build_exp0069.sh" > "${result_dir}/build.log" 2>&1
"${project_root}/scripts/check_exp0069_static.sh" > "${result_dir}/static_gate.json"
QBH_EXP0069_PACKAGE="${package}" "${project_root}/scripts/deploy_exp0069_block.sh" \
...
"${project_root}/scripts/run_exp0069.sh" control 1 on on \
    > "${result_dir}/correctness_control.jsonl"
"${project_root}/scripts/run_exp0069.sh" "${candidate_mode}" 1 on on \
...
for round in 1 2 3 4 5 6 7; do
    printf 'EXP-0069 paired round %d/%d\n' "${round}" "${paired_rounds}"
    if (( round % 2 == 1 )); then
        repeats=(1 10)
    ...
    for repeat in "${repeats[@]}"; do
        for mode in "${modes[@]}"; do
            ...
            "${project_root}/scripts/run_exp0069.sh" "${run_mode}" "${repeat}" on off \
                >> "${result_dir}/paired_${mode}_r${repeat}.jsonl"
        done
    done
done
...
python3 "${project_root}/scripts/validate_exp0069.py" "${result_dir}" "${package}" \
    > "${result_dir}/gate_summary.json"
python3 "${project_root}/scripts/validate_exp0069.py" "${result_dir}" "${package}" --report \
