...
from data_exp0255 import prior_data as old_prior, verified as previous_verified
from data_exp0233 import ids_of
from gptq_exp0221 import fetch
RESULT=BASE/'exp0256'
def verified(exp,name):
 if exp!='exp0255':return previous_verified(exp,name)
 ledger=BASE/exp/'EVIDENCE_SHA256.json'
 assert sha(ledger)=='0076296574d6c14bdfa8e711b0bb2b51bf8f0ed1934af14ba19b5f0e3d52e19d'
...
def prior_data():
 paths,ds,rows=old_prior();p=verified('exp0255','dataset.json');d=json.loads(p.read_text())
 return paths+[p],ds+[d],rows+d['samples']
def frozen():
 f=json.loads((RESULT/'dataset_freeze.json').read_text())
 assert sha(MEMORY/'docs/experiments/EXP-0256.md')==f['protocol_sha256']
...
def prepare():
 ...
 rng=np.random.default_rng(256);chosen={c:[] for c in CELLS};used=set();grams=set();hashes=set();sources=[]
 ...
 for lang in ['en','zh']:
  ...
  if len(chosen[cell])<64:
   ...
   for off in range(164000,174000,100):
 ...
 write('dataset.json',dict(version='qbh-EXP256-fixed-integer-rmse-confirmation-v1',seed=256,cells=CELLS,samples=samples,provenance=sources,tokenizer_sha256=sha(MODEL/'qwen3-tokenizer.json'),calibration_reference='EXP230_C64_65536',final_targets=4096,prior_paths=[str(p) for p in paths]))
 ...
 write('dataset_freeze.json',dict(protocol_sha256=sha(MEMORY/'docs/experiments/EXP-0256.md'),files={'dataset.json':sha(RESULT/'dataset.json')},references={str(p):sha(p) for p in refs},frozen_before_export_and_scoring=True))
