...
package_dir="${QBH_EXP0050_PACKAGE:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0042/block_package_layer14_m64_integer_attention_parallel}"
candidate_root="${EXP0050_CANDIDATE_REMOTE_ROOT:-/data/local/tmp/qwen3-block-htp/exp0050-candidate}"
control_root="${EXP0050_CONTROL_REMOTE_ROOT:-/data/local/tmp/qwen3-block-htp/exp0050-control}"
...
for artifact in \
    ...
    [[ -f "${artifact}" ]] || {
        printf 'missing EXP-0050 artifact: %s\n' "${artifact}" >&2
        exit 1
    }
done
