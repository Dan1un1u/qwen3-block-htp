...
result_root="${QBH_EXP0118_RESULT_ROOT:-/mnt/d/llm_exp/results/qwen3-block-htp/exp0118}"
artifact_root="${QBH_EXP0118_ARTIFACT_ROOT:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0118/artifacts}"
...
if ! git -C "${project_root}" diff --quiet || \
   ...
   [[ -n "$(git -C "${project_root}" ls-files --others --exclude-standard)" ]]; then
    printf 'source worktree must be clean before EXP-0118 formal collection\n' >&2
    exit 1
fi
...
"${project_root}/scripts/build_exp0118.sh" > "${result_dir}/build.log" 2>&1
"${project_root}/scripts/check_exp0118_static.sh" > "${result_dir}/static_gate.json"
QBH_EXP0118_U8_PACKAGE="${u8_package}" \
    "${project_root}/scripts/deploy_exp0118_block.sh" \
...
for cell in "${cells[@]}"; do
    ...
    "${project_root}/scripts/run_exp0118.sh" "${cell}" 1 on on > "${raw}"
...
for round in 1 2 3 4 5; do
    printf 'EXP-0118 paired round %d/%d\n' "${round}" "${rounds}"
    if ((round % 2 == 1)); then
        repeats=(1 10)
    ...
    for repeat in "${repeats[@]}"; do
        for cell in "${order[@]}"; do
            ...
            "${project_root}/scripts/run_exp0118.sh" \
...
python3 "${project_root}/scripts/analyze_exp0118.py" "${result_dir}" \
    > "${result_dir}/gate_summary.json"
python3 "${project_root}/scripts/analyze_exp0118.py" "${result_dir}" --report \
...
{
    printf 'experiment=EXP-0118\nsource_branch=%s\nsource_head=%s\n' \
        "${source_branch}" "${source_head}"
    printf 'parent=EXP-0112\nrecipe=W4U8\n'
    printf 'control=qk_head_pairs_main_q_expand\n'
    printf 'candidate=preemptible_16_k_tile_worker_assisted_q_expand\n'
