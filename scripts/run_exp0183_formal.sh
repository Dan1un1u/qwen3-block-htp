...
"${project_root}/scripts/check_exp0183_static.sh" > "${result_dir}/static_gate.json"
"${project_root}/scripts/deploy_exp0183.sh" > "${result_dir}/deploy.log" 2>&1
for cell in control k_tail; do
    out_cell="$([[ "${cell}" == k_tail ]] && printf quartet || printf control)"
    QBH_EXP0173_GENERATION_STEPS=34 \
        "${project_root}/scripts/run_exp0183.sh" "${cell}" \
        > "${result_dir}/boundary_${out_cell}.log" 2>&1
done
QBH_EXP0173_GENERATION_STEPS=193 QBH_GENERATION_BOUNDARY_AUDIT=1 \
QBH_GENERATION_AUDIT_DIR="${result_dir}/audit" \
    "${project_root}/scripts/run_exp0183.sh" k_tail \
    > "${result_dir}/audit/capture.log" 2>&1
"${python_exe}" "${project_root}/scripts/verify_exp0167_generation.py" \
    --audit-dir "${result_dir}/audit" --package "${package}" \
    --transformer-package "${transformer_package}" --steps 193 \
    --experiment-record 183 --experiment-label EXP-0183 \
    > "${result_dir}/audit/verify.log" 2>&1
QBH_EXP0173_GENERATION_STEPS=193 \
    "${project_root}/scripts/run_exp0183.sh" k_tail_poison \
...
for round in $(seq 1 10); do
    ...
    for cell in "${order[@]}"; do
        ...
        QBH_EXP0173_GENERATION_STEPS=193 QBH_GENERATION_BOUNDARY_AUDIT=0 \
            "${project_root}/scripts/run_exp0183.sh" "${cell}" \
            > "${result_dir}/raw/pair_${index}_${out_cell}.log" 2>&1
    done
done
...
"${python_exe}" "${project_root}/scripts/summarize_exp0183.py" \
