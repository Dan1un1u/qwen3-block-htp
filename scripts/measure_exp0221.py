...
from experiment_exp0221 import RESULT,OUTPUT,BASE,VARIANTS
...
REMOTE='/data/local/tmp/qwen3-block-htp/exp0221-'
FROZEN_REMOTE='/data/local/tmp/qwen3-block-htp/exp0219-A'

def deploy(variant):
    ev.dataset();root=REMOTE+variant
    assert adb('shell',f'test ! -e {root} && echo absent').strip()=='absent'
    package=BASE if variant=='original' else OUTPUT/variant
    ...
    changed=set(manifest.get('changed_files',[])) if variant!='original' else set()
...
def run(variant,phase,index=None):
    ...
    write_json(path.with_suffix('.provenance.json'),dict(remote_root=REMOTE+variant,variant=variant,experiment='EXP-0221'))
...
def main():
    p=argparse.ArgumentParser();p.add_argument('phase',choices=['deploy','quick','full','repeat','warmup','short','formal']);p.add_argument('--variant',choices=['original']+VARIANTS);a=p.parse_args()
    if a.phase=='deploy':assert a.variant;deploy(a.variant);return
    variants=[a.variant] if a.variant else (VARIANTS if a.phase in ['quick','full','repeat'] else ['original']+VARIANTS)
    for variant in variants:
        d=RESULT/variant;d.mkdir(parents=True,exist_ok=True);expected=d/'regression_expected.json'
        root=BASE if variant=='original' else OUTPUT/variant
