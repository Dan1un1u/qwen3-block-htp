...
def main():
 ...
 # All native code in the measured ABI127 binary must remain unchanged.
 ...
 summary=dict(experiment='EXP-0264',source_head=head,runtime_source_head=runtime['source_head'],source_branch='codex/exp-0264-w4u8-r4-fused-quantization',scope='one real layer0 M64+8M1 teacher-input replay; no fullmodel',short_rounds=5,formal_rounds=10,timed_RPCs=total,ABI=127,numerical_gate='pass_R4_components_and_conditional_wholeblock',known_R3_numerical_failure='unchanged',physical_gate='pass',measurement_integrity='pass',speed_eligible=all(p['ci95'][1]<=1.10 for p in perf.values()),stop_before_fullmodel=not all(p['ci95'][1]<=1.10 for p in perf.values()),stable_over10_confirmed=any(p['stable_over10'] for p in perf.values()),performance=perf,module_centers=modules,R4_breakdown_us=breakdown,physical=physical_summary,baseline_promoted=False,model_quality='not assessed',device_PPL=None,E2E_tokens_per_second=None)
 write(R/'independent_integrity.json',dict(pass_all=True,timed_RPCs=total,all_output_hashes_exact=True,all_layer_ledgers_checked=True,paired_statistics_independently_recomputed=True,independent_converter_pinned=True,native_source_unchanged=True));write(R/'counter_round_means.json',data)
 lines=['# EXP0264 R4 exact prepared constants and fused native output','','Branch '+summary['source_branch']+'; reporting source '+head+'; native runtime '+runtime['source_head']+' ABI127. Evidence '+str(R)+'; candidate artifacts '+str(O/'r4')+'.','', 'Two paired arms: current optimized R3 OPT2 + wideNR64/nativeW4, and the same path plus full6144 R4 before middle A8. Five short and ten alternating formal pairs, repeat1/repeat10; one prefillM64 and eight consecutive teacher-input M1 steps per repetition. KV starts empty and is computed/appended on device. No frozen-snapshot decode, discarded warmups, selection, extra rounds or fullmodel extrapolation.','', 'Primary medians of round means; paired effect is median within-round ratio, 10000bootstrap seed261. Decode latency averages the eight positions. Additive modules use the identical two median-Host-ranked rounds for all fields. All2970RPCs and layer ledgers independently reconstructed.','', '|Scope|Control us|R4 us|Paired time change|Ratio95% CI|','|---|---:|---:|---:|---|']
 ...
 docs=S/'docs/experiments';(docs/'EXP-0264-RESULTS.md').write_text(report);(docs/'EXP-0264-PROFILE.md').write_text((R/'FULL_PROFILING_REPORT.md').read_text())
