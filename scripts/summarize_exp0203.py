#!/usr/bin/env python3
"""Validate and summarize EXP-0203 cross-layer first-Q weight prefetch."""
...
def validate_run(run: dict[str, object], cell: str, steps: int) -> None:
    ...
    for index, profile in enumerate(profiles):
        ...
        base.require_equal(
            ...
            1, context + " O-to-Gate parent selector")
        base.require_equal(
            int(profile[
                "w4u8_decode_direct_n_gate_up_swiglu_stream"]),
            1, context + " Gate/Up SwiGLU parent selector")
        base.require_equal(
        ...
        base.require_equal(
            int(profile["w4u8_decode_direct_n_next_q_prefetch"]),
            enabled, context + " next-Q prefetch selector")
...
def main() -> None:
    ...
    gates = {
        ...
        "control_prefetch_disabled":
            next_q_prefetch["control_start_count"] == 0 and
            next_q_prefetch["control_consume_count"] == 0,
        "one_prefetch_per_layer_transition":
            next_q_prefetch["start_count"] == LAYERS - 1 and
            next_q_prefetch["consume_count"] == LAYERS - 1 and
            next_q_prefetch["overlap_count"] == LAYERS - 1,
        "prefetch_wait_below_lifetime":
            next_q_prefetch["wait_us"] <
            next_q_prefetch["lifetime_us"],
    ...
    summary = {
        ...
        "qkv_us": qkv,
        "next_q_prefetch": next_q_prefetch,
        "physical": physical,
        "audit": audit,
        "gates": gates,
    }
    ...
    report = [
        "# EXP-0203 Down-tail next-layer Q-weight prefetch report",
        "",
        f"Source commit: `{args.source_commit}`",
        "",
        "Both cells inherit EXP-0201. The candidate moves each next "
        "layer's first Q batch16 packed-W4 transfer into the current "
        "layer's final Down-HMX tail and consumes the resident carrier "
        "at the next QKV-ring entry.",
        "",
        "## End-to-end gate",
        "",
        "| Metric | EXP-0201 control | Next-Q candidate | Change |",
        "|---|---:|---:|---:|",
        ...
        *pair_rows,
        ...
        "",
        "| Module | EXP-0201 control | Next-Q candidate | Candidate speed change |",
        "|---|---:|---:|---:|",
        *module_rows,
        ...
        f"{qkv[CANDIDATE]:.1f} us/token. Cross-layer prefetch: "
        f"`{json.dumps(next_q_prefetch, sort_keys=True)}`.",
        "",
