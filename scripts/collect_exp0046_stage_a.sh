...
"${project_root}/scripts/check_exp0046_stage_a_static.sh" \
...
for round in $(seq 1 "${paired_rounds}"); do
    printf 'EXP-0046 Stage A paired round %d/%d\n' \
        "${round}" "${paired_rounds}"
    if (( round % 2 == 1 )); then
        repeat_order=(1 10)
        mode_order=(w4u8_mlp_input control)
    else
        repeat_order=(10 1)
        mode_order=(control w4u8_mlp_input)
    fi
...
python3 "${project_root}/scripts/validate_exp0046_stage_a.py" \
...

{
    ...
    printf 'control=row_major_post_norm_plus_mlp_input_pack\n'
    printf 'candidate=native_post_norm_to_gate_up_hmx_activation\n'
