#!/usr/bin/env python3
"""Validate and report EXP-0135 worker-3 streaming drain."""
...
import analyze_exp0134 as parent
import validate_exp0050 as base


REPEATS = (1, 10)
SAMPLES = 5
CELLS = ("stream3", "stream4")
...
def validate_record(record: dict, repeat: int, cell: str,
    ...
    compatible["experiment"] = "EXP-0134"
    compatible["w4f16_gate_up_extra_expand_worker"] = 1
    parent.validate_record(compatible, repeat, "worker4", audit=audit)
    base.require(record, "experiment", "EXP-0135")
    base.require(record, "w4f16_gate_up_extra_expand_worker", 1)
    base.require(record, "w4f16_gate_up_extra_stream_worker",
                 0 if cell == "stream3" else 1)
...
def build_summary(result_dir: Path, exp0109_dir: Path,
    ...
    if ((result_dir / "boot_id_before.txt").read_bytes() !=
        ...
        raise SystemExit("device boot ID changed during EXP-0135")
    ...
    if correctness["stream3"]["output_hash"] != \
            correctness["stream4"]["output_hash"]:
    ...
    return {
        ...
        "pc028": parent.parent.baseline_pc028(
            exp0109_dir, exp0124_dir, records[10]["stream3"]),
        "pc028_provenance": {
            "f16f16": str(exp0109_dir),
            "w4f16": str(result_dir) + "/paired_stream3_r10.jsonl",
            "w4u8": str(exp0124_dir) + "/paired_control_r10.jsonl",
        },
        "repeat10_candidate_modules":
            exp109.module_medians(records[10]["stream4"]),
    }
...
def add_candidate_modules(lines: list[str], summary: dict) -> None:
    ...
    lines.extend([
        "## EXP-0135 repeat-ten W4F16 module wall-time", "",
        "| Module | Stream-3 drain | Stream-4 drain | Speed |",
        "|---|---:|---:|---:|",
    ])
    ...
    for name, candidate in rows.items():
        ...
        lines.append(f"| {name} | {c:.1f} us | {candidate:.1f} us | {speed:+.1f}% |")
    lines.append("")


def render_report(summary: dict) -> str:
    lines = ["# EXP-0135 — W4F16 worker-3 streaming-drain report", ""]
    exp107.add_pc028(lines, summary)
    lines.extend([
        "PC-028 provenance: F16F16 uses frozen EXP-0109, W4U8 uses "
        "EXP-0124 control, and W4F16 is this experiment's stream-3 control.",
        "",
    ])
    ...
    lines.extend([
        ...
        f"| EXP-0135 local gate | {'PASS' if summary['local_gate_pass'] else 'FAIL'} |",
