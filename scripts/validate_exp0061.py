#!/usr/bin/env python3
"""Validate EXP-0061 paired Q/K batched HVX rsqrt evidence."""
...
import validate_exp0060 as previous
...
def validate_record(record: dict[str, object], repeat: int,
    ...
    fixed = {
        ...
        "u8_norm_reduction_mode": (
            "hvx_tree_qk_batched_rsqrt" if mode == "candidate"
            else "hvx_tree"
        ),
...
def build_summary(result_dir: Path, package_dir: Path) -> dict[str, object]:
    ...
    summary.update({
        "experiment": "EXP-0061",
        "control": "EXP-0060 exact HVX tree plus scalar Q/K rsqrt",
        "candidate": "four-vector batched HVX Q/K rsqrt",
    })
    return summary
...
def render_report(summary: dict[str, object]) -> str:
    lines = [
        ...
        "Both paths use EXP-0060's exact HVX word-tree square-sum. The "
        "candidate batches the 128 reciprocal-square-root inputs of each "
        "paired Q/K task into four HVX vectors, then preserves the existing "
        "gamma, RoPE and U8 requantization path.", "",
    ]
    for repeat in previous.previous.REPEATS:
        result = summary["repeat_results"][f"repeat{repeat}"]
        metrics = result["metrics"]
        lines.extend([
            f"## Repeat {repeat}", "",
            "| Metric (per block) | EXP-0060 control | Batched-rsqrt candidate | Delta | Paired delta |",
            "|---|---:|---:|---:|---:|",
        ])
        for field in (
    ...
    lines.extend([
        ...
        "## Decision", "",
        f"EXP-0061 local gate: **{'PASS' if summary['local_gate_pass'] else 'FAIL'}**.", "",
    ])
    return "\n".join(lines)
