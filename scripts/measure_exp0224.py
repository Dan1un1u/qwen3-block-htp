...
from experiment_exp0224 import RESULT,OUTPUT,VARIANTS
from output_scale_exp0224 import control_package
...
REMOTE='/data/local/tmp/qwen3-block-htp/exp0224-'
CONTROLS=['A0','C0']
ALL=['A0','A','C0','C']

def remote(v):
    if v in CONTROLS:return '/data/local/tmp/qwen3-block-htp/'+('exp0221-A' if v=='A0' else 'exp0223-C')
    return REMOTE+v

def package_path(v):return control_package(v[0]) if v in CONTROLS else OUTPUT/v

def deploy(variant):
    ev.dataset();root=remote(variant)
    assert variant in VARIANTS
    FROZEN_REMOTE=remote(variant+'0')
...
def run(variant,phase,index=None):
    ...
    write_json(path.with_suffix('.provenance.json'),dict(remote_root=remote(variant),variant=variant,experiment='EXP-0224'))
