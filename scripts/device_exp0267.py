...
def deploy():
 preflight();assert read(R/'export_audit.json')['pass_all'];pm=read(P/'manifest.json');byhash={v['sha256']:n for n,v in pm['files'].items()}
 for first in range(0,len(pm['files']),32):
  names=list(pm['files'])[first:first+32];raw=adb('shell','sha256sum '+' '.join(shlex.quote(BASE+'/'+n) for n in names)).stdout
  got={l.split(None,1)[1].removeprefix(BASE+'/'):l.split()[0] for l in raw.splitlines()};assert all(got[n]==pm['files'][n]['sha256'] for n in names)
 assert adb('shell','sha256sum '+SEED).stdout.split()[0]=='7683237318d42ac5cc80052fb53619205a3d82a0d1377bcbbaed78d7c7683b91'
 for local in [O/'sp2']+[O/f'layer{l}-{a}' for l in [0,14,27] for a in ['u8','sp2']]:
  m=read(local/'manifest.json');dest=REMOTE+'-models/'+local.name;assert adb('shell','test ! -e '+dest,check=False).returncode==0;adb('shell','mkdir -p '+dest);lines=['set -e'];checks=[]
  archive=R/(local.name+'.tar')
  with tarfile.open(archive,'x',dereference=True) as tar:
   for n,v in m['files'].items():
    assert sha(local/n)==v['sha256'];lines.append('mkdir -p '+shlex.quote(str(Path(n).parent)))
    if v['sha256'] in byhash:lines.append('ln -s '+shlex.quote(BASE+'/'+byhash[v['sha256']])+' '+shlex.quote(n))
    else:tar.add(local/n,arcname=n)
    checks.append(v['sha256']+'  '+n)
   tar.add(local/'manifest.json',arcname='manifest.json')
  script=R/(local.name+'-links.sh');script.write_text('\n'.join(lines)+'\n');check=R/(local.name+'-files.sha256');check.write_text('\n'.join(checks)+'\n')
  for p in [archive,script,check]:adb('push',win(p),dest+'/'+p.name)
  raw=adb('shell',f'cd {dest} && sh {script.name} && tar -xf {archive.name} && sha256sum -c {check.name}').stdout;assert raw.count(': OK')==len(m['files']) and 'FAILED' not in raw
  print('PACKAGE',local.name,'VERIFIED',len(m['files']),flush=True)
 write(R/'deployment.json',dict(pass_all=True,base=BASE,prefix=SEED,parent_manifest_sha256=sha(P/'manifest.json'),sp2_manifest_sha256=sha(O/'sp2/manifest.json')))
...
def run(mode,repeat,tag,count=1,layer=0,dump=False):
 ...
 e=dict(old.ENV,LD_LIBRARY_PATH=root,DSP_LIBRARY_PATH=root,ADSP_LIBRARY_PATH=root,QBH_SP2=str(mode),QBH_WIDE_SCORE='4',QBH_DENSE_R3='0',QBH_DENSE_R4='0',QBH_KV_CACHE_CAPACITY='128')
 if count==1:pkg=REMOTE+f'-models/layer{layer}-'+('sp2' if mode else 'u8')
 else:pkg=REMOTE+'-models/sp2' if mode else BASE
 if count==28:
...
def layer_profile(phase):
 ...
 for i in range(n):
  for repeat in [1,10]:
   for mode in ([0,8] if i%2==0 else [8,0]):
    z=run(mode,repeat,f'layer-{phase}/round{i:02d}-r{repeat}-m{mode}');gold=read(R/f'audit-layer0-m{mode}/validated.json');assert z['output_hashes']==gold['output_hashes']*repeat;rows.append(dict(round=i,**z))
 rng=np.random.default_rng(267);idx=rng.integers(0,n,(20000,n));perf={}
 for repeat in [1,10]:
  for key in ['prefill_ns','decode_ns']:
   pair=np.array([[next(z[key] for z in rows if z['round']==i and z['repeat']==repeat and z['mode']==m) for m in [0,8]] for i in range(n)]);ci=np.quantile(pair[idx,1].mean(1)/pair[idx,0].mean(1),[.025,.975]);perf[f'r{repeat}_{key}']=dict(u8_mean_ns=float(pair[:,0].mean()),sp2_mean_ns=float(pair[:,1].mean()),ratio=float(pair[:,1].mean()/pair[:,0].mean()),ci95=ci.tolist(),gate=bool(ci[1]<=1.1),auxiliary_only=repeat==1)
