...
remote_root="${EXP0128_REMOTE_ROOT:-/data/local/tmp/qwen3-block-htp/exp0128-u8}"
cell="${1:-stream32}"
repeat_count="${2:-1}"
...
case "${cell}" in
    stream32)
        ;;
    stream16)
        stream_region_tiles=16
        ;;
    *)
        printf 'usage: %s stream32|stream16 [repeat] [attribution] [audit]\n' "$0" >&2
        exit 2
        ;;
esac
...
"${adb_exe}" shell \
    "cd ${remote_root} && QBH_W4U8_STREAM_FENCE=single_fence QBH_W4U8_GATE_UP_RING_SLOTS=${ring_slots} QBH_W4U8_GATE_UP_STREAM_REGION_TILES=${stream_region_tiles} LD_LIBRARY_PATH=${remote_root} DSP_LIBRARY_PATH=${remote_root} ADSP_LIBRARY_PATH=${remote_root} ./qwen3_block_cli ${remote_root}/block_package_layer14_m64 W4U8 ${repeat_count} ${runtime_args}"
