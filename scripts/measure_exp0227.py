...
REMOTE='/data/local/tmp/qwen3-block-htp/exp0227-'
CONTROLS=['A0','R0']
ALL=['A0','R0','A','R']

def remote(v):
    return '/data/local/tmp/qwen3-block-htp/exp0224-A' if v=='A0' else '/data/local/tmp/qwen3-block-htp/exp0225-step100' if v=='R0' else REMOTE+v

def package_path(v):return OUTPUT.parent/'exp0224/A' if v=='A0' else OUTPUT.parent/'exp0225/step100' if v=='R0' else OUTPUT/v

def deploy(variant):
    ev.dataset();root=remote(variant)
    assert variant in VARIANTS
    FROZEN_REMOTE=remote('A0' if variant=='A' else 'R0')
...
def run(variant,phase,index=None):
    ...
    write_json(path.with_suffix('.provenance.json'),dict(remote_root=remote(variant),variant=variant,experiment='EXP-0227'))
