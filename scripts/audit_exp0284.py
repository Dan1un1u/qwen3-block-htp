...
def head(tag):
 ...
 if 'INT16' in tag:
  mode='fixed' if '-fixed-' in tag else 'greedy';teacher=read(R/(mode+'-int16-teacher.json'));assert codes==teacher['selected_codes'],(codes,teacher['selected_codes'])
  for step in range(16):
   actual=np.fromfile(d/f'generation_hidden_step{step:02d}_f32.bin','<f4');ref=np.fromfile(R/'frontend-reference'/f'{mode}_step{step:02d}_hidden_f32.bin','<f4');assert np.array_equal(actual,ref),(tag,step,int(np.count_nonzero(actual!=ref)))
