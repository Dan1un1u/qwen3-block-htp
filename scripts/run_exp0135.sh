...
remote_root="${EXP0135_REMOTE_ROOT:-/data/local/tmp/qwen3-block-htp/exp0135-w4f16}"
cell="${1:-stream3}"
repeat_count="${2:-1}"
attribution_mode="${3:-on}"
audit_mode="${4:-off}"
extra_stream_worker=0

case "${cell}" in
    stream3)
        ;;
    stream4)
        extra_stream_worker=1
        ;;
    *)
        printf 'usage: %s stream3|stream4 [repeat] [attribution] [audit]\n' "$0" >&2
        exit 2
        ;;
esac
...
"${adb_exe}" shell \
    "cd ${remote_root} && QBH_QKV_SCHEDULE=control QBH_W4F16_GROUP_FENCE=join_only QBH_W4F16_EXPAND_CLAIM_REGIONS=1 QBH_W4F16_GATE_UP_EXTRA_EXPAND_WORKER=1 QBH_W4F16_GATE_UP_EXTRA_STREAM_WORKER=${extra_stream_worker} LD_LIBRARY_PATH=${remote_root} DSP_LIBRARY_PATH=${remote_root} ADSP_LIBRARY_PATH=${remote_root} ./qwen3_block_cli ${remote_root}/block_package_layer14_m64 W4F16 ${repeat_count} ${runtime_args}"
