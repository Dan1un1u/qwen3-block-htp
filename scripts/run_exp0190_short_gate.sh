...
"${project_root}/scripts/build_exp0190.sh" > "${result_dir}/build.log" 2>&1
"${project_root}/scripts/check_exp0190_static.sh" > "${result_dir}/static_gate.json"
"${project_root}/scripts/deploy_exp0190.sh" > "${result_dir}/deploy.log" 2>&1

for cell in batch4 batch8; do
    QBH_EXP0173_GENERATION_STEPS=4 \
    QBH_GENERATION_BOUNDARY_AUDIT=1 \
    QBH_GENERATION_AUDIT_DIR="${result_dir}/audit/${cell}" \
        "${project_root}/scripts/run_exp0190.sh" "${cell}" \
