...
generation_mode="${QBH_GENERATION_MODE:-7}"
...
if [[ "${generation_mode}" != 4 && "${generation_mode}" != 5 && "${generation_mode}" != 6 && "${generation_mode}" != 7 ]]; then
    printf 'QBH_GENERATION_MODE must be 4 (EXP-0165 control), 5 (streaming HMX), 6 (DMA-HVX overlap), or 7 (coarse triple pipeline)\n' >&2
    exit 2
fi
if [[ "${QBH_EXP0166_DEPLOY:-0}" == 1 ]]; then
    "${project_root}/scripts/deploy_exp0166.sh"
fi
