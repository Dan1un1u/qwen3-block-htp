...
REMOTE='/data/local/tmp/qwen3-block-htp/exp0226-'
CONTROLS=['A0','old100']
ALL=['A0','old100',SELECTED]

def remote(v):
    return '/data/local/tmp/qwen3-block-htp/exp0224-A' if v=='A0' else '/data/local/tmp/qwen3-block-htp/exp0225-step100' if v=='old100' else REMOTE+v

def package_path(v):return OUTPUT.parent/'exp0224/A' if v=='A0' else OUTPUT.parent/'exp0225/step100' if v=='old100' else OUTPUT/v
...
def run(variant,phase,index=None):
    ...
    write_json(path.with_suffix('.provenance.json'),dict(remote_root=remote(variant),variant=variant,experiment='EXP-0226'))
