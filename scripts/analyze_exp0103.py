#!/usr/bin/env python3
"""Validate EXP-0103 and render its PC-027/PC-028 closure."""
...
PHYSICAL = tuple(dict.fromkeys((
    *base.COUNTERS, *base.RESOURCES, *exp84.PHYSICAL,
    ...
    "w4u8_qk_interleaved_pair_count",
...
def build_summary(result_dir: Path, package_dir: Path) -> dict[str, object]:
    if (result_dir / "boot_id_before.txt").read_bytes() != \
        ...
        raise SystemExit("device boot ID changed during EXP-0103")
    ...
    for mode in MODES:
        ...
        correctness[mode] = {
            ...
            "candidate_pair_count": record["w4u8_qk_interleaved_pair_count"],
        }
    ...
    for repeat in REPEATS:
        ...
        metrics = metric_set(records["control"], records["interleaved"])
        speed = all(
    ...
    selected = "interleaved" if local_gate else "control"
    return {
        "experiment": "EXP-0103",
        "control": "EXP-0084 serial-inner Q/K paired task",
        "candidate": "spill-free shared-invariant Q/K paired task",
        ...
        "pc028": {
            ...
            "w4u8_candidate": module_medians(records_by_repeat[10]["interleaved"]),
            "w4u8_provenance": (
                "EXP-0103 candidate" if local_gate else "EXP-0103 control"
            ),
...
def render_report(summary: dict[str, object]) -> str:
    lines = ["# EXP-0103 — Complete profiling report", ""]
    add_pc028(lines, summary)
    lines.extend([
        "The first simultaneous two-head prototype was rejected statically: "
        "it generated a 3.5 KiB vector stack frame. The measured candidate "
        "keeps the exact paired task and per-head arithmetic order, but hoists "
        "shared input-zero-point and output-encoding setup and compiles to a "
        "540-byte helper with no stack frame, versus 704 bytes for control.", "",
    ])
    for repeat in REPEATS:
    ...
    lines.extend([
        ...
        "## Decision", "",
        f"EXP-0103 local gate: **{'PASS' if summary['local_gate_pass'] else 'FAIL'}**. "
