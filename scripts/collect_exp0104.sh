...
package="${QBH_EXP0104_PACKAGE:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0042/block_package_layer14_m64_integer_attention_parallel}"
result_root="${QBH_EXP0104_RESULT_ROOT:-/mnt/d/llm_exp/results/qwen3-block-htp/exp0104}"
artifact_root="${QBH_EXP0104_ARTIFACT_ROOT:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0104/artifacts}"
...
if ! git -C "${project_root}" diff --quiet || \
   ...
   [[ -n "$(git -C "${project_root}" ls-files --others --exclude-standard)" ]]; then
    printf 'source worktree must be clean before EXP-0104 formal collection\n' >&2
    exit 1
fi
...
if [[ -e "${result_dir}" || -e "${artifact_dir}" ]]; then
    printf 'EXP-0104 evidence destination already exists\n' >&2
    exit 1
fi
mkdir -p "${result_dir}" "${artifact_dir}"
"${project_root}/scripts/build_exp0104.sh" > "${result_dir}/build.log" 2>&1
"${project_root}/scripts/check_exp0104_static.sh" \
    > "${result_dir}/static_gate.json"
QBH_EXP0104_PACKAGE="${package}" \
    "${project_root}/scripts/deploy_exp0104_block.sh" \
...
for mode in "${modes[@]}"; do
    ...
    "${project_root}/scripts/run_exp0104.sh" "${mode}" 1 on on > "${raw}"
...
for ((round = 1; round <= rounds; ++round)); do
    printf 'EXP-0104 round %d/%d\n' "${round}" "${rounds}"
    if ((round % 2 == 1)); then
        repeats=(1 10)
        order=(control quarter)
    else
        repeats=(10 1)
        order=(quarter control)
    fi
    for repeat in "${repeats[@]}"; do
        for mode in "${order[@]}"; do
            raw="${result_dir}/round_${round}_${mode}_r${repeat}_raw.jsonl"
            "${project_root}/scripts/run_exp0104.sh" \
...
python3 "${project_root}/scripts/analyze_exp0104.py" \
...
cp "${project_root}/hexagon_ReleaseG_toolv19_v79/ship/libqwen3_probe_skel.so" \
    "${artifact_dir}/"
{
    printf 'experiment=EXP-0104\nsource_head=%s\n' "${source_head}"
    printf 'control=serial_inner_two_head_qk_kernel\n'
    printf 'candidate=quarter_tiled_interleaved_two_head_qk_kernel\n'
