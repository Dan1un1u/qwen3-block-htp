...
grep -q '#define QBH_BLOCK_EXPERIMENT UINT32_C(209)' include/block_protocol.h
...
grep -q 'QBH_W4U8_DECODE_DIRECT_N_GATE_UP_SWIGLU_STREAM' \
    src/host/block_main.c scripts/run_exp0173.sh scripts/run_exp0209.sh
grep -q 'QBH_W4U8_DECODE_DIRECT_N_QKV_BATCH_N_TILES=16' scripts/run_exp0209.sh
grep -q 'QBH_W4U8_DECODE_DIRECT_N_Q_BATCH_N_TILES="${q_batch}"' scripts/run_exp0209.sh
grep -q 'QBH_EXP0176_O_BATCH_TILES=8' scripts/run_exp0209.sh
...
bash -n scripts/build_exp0209.sh scripts/deploy_exp0209.sh \
    scripts/run_exp0209.sh scripts/run_exp0209_short_gate.sh \
    scripts/run_exp0209_formal.sh scripts/check_exp0209_static.sh
python3 -m py_compile scripts/summarize_exp0189.py scripts/summarize_exp0209.py
printf '%s\n' '{"experiment":"EXP-0209","static_gate":"pass","candidate":"direct-W4-Q32-KV16","qnn_dependency":false}'
