#!/usr/bin/env python3
"""Frozen absmax GPTQ versus transformer-block scale reconstruction candidates, actual DSP evidence."""
import argparse
import json
import math
...
def quality():
    ...
    entries={'control_A':old['samples']['selected_A'],'control_R':json.loads(old225.read_text())['samples']['selected_step100']};determinism={};software={}
    for v in VARIANTS:
    ...
    out=dict(experiment='EXP-0227',summary=summaries,samples=entries,software_diagnostic={k:score(rows,teacher) for k,rows in software.items()},
        determinism=determinism,effectiveness={v:improved(summaries['selected_'+v],summaries['control_A' if v=='A' else 'control_R']) for v in VARIANTS},
        incremental_rotation=improved(summaries['selected_R'],summaries['selected_A']),profile_candidate=chosen,
    ...
    labels=['A0 EXP0224（冻结）','R0 EXP0225 step100（冻结）','A 原坐标 block 重建','R 固定学习旋转 block 重建']
...
def speed():
    ...
    paired={v:{mode:100*(median(a['host_us']/b['host_us'] for a,b in zip(data['A0' if v=='A' else 'R0'][mode],data[v][mode]))-1) for mode in ['prefill','decode']} for v in VARIANTS}
    ...
    for v in ['R0',*VARIANTS]:
        for mode in ['prefill','decode']:
            ...
            report.extend([f'## {control} versus {v} {mode}',table(['Field','R1 original','R1 candidate','R1 change','R10 original median','R10 candidate median','R10 change'],rows)])
