#!/usr/bin/env python3
"""Validate and report EXP-0113 W4U8 generation-release elision."""
...
EXP0109_FORMAL = exp112.EXP0109_FORMAL
EXP0111_FORMAL = exp112.EXP0111_FORMAL
TARGETS = exp112.TARGETS
W4U8_PIPELINE = exp112.W4U8_PIPELINE
...
def validate_record(record: dict, repeat: int, cell: str,
    ...
    base.require(record, "experiment", "EXP-0113")
    base.require(
        record, "w4u8_stream_fence_mode",
        "single_fence" if cell == "control" else "barrier_only",
    )
...
def build_summary(result_dir: Path, exp0109_dir: Path,
    ...
    if ((result_dir / "boot_id_before.txt").read_bytes() !=
        ...
        raise SystemExit("device boot ID changed during EXP-0113")
    ...
    for repeat in REPEATS:
        ...
        values = exp112.metrics(sides["control"], sides["candidate"])
        comparisons[f"repeat{repeat}"] = values
        physical.append(exp112.physical_equal(
...
def render_report(summary: dict) -> str:
    lines = ["# EXP-0113 — Complete profiling report", ""]
    exp112.add_pc028(lines, {
        **summary,
        "repeat10_selected_modules":
            summary["repeat10_candidate_modules"],
    })
    for repeat in REPEATS:
        ...
        exp112.add_table(
            lines, "HMX/HVX/DMA, publication and waits",
            tuple(dict.fromkeys((*exp107.OVERLAP, *W4U8_PIPELINE))),
            values,
        )
    ...
    lines.extend([
        ...
        f"| EXP-0113 local gate | {'PASS' if summary['local_gate_pass'] else 'FAIL'} |",
