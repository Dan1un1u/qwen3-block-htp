...
from device_exp0279 import adb,win,records
...
def full(fp,repeat,tag,audit=False):
 ...
 fixed=read(R/'fixed_tokens.json')['ids'];row=[0,2,34]+ids+fixed;f=d/'eval.bin';f.write_bytes(struct.pack('<4I',0x51424556,1,repeat,83)+b''.join(struct.pack('<83I',i,*row[1:]) for i in range(repeat)));remote_eval=root+'/'+tag.replace('/','_')+'.bin';adb('push',win(f),remote_eval);e['QBH_EVAL_FILE']=remote_eval
 if audit:
...
def validate(d,fp,repeat,audit=False):
 ...
 rs=records(d/'stdout.jsonl');ps=[v for v in rs if v.get('record')=='generation_profile'];assert len(ps)==34*repeat,(tag,len(ps))
 ...
 for step,v in enumerate(ps):
  assert v['generation_step']==step%34 and v['valid_length']==64+step%34 and v['logical_m']==(64 if step%34==0 else 1)
  ...
  for l in range(28):
   ...
   assert a['cache_valid_before']==(0 if step%34==0 else 63+step%34) and a['cache_valid_after']==64+step%34
 fs=[x for x in rs if x.get('generation_sequence_complete')];assert len(fs)==repeat and all(x['all_steps_pass'] for x in fs)
 codes=[(x['selected_token_id'],x['selected_logit_half_bits']) for x in rs if 'selected_logit_half_bits' in x];assert len(codes)==34*repeat and all(v==codes[i%34] for i,v in enumerate(codes))
 for i,x in enumerate(fs):assert x['total_host_wall_ns']==sum(v['host_wall_ns'] for v in ps[i*34:i*34+34])
 out=dict(pass_all=True,fp32=fp,repeat=repeat,audit=audit,profiles=len(ps),peak=max(v['vtcm_peak_plan_bytes'] for v in ps),prefill_ns=statistics.mean(v['host_wall_ns'] for v in ps if v['mode']=='prefill'),decode_ns=statistics.mean(v['host_wall_ns'] for v in ps if v['mode']=='decode'),selected_codes=[list(v) for v in codes[:34]])
