...
from data_exp0229 import BASE,SOURCE,MEMORY,MODEL,CELLS,sha,verified,preflight,hashed_grams
from data_exp0233 import prior_data as old_prior,ids_of
from gptq_exp0221 import fetch
RESULT=BASE/'exp0236'
OUTPUT=Path('/mnt/d/llm_exp/models/qwen3-block-htp/exp0236')
GROOT=OUTPUT.parent/'exp0234/G64'
CHECKPOINT=OUTPUT.parent/'exp0234/checkpoints/G64/layer27_hidden.npy'
def write(n,x):
 p=RESULT/n;p.parent.mkdir(parents=True,exist_ok=True)
 with p.open('x') as f:json.dump(x,f,indent=2,ensure_ascii=False);f.write('\n')
def prior_data():
 paths,ds,rows=old_prior();p=verified('exp0233','dataset.json');d=json.loads(p.read_text())
 return paths+[p],ds+[d],rows+d['samples']
def frozen():
 f=json.loads((RESULT/'dataset_freeze.json').read_text())
 assert sha(MEMORY/'docs/experiments/EXP-0236.md')==f['protocol_sha256']
...
def prepare():
 ...
 rng=np.random.default_rng(236);chosen={c:[] for c in CELLS};used=set();grams=set();hashes=set();sources=[]
 ...
 for lang in ['en','zh']:
  ...
  select(cell,rows,p);assert len(chosen[cell])==256;print('DATA_CELL_READY',cell,256,flush=True)
 for lang in ['en','zh']:
  ...
  for p in sorted(pool.values()):
   ...
   if len(chosen[cell])==256:break
  if len(chosen[cell])<256:
   from urllib.parse import urlencode
   for off in range(46000,54000,100):
    ...
    if len(chosen[cell])==256:break
  assert len(chosen[cell])==256,(cell,len(chosen[cell]));print('DATA_CELL_READY',cell,256,flush=True)
 samples=[]
 for i in range(256):
  for c in CELLS:
   r=chosen[c][i];r.update(id=len(samples),split='final',prompt_ids=r['token_ids'][:64],target_ids=r['token_ids'][64:]);samples.append(r)
 write('dataset.json',dict(version='qbh-EXP236-independent-v1',seed=236,cells=CELLS,samples=samples,provenance=sources,tokenizer_sha256=sha(MODEL/'qwen3-tokenizer.json'),calibration_reference='EXP230_C64_65536',final_targets=16384,prior_paths=[str(p) for p in paths]))
 ledger=json.loads(verified('exp0234','artifacts_sha256.json').read_text());cp=ledger['files'][str(CHECKPOINT.relative_to(Path(ledger['root'])))];assert sha(CHECKPOINT)==cp['sha256']
 package=verified('exp0234','G64/package.json');assert sha(GROOT/'manifest.json')==json.loads(package.read_text())['manifest_sha256']
 refs=paths+[wiki,package,verified('exp0234','artifacts_sha256.json'),CHECKPOINT,GROOT/'manifest.json',verified('exp0230','inputs/C64_calibration_u32.bin'),verified('exp0218','original_checkpoint_sha256.json')]
 write('dataset_freeze.json',dict(protocol_sha256=sha(MEMORY/'docs/experiments/EXP-0236.md'),files={'dataset.json':sha(RESULT/'dataset.json')},references={str(p):sha(p) for p in refs},frozen_before_export_and_scoring=True))
...
def audit():
 ...
 write('independent_data_audit.json',dict(pass_all=True,documents=1024,targets=16384,direct_tuple32_disjoint=True,all_windows_reconstructed=True,dataset_sha256=sha(RESULT/'dataset.json')));print('DATA_AUDIT_PASS',flush=True)
