...
from device_exp0273 import adb,win,records
...
def full(fp,repeat,tag,audit=False):
 preflight();assert read(R/'prerequisite_provenance.json')['inherited_exact_prerequisites_verified'];state=read(R/'runtime-l28.json');root=state['remote'];d=R/tag;d.mkdir(parents=True,exist_ok=False)
