#!/usr/bin/env python3
"""Validate EXP-0074 staged row-sliced W4U8 Softmax scheduling."""
...
def pipeline(mode: str) -> str:
    ...
    return stem + ("_staged_softmax" if mode == "candidate" else "")
...
def build_summary(result_dir: Path, package_dir: Path) -> dict[str, object]:
    ...
    for repeat in REPEATS:
        ...
        passed = speed_gate and invariant_gate and slicing_gate
        ...
        repeat_results[f"repeat{repeat}"] = {
            ...
            "sixteen_row_slice_task_gate": slicing_gate,
        }

    return {
        "experiment": "EXP-0074",
        "control": "EXP-0072 eight coarse full-GQA Attention tasks",
        "candidate": "QK / sixteen 32-row Softmax tasks / AV stages",
...
def add_table(lines: list[str], title: str, fields: tuple[str, ...],
    ...
    lines.extend([
        f"### {title}", "",
        "| Metric | coarse control | staged candidate | Delta | Paired delta |",
        "|---|---:|---:|---:|---:|",
    ])
    for field in fields:
...
def add_pc028_table(lines: list[str], summary: dict[str, object]) -> None:
    ...
    used = "EXP-0074" if summary["local_gate_pass"] else "EXP-0072"
    lines.extend([
...
def render_report(summary: dict[str, object]) -> str:
    lines = ["# EXP-0074 — Complete profiling report", ""]
    add_pc028_table(lines, summary)
    lines.extend([
        "The control runs eight coarse full-GQA Attention tasks. The "
        "candidate stages all QK work, sixteen independent 32-row paired-"
        "head Softmax tasks, and all AV work across the same six contexts. "
        "Each context owns one VTCM Softmax scratch bank; arithmetic, tensor "
        "bytes, HMX commands and tile pairs remain unchanged.", "",
    ])
    for repeat in REPEATS:
        ...
        lines.extend([
            ...
            f"sixteen row-slice tasks: **{'PASS' if result['sixteen_row_slice_task_gate'] else 'FAIL'}**.",
            "",
        ])
    lines.extend([
        ...
        "## Decision", "",
        f"EXP-0074 local gate: **{'PASS' if summary['local_gate_pass'] else 'FAIL'}**. "
