...
result_root="${QBH_RESULT_ROOT:-/mnt/d/llm_exp/results/qwen3-block-htp/exp0034}"
artifact_root="${QBH_ARTIFACT_ROOT:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0034/artifacts}"
...
"${project_root}/scripts/build_exp0034.sh" > "${result_dir}/build.log" 2>&1
QBH_STATIC_OUTPUT_DIR="${result_dir}/static" \
    "${project_root}/scripts/check_exp0034_static.sh" \
...
configs=(f16_control w4_control)
...
run_config() {
    ...
    case "${config}" in
        f16_control)
            "${project_root}/scripts/run_exp0034_block.sh" \
...
for round in 1 2 3 4 5; do
    if (( round % 2 == 1 )); then
        order=(f16_control w4_control)
    else
        order=(w4_control f16_control)
    fi
...
python3 "${project_root}/scripts/validate_exp0034_evidence.py" \
