#!/usr/bin/env python3
"""Validate and summarize EXP-0198 continuous Gate-to-Up ring evidence."""
...
def validate_run(run: dict[str, object], cell: str, steps: int) -> None:
    ...
    for index, profile in enumerate(profiles):
        ...
        base.require_equal(
            ...
            32, context + " Gate/Up batch")
        base.require_equal(
            int(profile["w4u8_decode_direct_n_gate_up_continuous"]),
            enabled, context + " continuous selector")
...
def main() -> None:
    ...
    report = [
        "# EXP-0198 continuous direct-n Gate-to-Up ring report",
        "",
        f"Source commit: `{args.source_commit}`",
        "",
        "Both cells use the EXP-0196 packed-W4 direct-n batch32 kernels. "
        "The candidate flattens the three Gate and three Up groups into one "
        "continuous ping-pong ring, overlapping the first Up DMA with the "
        "final Gate HMX command.",
        "",
        "## End-to-end gate",
        "",
        "| Metric | Independent control | Continuous candidate | Change |",
        "|---|---:|---:|---:|",
        ...
        *pair_rows,
        ...
        "",
        "| Module | Independent control | Continuous candidate | Candidate speed change |",
        "|---|---:|---:|---:|",
        *module_rows,
        ...
        f"{gate_up[CANDIDATE]:.1f} us/token.",
        "",
