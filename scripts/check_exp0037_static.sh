...
grep -q 'EXP-0037' "${output_dir}/host.strings.txt"
grep -q 'q_batch4' "${output_dir}/host.strings.txt"
grep -q 'qkv_batch4' "${output_dir}/host.strings.txt"
...
required = (
    "QBH_BLOCK_EXPERIMENT UINT32_C(37)",
    "QBH_BLOCK_ABI_VERSION UINT32_C(18)",
    "QBH_BLOCK_Q_BATCH4",
    "QBH_BLOCK_QKV_BATCH4",
    "qbh_qkv_projection_batch_tiles(",
    "qkv_hmx_batch_n_tiles",
    "qkv_hmx_command_count",
    "qkv_projection_wall_ticks",
    "qkv_batch_mode",
...
for fragment in required:
    if fragment not in combined:
        raise SystemExit(f"missing EXP-0037 contract fragment: {fragment}")
...
printf '{"experiment":"EXP-0037","stage":"A",'
printf '"q_only_batch4":true,"fixed_vtcm_request_bytes":8388608,'
