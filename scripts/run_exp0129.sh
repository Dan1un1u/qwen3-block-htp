...
remote_root="${EXP0129_REMOTE_ROOT:-/data/local/tmp/qwen3-block-htp/exp0129-w4f16}"
cell="${1:-worker3}"
repeat_count="${2:-1}"
...
"${adb_exe}" shell \
    "cd ${remote_root} && QBH_QKV_SCHEDULE=control QBH_W4F16_GROUP_FENCE=join_only LD_LIBRARY_PATH=${remote_root} DSP_LIBRARY_PATH=${remote_root} ADSP_LIBRARY_PATH=${remote_root} ./qwen3_block_cli ${remote_root}/block_package_layer14_m64 W4F16 ${repeat_count} ${runtime_args}"
