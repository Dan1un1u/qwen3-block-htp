#!/usr/bin/env python3
"""Validate paired EXP-0046 Stage-B native MLP-output evidence."""
...
SAMPLES = 7
REPEATS = (1, 10)
CONTROL = "w4u8_mlp_input"
CANDIDATE = "w4u8_mlp_io"
...
def validate_record(record: dict[str, object], repeat: int, mode: str) -> None:
    fixed = {
        ...
        "w4u8_mlp_output_unpack_skipped": repeat if mode == CANDIDATE else 0,
        "release_result": 0,
        "close_result": 0,
    }
    ...
    if int(record["w4u8_mlp_input_pack_ticks"]) != 0:
        raise SystemExit("Stage B retained W4U8 MLP input pack work")
    output_unpack_ticks = int(record["w4u8_mlp_output_unpack_ticks"])
    if mode == CONTROL and output_unpack_ticks <= 0:
        raise SystemExit("control did not execute the W4U8 MLP output unpack")
    if mode == CANDIDATE and output_unpack_ticks != 0:
        raise SystemExit("candidate retained W4U8 MLP output unpack work")
    for field in (
...
def derived(record: dict[str, object], field: str) -> float:
    if field == "mlp_output_boundary_ticks":
        return sum(
...
def compare(
    ...
    result["stage_b_speed_gate_pass"] = all(
        ...
        for field in (
            ...
            "mlp_output_boundary_ticks_change_percent",
            "paired_mlp_output_boundary_ticks_change_percent_median",
        )
    )
    return result
...
def main() -> None:
    ...
    for repeat in REPEATS:
        ...
        comparison = compare(records[CONTROL], records[CANDIDATE])
        gates.append(bool(comparison["stage_b_speed_gate_pass"]))
        summary["repeat_results"][f"repeat{repeat}"] = comparison

    summary["stage_b_speed_gate_pass"] = all(gates)
    summary["stage_b_gate_pass"] = all(gates)
