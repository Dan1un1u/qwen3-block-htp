...
REMOTE='/data/local/tmp/qwen3-block-htp/exp0288'
...
def run(package,tag,count=1,repeat=1,fp32=2,dump=False):
 ...
 if dump and fp32:
  errs=[]
  for step in range(2):
   rows=64 if not step else 1;a=np.fromfile(d/f'step{step:02d}_output.bin','<f4').reshape(rows,1024);b=np.fromfile(package_path(package)/('reference_w4u8_block_output_f32.bin' if not step else 'replay_decode_reference_00_f32.bin'),'<f4').reshape(64,1024)[:rows];delta=a.astype('f8')-b
...
if __name__=='__main__':
 p=argparse.ArgumentParser();p.add_argument('action');p.add_argument('--count',type=int,default=1);p.add_argument('--package',default='layer0-fp32-a01');p.add_argument('--tag',default='layer0-a01');p.add_argument('--repeat',type=int,default=1);p.add_argument('--fp32',type=int,default=2);p.add_argument('--dump',action='store_true');a=p.parse_args()
