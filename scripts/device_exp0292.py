"""EXP0292 provenance-sealed Qwen3-0.6B A16 device checks."""
import argparse,subprocess,shutil,shlex,statistics,json,struct,os
import numpy as np
from common_exp0289 import *
O=Path("/mnt/d/llm_exp/models/qwen3-block-htp/exp0292")
R=Path("/mnt/d/llm_exp/results/qwen3-block-htp/exp0292")
def preflight():
...
def run(package,tag,count=1,repeat=1,audit=True,full=False,greedy=False,opt=2,f16opt=4,fp32=1,component=False):
 ...
 e=dict(QBH_FP32_RESIDUAL=str(fp32),LD_LIBRARY_PATH=root,DSP_LIBRARY_PATH=root,ADSP_LIBRARY_PATH=root,QBH_VERTICAL_SLICE='1',QBH_REPLAY_SEQUENCE='1',QBH_SCAN_MODE='prefill',QBH_LOGICAL_M='64',QBH_KV_CACHE_LENGTH='0',QBH_KV_CACHE_CAPACITY='128',QBH_KV_CACHE_LAYOUT='hmx_native_f16',QBH_F16F16_OPT=str(f16opt),QBH_W4F16_DECODE_OPT=str(opt),QBH_REPLAY_DECODE_STEPS='1')
 args=ARGS
 if recipe=='W4F16':
