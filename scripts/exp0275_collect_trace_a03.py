...
from device_exp0275 import old,run,adb
from selected_audit_exp0275 import audit
r=e.R
assert e.read(r/'runtime-l1.json')['seal']['paper_trace'] is True
e.preflight()
state=e.read(r/'runtime-l1.json')
farf=state['remote']+'/qwen3_block_cli.farf'
assert adb('shell','test ! -e '+farf,check=False).returncode==0
adb('shell','touch '+farf)
e.write(r/'trace_log_transport_configuration_a03.json',dict(cause='SDK static FARF requires process-name .farf in DSP_LIBRARY_PATH to route to logcat',remote_farf=farf,configuration='empty file; diagnostic runtime only',production_timing_affected=False))
for a in e.ARMS:
 e.setarm(a);tag=f'trace-{a}-a03';log=r/f'{tag}-logcat.txt'
