#!/usr/bin/env python3
"""Frozen RTN versus three per-channel GPTQ candidates, actual DSP evidence."""
import argparse
import json
import math
...
from experiment_exp0221 import RESULT,VARIANTS
...
def quality():
    ...
    entries={'RTN_original':old['samples']['A'],'RTN_fold':old['samples']['B'],'RTN_R1R2':old['samples']['C']};determinism={};software={}
    for v in VARIANTS:
        ...
        for s in full['samples']:
            ...
            rows.append(r)
        entries['GPTQ_'+v]=rows;mismatches=[]
    ...
    summaries={k:score(rows,teacher) for k,rows in entries.items()};a=summaries['GPTQ_A'];c=summaries['GPTQ_C'];base=summaries['RTN_original']
    improved=lambda x,y:x['nll']<y['nll'] and x['tasks_correct']>y['tasks_correct']
    chosen=min(VARIANTS,key=lambda v:(summaries['GPTQ_'+v]['nll'],-summaries['GPTQ_'+v]['tasks_correct'],v))
    out=dict(experiment='EXP-0221',summary=summaries,samples=entries,software_diagnostic={k:score(rows,teacher) for k,rows in software.items()},
        determinism=determinism,A_effectiveness=improved(a,base),C_incremental_rotation=improved(c,a),profile_candidate=chosen,
        calibration_sha256=ev.digest(RESULT/'calibration.json'),dataset_sha256=ev.digest(ev.ROOT/'dataset_v1.json'),holdout_used=False,baseline_promoted=False)
    write_json(RESULT/'quality_summary.json',out)
    labels=['RTN 原始（冻结）','RTN gamma 折叠（冻结）','RTN R1/R2（冻结）','GPTQ 原始 A','GPTQ gamma 折叠 B','GPTQ R1/R2 C']
...
def speed():
    ...
    paired={v:{mode:100*(median(a['host_us']/b['host_us'] for a,b in zip(data['original'][mode],data[v][mode]))-1) for mode in ['prefill','decode']} for v in VARIANTS}
    ...
    for v in VARIANTS:
        for mode in ['prefill','decode']:
            ...
            report.extend([f'## Original versus {v} {mode}',table(['Field','R1 original','R1 candidate','R1 change','R10 original median','R10 candidate median','R10 change'],rows)])
