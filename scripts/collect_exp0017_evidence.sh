...
set -u
result_root="${QBH_RESULT_ROOT:-/mnt/d/llm_exp/results/qwen3-block-htp/exp0017}"
artifact_root="${QBH_ARTIFACT_ROOT:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0017}"
...
"${project_root}/scripts/build_exp0017.sh" > "${result_dir}/build.log" 2>&1
...
storages=(
    expanded_s8_control packed_w4_hvx_prescale packed_w4_hmx_postscale
    expanded_s8_control packed_w4_hvx_prescale packed_w4_hmx_postscale
)
projections=(gate_up_pair gate_up_pair gate_up_pair down down down)
plans=(
    expanded_s8_dma_chain2 slots8e7_chunk64_dma_chain4 slots8e7_chunk64_dma_chain4
    exp0005_full_bundle_control slots4_chunk96_dma_batch2 slots4_chunk96_dma_batch2
)
...
run_one() {
    ...
    local output="$4"
    if [[ "${deployed}" == "0" ]]; then
        "${project_root}/scripts/run_exp0017.sh" \
            "${storages[index]}" "${projections[index]}" "${pattern}" \
            "${repeat_count}" "${plans[index]}" linked_2d_dma \
            prepared_session single_invocation | tee -a "${output}"
        deployed=1
    else
        QBH_SKIP_DEPLOY=1 "${project_root}/scripts/run_exp0017.sh" \
            "${storages[index]}" "${projections[index]}" "${pattern}" \
            "${repeat_count}" "${plans[index]}" linked_2d_dma \
            prepared_session single_invocation | tee -a "${output}"
    fi
...
for pattern in "${patterns[@]}"; do
    for index in "${!plans[@]}"; do
        run_one "${index}" "${pattern}" 1 "${result_dir}/correctness_repeat1.jsonl"
    done
done
...
run_timing_matrix() {
    ...
    for round in 0 1 2 3 4; do
        for pattern_index in "${!patterns[@]}"; do
            local start=$(((round + pattern_index) % ${#plans[@]}))
...
python3 "${project_root}/scripts/validate_exp0017_evidence.py" \
