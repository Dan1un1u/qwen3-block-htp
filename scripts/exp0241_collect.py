...
def main():
 ...
 gold={cell:{x['replay_step']:x['output_hash'] for x in load(tag)} for cell,tag in [('control','smoke_control'),('direct','smoke_direct'),('lpbq32','smoke_lpbq')]}
 for f in (d.RESULT/'smoke_unit').glob('*.bin'):
  assert f.read_bytes()==(d.RESULT/'smoke_control'/f.name).read_bytes(),f.name
 for f in (d.RESULT/'smoke_direct').glob('*.bin'):
