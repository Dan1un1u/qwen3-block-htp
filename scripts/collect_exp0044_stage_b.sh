...
package="${QBH_EXP0044_PACKAGE:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0042/block_package_layer14_m64_integer_attention_parallel}"
result_root="${QBH_EXP0044_RESULT_ROOT:-/mnt/d/llm_exp/results/qwen3-block-htp/exp0044}"
artifact_root="${QBH_EXP0044_ARTIFACT_ROOT:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0044/artifacts}"
...
"${project_root}/scripts/build_exp0044.sh" \
    > "${result_dir}/build.log" 2>&1
"${project_root}/scripts/check_exp0044_static.sh" \
    > "${result_dir}/static_gate.json"
QBH_EXP0044_PACKAGE="${package}" \
    "${project_root}/scripts/deploy_exp0044_block.sh" \
...
QBH_EXP0044_PACKAGE="${package}" \
    "${project_root}/scripts/audit_exp0044_attention.sh" \
    "${result_dir}/attention_implementation_audit" candidate \
    > "${result_dir}/attention_audit.log"

for repeat in 1 10; do
    for mode in original stage_a candidate; do
        : > "${result_dir}/paired_${mode}_r${repeat}.jsonl"
    done
done

for round in $(seq 1 "${paired_rounds}"); do
    printf 'EXP-0044 Stage B paired round %d/%d\n' \
        "${round}" "${paired_rounds}"
    if (( round % 2 == 1 )); then
        repeat_order=(1 10)
        mode_order=(original stage_a candidate)
    else
        repeat_order=(10 1)
        mode_order=(candidate stage_a original)
    fi
    for repeat in "${repeat_order[@]}"; do
        for mode in "${mode_order[@]}"; do
            "${project_root}/scripts/run_exp0044_block.sh" \
                "${mode}" "${repeat}" on off \
                >> "${result_dir}/paired_${mode}_r${repeat}.jsonl"
        done
    done
...
python3 "${project_root}/scripts/validate_exp0044_stage_b.py" \
...
{
    printf 'experiment=EXP-0044\n'
    printf 'stage=B\n'
    printf 'source_branch=%s\n' "$(git -C "${project_root}" branch --show-current)"
    printf 'source_head=%s\n' "${source_head}"
    printf 'execution_unit=qwen3_layer14_complete_block_m64\n'
    printf 'original=EXP0042_integer_Attention\n'
    printf 'stage_a=fused_K_Norm_RoPE_carrier_bias\n'
    printf 'candidate=generation_safe_QK_prep_overlap_with_QKV_projection\n'
    ...
    printf 'audit_dump_ddr_is_diagnostic_only=true\n'
    printf 'source_package_experiment=EXP-0042\n'
    printf 'package=%s\n' "${package}"
    printf 'result_dir=%s\n' "${result_dir}"
