...
M=Path('/mnt/d/llm_exp/models/qwen3-block-htp');R=Path('/mnt/d/llm_exp/results/qwen3-block-htp/exp0277');O=M/'exp0277'
def sha(p):
 h=hashlib.sha256()
...
def verify(p,expected):
 ...
 return m
def main():
 out=subprocess.check_output(['python3','/home/daniuniu/work/qwen3-block-htp-project-memory/scripts/project_memory.py','preflight','--source-worktree',str(S)],text=True);assert 'EXPERIMENT=EXP-0277' in out
 base=M/'exp0257/package';bm=verify(base,'ab2442027f315797dc7ff8f9c49b0d6bf27a3de3c958e831eddf4e1953f915bb')
 O.mkdir(exist_ok=False)
 for key,src,count,origlayer in [('layer14-a8',M/'exp0269/layer14-fp32-a01',1,14),('full-a8',M/'exp0271/sp2-fp32',28,0)]:
  cfg=read(R.parent/'exp0276'/('deployment-layer14-fp32-a01.json' if count==1 else 'deployment-sp2-fp32.json'));m=verify(src,cfg['manifest_sha256']);dst=O/key;dst.mkdir();changed=[]
  for n,v in m['files'].items():
   ...
   if name in ['qparams_u8.bin','silu_up_lut_u16.bin']:
    li=origlayer if count==1 else (int(Path(n).parts[0][5:]) if n.startswith('layer') else 0)
    source=base/f'layer{li}'/name;changed.append(n)
    if name=='qparams_u8.bin':
     a=load_qparams_bin(src/n);b=load_qparams_bin(source);assert set(a)==set(b)
     for k in a:
      if k!='middle':assert a[k]==b[k],(n,k)
     assert a['middle']['minimum']==b['middle']['minimum'] and a['middle']['maximum']==b['middle']['maximum']
  ...
  for step in range(2):
   ...
   for i in range(len(past)):
    x,past[i],d=layer(x,dst/f'layer{i}',cos,sin,past[i],sp2=False)
  ...
  write(dst/'manifest.json',dict(experiment='EXP-0277',recipe='nativeW4 ordinaryA8 FP32residual2',parent=str(src),parent_sha256=sha(src/'manifest.json'),ordinary_middle_parent=str(base),ordinary_parent_sha256=sha(base/'manifest.json'),changed_metadata=changed,new_weight_codes=False,reference_scope='selectedlayer14' if count==1 else 'first3 layers only; full frontend separately checked',files=files))
