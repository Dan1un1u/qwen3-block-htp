...
remote_root="${REMOTE_ROOT:-/data/local/tmp/qwen3-block-htp/exp0014}"
...
remote_log="${remote_root}/run_${storage}_${projection}_${pattern}_${repeat_count}_${physical_plan}_${output_assembly}.log"
if [[ "${QBH_DETACH:-0}" == "1" ]]; then
    "${adb_exe}" shell "rm -f ${remote_log}"
    "${adb_exe}" shell \
        "cd ${remote_root} && (LD_LIBRARY_PATH=${remote_root} DSP_LIBRARY_PATH=${remote_root} ADSP_LIBRARY_PATH=${remote_root} ./qwen3_probe_cli ${storage} ${projection} ${pattern} ${repeat_count} ${physical_plan} ${output_assembly} > ${remote_log} 2>&1 < /dev/null &)"
    printf 'DETACHED_LOG=%s\n' "${remote_log}"
    exit 0
fi

"${adb_exe}" shell \
    "cd ${remote_root} && LD_LIBRARY_PATH=${remote_root} DSP_LIBRARY_PATH=${remote_root} ADSP_LIBRARY_PATH=${remote_root} ./qwen3_probe_cli ${storage} ${projection} ${pattern} ${repeat_count} ${physical_plan} ${output_assembly}"
