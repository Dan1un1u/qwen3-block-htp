...
import numpy as np
R=Path('/mnt/d/llm_exp/results/qwen3-block-htp/exp0250');P=R.parent/'exp0249'
...
for arm,param in [('OFF','A8'),('R3','R3_ALL')]:
 ...
 for layer in range(28):
  ...
  methods={'wide_float':np.floor(ideal*255+.5),'wide_exact':np.where(single,255,exact),'wide_sole':np.where(single,255,sole)};metrics={}
...
with (R/'local_wide_precision.json').open('x') as f:json.dump(out,f,indent=2)
for arm,layers in results.items():
 print(arm,{m:dict(mean_L1=float(np.mean([l['methods'][m]['mean_row_L1'] for l in layers])),mean_mass=float(np.mean([l['methods'][m]['row_mass_mean'] for l in layers]))) for m in ['wide_float','wide_exact','wide_sole']})
