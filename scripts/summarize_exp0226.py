#!/usr/bin/env python3
"""Frozen absmax GPTQ versus transformer-learned R1R2 GPTQ candidates, actual DSP evidence."""
import argparse
import json
import math
...
from learned_rotation_exp0226 import RESULT
from measure_exp0226 import ALL,VARIANTS,SELECTED
...
def quality():
    ...
    previous=RESULT.parent/'exp0224'
    assert ev.digest(previous/'quality_summary.json')=='0ad5f0d13ab63a422e85e8999096e8fb3b3f8b486def1c3ff8ed966745dd9b5a'
    old=json.loads((previous/'quality_summary.json').read_text())
    old225=RESULT.parent/'exp0225/quality_summary.json';assert ev.digest(old225)=='8cc8e996942ec28f92a974db1933f5ed97df55a6f7bcce3f38cfaf5dd7edc96e'
    entries={'control_A':old['samples']['selected_A'],'control_old100':json.loads(old225.read_text())['samples']['selected_step100']};determinism={};software={}
    for v in VARIANTS:
    ...
    chosen=SELECTED  # Already frozen by separate validation before qbh-lite scores.
    out=dict(experiment='EXP-0226',summary=summaries,samples=entries,software_diagnostic={k:score(rows,teacher) for k,rows in software.items()},
        determinism=determinism,effectiveness={v:improved(summaries['selected_'+v],summaries['control_A']) for v in VARIANTS},
        incremental_rotation=improved(summaries['selected_'+chosen],summaries['control_A']),profile_candidate=chosen,
        calibration_sha256=ev.digest(RESULT.parent/'exp0221/calibration.json'),dataset_sha256=ev.digest(ev.ROOT/'dataset_v1.json'),holdout_used=False,baseline_promoted=False)
    write_json(RESULT/'quality_summary.json',out)
    labels=['A EXP0224（冻结）','旧采样 EXP0225 step100','新采样 R1R2 '+SELECTED]
...
def speed():
    ...
    paired={v:{mode:100*(median(a['host_us']/b['host_us'] for a,b in zip(data['A0'][mode],data[v][mode]))-1) for mode in ['prefill','decode']} for v in VARIANTS}
    write_json(RESULT/'speed_summary.json',dict(data=data,times=times,paired_speed_percent=paired,profile_candidate=chosen,formal_invocations=480,formal_layer_ledgers=13440))
    ...
    for v in ['old100',*VARIANTS]:
        for mode in ['prefill','decode']:
            ...
            report.extend([f'## EXP0224 A versus learned {v} {mode}',table(['Field','R1 original','R1 candidate','R1 change','R10 original median','R10 candidate median','R10 change'],rows)])
