...
remote_root="${EXP0170_REMOTE_ROOT:-/data/local/tmp/qwen3-block-htp/exp0170-w4u8}"
generation_steps="${QBH_EXP0170_GENERATION_STEPS:-193}"
softmax_mode="${QBH_EXP0170_SOFTMAX_MODE:-hvx_tile4}"
audit="${QBH_GENERATION_BOUNDARY_AUDIT:-0}"
audit_dir="${QBH_GENERATION_AUDIT_DIR:-}"
remote_audit_dir="${remote_root}/generation-audit-${softmax_mode}"
runtime_env="QBH_W4U8_STREAM_FENCE=single_fence QBH_W4U8_GATE_UP_RING_SLOTS=16 QBH_W4U8_QKV_RING_EXPAND_WORKERS=3 QBH_KV_CACHE_LAYOUT=hmx_native_u8_segmented_v4 QBH_W4U8_PREFILL_CACHE_MODE=reuse QBH_W4U8_DELTA_RECONSTRUCTION=serial QBH_W4U8_DECODE_SOFTMAX=${softmax_mode}"
...
case "${softmax_mode}" in
scalar|hvx_tile4) ;;
*) printf 'invalid EXP-0170 Softmax mode: %s\n' "${softmax_mode}" >&2; exit 2 ;;
esac
if [[ "${QBH_EXP0170_DEPLOY:-0}" == 1 ]]; then
    "${project_root}/scripts/deploy_exp0170.sh"
fi
