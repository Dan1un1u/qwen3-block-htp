...
from data_exp0254 import prior_data as old_prior, verified as previous_verified
from data_exp0233 import ids_of
from gptq_exp0221 import fetch
RESULT=BASE/'exp0255'
def verified(exp,name):
 if exp!='exp0254':return previous_verified(exp,name)
 ledger=BASE/exp/'EVIDENCE_SHA256.json'
 assert sha(ledger)=='0b1a0b0360cb93e0fd2f99fef83b3ba12b065b6a6d682bb0ee871405cfa92ed2'
...
def prior_data():
 paths,ds,rows=old_prior();p=verified('exp0254','dataset.json');d=json.loads(p.read_text())
 return paths+[p],ds+[d],rows+d['samples']
def frozen():
 f=json.loads((RESULT/'dataset_freeze.json').read_text())
 assert sha(MEMORY/'docs/experiments/EXP-0255.md')==f['protocol_sha256']
...
def prepare():
 ...
 rng=np.random.default_rng(255);chosen={c:[] for c in CELLS};used=set();grams=set();hashes=set();sources=[]
 ...
 for lang in ['en','zh']:
  ...
  if len(chosen[cell])<64:
   ...
   for off in range(154000,164000,100):
 ...
 write('dataset.json',dict(version='qbh-EXP255-residual-calibration-two-panels-v1',seed=255,cells=CELLS,samples=samples,provenance=sources,tokenizer_sha256=sha(MODEL/'qwen3-tokenizer.json'),calibration_reference='EXP230_C64_65536',final_targets=4096,prior_paths=[str(p) for p in paths]))
 ...
 write('dataset_freeze.json',dict(protocol_sha256=sha(MEMORY/'docs/experiments/EXP-0255.md'),files={'dataset.json':sha(RESULT/'dataset.json')},references={str(p):sha(p) for p in refs},frozen_before_export_and_scoring=True))
