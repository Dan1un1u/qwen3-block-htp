...
write(R/'ARTIFACT_PROVENANCE.json',dict(source_head=subprocess.check_output(['git','rev-parse','HEAD'],cwd=S,text=True).strip(),source_files={str(p.relative_to(S)):sha(p) for p in files},runtime=rt,native_source_unchanged_since_stage=True,ABI=119,toolchain='SDK6.6 Tools19.0.07 V79 NDKr26c',package_manifest_sha256=sha(O/'r3/manifest.json'),prefix_sha256=a['seed_sha256'],prompts_sha256=sha(R/'prompts.json'),protocol_sha256=sha(M/'docs/experiments/EXP-0258.md'),binaries_and_all_package_files_reverified=True,boot_unchanged=True,numerical_exception='PC072 only understood R3 ideal-reference gate; failed status retained'))
