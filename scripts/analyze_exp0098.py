...
TARGETS = ("down_ticks", "host_wall_ns_per_block")
...
PHYSICAL = tuple(dict.fromkeys((
    *base.COUNTERS, *base.RESOURCES,
    "block_invocation_count", "w4u8_mlp_down_hmx_batch_n_tiles",
...
def build_summary(result_dir: Path, package_dir: Path) -> dict[str, object]:
    ...
    for mode in MODES:
        ...
        correctness[mode] = {
            ...
            "down_commands": record["w4u8_mlp_down_hmx_command_count"],
            "in_command_releases": record[
                "w4u8_mlp_down_in_command_slot_release_count"
            ],
            "producer_progress_commands": record[
                "w4u8_mlp_down_producer_progress_command_count"
            ],
        }
    ...
    for repeat in REPEATS:
        ...
        repeat_results[f"repeat{repeat}"] = {
            "metrics": metrics,
            "down_and_host_speed_gate": speed,
        }
...
def render_report(summary: dict[str, object]) -> str:
    ...
    for repeat in REPEATS:
        ...
        lines.extend([
            "Down and complete Host speed gate: "
            f"**{'PASS' if result['down_and_host_speed_gate'] else 'FAIL'}**.", "",
        ])
    lines.extend([
