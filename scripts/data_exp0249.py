...
from data_exp0246 import prior_data as old_prior, verified as previous_verified
from data_exp0233 import ids_of
from gptq_exp0221 import fetch
RESULT=BASE/'exp0249'
def verified(exp,name):
 if exp!='exp0246':return previous_verified(exp,name)
 ledger=BASE/exp/'EVIDENCE_SHA256.json'
 assert sha(ledger)=='de6b2a5bcd10369a9c0f188ae11af115e04f951fb3f970a1b0a0d5914f9e25dc'
...
def prior_data():
 paths,ds,rows=old_prior();p=verified('exp0246','dataset.json');d=json.loads(p.read_text())
...
def prepare():
 ...
 rng=np.random.default_rng(249);chosen={c:[] for c in CELLS};used=set();grams=set();hashes=set();sources=[]
 ...
 for lang in ['en','zh']:
  ...
  if len(chosen[cell])<64:
   ...
   for off in range(104000,114000,100):
 ...
 write('dataset.json',dict(version='qbh-EXP249-integer-attention-final-v1',seed=249,cells=CELLS,samples=samples,provenance=sources,tokenizer_sha256=sha(MODEL/'qwen3-tokenizer.json'),calibration_reference='EXP230_C64_65536',final_targets=4096,prior_paths=[str(p) for p in paths]))
 ...
 write('dataset_freeze.json',dict(protocol_sha256=sha(MEMORY/'docs/experiments/EXP-0249.md'),files={'dataset.json':sha(RESULT/'dataset.json')},references={str(p):sha(p) for p in refs},frozen_before_export_and_scoring=True))
