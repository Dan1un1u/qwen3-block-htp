...
package="${QBH_EXP0105_PACKAGE:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0042/block_package_layer14_m64_integer_attention_parallel}"
result_root="${QBH_EXP0105_RESULT_ROOT:-/mnt/d/llm_exp/results/qwen3-block-htp/exp0105}"
artifact_root="${QBH_EXP0105_ARTIFACT_ROOT:-/mnt/d/llm_exp/models/qwen3-block-htp/exp0105/artifacts}"
...
if ! git -C "${project_root}" diff --quiet || \
   ...
   [[ -n "$(git -C "${project_root}" ls-files --others --exclude-standard)" ]]; then
    printf 'source worktree must be clean before EXP-0105 formal collection\n' >&2
    exit 1
fi
...
if [[ -e "${result_dir}" || -e "${artifact_dir}" ]]; then
    printf 'EXP-0105 evidence destination already exists\n' >&2
    exit 1
fi
mkdir -p "${result_dir}" "${artifact_dir}"
"${project_root}/scripts/build_exp0105.sh" > "${result_dir}/build.log" 2>&1
"${project_root}/scripts/check_exp0105_static.sh" \
    > "${result_dir}/static_gate.json"
QBH_EXP0105_PACKAGE="${package}" \
    "${project_root}/scripts/deploy_exp0105_block.sh" \
...
for mode in "${modes[@]}"; do
    ...
    "${project_root}/scripts/run_exp0105.sh" "${mode}" 1 on on > "${raw}"
...
for ((round = 1; round <= rounds; ++round)); do
    ...
    for repeat in "${repeats[@]}"; do
        for mode in "${order[@]}"; do
            ...
            "${project_root}/scripts/run_exp0105.sh" \
...
python3 "${project_root}/scripts/analyze_exp0105.py" \
...
cp "${project_root}/hexagon_ReleaseG_toolv19_v79/ship/libqwen3_probe_skel.so" \
    "${artifact_dir}/"
{
    printf 'experiment=EXP-0105\nsource_head=%s\n' "${source_head}"
    printf 'control=serial_per_head_softmax_reductions\n'
    printf 'candidate=dual_head_interleaved_softmax_reductions\n'
