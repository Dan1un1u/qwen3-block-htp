...
/* Exhaustive FP SwiGLU audit, separate from model runtime timing. */
...
static int lsp2_gather_audit(uint8_t *shared,uint32_t bytes,uint8_t *vtcm,uint32_t vbytes) {
 ...
 if(scratch+896U>vtcm+vbytes)return AEE_ENOMEMORY;
 ...
 if(h->mode==4U)qbh_mlp_gate_up_sp2_lut_pipelined_hvx(g,u,out+131072U,out+196608U,65536U,lut,scratch);
   ...
   *(HVX_Vector *)(scratch+384U)=Q6_V_vror_VR(*(const HVX_Vector *)(u+(i&~127U)),i&127U);
   ...
   for(uint32_t j=0;j<32U;j++)if(scratch[512U+j]!=(uint8_t)lut[i+j] || scratch[640U+j]!=(uint8_t)(lut[i+j]>>8))return AEE_EFAILED;
 ...
 h->peak_bytes=(uint32_t)(scratch+896U-vtcm);h->status=mismatch?AEE_EFAILED:AEE_SUCCESS;
 return h->status;
}
...
#include "llama_r4_butterfly_probe.inc"
int lsp2_run(int fd,uint32_t bytes,uint8_t *vtcm,uint32_t vbytes,uint32_t ctx){
 ...
 struct lsp2_header *h=(struct lsp2_header*)shared;
 if(bytes>=128U && h->mode>=16U && h->mode<=18U) {
  ret=bf69_run(shared,bytes,vtcm,vbytes);
  ...
  *(HVX_Vector*)(pack_scratch+128)=c.mode>=5 ? Q6_Vb_vpacko_VhVh(v,v) : Q6_V_vxor_VV(Q6_Vb_vpacko_VhVh(v,v),Q6_V_vsplat_R(0x80808080));
  for(uint32_t j=0;j<2 && k+32*j<c.k;j++){
   uint32_t off=((k/32)+j)*2048+row*32;
   memcpy(a0+off,pack_scratch+j*32,32);
   if(c.mode==1 || c.mode==5)memcpy(a0+off+c.rows*32,pack_scratch+128+j*32,32);
   if(c.mode==2 || c.mode==6)memcpy(a1+off,pack_scratch+128+j*32,32);
  }
 }
 ...
 for(uint32_t nt=0;nt<c.n/32;nt++){
  ...
  uint32_t passes=(c.mode==2 || c.mode==6)?2:1;
  ...
  unpack_digits(raw0,wide0,c.mode==3?1:4);if(c.mode==2 || c.mode==6)unpack_digits(raw1,wide1,4);
  for(uint32_t row=0;row<c.rows;row++){
   {
    HVX_Vector v=*(const HVX_Vector*)(wide0+row*32);
    if(c.mode==1 || c.mode==2 || c.mode==5 || c.mode==6){
     const int32_t *hp=(c.mode==1 || c.mode==5)?wide0+(row+c.rows)*32:wide1+row*32;
     ...
     if(c.mode>=5) v=Q6_Vw_vadd_VwVw(v,Q6_Vw_vsub_VwVw(high,Q6_Vw_vasl_VwR(sum,1)));
    }
