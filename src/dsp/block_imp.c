    ...
    if ((header->common_ops_mask &
        ...
        (header->residual_mode ==
         ...
         header->attention_hvx_contexts !=
             QBH_BLOCK_MAX_ATTENTION_HVX_CONTEXTS) ||
        header->mlp_mode >
            QBH_BLOCK_MLP_W4U8_STREAMING_GATE_UP_PREFETCH ||
...
static int qbh_run_w4u8_streaming_mlp(
    ...
    struct qbh_w4_initial_prefetch *gate_up_prefetch) {
...

    {
        ...
        start = HAP_perf_get_qtimer_count();
        result = qbh_run_chunked_w4_pipeline_external_prefetched(
            &gate_up_phase, &gate_up_layout,
            shared + header->w4u8_gate_up_bundle_offset,
            mlp_arena + gate_up_layout.vtcm_activation_offset,
            mlp_arena, &handoff, &runner, gate_up_prefetch,
            &header->w4u8_gate_up_cross_prefetch_wait_ticks,
            &header->w4u8_gate_up_cross_prefetch_lifetime_ticks);
...
static int qbh_run_one_block(struct qbh_block_header *header,
    ...
    struct qbh_block_w4f16_cross_prefetch cross_prefetch;
    struct qbh_w4_initial_prefetch w4u8_gate_up_prefetch;
    ...
    memset(&w4u8_gate_up_prefetch, 0,
           sizeof(w4u8_gate_up_prefetch));
...
#undef QBH_COPY_U8_ATTN_AUDIT
    ...
    if (header->mlp_mode ==
            QBH_BLOCK_MLP_W4U8_STREAMING_GATE_UP_PREFETCH) {
        if (qbh_start_chunked_w4_initial_prefetch(
                &w4u8_gate_up_layout,
                shared + header->w4u8_gate_up_bundle_offset,
                buffers->q, &w4u8_gate_up_prefetch) !=
            AEE_SUCCESS) {
            return QBH_BLOCK_STATUS_MLP_STREAM_FAILED;
        }
        ++header->w4u8_gate_up_cross_prefetch_count;
    }
