...
enum qbh_block_u8_attention_stage {
    QBH_BLOCK_U8_ATTENTION_STAGE_GROUP = 0,
    QBH_BLOCK_U8_ATTENTION_STAGE_QK = 1,
    QBH_BLOCK_U8_ATTENTION_STAGE_SOFTMAX = 2,
    QBH_BLOCK_U8_ATTENTION_STAGE_AV = 3,
};
...
struct qbh_block_w4f16_pool {
    ...
    uint32_t u8_attention_stage;
    volatile uint32_t u8_attention_v_ready[QBH_BLOCK_KV_HEADS];
    volatile uint32_t u8_attention_softmax_ready[
        QBH_BLOCK_KV_HEADS * QBH_BLOCK_W4U8_SOFTMAX_ROW_SLICES];
    uint32_t u8_attention_stage_generation;
...
static int qbh_attention_u8_staged_softmax_enabled(uint32_t mode);
...
static int qbh_header_valid(const struct qbh_block_header *header,
    ...
    if ((header->common_ops_mask &
        ...
        header->attention_pipeline_mode >
            QBH_BLOCK_ATTENTION_PIPELINE_U8_LOG2_GQA_QKV_OVERLAP_VGATHER_VDEAL_FUSED_QK_REQUANT_HMX_BATCH_LUT_TEMPLATES_GQA_BATCH_STAGED_SOFTMAX ||
...
static void qbh_attention_u8_staged_qk_run_tasks(
    struct qbh_block_w4f16_pool *pool,
    struct qbh_block_w4f16_job *job) {
    struct qbh_block_header *header = pool->attention_header;
    struct qbh_block_buffers *buffers = pool->attention_buffers;
    ...
    for (;;) {
        ...
        job->u8_attention_v_pack_ticks +=
            HAP_perf_get_qtimer_count() - start;
        pool->u8_attention_v_ready[group] =
            pool->u8_attention_stage_generation;
        asm volatile("release(%0):at"
                     :
                     : "r"(&pool->u8_attention_v_ready[group])
                     : "memory");
...
static void qbh_attention_u8_staged_softmax_run_tasks(
    struct qbh_block_w4f16_pool *pool,
    struct qbh_block_w4f16_job *job) {
    struct qbh_block_header *header = pool->attention_header;
    struct qbh_block_buffers *buffers = pool->attention_buffers;
    ...
    uint64_t template_start = HAP_perf_get_qtimer_count();
    ...
    job->u8_attention_softmax_ticks +=
        HAP_perf_get_qtimer_count() - template_start;
    for (;;) {
        const uint32_t task =
            qbh_atomic_fetch_increment(&pool->next_attention_task);
        const uint32_t group =
            task / QBH_BLOCK_W4U8_SOFTMAX_ROW_SLICES;
        const uint32_t first_row =
            (task % QBH_BLOCK_W4U8_SOFTMAX_ROW_SLICES) *
        ...
        job->u8_attention_softmax_ticks +=
            HAP_perf_get_qtimer_count() - start;
        ++job->attention_softmax_task_count;
        pool->u8_attention_softmax_ready[task] =
            pool->u8_attention_stage_generation;
        asm volatile("release(%0):at"
                     :
                     : "r"(&pool->u8_attention_softmax_ready[task])
                     : "memory");
        if (telemetry_ptr != NULL) {
...
static void qbh_attention_u8_staged_av_run_tasks(
    struct qbh_block_w4f16_pool *pool,
    struct qbh_block_w4f16_job *job) {
    struct qbh_block_header *header = pool->attention_header;
    struct qbh_block_buffers *buffers = pool->attention_buffers;
    ...
    for (;;) {
        ...
        while (pool->u8_attention_v_ready[group] !=
                   pool->u8_attention_stage_generation ||
...
static void qbh_attention_u8_staged_pool_run_tasks(
    struct qbh_block_w4f16_pool *pool,
    struct qbh_block_w4f16_job *job) {
    if (pool->u8_attention_stage ==
        QBH_BLOCK_U8_ATTENTION_STAGE_QK) {
        qbh_attention_u8_staged_qk_run_tasks(pool, job);
    } else if (pool->u8_attention_stage ==
               QBH_BLOCK_U8_ATTENTION_STAGE_SOFTMAX) {
        qbh_attention_u8_staged_softmax_run_tasks(pool, job);
    } else if (pool->u8_attention_stage ==
               QBH_BLOCK_U8_ATTENTION_STAGE_AV) {
        qbh_attention_u8_staged_av_run_tasks(pool, job);
    } else {
        pool->attention_gqa_abort = 1U;
        asm volatile("barrier" ::: "memory");
    }
...
static void qbh_attention_u8_pool_run_tasks(
    struct qbh_block_w4f16_pool *pool,
    struct qbh_block_w4f16_job *job) {
    if (qbh_attention_u8_staged_softmax_enabled(
            pool->attention_header->attention_pipeline_mode)) {
        qbh_attention_u8_staged_pool_run_tasks(pool, job);
        return;
    }
    struct qbh_block_header *header = pool->attention_header;
    struct qbh_block_buffers *buffers = pool->attention_buffers;
...
static void qbh_attention_u8_reset_job_metrics(
    struct qbh_block_w4f16_job *job) {
...
static uint32_t qbh_hvx_pool_u8_attention_run_stage(
    struct qbh_block_header *header,
    struct qbh_block_w4f16_pool *pool,
    uint32_t stage, uint32_t task_count) {
    struct qbh_block_w4f16_job main_job;
    uint64_t wait_start;
    ...
    memset(&main_job, 0, sizeof(main_job));
    main_job.worker_index = header->attention_hvx_contexts - 1U;
    qbh_attention_u8_reset_job_metrics(&main_job);
    pool->u8_attention_stage = stage;
    pool->attention_task_count = task_count;
    pool->next_attention_task = 0U;
    for (uint32_t worker_index = 0U;
         worker_index < pool->active_worker_count; ++worker_index) {
        ...
        qbh_attention_u8_reset_job_metrics(job);
        job->command_kind =
            QBH_BLOCK_HVX_POOL_U8_GQA_ATTENTION;
    }
    asm volatile("barrier" ::: "memory");
    ...
    return pool->attention_gqa_abort == 0U
        ? completed_groups : UINT32_MAX;
}

static int qbh_hvx_pool_u8_attention_staged(
    struct qbh_block_header *header,
    struct qbh_block_w4f16_pool *pool,
    struct qbh_block_buffers *buffers,
    struct qbh_block_hmx_worker *worker) {
    ...
    header->u8_attention_probability_row_sum_min = UINT32_MAX;
    ++pool->u8_attention_stage_generation;
    if (pool->u8_attention_stage_generation == 0U) {
        memset((void *)pool->u8_attention_v_ready, 0,
               sizeof(pool->u8_attention_v_ready));
        memset((void *)pool->u8_attention_softmax_ready, 0,
               sizeof(pool->u8_attention_softmax_ready));
        pool->u8_attention_stage_generation = 1U;
    }

    if (qbh_hvx_pool_u8_attention_run_stage(
            header, pool, QBH_BLOCK_U8_ATTENTION_STAGE_QK,
            QBH_BLOCK_KV_HEADS) == UINT32_MAX ||
        qbh_hvx_pool_u8_attention_run_stage(
            header, pool, QBH_BLOCK_U8_ATTENTION_STAGE_SOFTMAX,
            QBH_BLOCK_KV_HEADS *
                QBH_BLOCK_W4U8_SOFTMAX_ROW_SLICES) == UINT32_MAX) {
        return -1;
    }
    completed_groups = qbh_hvx_pool_u8_attention_run_stage(
        header, pool, QBH_BLOCK_U8_ATTENTION_STAGE_AV,
        QBH_BLOCK_KV_HEADS);
    if (completed_groups != QBH_BLOCK_KV_HEADS) {
        return -1;
    }
    header->u8_attention_qk_execution_count +=
    ...
    header->u8_attention_av_execution_count +=
        completed_groups * QBH_ATTENTION_Q_HEADS_PER_GROUP *
        QBH_ATTENTION_HEAD_DIM_TILES;
    header->hmx_command_count += completed_groups * 2U;
    header->hmx_u8s8_tile_pair_count +=
        completed_groups * QBH_ATTENTION_Q_HEADS_PER_GROUP *
        (QBH_ATTENTION_SCORE_TILES *
             QBH_ATTENTION_HEAD_DIM_TILES +
...
static int qbh_hvx_pool_u8_attention(
    ...
    if (qbh_attention_u8_staged_softmax_enabled(
            header->attention_pipeline_mode)) {
        return qbh_hvx_pool_u8_attention_staged(
            header, pool, buffers, worker);
    }
    memset(&main_job, 0, sizeof(main_job));
    main_job.worker_index = header->attention_hvx_contexts - 1U;
    pool->attention_header = header;
