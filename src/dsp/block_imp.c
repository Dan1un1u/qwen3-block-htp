...
static int qbh_header_valid(const struct qbh_block_header *header,
    ...
    if (header == NULL || header->magic != QBH_BLOCK_MAGIC ||
        ...
        (QBH_LLAMA_SP2(header)!=0U && QBH_LLAMA_SP2(header)!=3U && QBH_LLAMA_SP2(header)!=4U && QBH_LLAMA_SP2(header)!=5U && QBH_LLAMA_SP2(header)!=6U && QBH_LLAMA_SP2(header)!=7U && QBH_LLAMA_SP2(header)!=8U && QBH_LLAMA_SP2(header)!=9U) ||
        (QBH_LLAMA_SP2(header)>=5U &&
...
static int qbh_run_w4u8_direct_n_projection(
    ...
    while (current_first < n_tiles) {
        ...
        qbh_hmx_start(
            worker, fp32_projection ? QBH_BLOCK_HMX_W4_FP32_RESIDUAL : (QBH_LLAMA_SP2(header) && desc==&header->projections[QBH_BLOCK_PROJ_DOWN]) ? QBH_BLOCK_HMX_SP2_W4 : QBH_BLOCK_HMX_U8N4, activation_tiles,
        ...
        ++header->w4u8_decode_direct_n_hmx_command_count;
        header->hmx_u8s8_tile_pair_count +=
            (uint64_t)k_tiles * current_tiles * ((QBH_LLAMA_SP2(header) && desc==&header->projections[QBH_BLOCK_PROJ_DOWN] && header->logical_m==64U) ? 2U : 1U);
...
static int qbh_start_w4u8_gate_up_swiglu_stream(
    ...
    pool->u8_sp2_high = QBH_LLAMA_SP2(header) ? middle + 128U : NULL;
    pool->u8_swiglu_lut =
...
static int qbh_run_w4u8_direct_n_mlp(
...
#ifdef QBH_MODEL_LLAMA32
        if(stream_sp2 && qbh_llama_sp2_prefill_start(pool,buffers,gate_native,up_native,middle_native,QBH_LLAMA_SP2(header)>=6U))return -1;
#endif
...
#ifdef QBH_MODEL_LLAMA32
        if(stream_sp2)qbh_llama_sp2_prefill_finish(header,pool,buffers,up_status);
#endif
    ...
    if(QBH_LLAMA_SP2(header) && !stream_down && down_result!=0) {
