...
struct qbh_sp2_epilogue {
    ...
    uint8_t *output[2];
    volatile uint32_t ready[2],done[2],abort;
    uint32_t produced,tiles;
    int32_t zero_point;
};
...
struct qbh_block_w4f16_job {
    ...
    uint32_t u8_decode_q_pair_row4_call_count;
    uint32_t u8_decode_k_pair_row4_call_count;
    uint32_t u8_decode_qk_rows_processed;
    uint32_t u8_decode_k_temp_carrier_skipped_count;
    uint32_t u8_qk_padding_poison_pair_count;
...
struct qbh_block_w4f16_pool {
    ...
    uint32_t fp32_norm_opt,fp32_norm_native;
...
struct qbh_w4u8_qkv_ring_state {
    ...
    uint32_t qk_prepare_rows;
    uint32_t decode_qk_row_contract;
...
struct qbh_block_w4u8_gate_prefetch {
...
static uint64_t qbh_fnv1a64_u8_native_rows(
    const uint8_t *heads, uint32_t head_count, uint32_t rows) {
    uint64_t hash = UINT64_C(1469598103934665603);
    for (uint32_t head = 0U; head < head_count; ++head) {
        const uint8_t *head_tiles = heads +
...
static void qbh_mix_observed_hash(uint64_t *aggregate, uint64_t value) {
    if (*aggregate == 0U) {
        *aggregate = value;
    }
}
...
#undef QBH_MAP_GATE_RANGE
    for (uint32_t projection = 0U;
        ...
        hash = qbh_map_gate_mix_range(
            hash, shared, desc->direct_n_weight_offset,
...
static int qbh_plan_buffers(uint8_t *vtcm, uint32_t vtcm_bytes,
    ...
    if (sp2_mode) {
        ...
        if (high_bytes + (fp32_residual ? 0U : scratch_bytes) >
            QBH_BLOCK_M * QBH_BLOCK_MAX_K * (fp32_residual ? 1U : sizeof(uint16_t))) return -1;
        /* EXP0269: FP32 residual trades the idle half of hmx_activation for
         * wider hidden storage. Normalized is dead while O/Down raw slots are
    ...
    if (variant == QBH_BLOCK_W4U8 &&
        ...
        buffers->persistent_k_tail = qbh_arena_alloc_aligned(
            &arena, QBH_KV_CACHE_HMX_U8_K_VTCM_TAIL_ATLAS_BYTES,
            QBH_BLOCK_ALIGNMENT);
    }
...
static int qbh_slice_layer_desc_valid(
    ...
    for (uint32_t index = 0U;
        ...
        if (header->w4u8_decode_projection_mode ==
            ...
            if (header->variant != QBH_BLOCK_W4U8 ||
                desc->direct_n_weight_bytes != expected_weight ||
...
static int qbh_generation_enabled(uint32_t mode) {
    return qbh_generation_f16f16_enabled(mode) ||
           qbh_generation_w4f16_enabled(mode) ||
...
static int qbh_generation_request_valid(
    ...
    if (header->generation_mode == QBH_BLOCK_GENERATION_DISABLED) {
        return header->generation_token_ids_offset == 0U &&
               ...
               header->generation_lm_head.direct_n_weight_offset == 0U &&
               header->generation_lm_head.direct_n_weight_bytes == 0U &&
    ...
    if (header->evaluation_mode > 2U ||
        (header->evaluation_mode == 1U &&
         (header->evaluation_target_token >= QBH_QWEN3_VOCAB_SIZE ||
          (header->generation_mode != QBH_BLOCK_GENERATION_GREEDY_F16F16 &&
           ...
           header->generation_mode != QBH_BLOCK_GENERATION_GREEDY_W4U8_BATCH8_RESIDENT_BIAS))) ||
        ...
        (header->variant !=
             (qbh_generation_f16f16_enabled(header->generation_mode)
                  ...
                  : qbh_generation_w4f16_enabled(header->generation_mode)
                        ? QBH_BLOCK_W4F16 : QBH_BLOCK_W4U8)) ||
        ...
        header->generation_embedding_bytes !=
            QBH_QWEN3_VOCAB_SIZE * QBH_BLOCK_HIDDEN *
                (qbh_generation_w4u8_enabled(header->generation_mode) && !QBH_FP32_RESIDUAL(header)
    ...
    if (qbh_generation_f16f16_enabled(header->generation_mode)) {
        return head->k == QBH_BLOCK_HIDDEN &&
               ...
               qbh_range_valid(head->weight_offset, head->weight_bytes, shared_bytes) &&
               head->scale_offset == 0U && head->scale_bytes == 0U &&
               head->bias_offset == 0U && head->bias_bytes == 0U &&
               head->direct_n_weight_offset == 0U && head->direct_n_weight_bytes == 0U;
    }
    ...
    if (qbh_generation_w4u8_enabled(header->generation_mode)) {
        const int direct_n_valid =
            header->w4u8_decode_projection_mode ==
                    QBH_BLOCK_W4U8_DECODE_PROJECTION_DIRECT_N
                ? head->direct_n_weight_bytes == head->weight_bytes &&
                      qbh_range_valid(head->direct_n_weight_offset,
                                      head->direct_n_weight_bytes,
                                      shared_bytes)
                : head->direct_n_weight_offset == 0U &&
                      head->direct_n_weight_bytes == 0U;
        return direct_n_valid &&
               head->bias_bytes ==
...
static int qbh_header_valid(const struct qbh_block_header *header,
    ...
    if (header == NULL || header->dense_r3_mode ||
        (header->dense_r4_mode && (header->dense_r4_mode>2U || header->dense_r4_optimization!=6U || QBH_SP2(header)!=8U)) ||
        (header->variant == QBH_BLOCK_W4U8
    ...
    if (header == NULL || header->magic != QBH_BLOCK_MAGIC ||
        ...
        (QBH_U8_PREFILL_OPT(header) &&
         (QBH_SP2(header) || header->variant!=QBH_BLOCK_W4U8 ||
          ...
          !header->w4u8_decode_direct_n_gate_up_continuous ||
          header->attention_hvx_contexts<4U ||
          header->projections[QBH_BLOCK_PROJ_GATE].lpbq_mode ||
          header->projections[QBH_BLOCK_PROJ_UP].lpbq_mode ||
          header->projections[QBH_BLOCK_PROJ_DOWN].lpbq_mode)) ||
        ...
        (header->w4u8_decode_projection_mode ==
         ...
         header->w4u8_decode_direct_n_mask != 0U) ||
        (header->w4u8_decode_projection_mode ==
             QBH_BLOCK_W4U8_DECODE_PROJECTION_DIRECT_N &&
         header->w4u8_decode_direct_n_mask == 0U) ||
        (header->w4u8_decode_projection_mode !=
         ...
         (header->variant != QBH_BLOCK_W4U8 ||
          ...
          header->replay_mode != QBH_BLOCK_REPLAY_CONTINUOUS ||
          (QBH_VERTICAL_SLICE_LAYER_COUNT != 1U && !qbh_generation_w4u8_enabled(header->generation_mode) && !QBH_STAGED_A8_REPLAY))) ||
        ...
        (header->w4u8_decode_qk_norm_rope_rows !=
             QBH_BLOCK_W4U8_QK_PREP_FULL_ROWS &&
         header->variant != QBH_BLOCK_W4U8) ||
        header->w4u8_decode_qk_padding_poison > 1U ||
        (header->w4u8_decode_qk_padding_poison != 0U &&
         (header->variant != QBH_BLOCK_W4U8 ||
          header->w4u8_decode_qk_norm_rope_rows !=
              QBH_BLOCK_W4U8_QK_PREP_DECODE_ROWS)) ||
        ...
        header->w4u8_decode_direct_n_gate_up_continuous > 1U ||
        ...
        (header->w4u8_decode_direct_n_q_batch_n_tiles != 0U &&
         header->w4u8_decode_direct_n_q_batch_n_tiles != 32U) ||
        (header->w4u8_decode_direct_n_q_batch_n_tiles != 0U &&
         (header->variant != QBH_BLOCK_W4U8 ||
          header->w4u8_decode_projection_mode !=
              QBH_BLOCK_W4U8_DECODE_PROJECTION_DIRECT_N ||
...
static int qbh_dma_start_w4u8_direct_n_prefetch(
    ...
    result = qbh_dma_start_weight_prefetch(
        &descriptors[0], weight_destination, weight_source, first_bytes);
    if (result == 0) {
        result = qbh_dma_wait_weight_prefetch(&descriptors[0]);
    }
    if (result != 0 || qbh_dma_wait_idle() != 0) {
    ...
    *descriptor_count = 3U;
    return qbh_dma_start_w4u8_batch_prefetch(
        ...
        weight_bytes - first_bytes,
        bias_destination, bias_source, bias_bytes);
}
    ...
    for (uint32_t batch_index = 0U;
        ...
        if (batch->desc ==
            ...
            state->head_publish_count +=
                batch->n_tiles / QBH_ATTENTION_HEAD_DIM_TILES;
        }
        if (state->direct_n_weights != 0U) {
            qurt_sem_up(&state->compressed_free[slot]);
            ++state->header->w4u8_decode_direct_n_hmx_command_count;
        } else {
            qurt_sem_up(&state->expanded_free[slot]);
        }
    }
        ...
        if (worker->kind == QBH_BLOCK_HMX_FP16) {
            ...
            if(worker->sp2_hvx_locked) {
                worker->command_status=qurt_hvx_unlock();worker->sp2_hvx_locked=0U;
                   ...
                   worker->m_tiles <= QBH_ATTENTION_Q_HEADS_PER_GROUP &&
                   worker->n_tiles >= 1U &&
                   worker->n_tiles <= QBH_BLOCK_SCAN_MAX_KV_TILES) {
            for (uint32_t m_tile = 0U;
                 m_tile < worker->m_tiles; ++m_tile) {
                const uint8_t *activation =
                    (const uint8_t *)worker->activation +
...
static void qbh_w4u8_qkv_ring_expand_worker_run(
    ...
    const uint32_t task_count =
        state != NULL
            ? state->batch_count * state->tiles_per_batch
            : 0U;
    ...
    while (state != NULL && state->abort_status == 0U) {
        ...
        } else qbh_unpack_w4_to_s8_hvx_relaxed(
            state->compressed_slots[slot] +
                (size_t)tile * state->k_tiles *
                    QBH_W4_PACKED_TILE_BYTES,
        ...
        if (completed == state->tiles_per_batch) {
            asm volatile("barrier" ::: "memory");
            qurt_sem_up(&state->compressed_free[slot]);
    ...
    for(uint32_t tile=index;tile<QBH_BLOCK_INTERMEDIATE/32U;tile+=3U) {
        if(pool->sp2_prefill_stream) {
            while(!pool->u8_swiglu_ready[tile/32U]) {
                if(pool->u8_swiglu_abort)return;
                asm volatile("pause(#8)" ::: "memory");
            }
            asm volatile("barrier" ::: "memory");
        }
        if(pool->u8_r4_act) {
            ...
            qbh_mlp_gate_up_sp2_compact_hvx(pool->u8_swiglu_gate+(size_t)tile*2048U,
                pool->u8_swiglu_up+(size_t)tile*2048U,pool->u8_swiglu_middle+(size_t)tile*2048U,
                pool->u8_sp2_high+(size_t)tile*2048U,2048U,pool->u8_swiglu_lut,
                pool->u8_swiglu_gather_scratch+(size_t)index*256U,pool->attention_buffers->normalized+index*4096U);
        ...
        else if (pool->u8_sp2_high)
            qbh_mlp_gate_up_sp2_lut_hvx(pool->u8_swiglu_gate+(size_t)tile*2048U,
                pool->u8_swiglu_up+(size_t)tile*2048U,
                pool->u8_swiglu_middle+(size_t)tile*2048U,
                pool->u8_sp2_high+(size_t)tile*2048U,2048U,
                pool->u8_swiglu_lut,pool->u8_swiglu_gather_scratch+(size_t)index*256U);
...
static void qbh_w4u8_swiglu_stream_worker_run(
    ...
    for (uint32_t group = 0U;
        ...
        for (uint32_t tile = 0U;
            ...
            else if(pool->u8_sp2_high) qbh_mlp_gate_up_sp2_lut_hvx(
...
/* EXP0274: row-major producer and optimized HVX consumer pack. All four
 * independent Norm workers and ordered FP32 arithmetic are retained. */
static __attribute__((noinline)) void qbh_paper_pack_norm(
    const uint8_t *src,uint8_t *dst) {
    for(uint32_t c=0;c<QBH_BLOCK_HIDDEN;c+=128U)for(uint32_t row=0;row<64U;row+=4U) {
        HVX_Vector a=*(const HVX_Vector *)(src+row*QBH_BLOCK_HIDDEN+c);
        HVX_Vector b=*(const HVX_Vector *)(src+(row+1U)*QBH_BLOCK_HIDDEN+c);
        HVX_Vector d=*(const HVX_Vector *)(src+(row+2U)*QBH_BLOCK_HIDDEN+c);
        HVX_Vector e=*(const HVX_Vector *)(src+(row+3U)*QBH_BLOCK_HIDDEN+c);
        HVX_VectorPair ab=Q6_W_vshuff_VVR(b,a,-32),de=Q6_W_vshuff_VVR(e,d,-32);
        HVX_VectorPair x=Q6_W_vshuff_VVR(Q6_V_lo_W(de),Q6_V_lo_W(ab),-64);
        HVX_VectorPair y=Q6_W_vshuff_VVR(Q6_V_hi_W(de),Q6_V_hi_W(ab),-64);
        uint8_t *o=dst+(c/32U)*2048U+row*32U;
        *(HVX_Vector *)o=Q6_V_lo_W(x);*(HVX_Vector *)(o+2048U)=Q6_V_hi_W(x);
        *(HVX_Vector *)(o+4096U)=Q6_V_lo_W(y);*(HVX_Vector *)(o+6144U)=Q6_V_hi_W(y);
    }
    asm volatile("barrier":::"memory");
}
...
static void qbh_generation_hvx_argmax_group(
    ...
    if (header->evaluation_mode == 1U) {
        for (uint32_t tile = 0U; tile < group_tiles; ++tile) {
            size_t offset = qbh_hmx_fp16_matrix_tile_offset(0U, tile, group_tiles);
            for (uint32_t lane = 0U; lane < QBH_HMX_FP16_COLS; ++lane) {
                uint16_t code;
                const __fp16 *value = hmx_output + offset + qbh_hmx_fp16_tile_offset(0U, lane);
                memcpy(&code, value, sizeof(code));
                ++hist[code];
                if ((n_tile + tile) * QBH_HMX_FP16_COLS + lane == header->evaluation_target_token) {
                    header->evaluation_target_code = code;
                    ++header->evaluation_target_seen;
                }
            }
        }
    }
...
/* EXP-0217: FP16 stored head weights, streamed through existing aligned
 * phase-dead projection buffers. No W4 expansion or logits DDR boundary. */
static int qbh_run_generation_head_f16f16(
    struct qbh_block_header *header, uint8_t *shared,
    struct qbh_block_buffers *buffers, struct qbh_block_hmx_worker *worker,
    uint32_t logical_rows, uint32_t generation_step) {
    const struct qbh_block_projection_desc *head = &header->generation_lm_head;
    const uint32_t k_tiles = QBH_BLOCK_HIDDEN / QBH_HMX_FP16_COLS;
    const uint32_t n_tiles = QBH_QWEN3_VOCAB_SIZE / QBH_HMX_FP16_COLS;
    const uint32_t group_limit = 8U;
    ...
    uint32_t best_token = 0U;
    uint16_t best_bits = 0U;
    if (logical_rows == 0U || generation_step >= header->generation_expected_token_count ||
        header->mlp_mode != QBH_BLOCK_MLP_CROUTON_NATIVE_BATCH8) return -1;
    if (qbh_dma_copy(header, buffers->input_norm_weight,
                     shared + header->generation_final_norm_offset,
    ...
    header->generation_lm_head_batch_n_tiles = group_limit;
    header->generation_lm_head_n_tiles = n_tiles;
    for (uint32_t n_tile = 0U; n_tile < n_tiles; n_tile += group_limit) {
        uint32_t count = n_tiles - n_tile;
        uint64_t stage;
    ...
    header->generation_selected_token_id = best_token;
    header->generation_selected_logit_half_bits = best_bits;
    header->generation_expected_token_id = ((const uint32_t *)(shared +
        header->generation_expected_token_ids_offset))[generation_step];
    header->generation_token_match = best_token == header->generation_expected_token_id;
    header->generation_lm_head_ticks += HAP_perf_get_qtimer_count() - start;
    return 0;
}
...
static void qbh_generation_hvx_argmax_u8_group(
    ...
    if (header->evaluation_mode == 1U) {
        for (uint32_t tile = 0U; tile < group_tiles; ++tile) {
            for (uint32_t lane = 0U; lane < QBH_HMX_OUTPUT_CHANNELS; ++lane) {
                uint8_t code = hmx_output[(size_t)tile * QBH_HMX_OUTPUT_BYTES + lane];
                ++hist[code];
                if ((first_n_tile + tile) * QBH_HMX_OUTPUT_CHANNELS + lane == header->evaluation_target_token) {
                    header->evaluation_target_code = code;
                    ++header->evaluation_target_seen;
                }
            }
...
static int qbh_run_generation_head_w4u8(
    ...
    const uint32_t direct_n_decode =
        ...
        header->w4u8_decode_projection_mode ==
            QBH_BLOCK_W4U8_DECODE_PROJECTION_DIRECT_N &&
        (header->w4u8_decode_direct_n_mask &
         QBH_BLOCK_W4U8_DIRECT_N_LM_HEAD) != 0U;
    const uint32_t decode_batch16 =
        direct_n_decode == 0U && resident_bias != 0U &&
        header->scan_mode == QBH_BLOCK_SCAN_DECODE &&
        logical_rows == 1U &&
        header->w4u8_decode_lm_head_group_tiles == 16U;
    ...
    if (resident_bias != 0U) {
        ...
        if (direct_n_decode != 0U) {
            ...
            header->w4u8_decode_direct_n_expand_bytes_avoided +=
                (uint64_t)n_tiles * k_tiles * QBH_HMX_WEIGHT_BYTES;
        }
...
/* EXP0240: compressed LPBQ stays in DDR; expansion is entirely VTCM.
 * Double-buffer DMA+HVX overlaps the previous integer HMX command. */
static int qbh_run_w4u8_lpbq32_projection(
    struct qbh_block_header *header, const uint8_t *shared,
    const struct qbh_block_projection_desc *desc,
    struct qbh_block_buffers *buffers, struct qbh_block_hmx_worker *worker,
    ...
    for (uint32_t first = 0U, slot = 0U; first < nt; first += batch, slot ^= 1U) {
        ...
        uint64_t start = HAP_perf_get_qtimer_count();
        int rc = qbh_dma_start_w4u8_batch_prefetch(dma, packed[slot],
            shared + desc->lpbq_weight_offset + (size_t)first * stride,
            count * stride, bias[slot], shared + desc->bias_offset + (size_t)first * QBH_HMX_BIAS_BYTES,
            count * QBH_HMX_BIAS_BYTES);
        if (rc == 0) rc = qbh_dma_wait_w4u8_batch_prefetch(dma);
        header->weight_dma_ticks += HAP_perf_get_qtimer_count() - start;
        if (rc != 0) {
            if (previous_active) (void)qbh_hmx_wait(worker);
            qbh_record_projection_failure(header, desc, first, 90U, rc);
            return -1;
        }
        header->weight_ddr_read_bytes += (uint64_t)count * (stride + QBH_HMX_BIAS_BYTES);
        ...
        else header->w4u8_mlp_weight_expand_ticks += HAP_perf_get_qtimer_count() - start;
        if (previous_active) {
            start = HAP_perf_get_qtimer_count();
            rc = qbh_hmx_wait(worker);
...
static int qbh_run_w4u8_direct_n_projection(
    ...
    const uint8_t *activation_tiles, uint8_t *output_tiles,
    uint32_t batch_tiles) {
    if (desc != NULL && desc->lpbq_mode != 0U)
        return qbh_run_w4u8_lpbq32_projection(header, shared, desc, buffers,
            worker, activation_tiles, output_tiles);
    struct qbh_dma_aligned_desc_1d descriptors[3]
        __attribute__((aligned(64)));
    ...
    if (result != 0) {
        qbh_record_projection_failure(header, desc, 0U, 80U, result);
        return -1;
    }
    header->weight_ddr_read_bytes +=
        (uint64_t)current_tiles *
    ...
    worker->sp2_mode = QBH_SP2(header);
    ...
    while (current_first < n_tiles) {
        ...
        worker->fp32_first_channel=current_first*32U;
        qbh_hmx_start(
            worker, fp32_projection ? QBH_BLOCK_HMX_W4_FP32_RESIDUAL : (QBH_SP2(header) && desc==&header->projections[QBH_BLOCK_PROJ_DOWN]) ? QBH_BLOCK_HMX_SP2_W4 : QBH_BLOCK_HMX_U8N4, activation_tiles,
            weight_slots[current_slot], bias_slots[current_slot],
            output_tiles + (size_t)current_first * QBH_HMX_OUTPUT_BYTES,
        ...
        if(buffers->sp2_prefill_ready && desc==&header->projections[QBH_BLOCK_PROJ_UP]) {
            uint32_t group=current_first/32U;
            buffers->sp2_prefill_ready[group]=1U;
            asm volatile("release(%0):at" :: "r"(&buffers->sp2_prefill_ready[group]) : "memory");
    ...
    if (desc == &header->projections[QBH_BLOCK_PROJ_O]) {
        header->w4u8_o_batch_count +=
            (n_tiles + batch_tiles - 1U) / batch_tiles;
    }
    header->w4u8_decode_direct_n_expand_bytes_avoided +=
        (uint64_t)n_tiles * k_tiles * QBH_HMX_WEIGHT_BYTES;
    return 0;
...
static int qbh_start_w4u8_direct_n_gate_prefetch(
    struct qbh_block_header *header, const uint8_t *shared,
    struct qbh_block_buffers *buffers,
    struct qbh_block_w4u8_gate_prefetch *state) {
    const struct qbh_block_projection_desc *desc =
        &header->projections[QBH_BLOCK_PROJ_GATE];
    const uint32_t n_tiles = 32U;
    const uint32_t k_tiles = desc->k / QBH_HMX_INPUT_CHANNELS;
    const uint32_t tile_bytes = k_tiles * QBH_W4_PACKED_TILE_BYTES;
    int result;

    if (header == NULL || shared == NULL || buffers == NULL ||
        state == NULL || state->active != 0U ||
        ...
        (header->w4u8_decode_direct_n_mask &
         QBH_BLOCK_W4U8_DIRECT_N_MLP) == 0U ||
        header->w4u8_decode_direct_n_gate_up_batch_n_tiles != n_tiles ||
        header->w4u8_decode_direct_n_gate_up_continuous == 0U ||
        desc->direct_n_weight_bytes != desc->weight_bytes ||
        desc->n / QBH_HMX_OUTPUT_CHANNELS < n_tiles) {
        return -1;
    }

    state->target = desc;
    state->weight_slot = buffers->expanded_weight;
    state->bias_slot = buffers->rope_cos;
    state->n_tiles = n_tiles;
    state->start_ticks = HAP_perf_get_qtimer_count();
    result = qbh_dma_start_w4u8_batch_prefetch(
        state->descriptors, state->weight_slot,
        shared + desc->direct_n_weight_offset,
        n_tiles * tile_bytes, state->bias_slot,
        shared + desc->bias_offset,
        n_tiles * QBH_HMX_BIAS_BYTES);
    if (result != 0) {
        memset(state, 0, sizeof(*state));
        qbh_record_projection_failure(header, desc, 0U, 86U, result);
        return -1;
    }
    state->active = 1U;
    header->weight_ddr_read_bytes +=
        (uint64_t)n_tiles * (tile_bytes + QBH_HMX_BIAS_BYTES);
    header->w4u8_decode_direct_n_weight_ddr_read_bytes +=
        (uint64_t)n_tiles * tile_bytes;
    header->weight_dma_descriptor_count += 2U;
    ++header->w4u8_o_gate_prefetch_start_count;
    return 0;
}

static int qbh_consume_w4u8_direct_n_gate_prefetch(
    struct qbh_block_header *header,
    struct qbh_block_w4u8_gate_prefetch *state,
    const struct qbh_block_projection_desc *target,
    uint8_t *weight_slot, uint8_t *bias_slot, uint32_t n_tiles) {
    const uint64_t wait_start = HAP_perf_get_qtimer_count();
    uint64_t end;
    int result;

    if (header == NULL || state == NULL || state->active == 0U ||
        state->target != target || state->weight_slot != weight_slot ||
        state->bias_slot != bias_slot || state->n_tiles != n_tiles) {
        return -1;
    }
    result = qbh_dma_wait_w4u8_batch_prefetch(state->descriptors);
    end = HAP_perf_get_qtimer_count();
    header->w4u8_o_gate_prefetch_wait_ticks += end - wait_start;
    header->w4u8_o_gate_prefetch_lifetime_ticks +=
        end - state->start_ticks;
    header->weight_dma_ticks += end - state->start_ticks;
    state->active = 0U;
    if (result != 0) {
        qbh_record_projection_failure(header, target, 0U, 87U, result);
        return -1;
    }
    ++header->w4u8_o_gate_prefetch_consume_count;
    return 0;
}

static void qbh_drain_w4u8_direct_n_gate_prefetch(
    struct qbh_block_header *header,
    struct qbh_block_w4u8_gate_prefetch *state) {
    if (header == NULL || state == NULL || state->active == 0U) {
        return;
    }
    (void)qbh_consume_w4u8_direct_n_gate_prefetch(
        header, state, state->target, state->weight_slot,
 ...
 * final Gate HMX command is active instead of after a full ring drain. */
static int qbh_run_w4u8_direct_n_gate_up_pair(
    struct qbh_block_header *header, const uint8_t *shared,
    struct qbh_block_buffers *buffers,
    struct qbh_block_hmx_worker *worker,
    struct qbh_block_w4f16_pool *pool,
    ...
    const struct qbh_block_projection_desc *descs[2] = {
        &header->projections[QBH_BLOCK_PROJ_GATE],
        &header->projections[QBH_BLOCK_PROJ_UP]};
    uint8_t *outputs[2] = {gate_output_tiles, up_output_tiles};
    uint8_t *weight_slots[2] = {
        buffers->expanded_weight, buffers->expanded_weight_alt};
    uint8_t *bias_slots[2] = {
        buffers->rope_cos,
        buffers->rope_cos + 32U * QBH_HMX_BIAS_BYTES};
    const uint32_t batch_tiles = 32U;
    const uint32_t k_tiles =
        descs[0]->k / QBH_HMX_INPUT_CHANNELS;
    const uint32_t n_tiles =
        descs[0]->n / QBH_HMX_OUTPUT_CHANNELS;
    const uint32_t tile_bytes =
        k_tiles * QBH_W4_PACKED_TILE_BYTES;
    const uint32_t prefill_pair=header && header->logical_m==64U &&
        (QBH_SP2(header)>=7U || QBH_U8_PREFILL_OPT(header)>=3U);
    ...
    if (header == NULL || shared == NULL || buffers == NULL ||
        ...
        header->variant != QBH_BLOCK_W4U8 || (header->logical_m != 1U && !prefill_pair) ||
        header->w4u8_decode_projection_mode !=
            QBH_BLOCK_W4U8_DECODE_PROJECTION_DIRECT_N ||
        header->w4u8_decode_direct_n_gate_up_continuous == 0U ||
        header->w4u8_decode_direct_n_gate_up_batch_n_tiles != batch_tiles ||
        (header->w4u8_decode_direct_n_mask &
         QBH_BLOCK_W4U8_DIRECT_N_MLP) == 0U ||
        descs[0]->k != descs[1]->k ||
        descs[0]->n != descs[1]->n ||
        descs[0]->direct_n_weight_bytes != descs[0]->weight_bytes ||
        descs[1]->direct_n_weight_bytes != descs[1]->weight_bytes ||
    ...
    if (!prefill_pair && header->w4u8_decode_direct_n_o_gate_prefetch != 0U && !(header->paper_pipeline_disable&16U)) {
        ...
        result = qbh_dma_start_w4u8_batch_prefetch(
            descriptors, weight_slots[0],
            shared + descs[0]->direct_n_weight_offset,
            current_tiles * tile_bytes, bias_slots[0],
            shared + descs[0]->bias_offset,
            current_tiles * QBH_HMX_BIAS_BYTES);
        if (result == 0) {
    ...
    if (swiglu_stream != 0U && !prefill_pair) {
    ...
    for (;;) {
        ...
        qbh_hmx_start(
            worker, QBH_BLOCK_HMX_U8N4, activation_tiles,
            weight_slots[current_slot], bias_slots[current_slot],
            outputs[projection] +
                (size_t)current_first * QBH_HMX_OUTPUT_BYTES,
            1U, k_tiles, current_tiles);
        ++header->hmx_command_count;
        ++header->w4u8_decode_direct_n_hmx_command_count;
        header->hmx_u8s8_tile_pair_count +=
        ...
        if (next_projection < 2U) {
            ...
            result = qbh_dma_start_w4u8_batch_prefetch(
                descriptors, weight_slots[next_slot],
                shared + next_desc->direct_n_weight_offset +
                    (size_t)next_first * tile_bytes,
                next_tiles * tile_bytes, bias_slots[next_slot],
                shared + next_desc->bias_offset +
                    (size_t)next_first * QBH_HMX_BIAS_BYTES,
                next_tiles * QBH_HMX_BIAS_BYTES);
            if (result == 0) {
                result = qbh_dma_wait_w4u8_batch_prefetch(descriptors);
        ...
        if(prefill_pair && projection==1U) {
            uint32_t g=current_first/batch_tiles;
            buffers->sp2_prefill_ready[g]=1U;
            asm volatile("release(%0):at" :: "r"(&buffers->sp2_prefill_ready[g]) : "memory");
            ++header->w4u8_gate_up_swiglu_publish_count;
        }
        if (!prefill_pair && swiglu_stream != 0U && projection == 1U &&
    ...
    header->w4u8_decode_direct_n_expand_bytes_avoided +=
        2U * (uint64_t)n_tiles * k_tiles * QBH_HMX_WEIGHT_BYTES;
    return 0;
}
...
static int qbh_run_w4u8_qkv_ring(
    ...
    state.q_tiles_per_batch = state.direct_n_weights != 0U &&
            header->w4u8_decode_direct_n_q_batch_n_tiles != 0U
    ...
    state.slot_count = descs[0]->lpbq_mode != 0U ? 2U : state.direct_n_weights != 0U &&
            state.tiles_per_batch >= 16U
    ...
    if((header->paper_pipeline_disable&1U) && !state.direct_n_weights)return -1;
    state.expand_worker_count = state.direct_n_weights != 0U
        ? 0U : header->w4u8_qkv_ring_expand_workers;
    state.decode_qk_row_contract =
        logical_rows == 1U && past_tokens != 0U;
    state.qk_prepare_rows = state.decode_qk_row_contract != 0U
        ? header->w4u8_decode_qk_norm_rope_rows
        : QBH_BLOCK_W4U8_QK_PREP_FULL_ROWS;
    if (state.decode_qk_row_contract != 0U) {
        header->w4u8_qk_norm_rope_rows_observed =
            state.qk_prepare_rows;
    }
    ...
    for (uint32_t slot = 0U; slot < state.slot_count; ++slot) {
        if (state.direct_n_weights != 0U &&
            ...
            state.bias_slots[slot] = slot >= 2U
                ? buffers->input_norm_weight +
                      (size_t)(slot - 2U) * state.tiles_per_batch *
                          QBH_HMX_BIAS_BYTES
                : buffers->scale_or_bias +
                      (size_t)slot * state.tiles_per_batch *
                          QBH_HMX_BIAS_BYTES;
        } else {
    ...
    for (uint32_t projection = 0U; projection < 3U; ++projection) {
        ...
        for (uint32_t first = 0U; first < n_tiles;
            ...
            batch->n_tiles = projection_batch_tiles;
        }
    }
    if (state.batch_count != state.expected_batch_count) {
        goto cleanup;
    }
    ...
    for (uint32_t index = 0U;
        ...
        job->u8_decode_q_pair_row4_call_count = 0U;
        job->u8_decode_k_pair_row4_call_count = 0U;
        job->u8_decode_qk_rows_processed = 0U;
        job->u8_decode_k_temp_carrier_skipped_count = 0U;
        job->u8_qk_padding_poison_pair_count = 0U;
    ...
    for (uint32_t index = 0U;
         index < pool->active_worker_count; ++index) {
        if(!(header->paper_pipeline_disable&1U))qurt_sem_up(&pool->command_ready[index]);
    }
    pool_started = 1;
    hmx_start = HAP_perf_get_qtimer_count();
    ...
    for (uint32_t batch_index = 0U;
        ...
        const uint32_t slot = batch_index % state.slot_count;
        const uint32_t compressed_tile_bytes =
            state.k_tiles * (batch->desc->lpbq_mode != 0U ? 528U : QBH_W4_PACKED_TILE_BYTES);
        ...
        if (qbh_dma_start_w4u8_batch_prefetch(
                ...
                shared +
                    (batch->desc->lpbq_mode != 0U ? batch->desc->lpbq_weight_offset : state.direct_n_weights != 0U
...
finish:
    ...
    if (pool_started != 0) {
        if(header->paper_pipeline_disable&1U)
            for(uint32_t i=0;i<pool->active_worker_count;i++)qurt_sem_up(&pool->command_ready[i]);
    ...
    result = state.abort_status == 0U &&
             ...
             state.head_publish_count ==
                 QBH_BLOCK_HEADS + QBH_BLOCK_KV_HEADS
                 ? 0 : -1;
    if (result == 0 && state.direct_n_weights != 0U) {
        header->w4u8_decode_direct_n_projection_count += 3U;
    }
...
static void qbh_attention_u8_qk_prep_pool_run_head_pair_tasks(
    ...
    struct qbh_block_header *header = pool->attention_header;
    struct qbh_block_buffers *buffers = pool->attention_buffers;
    const struct qbh_w4u8_qkv_ring_state *ring =
        (const struct qbh_w4u8_qkv_ring_state *)pool->qkv_ring_state;
    const uint32_t decode_qk_row_contract =
        ring != NULL ? ring->decode_qk_row_contract : 0U;
    const uint32_t qk_prepare_rows =
        decode_qk_row_contract != 0U
            ? ring->qk_prepare_rows
            : QBH_BLOCK_W4U8_QK_PREP_FULL_ROWS;
    const uint32_t row4 =
        decode_qk_row_contract != 0U &&
        qk_prepare_rows == QBH_BLOCK_W4U8_QK_PREP_DECODE_ROWS;
    const uint32_t q_pair_count = QBH_BLOCK_HEADS / 2U;
    ...
    for (;;) {
        ...
        if (task < q_pair_count) {
            ...
            start = HAP_perf_get_qtimer_count();
            if (row4 != 0U) {
                qbh_hvx_qk_norm_rope_u8_native_head_pair_rows(
                    ...
                    buffers->attention_projection +
                        QBH_QK_ROPE_SF32_CACHE_OFFSET,
                    qk_prepare_rows);
                if (header->w4u8_decode_qk_padding_poison != 0U) {
                    qbh_hvx_poison_u8_native_head_pair_padding(
                        first_q_head, second_q_head,
                        qk_prepare_rows);
                    ++job->u8_qk_padding_poison_pair_count;
                }
                ++job->u8_decode_q_pair_row4_call_count;
            } else {
            ...
            job->u8_attention_qk_norm_rope_ticks +=
                HAP_perf_get_qtimer_count() - start;
            job->attention_qk_norm_task_count += 2U;
            if (decode_qk_row_contract != 0U) {
                job->u8_decode_qk_rows_processed +=
                    2U * qk_prepare_rows;
            }
            continue;
        }

        {
            ...
            if (row4 != 0U) {
                ...
                ++job->u8_decode_k_pair_row4_call_count;
                job->u8_decode_k_temp_carrier_skipped_count += 2U;
            } else {
...
static void qbh_hvx_pool_u8_qk_prep_publish(
    ...
    for (uint32_t tile_end = first_n_tile + tiles_per_head;
         tile_end <= end_tile; tile_end += tiles_per_head) {
        uint32_t task = tile_end / tiles_per_head - 1U;
        if (desc == &header->projections[QBH_BLOCK_PROJ_K]) {
            task += QBH_BLOCK_HEADS;
        }
        if (task >= QBH_BLOCK_HEADS + QBH_BLOCK_KV_HEADS) {
...
static int qbh_hvx_pool_u8_qk_prep_wait_async(
    ...
    for (uint32_t worker = 0U;
        ...
        header->w4u8_decode_q_pair_row4_call_count +=
            job->u8_decode_q_pair_row4_call_count;
        header->w4u8_decode_k_pair_row4_call_count +=
            job->u8_decode_k_pair_row4_call_count;
        header->w4u8_decode_qk_rows_processed +=
            job->u8_decode_qk_rows_processed;
        header->w4u8_decode_k_temp_carrier_skipped_count +=
            job->u8_decode_k_temp_carrier_skipped_count;
        header->w4u8_qk_padding_poison_pair_count +=
            job->u8_qk_padding_poison_pair_count;
 ...
 * Low plane uses the dedicated middle allocation, never the still-live Up
 * input in q. Each worker owns disjoint tiles and private gather scratch. */
static int qbh_llama_sp2_prefill_start(struct qbh_block_w4f16_pool *pool,
    struct qbh_block_buffers *buffers,const uint8_t *gate,const uint8_t *up,uint8_t *middle,uint32_t pipelined) {
    if(!pool || pool->worker_count<3U)return -1;
    ...
    for(uint32_t i=0;i<QBH_BLOCK_INTERMEDIATE/(32U*32U);i++)pool->u8_swiglu_ready[i]=0U;
    buffers->sp2_prefill_ready=pool->u8_swiglu_ready;
    pool->active_worker_count=3U;pool->extra_expand_worker_index=UINT32_MAX;
    for(uint32_t i=0;i<3U;i++)pool->jobs[i].command_kind=QBH_BLOCK_HVX_POOL_LLAMA_SWIGLU;
    asm volatile("barrier" ::: "memory");
    for(uint32_t i=0;i<3U;i++)(void)qurt_sem_up(&pool->command_ready[i]);
    return 0;
}
static void qbh_llama_sp2_prefill_finish(struct qbh_block_header *h,
    struct qbh_block_w4f16_pool *pool,struct qbh_block_buffers *buffers,int status) {
    uint64_t t=HAP_perf_get_qtimer_count();
    if(status) {pool->u8_swiglu_abort=1U;asm volatile("barrier" ::: "memory");}
    qbh_w4f16_pool_wait(pool);pool->active_worker_count=0U;
...
static int qbh_run_w4u8_direct_n_mlp(
    ...
    uint8_t *gate_native = buffers->gate;
    ...
    if (header == NULL || shared == NULL || buffers == NULL ||
        ...
        (((uintptr_t)gate_native | (uintptr_t)up_native) &
         (QBH_BLOCK_ALIGNMENT - 1U)) != 0U) {
        return -1;
    }
    buffers->r4_prefill_ready=NULL;buffers->r4_prefill_prepared=0U;
    /* Stream into dedicated middle: q still supplies the unfinished Up.
     * U8 uses the original LUT/pack and one-pass native Down unchanged. */
    const uint32_t stream_prefill = prefill_direct &&
        (QBH_SP2(header)>=5U || QBH_U8_PREFILL_OPT(header)>=2U) && !(header->paper_pipeline_disable&2U);
    buffers->sp2_prefill_ready=NULL;
    middle_native = (stream_prefill || (QBH_SP2(header) && prefill_direct)) ? buffers->middle : mlp_arena;
     ...
     * whose lifetime can overlap that address.  Keep the native HMX output
    ...
    start = HAP_perf_get_qtimer_count();
#if defined(QBH_MODEL_LLAMA32) || defined(QBH_NATIVE_SP2)
    if(stream_prefill && (QBH_SP2(header)>=7U || QBH_U8_PREFILL_OPT(header)>=3U)) {
        if(qbh_llama_sp2_prefill_start(pool,buffers,gate_native,up_native,middle_native,QBH_SP2(header)>=6U))return -1;
    ...
    if (prefill_direct == 0U &&
        ...
        if (qbh_run_w4u8_direct_n_projection(
                header, shared,
...
#if defined(QBH_MODEL_LLAMA32) || defined(QBH_NATIVE_SP2)
        if(stream_prefill && qbh_llama_sp2_prefill_start(pool,buffers,gate_native,up_native,middle_native,QBH_SP2(header)>=6U))return -1;
#endif
...
#if defined(QBH_MODEL_LLAMA32) || defined(QBH_NATIVE_SP2)
        if(stream_prefill)qbh_llama_sp2_prefill_finish(header,pool,buffers,up_status);
#endif
    ...
    if (header->dense_r4_mode != 0U) {
        ...
        header->activation_ticks+=HAP_perf_get_qtimer_count()-start;
#endif
    } else if (stream_prefill || (prefill_direct == 0U &&
    ...
    const uint32_t stream_down=QBH_SP2(header)==8U && prefill_direct && !(header->paper_pipeline_disable&8U);
    if(stream_down) {
    ...
    if (QBH_SP2(header) && !stream_down && qurt_hvx_unlock()!=AEE_SUCCESS) return -1;
    ...
    if(QBH_SP2(header) && !stream_down && down_result!=0) {
    ...
    if (output_native != 0U) {
        ...
        start = HAP_perf_get_qtimer_count();
        for (uint32_t tile = 0U;
             tile < QBH_BLOCK_HIDDEN / QBH_HMX_OUTPUT_CHANNELS;
             ++tile) {
            qbh_unpack_u8_output(
                down_native + (size_t)tile * QBH_HMX_OUTPUT_BYTES,
                buffers->down, QBH_BLOCK_HIDDEN,
                tile * QBH_HMX_OUTPUT_CHANNELS);
        }
...
static void qbh_hvx_update_v_tail_row(
    uint8_t *tail, const uint8_t *row, uint32_t active_row) {
    for (uint32_t tile = 0U;
         tile < QBH_ATTENTION_HEAD_DIM_TILES; ++tile) {
        memcpy(
            tail + (size_t)tile * QBH_HMX_WEIGHT_BYTES +
                (size_t)active_row * QBH_HMX_OUTPUT_CHANNELS,
            row + (size_t)tile * QBH_HMX_OUTPUT_CHANNELS,
            QBH_HMX_OUTPUT_CHANNELS);
    }
    asm volatile("barrier" ::: "memory");
}
...
static int qbh_scan_append_u8_kv_hmx_segmented(
    ...
    const int attention_publish_v =
        qbh_hmx_native_u8_attention_publish_v_cache_formats(
            header->kv_cache_k_format, header->kv_cache_v_format);
    const int vtcm_tail_v =
        qbh_hmx_native_u8_vtcm_tail_v_cache_formats(
            header->kv_cache_k_format, header->kv_cache_v_format);
    const int partial_k_vtcm =
        qbh_hmx_native_u8_partial_k_vtcm_tail_cache_formats(
            header->kv_cache_k_format, header->kv_cache_v_format);
    ...
    for (uint32_t head = 0U;
        ...
        uint8_t *v_tail = v_tail_base +
            (size_t)active_row * QBH_BLOCK_HEAD_DIM;
        uint8_t *k_vtcm_head = partial_k_vtcm &&
                head < QBH_KV_CACHE_HMX_U8_K_VTCM_TAIL_CACHED_HEADS
            ? buffers->persistent_k_tail +
                  (size_t)head *
                      QBH_KV_CACHE_HMX_U8_K_VTCM_TAIL_HEAD_BYTES
            : NULL;

        qbh_attention_u8_native_head_to_row_major(k_head, row, 1U);
        if (k_vtcm_head == NULL || !session_k_vtcm) {
            if (qbh_scan_cache_dma(
                    header, k_tail, row,
                    QBH_BLOCK_HEAD_DIM, 0U) != 0) {
                return -1;
            ...
            ++header->u8_cache_k_vtcm_tail_ddr_write_skip_count;
            header->u8_cache_k_vtcm_tail_ddr_write_skip_bytes +=
                QBH_BLOCK_HEAD_DIM;
        }
        if (k_vtcm_head != NULL) {
        ...
        if (quartet_v) {
            ...
            if (vtcm_tail_v) {
                ...
                qbh_hvx_update_v_tail_row(
                    vtcm_head, row, active_row);
        ...
        if (active_row + 1U ==
            ...
            if (k_vtcm_head != NULL) {
                uint32_t *packed_bias = (uint32_t *)(packed +
                    QBH_KV_CACHE_HMX_U8_SEGMENT_WEIGHT_BYTES);
                qbh_hvx_copy_aligned_bytes(
                    packed, k_vtcm_head,
                    QBH_KV_CACHE_HMX_U8_K_VTCM_TAIL_WEIGHT_BYTES);
                ...
                ++header->u8_cache_k_vtcm_tail_seal_count;
            } else {
                if (qbh_scan_cache_dma(
                        header, row,
                        shared + header->kv_cache_k_offset +
            ...
            if (quartet_v) {
                ...
                qbh_attention_u8_publish_v_row_group_hvx(
                    vtcm_head +
                        (QBH_KV_CACHE_HMX_U8_SEGMENT_TOKENS / 4U - 1U) *
                            sizeof(HVX_Vector),
                    QBH_HMX_WEIGHT_BYTES, 4U,
                    &buffers->attention_configs[head], v_lut_scratch,
                    header->numerical_audit_enabled != 0U
                        ? &header
                               ->u8_attention_v_recenter_saturation_count
                        : NULL);
                ++header->u8_cache_v_vtcm_tail_publish_count;
                ++header->u8_cache_v_vtcm_tail_seal_count;
            } else {
                if (qbh_scan_cache_dma(
                        header, row, v_tail_base,
                        QBH_KV_CACHE_HMX_U8_SEGMENT_TAIL_BYTES, 1U) != 0) {
                    return -1;
                }
                if (!v_lut_valid ||
...
static uint32_t qbh_f16_ieee_bits(uint16_t h) {
    uint32_t sign=(uint32_t)(h&0x8000U)<<16,exp=(h>>10)&31U,m=h&1023U;
    if(!exp) {
        if(!m)return sign;
        int e=-14;while(!(m&1024U)){m<<=1;--e;}
        return sign|((uint32_t)(e+127)<<23)|((m&1023U)<<13);
    }
...
static uint32_t qbh_f16_conversion_sentinels(struct qbh_block_header *h) {
    ...
    uint32_t errors=0;
    for(uint32_t base=0;base<65536U;base+=64U) {
        for(uint32_t i=0;i<64U;++i)half[i]=(uint16_t)(base+i);
        qbh_f16_to_f32_contiguous((const __fp16 *)half,values);
    ...
    for(uint32_t base=0;base<0x3c00U;base+=64U)for(int delta=-1;delta<=1;++delta) {
        for(uint32_t i=0;i<64U;++i) {
            union{float f;uint32_t u;}a={.u=qbh_f16_ieee_bits((uint16_t)(base+i))};
            union{float f;uint32_t u;}b={.u=qbh_f16_ieee_bits((uint16_t)(base+i+1U))};
            union{float f;uint32_t u;}mid={.f=(a.f+b.f)*0.5f};mid.u+=delta;values[i]=mid.f;
        }
        qbh_f32_to_f16_contiguous(values,(__fp16 *)back);
        for(uint32_t i=0;i<64U;++i) {
    ...
    if (vtcm_k_head != NULL) {
        if (header->generation_boundary_audit_enabled != 0U &&
            !qbh_hmx_native_u8_session_k_vtcm_tail_cache_formats(
                header->kv_cache_k_format,
                header->kv_cache_v_format)) {
            int8_t *audit_weight = (int8_t *)(row_scratch +
                QBH_KV_CACHE_HMX_U8_SEGMENT_TAIL_BYTES);
            uint32_t *audit_bias = (uint32_t *)(audit_weight +
                QBH_KV_CACHE_HMX_U8_SEGMENT_WEIGHT_BYTES);
            if (tail_k_rows == NULL ||
                qbh_scan_cache_dma(
            ...
            for (uint32_t output = 0U;
                 output < QBH_HMX_OUTPUT_CHANNELS; ++output) {
                audit_bias[output] = slot->qk_bias[0];
                audit_bias[QBH_HMX_OUTPUT_CHANNELS + output] =
                    UINT32_C(128) * divisor + rounding;
            }
            qbh_attention_u8_patch_k_delta_rows_hvx(
                row_scratch, tail_rows, config,
                audit_weight, audit_bias);
            header->u8_attention_fused_k_operand_mismatch_count +=
                memcmp(audit_weight, vtcm_k_head,
                       QBH_KV_CACHE_HMX_U8_K_VTCM_TAIL_WEIGHT_BYTES) != 0;
            header->u8_attention_fused_k_operand_mismatch_count +=
                memcmp(audit_bias + QBH_HMX_OUTPUT_CHANNELS,
                       vtcm_k_head +
                           QBH_KV_CACHE_HMX_U8_K_VTCM_TAIL_WEIGHT_BYTES,
                       tail_rows * sizeof(uint32_t)) != 0;
        }
        qbh_hvx_copy_aligned_bytes(
            tail_weight, vtcm_k_head,
            QBH_KV_CACHE_HMX_U8_K_VTCM_TAIL_WEIGHT_BYTES);
        memcpy(
            tail_bias + QBH_HMX_OUTPUT_CHANNELS,
            vtcm_k_head +
                QBH_KV_CACHE_HMX_U8_K_VTCM_TAIL_WEIGHT_BYTES,
            tail_rows * sizeof(uint32_t));
        header->u8_cache_k_vtcm_tail_native_load_bytes +=
            QBH_KV_CACHE_HMX_U8_K_VTCM_TAIL_WEIGHT_BYTES;
        header->u8_cache_k_vtcm_tail_correction_load_bytes +=
            tail_rows * sizeof(uint32_t);
        return 0;
    }
    qbh_hvx_zero_aligned_bytes(
...
static int qbh_scan_prepare_u8_segmented_short_v(
    ...
    if (quartet_v || vtcm_tail_v) {
        ...
        if (vtcm_tail_v) {
            for (uint32_t tile = 0U;
                ...
                qbh_hvx_copy_aligned_bytes(
                    slot->weight +
                        (size_t)sealed_segments * QBH_HMX_WEIGHT_BYTES +
                        (size_t)tile * destination_stride,
                    tail_v_rows +
                        (size_t)tile * QBH_HMX_WEIGHT_BYTES,
                    transferred_bytes);
            }
            header->u8_cache_v_vtcm_tail_native_load_bytes +=
        ...
        if ((attention_publish_v || vtcm_tail_v) &&
            partial_rows == 0U &&
            completed_groups != 0U) {
        ...
        if (partial_rows != 0U) {
            ...
            header->u8_attention_v_pack_ticks +=
                HAP_perf_get_qtimer_count() - start;
            if (vtcm_tail_v) {
                header->u8_cache_v_vtcm_tail_partial_pack_rows +=
                    partial_rows;
            } else {
...
static int qbh_scan_u8_attention(
    ...
    /* Score/probability planes hold all query heads at physical M64.
     * GQA4/head64 needs four times the K/V plane size, unlike Qwen GQA2/head128. */
    const uint32_t plane_width = QBH_BLOCK_HEAD_DIM >
            QBH_ATTENTION_Q_HEADS_PER_GROUP * QBH_BLOCK_M
        ? QBH_BLOCK_HEAD_DIM : QBH_ATTENTION_Q_HEADS_PER_GROUP * QBH_BLOCK_M;
    const uint32_t plane_bytes = padded_tokens * plane_width;
...
static int qbh_run_one_block(struct qbh_block_header *header,
    ...
    memset(&gate_prefetch, 0, sizeof(gate_prefetch));
    ...
    if (header->variant == QBH_BLOCK_W4U8 &&
        ...
        (header->generation_boundary_audit_enabled != 0U ||
         header->numerical_audit_enabled != 0U)) {
        qbh_mix_observed_hash(
            &header->w4u8_decode_q_valid_row_hash,
            qbh_fnv1a64_u8_native_rows(
                buffers->q, QBH_BLOCK_HEADS, 1U));
        qbh_mix_observed_hash(
            &header->w4u8_decode_k_valid_row_hash,
            qbh_fnv1a64_u8_native_rows(
                buffers->k, QBH_BLOCK_KV_HEADS, 1U));
    }
...
#undef QBH_COPY_U8_ATTN_AUDIT
    ...
    if (header->logical_m == 1U &&
        ...
        qbh_start_w4u8_direct_n_gate_prefetch(
...
AEEResult qbh_run_block_rpc(int32_t shared_fd, uint32_t shared_bytes,
    ...
    struct qbh_block_buffers buffers;
    uint8_t *persistent_v_tail_atlas = NULL;
    uint8_t *persistent_k_tail_atlas = NULL;
    struct qbh_block_hmx_worker worker;
    struct qbh_block_w4f16_pool w4f16_pool;
    qurt_thread_attr_t attributes;
    ...
    persistent_v_tail_atlas = buffers.persistent_v_tail;
    persistent_k_tail_atlas = buffers.persistent_k_tail;
    ...
    if (qbh_slice_enabled(header)) {
        ...
        if (persistent_v_tail_atlas != NULL && generation_step == 0U) {
            qbh_hvx_zero_aligned_bytes(
                persistent_v_tail_atlas,
                QBH_KV_CACHE_HMX_U8_V_VTCM_TAIL_ATLAS_BYTES);
            ++header->u8_cache_v_vtcm_tail_init_count;
            header->u8_cache_v_vtcm_tail_init_bytes +=
                QBH_KV_CACHE_HMX_U8_V_VTCM_TAIL_ATLAS_BYTES;
        }
        if (persistent_k_tail_atlas != NULL && generation_step == 0U) {
            qbh_hvx_zero_aligned_bytes(
                persistent_k_tail_atlas,
                QBH_KV_CACHE_HMX_U8_K_VTCM_TAIL_ATLAS_BYTES);
            ++header->u8_cache_k_vtcm_tail_init_count;
            header->u8_cache_k_vtcm_tail_init_bytes +=
                QBH_KV_CACHE_HMX_U8_K_VTCM_TAIL_ATLAS_BYTES;
        }
        ...
        for (uint32_t slice_index = 0U;
            ...
            if (persistent_v_tail_atlas != NULL) {
                buffers.persistent_v_tail = persistent_v_tail_atlas +
                    (size_t)slice_index *
                        QBH_KV_CACHE_HMX_U8_V_VTCM_TAIL_LAYER_BYTES;
            }
            if (persistent_k_tail_atlas != NULL) {
                buffers.persistent_k_tail = persistent_k_tail_atlas +
                    (size_t)slice_index *
        ...
        if (generation_enabled != 0U) {
            ...
            if (header->evaluation_mode == 1U) {
                ...
                struct qbh_eval_score score = qbh_eval_reduce(qbh_generation_histogram(header,&buffers),
                    ...
                    header->evaluation_target_code);
                header->evaluation_vocab_count = score.count;
                header->evaluation_rank = score.rank;
                header->evaluation_target_ties = score.target_ties;
                header->evaluation_max_ties = score.max_ties;
                header->evaluation_saturated = score.saturated;
                header->evaluation_nonfinite = score.nonfinite;
                header->evaluation_target_logit = score.target_logit;
                header->evaluation_logsumexp = score.logsumexp;
                header->evaluation_nll = score.nll;
                header->generation_lm_head_ticks += evaluation_setup_ticks + HAP_perf_get_qtimer_count() - start;
                if (score.count != QBH_QWEN3_VOCAB_SIZE || header->evaluation_target_seen != 1U) {
                    header->dsp_status = QBH_BLOCK_STATUS_LM_HEAD_FAILED;
                    result = AEE_EFAILED;
                    goto stop_worker;
                }
            }
