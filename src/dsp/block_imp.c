...
/* EXP-0212 decode-only cross-layer overlap.  The next layer's first Q32
 * packed-W4 carrier is transferred into the same phase-dead slot that its
 * QKV ring will consume.  The transfer replaces the normal first-Q weight
 * descriptor; only the bias remains to be fetched at QKV entry. */
static int qbh_start_w4u8_next_q_weight_prefetch(
    ...
    if (header == NULL || shared == NULL || buffers == NULL ||
        ...
        (header->w4u8_decode_direct_n_mask &
         QBH_BLOCK_W4U8_DIRECT_N_QKV) == 0U ||
        header->w4u8_decode_direct_n_qkv_batch_n_tiles != 16U ||
        header->w4u8_decode_direct_n_q_batch_n_tiles != n_tiles ||
