...
static int qbh_parse_w4u8_gate_up_decode_mode(
    const char *text, uint32_t *mode) {
    if (strcmp(text, "single") == 0 ||
        strcmp(text, "control") == 0) {
        *mode = QBH_W4U8_DECODE_CONTROL;
        return 0;
    }
    if (strcmp(text, "interleaved2") == 0) {
        *mode = QBH_W4U8_DECODE_INTERLEAVED2;
        return 0;
    }
    return -1;
}

static const char *qbh_w4u8_gate_up_decode_mode_name(
    uint32_t mode) {
    return mode == QBH_W4U8_DECODE_INTERLEAVED2
               ? "interleaved2"
               : "single";
}
...
int main(int argc, char **argv) {
    ...
    uint32_t w4u8_gate_up_decode_mode =
        QBH_W4U8_DECODE_CONTROL;
    uint32_t w4u8_gate_up_worker_count = 3U;
    ...
    if (argc < 3 || argc > 26 ||
        ...
        (argc >= 25 && qbh_parse_w4u8_gate_up_decode_mode(
                           argv[24],
                           &w4u8_gate_up_decode_mode) != 0) ||
        (argc >= 26 && qbh_parse_u32(
                           argv[25],
                           &w4u8_gate_up_worker_count) != 0) ||
        ...
        w4u8_gate_up_decode_mode > QBH_W4U8_DECODE_INTERLEAVED2 ||
        (w4u8_gate_up_worker_count != 2U &&
         w4u8_gate_up_worker_count != 3U) ||
        ((w4u8_gate_up_decode_mode != QBH_W4U8_DECODE_CONTROL ||
          w4u8_gate_up_worker_count != 3U) &&
         (variant != QBH_BLOCK_W4U8 ||
          !qbh_block_mlp_is_w4u8_streaming(mlp_mode))) ||
        (variant == QBH_BLOCK_W4U8 &&
        ...
        fprintf(stderr, "usage: %s PACKAGE_DIR VARIANT [repeat_count] "
                        ...
                        "[w4u8_gate_up_decode:single|interleaved2] "
                        "[w4u8_gate_up_workers:2|3]\n",
                argv[0]);
        return 2;
    }
    ...
    header->w4u8_gate_up_decode_mode =
        w4u8_gate_up_decode_mode;
    header->w4u8_gate_up_worker_count =
        w4u8_gate_up_worker_count;
    ...
    printf(
        ...
        "\"w4u8_gate_up_decode_mode\":\"%s\","
        "\"w4u8_gate_up_worker_count\":%" PRIu32 ","
        "\"attribution_mode\":\"%s\","
        ...
        qbh_w4u8_gate_up_decode_mode_name(
            header->w4u8_gate_up_decode_mode),
        header->w4u8_gate_up_worker_count,
