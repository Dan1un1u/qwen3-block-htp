    ...
    for (uint32_t n_tile = 0; n_tile < n_tiles; ++n_tile) {
        for (uint32_t output = 0; output < 32U; ++output) {
            ...
            if (QBH_FP32_RESIDUAL(header) &&
                (projection_index==QBH_BLOCK_PROJ_O || projection_index==QBH_BLOCK_PROJ_DOWN)) {
                if(desc->lpbq_mode || (int64_t)positive_sum*255>8388607 ||
                   (int64_t)negative_sum*255< -8388608 ||
                   (projection_index==QBH_BLOCK_PROJ_DOWN &&
                    (int64_t)(positive_sum-negative_sum)*24576>INT32_MAX)) return -1;
                float real_scale=input_qparam->scale*scales[global_output];
                if(!(real_scale>0.0f) || !isfinite(real_scale))return -1;
                memcpy(&bias[(size_t)n_tile*64U+output],&real_scale,sizeof(real_scale));
...
static struct qbh_error_metrics qbh_compare_f32(
...
#define QBH_REPLAY_PROFILE_U64(field) \
    ...
    QBH_REPLAY_PROFILE_U32(w4u8_decode_direct_n_q_batch_n_tiles);
    QBH_REPLAY_PROFILE_U32(w4u8_decode_direct_n_down_batch_n_tiles);
    QBH_REPLAY_PROFILE_U32(w4u8_decode_direct_n_down_single_dma);
    QBH_REPLAY_PROFILE_U32(w4u8_decode_direct_n_o_single_dma);
    QBH_REPLAY_PROFILE_U32(w4u8_qkv_ring_slot_count);
    QBH_REPLAY_PROFILE_U32(w4u8_qkv_ring_expand_worker_count);
    QBH_REPLAY_PROFILE_U32(w4u8_qkv_ring_prep_worker_count);
    QBH_REPLAY_PROFILE_U32(w4u8_qkv_ring_dispatch_count);
    QBH_REPLAY_PROFILE_U32(w4u8_qkv_ring_batch_count);
    QBH_REPLAY_PROFILE_U32(w4u8_qkv_ring_expand_task_count);
    QBH_REPLAY_PROFILE_U32(w4u8_qkv_ring_hmx_dispatch_count);
    QBH_REPLAY_PROFILE_U32(w4u8_qkv_ring_head_publish_count);
    QBH_REPLAY_PROFILE_U64(w4u8_qkv_ring_pipeline_ticks);
    QBH_REPLAY_PROFILE_U64(w4u8_qkv_ring_dma_wait_ticks);
    QBH_REPLAY_PROFILE_U64(w4u8_qkv_ring_producer_slot_wait_ticks);
    QBH_REPLAY_PROFILE_U64(w4u8_qkv_ring_expand_ticks);
    QBH_REPLAY_PROFILE_U64(w4u8_qkv_ring_hmx_ready_wait_ticks);
    QBH_REPLAY_PROFILE_U64(w4u8_qkv_ring_hmx_compute_ticks);
    QBH_REPLAY_PROFILE_U64(w4u8_qkv_ring_pool_wait_ticks);
    QBH_REPLAY_PROFILE_U32(w4u8_o_gate_prefetch_start_count);
    ...
    QBH_REPLAY_PROFILE_U32(w4u8_swiglu_rows_observed);
    QBH_REPLAY_PROFILE_U32(w4u8_decode_swiglu_row4_call_count);
    QBH_REPLAY_PROFILE_U32(w4u8_decode_swiglu_vector_count);
    QBH_REPLAY_PROFILE_U32(w4u8_decode_swiglu_padding_poison_count);
    QBH_REPLAY_PROFILE_U64(w4u8_decode_swiglu_valid_row_hash);
    ...
    QBH_REPLAY_PROFILE_U32(u8_cache_k_vtcm_tail_cached_head_count);
    QBH_REPLAY_PROFILE_U32(u8_cache_k_vtcm_tail_fallback_head_count);
    ...
    QBH_REPLAY_PROFILE_U32(u8_cache_k_vtcm_tail_ddr_write_skip_count);
    QBH_REPLAY_PROFILE_U64(u8_cache_k_vtcm_tail_ddr_write_skip_bytes);
        ...
        for (uint32_t step = 0U; step <= steps; ++step) {
            ...
            for(uint32_t i=0;i<QBH_VERTICAL_SLICE_LAYER_COUNT;++i) {
                if(state->layers[QBH_VERTICAL_SLICE_FIRST_LAYER+i].valid_length != before+h->logical_m) {
                    fprintf(stderr,"slice state failure check_line=%u layer=%u rc=%d dsp=%d valid=%u expected=%u conversion_mismatches=%u sentinel_kind=%u input=%08x expected=%08x actual=%08x\n",h->projection_failure_step,i,rc,h->dsp_status,state->layers[QBH_VERTICAL_SLICE_FIRST_LAYER+i].valid_length,before+h->logical_m,h->w4f16_decode_conversion_audit_mismatches,h->w4f16_decode_sentinel_kind,h->w4f16_decode_sentinel_input,h->w4f16_decode_sentinel_expected,h->w4f16_decode_sentinel_actual);return -1;
                }
            }
            ...
            if (dump != NULL && rep == 0U) {
                ...
                if(h->variant!=QBH_BLOCK_W4U8) for(uint32_t li=0;li<QBH_VERTICAL_SLICE_LAYER_COUNT;++li) {
                    ...
                    snprintf(name,sizeof(name),"step%02u_layer%02u_v.bin",step,li);
                    if(qbh_write_named_tensor(dump,name,shared+cs->v_offset,cs->v_bytes))return -1;
                }
                if (step == 0U) {
...
int main(int argc, char **argv) {
    ...
    uint32_t w4u8_decode_direct_n_q_batch_n_tiles = 0U;
    uint32_t w4u8_decode_direct_n_down_batch_n_tiles = 2U;
    uint32_t w4u8_decode_direct_n_down_single_dma = 0U;
    uint32_t w4u8_decode_direct_n_o_single_dma = 0U;
    ...
    {
        const char *value = getenv(
            "QBH_W4U8_DECODE_DIRECT_N_QKV_BATCH_N_TILES");
        if (value != NULL && value[0] != '\0' &&
            qbh_parse_u32(
                value,
                &w4u8_decode_direct_n_qkv_batch_n_tiles) != 0) {
            w4u8_decode_direct_n_qkv_batch_n_tiles = UINT32_MAX;
        }
    }
    {
        const char *value = getenv(
            "QBH_W4U8_DECODE_DIRECT_N_Q_BATCH_N_TILES");
        if (value != NULL && value[0] != '\0' &&
            qbh_parse_u32(
                value,
                &w4u8_decode_direct_n_q_batch_n_tiles) != 0) {
            w4u8_decode_direct_n_q_batch_n_tiles = UINT32_MAX;
        }
    }
    {
        const char *value = getenv(
            "QBH_W4U8_DECODE_DIRECT_N_DOWN_BATCH_N_TILES");
        if (value != NULL && value[0] != '\0' &&
            qbh_parse_u32(
                value,
                &w4u8_decode_direct_n_down_batch_n_tiles) != 0) {
            w4u8_decode_direct_n_down_batch_n_tiles = UINT32_MAX;
        }
    }
    {
        const char *value = getenv(
            "QBH_W4U8_DECODE_DIRECT_N_DOWN_SINGLE_DMA");
        if (value != NULL && value[0] != '\0' &&
            qbh_parse_u32(
                value,
                &w4u8_decode_direct_n_down_single_dma) != 0) {
            w4u8_decode_direct_n_down_single_dma = UINT32_MAX;
        }
    }
    {
        const char *value = getenv(
            "QBH_W4U8_DECODE_DIRECT_N_O_SINGLE_DMA");
        if (value != NULL && value[0] != '\0' &&
            qbh_parse_u32(
                value,
                &w4u8_decode_direct_n_o_single_dma) != 0) {
            w4u8_decode_direct_n_o_single_dma = UINT32_MAX;
        }
    }
    {
        const char *value = getenv(
            "QBH_W4U8_DECODE_DIRECT_N_GATE_UP_CONTINUOUS");
        if (value != NULL && value[0] != '\0' &&
            qbh_parse_u32(
                value,
                &w4u8_decode_direct_n_gate_up_continuous) != 0) {
            w4u8_decode_direct_n_gate_up_continuous = UINT32_MAX;
        }
    }
    {
        const char *value = getenv(
            "QBH_W4U8_DECODE_DIRECT_N_O_GATE_PREFETCH");
        if (value != NULL && value[0] != '\0' &&
            qbh_parse_u32(
                value,
    ...
    {
        const char *value = getenv("QBH_W4U8_DECODE_SWIGLU_ROWS");
        if (value != NULL && value[0] != '\0' &&
            qbh_parse_u32(value, &w4u8_decode_swiglu_rows) != 0) {
            w4u8_decode_swiglu_rows = UINT32_MAX;
        }
    }
    {
        const char *value = getenv(
            "QBH_W4U8_DECODE_SWIGLU_PADDING_POISON");
        if (value != NULL && value[0] != '\0' &&
            qbh_parse_u32(
                value, &w4u8_decode_swiglu_padding_poison) != 0) {
            w4u8_decode_swiglu_padding_poison = UINT32_MAX;
        }
    }
    {
            ...
            if (strcmp(layout, "row_major") == 0) {
                ...
                kv_cache_v_format =
                    QBH_KV_CACHE_FORMAT_HMX_U8_V_VTCM_TAIL_V7;
                           ...
                           layout,
                           "hmx_native_u8_segmented_vtcm_k7_v8") == 0) {
                kv_cache_k_format =
                    QBH_KV_CACHE_FORMAT_HMX_U8_K_PARTIAL_VTCM_TAIL_V8;
                kv_cache_v_format =
                    QBH_KV_CACHE_FORMAT_HMX_U8_V_VTCM_TAIL_V7;
                           ...
                           layout,
                           "hmx_native_u8_segmented_vtcm_k7_session_v9") ==
                       0) {
                kv_cache_k_format =
                    QBH_KV_CACHE_FORMAT_HMX_U8_K_SESSION_VTCM_TAIL_V9;
                kv_cache_v_format =
    ...
    if (argc < 3 || argc > 26 ||
        ...
        w4u8_decode_direct_n_gate_up_continuous > 1U ||
        (w4u8_decode_direct_n_gate_up_continuous != 0U &&
         (variant != QBH_BLOCK_W4U8 ||
          ...
          (w4u8_decode_direct_n_mask &
           QBH_BLOCK_W4U8_DIRECT_N_MLP) == 0U ||
          w4u8_decode_direct_n_gate_up_batch_n_tiles != 32U)) ||
        w4u8_decode_direct_n_o_gate_prefetch > 1U ||
        ...
        (w4u8_decode_direct_n_q_batch_n_tiles != 0U &&
         w4u8_decode_direct_n_q_batch_n_tiles != 32U) ||
        (w4u8_decode_direct_n_q_batch_n_tiles != 0U &&
         (variant != QBH_BLOCK_W4U8 ||
          w4u8_decode_projection_mode !=
              QBH_BLOCK_W4U8_DECODE_PROJECTION_DIRECT_N ||
        ...
        (w4u8_decode_direct_n_down_batch_n_tiles != 2U &&
         (variant != QBH_BLOCK_W4U8 ||
          ...
          (w4u8_decode_direct_n_mask &
           QBH_BLOCK_W4U8_DIRECT_N_MLP) == 0U)) ||
        w4u8_decode_direct_n_down_single_dma > 1U ||
        (w4u8_decode_direct_n_down_single_dma != 0U &&
         (variant != QBH_BLOCK_W4U8 ||
          w4u8_decode_projection_mode !=
              QBH_BLOCK_W4U8_DECODE_PROJECTION_DIRECT_N ||
          (w4u8_decode_direct_n_mask &
        ...
        (w4u8_decode_direct_n_o_single_dma != 0U &&
         (variant != QBH_BLOCK_W4U8 ||
          ...
          w4u8_decode_o_batch_n_tiles != 16U)) ||
        ...
        (w4u8_decode_swiglu_rows !=
         ...
         (variant != QBH_BLOCK_W4U8 ||
          ...
          (w4u8_decode_direct_n_mask &
           QBH_BLOCK_W4U8_DIRECT_N_MLP) == 0U)) ||
        w4u8_decode_swiglu_padding_poison > 1U ||
        (w4u8_decode_swiglu_padding_poison != 0U &&
         (variant != QBH_BLOCK_W4U8 ||
          w4u8_decode_swiglu_rows !=
              QBH_BLOCK_W4U8_SWIGLU_DECODE_ROWS)) ||
    ...
    if (generation_mode != QBH_BLOCK_GENERATION_DISABLED &&
        (qbh_prepare_slot(
         ...
         qbh_prepare_slot(
             ...
             qbh_generation_w4u8_enabled(generation_mode) && !qbh_host_fp32_residual()
