...
static int qbh_parse_attention_pipeline_mode(
    ...
    if (strcmp(text,
               "u8_log2_gqa_qkv_overlap_vgather_vdeal_fused_qk_requant_hmx_batch_lut_templates_audit_reductions") == 0 ||
        strcmp(text,
               "integer_gqa_qkv_overlap_vgather_vdeal_fused_qk_requant_hmx_batch_lut_templates_audit_reductions") == 0) {
        *mode =
            QBH_BLOCK_ATTENTION_PIPELINE_U8_LOG2_GQA_QKV_OVERLAP_VGATHER_VDEAL_FUSED_QK_REQUANT_HMX_BATCH_LUT_TEMPLATES_AUDIT_REDUCTIONS;
        return 0;
    }
    return -1;
}
...
static const char *qbh_attention_pipeline_mode_name(uint32_t mode) {
    ...
    if (mode ==
        QBH_BLOCK_ATTENTION_PIPELINE_U8_LOG2_GQA_QKV_OVERLAP_VGATHER_VDEAL_FUSED_QK_REQUANT_HMX_BATCH_LUT_TEMPLATES_AUDIT_REDUCTIONS) {
        return "u8_log2_gqa_qkv_overlap_vgather_vdeal_fused_qk_requant_hmx_batch_lut_templates_audit_reductions";
    }
    return "control";
}
