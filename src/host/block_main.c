...
static int qbh_parse_mlp_mode(const char *text, uint32_t *mode) {
    ...
    if (strcmp(
            text,
            "w4u8_streaming_persistent_mlp_hvx_exact_affine") == 0 ||
        strcmp(text, "w4u8_exact_affine") == 0) {
        *mode =
...
static const char *qbh_mlp_mode_name(uint32_t mode) {
    ...
    if (mode ==
        QBH_BLOCK_MLP_W4U8_STREAMING_PERSISTENT_MLP_HVX_EXACT_AFFINE) {
        return "w4u8_streaming_persistent_mlp_hvx_exact_affine";
    }
    return "control";
}
...
int main(int argc, char **argv) {
    ...
    if (argc < 3 || argc > 24 ||
        ...
        fprintf(stderr, "usage: %s PACKAGE_DIR VARIANT [repeat_count] "
                        ...
                        "w4u8_streaming_persistent_mlp_hvx_exact_affine] "
    ...
    printf(
        ...
        "\"w4u8_mlp_exact_affine_activation_task_count\":%" PRIu32 ","
