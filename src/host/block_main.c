...
static int qbh_run_replay_sequence(
    ...
    for (uint32_t step = 0U; step < total_steps; ++step) {
        ...
        if (rpc_result != AEE_SUCCESS) {
            fprintf(stderr, "replay step %" PRIu32 " RPC failed: 0x%08x dsp_status=%u vtcm_peak=%u r4_calls=%u header_failure_site=%u\n",
...
int main(int argc, char **argv) {
    ...
    header->llama_sp2_down_backend=getenv("QBH_SP2_DOWN_HVX")?(uint32_t)atoi(getenv("QBH_SP2_DOWN_HVX")):0U;
    if(header->llama_sp2_down_backend>1U)return 2;
