...
int main(int argc, char **argv) {
    ...
    uint32_t w4u8_qkv_tail_prep_mode =
    ...
    {
        const char *tail_prep = getenv("QBH_W4U8_QKV_TAIL_PREP");
        if (tail_prep != NULL && tail_prep[0] != '\0') {
            if (strcmp(tail_prep, "control") == 0) {
                w4u8_qkv_tail_prep_mode =
                    QBH_BLOCK_W4U8_QKV_TAIL_PREP_CONTROL;
            } else if (strcmp(tail_prep, "main_drain") == 0) {
                w4u8_qkv_tail_prep_mode =
                    QBH_BLOCK_W4U8_QKV_TAIL_PREP_MAIN_DRAIN;
            } else {
                w4u8_qkv_tail_prep_mode = UINT32_MAX;
            }
        }
    }
    if (argc < 3 || argc > 26 ||
    ...
    printf(
        ...
        "\"w4u8_qkv_tail_prep_mode\":%" PRIu32 ","
