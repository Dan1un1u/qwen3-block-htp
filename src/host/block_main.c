...
static int qbh_parse_attention_pipeline_mode(
    ...
    if (strcmp(text,
               "u8_log2_gqa_qkv_overlap_vgather_vdeal_fused_qk_requant_hmx_batch_lut_templates_gqa_batch_staged_softmax") == 0 ||
        strcmp(text,
               "integer_gqa_qkv_overlap_vgather_vdeal_fused_qk_requant_hmx_batch_lut_templates_gqa_batch_staged_softmax") == 0) {
        *mode =
...
static const char *qbh_attention_pipeline_mode_name(uint32_t mode) {
    ...
    if (mode ==
        QBH_BLOCK_ATTENTION_PIPELINE_U8_LOG2_GQA_QKV_OVERLAP_VGATHER_VDEAL_FUSED_QK_REQUANT_HMX_BATCH_LUT_TEMPLATES_GQA_BATCH_STAGED_SOFTMAX) {
        return "u8_log2_gqa_qkv_overlap_vgather_vdeal_fused_qk_requant_hmx_batch_lut_templates_gqa_batch_staged_softmax";
    }
    return "control";
}
